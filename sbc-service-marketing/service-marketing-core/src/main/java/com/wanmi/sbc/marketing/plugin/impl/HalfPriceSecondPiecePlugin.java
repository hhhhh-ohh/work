...
public class HalfPriceSecondPiecePlugin implements IGoodsListPlugin, IGoodsDetailPlugin, ITradeCommitPlugin {
    ...
    public void goodsListFilter(List<GoodsInfoVO> goodsInfos, MarketingPluginRequest request) {
        ...
        List<MarketingResponse> marketingList = request.getMarketingMap().values().stream()
                ...
                .filter(marketing -> MarketingType.HALF_PRICE_SECOND_PIECE == marketing.getMarketingType())
    ...
    public void goodsDetailFilter(GoodsInfoDetailByGoodsInfoResponse detailResponse, MarketingPluginRequest request) {
        ...
        if (CollectionUtils.isEmpty(marketingList)
                || marketingList.stream().filter(marketing -> MarketingType.HALF_PRICE_SECOND_PIECE == marketing.getMarketingType()).count() < 1) {
            return;
        }
        MarketingResponse marketingObj =
                marketingList.stream().filter(marketing ->
                        MarketingType.HALF_PRICE_SECOND_PIECE == marketing.getMarketingType()).findFirst().orElse(new MarketingResponse());
        //填充营销描述<营销编号,描述>
        Map<Long, String> labelMap = this.getLabelMap(marketingList);
        MarketingLabelVO label = new MarketingLabelVO();
    ...
    public TradeMarketingResponse wraperMarketingFullInfo(TradeMarketingPluginRequest request) {
        ...
        List<TradeItemInfo> itemInfoList =
                tradeItems.stream().sorted(Comparator.comparing(TradeItemInfo::getPrice)).toList();
        ...
        // 将商品信息封装成新对象便计算优惠
        List<TradeItemInfo> discountInfoList = new ArrayList<>();
        exitFlag: for (TradeItemInfo item : itemInfoList) {
        ...
        // 有折扣
        if (grade > 0) {
            for (TradeItemInfo tradeItemInfo : discountInfoList) {
                if (level.getDiscount().compareTo(BigDecimal.ZERO) == 0) {
                    ...
                    halfPriceSecondPiece =
                            halfPriceSecondPiece.add(tradeItemInfo.getPrice().multiply((BigDecimal.valueOf(10).subtract(level.getDiscount()).multiply(BigDecimal.valueOf(0.1)))).setScale(2, RoundingMode.HALF_UP));
                }
            }
            // 循环折扣次数计算
...
//                            //第N件以最小的商品金额计算折扣
//                            halfPriceSecondPiece =
//                                    halfPriceSecondPiece.add(itemInfoList.get(0).getPrice().multiply((BigDecimal.valueOf(10).subtract(level.getDiscount()).multiply(BigDecimal.valueOf(0.1)))).setScale(2, RoundingMode.HALF_UP));
//                        }
//                    }
//                } else {
...
//                        //第N件以最小的商品金额计算折扣
//                        halfPriceSecondPiece =
//                                halfPriceSecondPiece.add(gradeTradeItemInfo.getPrice().multiply((BigDecimal.valueOf(10).subtract(level.getDiscount()).multiply(BigDecimal.valueOf(0.1)))).setScale(2, RoundingMode.HALF_UP));
//                    }
//                }
//            }
        ...
        response.setTradeMarketing(TradeMarketing.builder()
                .halfPriceSecondPieceLevel(KsBeanUtil.copyPropertiesThird(level, MarketingHalfPriceSecondPieceLevelVO.class))
                //优惠金额
