...
public class DistributionRecordService {
    ...
    public DistributionRecord getById(String id) {
        DistributionRecord distributionRecord = distributionRecordRepository.findById(id).orElse(null);
        if (Objects.isNull(distributionRecord) || Objects.isNull(distributionRecord.getDeleteFlag()) ||
                DeleteFlag.YES.equals(distributionRecord.getDeleteFlag())){
            throw new SbcRuntimeException(CommonErrorCodeEnum.K999999, new Object[] {"分销记录不存在"},this.getDeleteIndex(id));
        }
     ...
     * @description 查询数量
     * @author  xuyunpeng
     * @date 2021/6/7 4:14 下午
     * @param queryReq
     * @return
     */
    public Long count(DistributionRecordQueryRequest queryReq) {
        return distributionRecordRepository.count(DistributionRecordWhereCriteriaBuilder.build(queryReq));
    }
    ...
    public DistributionRecordByInviteeIdResponse getPerformanceByInviteeId(String distributorId) {
        ...
        if (!CollectionUtils.isEmpty(distributionRecords)) {
            ...
            List<String> distributorIds = distributionRecords.stream().filter(d -> d.getDistributorId() != null).map(DistributionRecord::getDistributorId).distinct().collect(toList());
     ...
     * 拼凑删除es-提供给findOne去调
     * @param id 编号
     * @return "es_distribution_record:{id}"
     */
    private Object getDeleteIndex(String id){
        return String.format(EsConstants.DELETE_SPLIT_CHAR, EsConstants.DISTRIBUTION_RECORD, id);
    }
