...
public class MarketingBuyoutPriceFullBaleLevelService {
    ...
    public void listFullBaleLevel(List<MarketingPageVO> marketingList) {
        ...
        List<MarketingBuyoutPriceLevel> priceLevelList =
                marketingBuyoutPriceLevelRepository.findByMarketingIdIn(marketingIds);
        Map<Long, List<MarketingBuyoutPriceLevel>> levelMap =
                priceLevelList.stream().collect(Collectors.groupingBy(MarketingBuyoutPriceLevel::getMarketingId));
        for (MarketingPageVO marketingId : marketingList) {
            ...
            List<String> levelsList = levels.stream().map(marketing ->
                    marketing.getFullAmount() + "元任选" + marketing.getChoiceCount() + "件").
