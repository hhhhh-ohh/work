...
public class GrouponActivityWhereCriteriaBuilder {
    public static Specification<GrouponActivity> build(GrouponActivityQueryRequest queryRequest) {
        return (root, cquery, cbuild) -> {
            List<Predicate> predicates = new ArrayList<>(30);
            // 批量查询-活动IDList
            ...
            // 不等于自身id
            if (StringUtils.isNotEmpty(queryRequest.getNotId())) {
                predicates.add(cbuild.notEqual(root.get("grouponActivityId"), queryRequest.getNotId()));
            }
