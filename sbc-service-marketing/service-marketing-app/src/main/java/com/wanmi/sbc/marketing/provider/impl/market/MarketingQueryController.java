...
public class MarketingQueryController implements MarketingQueryProvider {
    ...
    public BaseResponse<List<String>> queryExistsSkuByMarketingType(@RequestBody @Valid ExistsSkuByMarketingTypeRequest request) {
        // 默认正常
        SkuExistsRequest skuExistsRequest = KsBeanUtil.convert(request.getSkuExistsDTO(), SkuExistsRequest.class);
        skuExistsRequest.setPluginType(PluginType.NORMAL);
    ...
    public BaseResponse<MarketingPageResponse> page(@RequestBody @Valid MarketingPageRequest marketingPageRequest) {
        MarketingQueryListRequest marketingQueryListRequest = KsBeanUtil.convert(
                        marketingPageRequest.getMarketingPageDTO(),
                        MarketingQueryListRequest.class);
        // 默认查询SBC营销
        marketingQueryListRequest.setPluginType(PluginType.NORMAL);
        MicroServicePage<MarketingPageVO> marketingPage = marketingService.getMarketingPage(marketingQueryListRequest,
                marketingPageRequest.getStoreId());
        // 判断是否需要展示营销规和店铺名称
        ...
        List<MarketingPageVO> marketingList = marketingPage.getContent();
        // 判断是否需要展示第二件半价营销规
        if (marketingPageRequest.getMarketingPageDTO().getMarketingSubType()
                == MarketingSubType.HALF_PRICE_SECOND_PIECE
                || (Objects.equals(marketingPageRequest.getIsRule(),Boolean.TRUE) && CollectionUtils.isNotEmpty(marketingList))) {
            marketingHalfPriceSecondPieceLevelService.halfPriceSecondPieceLevel(
                    marketingPage.getContent());
        }
        //只展示满减，满赠，满折的优惠规则
        if(Objects.equals(marketingPageRequest.getIsRule(),Boolean.TRUE) && CollectionUtils.isNotEmpty(marketingList)){
            marketingList.forEach(marketingVO->{
                marketingService.joinMarketingLevels(marketingVO);
            });
            marketingPage.setContent(marketingList);
        }

        if (MarketingType.SUITS.equals(marketingQueryListRequest.getMarketingType())){
            List<Long> marketingIds =
                    marketingPage.getContent().stream().map(MarketingPageVO::getMarketingId).collect(Collectors.toList());
            //组合购商品信息列表
            List<MarketingSuitsSku> marketingSuitsSkuList = marketingSuitsSkuService.getByMarketingIds(marketingIds);
            Map<Long, List<MarketingSuitsSku>> marketingIdToMarketingPageVOsMap =
                    marketingSuitsSkuList.stream().collect(Collectors.groupingBy(MarketingSuitsSku::getMarketingId));

            Map<Long, MarketingPageVO> marketingIdToMarketingPageVOMap =
                    marketingPage.getContent().stream().collect(Collectors.toMap(MarketingPageVO::getMarketingId,
                    Function.identity()));
            marketingPage.forEach(marketing -> {
                List<MarketingSuitsSku> marketingSuitsSkus =
                        marketingIdToMarketingPageVOsMap.get(marketing.getMarketingId());
                if (CollectionUtils.isNotEmpty(marketingSuitsSkus)){
                    MarketingScopeVO marketingScopeVO = new MarketingScopeVO();
                    marketingScopeVO.setMarketingId(marketing.getMarketingId());
                    marketingScopeVO.setScopeId(marketingSuitsSkus.get(0).getSkuId());
                    marketing.setMarketingScopeList(Collections.singletonList(marketingScopeVO));
                }
            });
        }
    ...
    public BaseResponse<MarketingGetByIdForSupplierResponse> getByIdForSupplier(@RequestBody @Valid MarketingGetByIdRequest getByIdRequest) {
        ...
        if (MarketingScopeType.SCOPE_TYPE_CUSTOM.equals(marketingForEndVO.getScopeType())) {
    ...
    public BaseResponse<MarketingByIdAndSubtypeResponse> getByMarketingId(@RequestBody @Valid MarketingGetByIdRequest getByIdRequest){
        ...
        if(!(marketingResponse.getSubType() == MarketingSubType.SUITS_GOODS)){
     ...
     */
    @Override
    public BaseResponse<MarketingGetByIdForCustomerResponse> getMarketingSimpleByIdForCustomer(@RequestBody @Valid MarketingGetByIdRequest getByIdRequest) {
        MarketingForEndVO marketingForEndVO =
                KsBeanUtil.convert(marketingService.getMarketingSimpleByIdForCustomer(getByIdRequest.getMarketingId()),
                        MarketingForEndVO.class);
     ...
     */
    @Override
    public BaseResponse<MarketingSimpleGetByIdForCustomerResponse> getSimpleByIdForCustomer(@RequestBody @Valid MarketingGetByIdRequest getByIdRequest) {
        MarketingSimpleVO response = marketingService.getMarketingSimpleByIdForCustomer(getByIdRequest.getMarketingId());

        return BaseResponse.success(MarketingSimpleGetByIdForCustomerResponse.builder().marketingSimpleVO(response).build());
    }
