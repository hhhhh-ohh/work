...
public class DistributionRecordQueryController implements DistributionRecordQueryProvider {
    ...
    public BaseResponse<DistributionRecordExportResponse> export(@RequestBody @Valid DistributionRecordExportRequest distributionRecordExportReq) {
        ...
        DistributionRecordExportResponse finalRes = new DistributionRecordExportResponse(data);
        return BaseResponse.success(finalRes);
    }

    @Override
    public BaseResponse<Long> countForExport(@Valid DistributionRecordExportRequest distributionRecordExportReq) {
        DistributionRecordQueryRequest queryReq = new DistributionRecordQueryRequest();
        KsBeanUtil.copyPropertiesThird(distributionRecordExportReq, queryReq);
        Long total = distributionRecordService.count(queryReq);
    ...
    private Page<DistributionRecordVO> getDistributionRecordVOPage(@Valid @RequestBody DistributionRecordQueryRequest queryReq) {
        ...
        if (!CollectionUtils.isEmpty(distributionRecordList) && distributionRecordList.size() > 0) {
            ...
            List<String> cusotmerIds = distributionRecordList.stream().filter(d->d.getCustomerId() != null).map(DistributionRecord::getCustomerId).collect(toList());
            ...
            List<String> distributorIds = distributionRecordList.stream().filter(d->d.getDistributorId() != null).map(DistributionRecord::getDistributorId).collect(toList());
            ...
            List<Long> storeIds = distributionRecordList.stream().filter(d->d.getStoreId() != null).map(DistributionRecord::getStoreId).collect(toList());
            ...
            //根据货品ids获取货品信息
            List<String> goodsInfoIds = distributionRecordList.stream().filter(d->d.getGoodsInfoId() != null).map(DistributionRecord::getGoodsInfoId).collect(toList());
