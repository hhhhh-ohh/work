...
public class CouponCustomerRightsController implements CouponCustomerRightsProvider {
    ...
    public BaseResponse customerRightsIssueCoupons() {
        ...
        rightsList.forEach(rights -> {
            ...
            if (CollectionUtils.isNotEmpty(levelIds)) {
                ...
                customerIds.forEach(customerId -> couponCodeService.sendBatchCouponCodeByCustomer(finalGetCouponGroupResponse, customerId, rights.getActivityId(),null));
            }
        });
    ...
    @Override
    public BaseResponse customerRightsRepeatCoupons(@Valid @RequestBody RightsCouponRequest rightsCouponRequest) {
        couponCodeService.sendBatchCouponCodeByCustomer(rightsCouponRequest);
        return BaseResponse.SUCCESSFUL();
    }
