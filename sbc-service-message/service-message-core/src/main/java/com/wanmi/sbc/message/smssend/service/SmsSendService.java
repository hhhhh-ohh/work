...
public class SmsSendService {
	...
	public SmsSend add(SmsSend entity) {
		if(entity.getSendType()==SendType.NOW){
			...
			mqSendDTO.setTopic(ProducerTopic.SMS_SEND_DATA);
			...
			if(entity.getSendTime().isBefore(LocalDateTime.now())){
				throw new SbcRuntimeException(MessageErrorCodeEnum.K090013);
			}
			if(entity.getSendTime().getHour()>Constants.NUM_22 || entity.getSendTime().getHour()<Constants.EIGHT){
				throw new SbcRuntimeException(MessageErrorCodeEnum.K090013);
			}
	...
	public void scanSendTask(){
		...
		if(CollectionUtils.isNotEmpty(list)){
			for (SmsSend smsSend : list) {
				MqSendDTO mqSendDTO = new MqSendDTO();
				mqSendDTO.setTopic(ProducerTopic.SMS_SEND_DATA);
				mqSendDTO.setData(JSONObject.toJSONString(smsSend));
				mqSendProvider.send(mqSendDTO);
