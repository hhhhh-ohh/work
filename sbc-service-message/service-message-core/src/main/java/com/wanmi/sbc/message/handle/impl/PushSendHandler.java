...
public class PushSendHandler implements MessageDelivery {
    ...
    public void handle(MessageMQRequest request) {
        log.info("Push节点{}接收消息：{}", request.getNodeCode(), JSON.toJSONString(request));
        ...
        if(Objects.nonNull(pushCustomerEnable)){
            ...
            //节点开关开启组装消息
            if(node.getStatus() == 1 && SwitchFlag.OPEN == appSwitch && time){
    ...
    private boolean checkTime(){
        ...
        if(hour >= Constants.NUM_22 || hour<Constants.SEVEN){
            return false;
        }else{
            return true;
        }
