...
public class IteratorUtils {
     ...
     * @param list 入参list
     * @param partitionSize 每批多少数据
     * @param function 封装处理方法
     * @param <R>      要分区的对象类型
     * @param <T>      分区后的对象类型
     * @return java.util.List<T> 返回数据
     **/
    public static <R,T> List<T> partition(
            List<R> list, int partitionSize, Function<List<R>, CompletableFuture<List<T>>> function) {
        List<List<R>> goodsIdList = Lists.partition(list, partitionSize);
        List<CompletableFuture<List<T>>> futures =
                goodsIdList.parallelStream().map(function).collect(Collectors.toList());
