...
/*    private static final byte setLoNibble(byte dest, int b)
    ...
    protected final void next() {
        mhiTag = (System.currentTimeMillis() + (JVMHASH * 4294967296L))
                ^ MACHINEID;
        long nextLong = M_RANDOM.nextLong();
        while (nextLong == Long.MIN_VALUE) {
            // Math.abs(Long.MIN_VALUE) 会返回其自身(Integer.MIN_VALUE同理)，无法得到预期的绝对值
            // 可能对生成的UUID产生影响，这里特殊处理一下
            // 若 nextLong 随机到了 Long.MIN_VALUE，则重新随机 非Long.MIN_VALUE 的值
            nextLong = M_RANDOM.nextLong();
        }
        mloTag = EPOCH + Math.abs(nextLong);
        muuid = toString(this.toByteArray());
    }
