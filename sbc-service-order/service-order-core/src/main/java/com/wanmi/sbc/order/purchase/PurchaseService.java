...
public class PurchaseService {
    ...
    @Autowired
    protected GoodsCustomerFollowRepository goodsCustomerFollowRepository;
    ...
    @Autowired
    protected GoodsInfoSpecDetailRelQueryProvider goodsInfoSpecDetailRelQueryProvider;

    @Autowired
    protected GoodsIntervalPriceProvider goodsIntervalPriceProvider;

    @Autowired
    protected MarketingPluginProvider marketingPluginProvider;
    ...
    @Autowired
    protected MarketingQueryProvider marketingQueryProvider;
    ...
    @Autowired
    protected CouponCacheProvider couponCacheProvider;

    @Autowired
    protected CompanyInfoQueryProvider companyInfoQueryProvider;


    @Resource
    protected DistributorGoodsInfoQueryProvider distributorGoodsInfoQueryProvider;

    @Autowired
    protected GoodsPriceAssistProvider goodsPriceAssistProvider;

    @Autowired
    protected DistributionSettingQueryProvider distributionSettingQueryProvider;
    ...
    @Autowired
    protected GoodsRestrictedSaleQueryProvider goodsRestrictedSaleQueryProvider;

    @Autowired
    protected GoodsCommonQueryProvider goodsCommonQueryProvider;
    ...
    @Autowired
    protected LinkedMallStockQueryProvider linkedMallStockQueryProvider;

    @Autowired
    protected ThirdAddressQueryProvider thirdAddressQueryProvider;
    ...
    @Autowired
    protected DistributionCacheService distributionCacheService;
    ...
    public void save(PurchaseRequest request) {
        ...
        if (ThirdPlatformType.LINKED_MALL.equals(goodsInfo.getThirdPlatformType())) {
            ...
            if (stocks != null) {
                for (LinkedMallStockVO spuStock : stocks) {
                    Optional<LinkedMallStockVO.SkuStock> stock = spuStock.getSkuList().stream().filter(v -> String.valueOf(spuStock.getItemId()).equals(thirdPlatformSpuId) && String.valueOf(v.getSkuId()).equals(thirdPlatformSkuId)).findFirst();
                    if (stock.isPresent()) {
        ...
        if (goodsInfo.getPluginType() == PluginType.O2O) {
            goodsInfo.setStoreId(request.getStoreId());
            goodsInfo = stockService.setStock(goodsInfo);
        }
        if (request.getVerifyStock() && goodsInfo.getStock() < request.getGoodsNum()) {
            throw new SbcRuntimeException(GoodsErrorCodeEnum.K030062, new Object[]{goodsInfo.getStock()});
        }

        if(goodsInfo.getAddedFlag() == AddedFlag.NO.toValue() || goodsInfo.getAuditStatus() != CheckStatus.CHECKED) {
        ...
        sort(PurchaseRequest.builder().customerId(request.getCustomerId()).goodsInfoIds(Collections.singletonList(request.getGoodsInfoId())).build());
    }
    ...
    public void batchSave(PurchaseRequest request) {
        List<String> goodsInfoIds =
                request.getGoodsInfos().stream().map(GoodsInfoDTO::getGoodsInfoId).collect(Collectors.toList());
        ...
        Integer countNum = countGoods(request.getCustomerId(), request.getInviteeId());
        //获取每一个商品要购买的数量
        Map<String, Long> goodsBuycount = request.getGoodsInfos().stream().collect(Collectors.toMap(GoodsInfoDTO::getGoodsInfoId, GoodsInfoDTO::getBuyCount));

        // 查询供应上商品的库存
        Map<String, Long> providerStock = new HashMap<>();
        if (request.getVerifyStock()) {
            List<String> providerSkuIds =
                    goodsInfoList.stream()
                            .filter(
                                    goodsInfoVO ->
                                            StringUtils.isNotBlank(
                                                    goodsInfoVO.getProviderGoodsInfoId()))
                            .map(GoodsInfoVO::getProviderGoodsInfoId)
                            .collect(Collectors.toList());
            if (CollectionUtils.isNotEmpty(providerSkuIds)) {
                List<GoodsInfoVO> providerGoodsInfoList = goodsInfoQueryProvider.listByCondition(
                        GoodsInfoListByConditionRequest.builder().delFlag(Constants.no)
                                .goodsInfoIds(providerSkuIds)
                                .build()
                ).getContext().getGoodsInfos();
                if (CollectionUtils.isNotEmpty(providerGoodsInfoList)) {
                    providerStock = providerGoodsInfoList.stream().collect(Collectors.toMap(GoodsInfoVO::getGoodsInfoId, GoodsInfoVO::getStock));
                }
            }
        }

        Map<String, Long> finalProviderStock = providerStock;
        goodsInfoList.forEach(
                goods -> {
                    //验证上下架、删除
                    if(!Integer.valueOf(1).equals(goods.getAddedFlag()) ||
                            !DeleteFlag.NO.equals(goods.getDelFlag()) ||
                            !CheckStatus.CHECKED.equals(goods.getAuditStatus())){
                        throw new SbcRuntimeException(
                                CommonErrorCodeEnum.K000008);
                    }
                    // 验证库存
                    if (request.getVerifyStock()) {
                        ...
                        if (StringUtils.isNotBlank(goods.getProviderGoodsInfoId())) {
                            stock =
                                    finalProviderStock.containsKey(goods.getProviderGoodsInfoId())
                                                    && Objects.nonNull(
                                                            finalProviderStock.get(
                                                                    goods.getProviderGoodsInfoId()))
                                            ? finalProviderStock.get(goods.getProviderGoodsInfoId())
                                            : 0L;
                        }
                        if (stock < goodsBuycount.get(goods.getGoodsInfoId())) {
                            throw new SbcRuntimeException(
                                    GoodsErrorCodeEnum.K030062, new Object[] {goods.getStock()});
                        }
                    }
                    Purchase purchases =
                            purchaseList.stream()
                                    .map(
                                            purchase -> {
                                                if (goods.getGoodsInfoId()
                                                        .equals(purchase.getGoodsInfoId())) {
                                                    return purchase;
                                                }
                                                return null;
                                            })
                                    ...
                                    .findFirst()
                                    .orElse(null);
                    request.getGoodsInfos()
                            .forEach(
                                    info -> {
                                        if (info.getBuyCount() <= 0) {
                                            throw new SbcRuntimeException(
                                                    CommonErrorCodeEnum.K000009);
                                        }
                                        if (goods.getGoodsInfoId().equals(info.getGoodsInfoId())) {
                                            request.setGoodsNum(info.getBuyCount());
                                        }
                                    });
                    request.setFirstPurchasePrice(goods.getMarketPrice());
                    if (purchases != null) {
                        // 商品详情页加入购物车不需要校验购物车中已选商品数量
                        //                if (goods.getStock() < (request.getGoodsNum() +
                        // purchases.getGoodsNum())) {
                        //                    throw new SbcRuntimeException(GoodsErrorCodeEnum.K030062, new
                        // Object[]{goods.getStock()});
                        //                }
                        updatePurchase(purchases, request, goods);
                    } else {
                        List<GoodsInfoDTO> goodsInfos =
                                request.getGoodsInfos().stream()
                                        .filter(
                                                info ->
                                                        info.getGoodsInfoId()
                                                                .equals(goods.getGoodsInfoId()))
                                        .collect(Collectors.toList());
     ...
     */
    @Transactional
    public BaseResponse batchSaveNew(PurchaseRequest request) {
        List<String> goodsInfoIds =
                request.getGoodsInfos().stream().map(GoodsInfoDTO::getGoodsInfoId).collect(Collectors.toList());
        List<GoodsInfoVO> goodsInfoList = goodsInfoQueryProvider.listByCondition(
                GoodsInfoListByConditionRequest.builder().delFlag(Constants.no)
        ...
        if (CollectionUtils.isEmpty(goodsInfoList)) {
            return BaseResponse.info(OrderErrorCodeEnum.K050116.getCode(), "加购失败");
        }
        ...
        Map<String, Long> finalProviderStock = providerStock;
        // 库存不足商品
        List<String> errorGoods = new ArrayList<>();
        List<String> outNum = new ArrayList<>();
        for (GoodsInfoVO goods : goodsInfoList) {
            // 验证商品有效性
            if(goods.getAddedFlag() == AddedFlag.NO.toValue() || goods.getAuditStatus() != CheckStatus.CHECKED ||
                    !DeleteFlag.NO.equals(goods.getDelFlag())) {
                errorGoods.add(goods.getGoodsInfoId());
                continue;
            }
            // 验证库存
            Long stock = goods.getStock();
            ...
            request.setFirstPurchasePrice(goods.getMarketPrice());
            if (purchases != null) {
                this.updatePurchase(purchases, request, goods);
            } else {
                List<GoodsInfoDTO> goodsInfos =
                        request.getGoodsInfos().stream()
                                .filter(
                ...
                try{
                    this.addPurchase(goods, countNum, request);
                    countNum++;
                } catch (Exception e) {
                    if (e instanceof SbcRuntimeException) {
                        if(OrderErrorCodeEnum.K050031.getCode().equals(((SbcRuntimeException) e).getErrorCode())) {
                            outNum.add(goods.getGoodsInfoId());
                        } else {
                            throw e;
                        }
                    } else {
        ...
        request.setGoodsInfoIds(goodsInfoIds);
        sort(request);
        if (CollectionUtils.isNotEmpty(errorGoods)) {
            if (errorGoods.size() == goodsInfoIds.size()) {
                return BaseResponse.info(OrderErrorCodeEnum.K050115.getCode(), "加购失败");
            } else {
                return BaseResponse.info(OrderErrorCodeEnum.K050115.getCode(), "部分商品不可加购");
            }
        }
        if(CollectionUtils.isNotEmpty(outNum)) {
            return BaseResponse.info(OrderErrorCodeEnum.K050031.getCode(), "购物车容量达到上限！");
        }
    ...
    public void mergePurchase(PurchaseMergeRequest request) {
        ...
        goodsInfoList.forEach(goods -> {
            ...
            if (purchaseOptional.isPresent()) {
                ...
                this.updatePurchase(purchase, PurchaseRequest.builder().goodsNum(goodsNum).isCover(Boolean.TRUE).build(), goods);
...
//                        .goodsNum(goodsNum).isCover(true).build());
        ...
        if (goodsInfos.size() > 0) {
            ...
            sort(PurchaseRequest.builder().customerId(customer.getCustomerId()).goodsInfoIds(request.getPurchaseMergeDTOList().stream().map(PurchaseMergeDTO::getGoodsInfoId).collect(Collectors.toList())).build());
        }
    }

    private void sort(PurchaseRequest request){
        ListByCustomerIdAndGoodsInfoIdResp resp =
                goodsMarketingQueryProvider.queryByCustomerIdAndGoodsInfoId(ListByCustomerIdAndGoodsInfoIdReq.builder()
                        .customerId(request.getCustomerId()).goodsInfoIds(request.getGoodsInfoIds())
                        .build()).getContext();
        Map<Long, List<GoodsMarketingVO>> listMap =
                resp.getGoodsMarketings().stream().collect(Collectors.groupingBy(GoodsMarketingVO::getMarketingId));
        listMap.forEach((k, v) -> {
            if (CollectionUtils.isNotEmpty(v)) {
                List<String> goodsInfoIds =
                        v.stream().map(GoodsMarketingVO::getGoodsInfoId).collect(Collectors.toList());
                List<Purchase> purchases = purchaseRepository.queryByGoodsInfoIdAndCustomerId(goodsInfoIds,
                        request.getCustomerId());
                for (int i = 0; i < purchases.size(); i++) {
                    Purchase item = purchases.get(i);
    ...
    private void addPurchase(GoodsInfoVO goodsInfo, Integer countNum, PurchaseRequest request) {
        if (GoodsType.REAL_GOODS.toValue() != goodsInfo.getGoodsType() || Constants.yes.equals(goodsInfo.getIsBuyCycle())) {
            throw new SbcRuntimeException(OrderErrorCodeEnum.K050117);
        }
        ...
        if (Objects.nonNull(purchase)) {
            ...
            purchase.setOrderSort(0);
            purchase.setGoodsNum(request.getGoodsNum());
            //判断第一次加入购物车的价格是否存在/是否为空,如果不存/为空在则更新价格
            if (purchase.getFirstPurchasePrice() == null){
                purchase.setFirstPurchasePrice(request.getFirstPurchasePrice());
            }
            purchaseRepository.save(purchase);
            return;
        }
        if (countNum >= Constants.PURCHASE_MAX_SIZE) {
            throw new SbcRuntimeException(OrderErrorCodeEnum.K050031, new Object[]{Constants.PURCHASE_MAX_SIZE});
        }
...
//        purchase.setBrandId(goodsInfo.getBrandId());
        ...
        purchase.setOrderSort(0);
        purchase.setPluginType(goodsInfo.getPluginType());
        //加入购物车的价格
        purchase.setFirstPurchasePrice(request.getFirstPurchasePrice());
        purchaseRepository.save(purchase);
    }
     ...
     * @param request
     */
    private void updatePurchase(Purchase purchase, PurchaseRequest request, GoodsInfoVO goods) {
        if (Constants.yes.equals(goods.getIsBuyCycle())) {
            throw new SbcRuntimeException(OrderErrorCodeEnum.K050117);
        }
    ...
    public MiniPurchaseResponse miniListFront(PurchaseFrontMiniRequest frontReq) {
        ...
        if (TerminalSource.PC.equals(frontReq.getTerminalSource())) {
            skuIdList = goodsInfoQueryProvider
                    .filterCycleGoodsInfoId(GoodsInfoFilterCycleRequest.builder().goodsInfoIds(skuIdList).build())
        ...
        // 按照前端传入的采购单顺序进行排序
        List<String> finalSkuIdList = skuIdList;
        goodsInfoList.sort(Comparator.comparingInt((goods) -> finalSkuIdList.indexOf(goods.getGoodsInfoId())));
...
//                .getGoodsInfoVOList());
        ...
        // 第三方平台填充状态
        thirdPlatformTradeService.fillCart(response.getGoodsInfos(), null);
    ...
    public MiniPurchaseResponse miniList(PurchaseRequest request, CustomerDTO customer) {
        ...
        request.setPageSize(Constants.PURCHASE_MAX_SIZE);
        ...
        if (TerminalSource.PC.equals(request.getTerminalSource())) {
            cycleIds = idsResponse.getGoodsInfos().stream()
                    .filter(goodsInfoVO -> Constants.yes.equals(goodsInfoVO.getIsBuyCycle()))
                    .map(GoodsInfoVO::getGoodsInfoId)
                    .distinct()
                    .collect(Collectors.toList());
        }
        List<String> finalCycleIds = cycleIds;
        List<Purchase> purchaseList = purchases.stream()
                .filter(purchase -> !finalCycleIds.contains(purchase.getGoodsInfoId())).limit(Constants.FIVE)
                .collect(Collectors.toList());
        List<String> purchaseGoodsInfoIds = purchaseList.stream().map(Purchase::getGoodsInfoId).collect(Collectors.toList());
        List<GoodsInfoVO> goodsInfoVOList = idsResponse.getGoodsInfos().stream()
                .filter(goodsInfoVO -> purchaseGoodsInfoIds.contains(goodsInfoVO.getGoodsInfoId()))
                .collect(Collectors.toList());


        //计算区间价 - 1052087 购物车重构  mini购物车不在展示商品价格
//        GoodsIntervalPriceByCustomerIdRequest priceRequest = new GoodsIntervalPriceByCustomerIdRequest();
...
//        miniPurchaseResponse.setGoodsIntervalPrices(iepResponse.getGoodsIntervalPrices());
        //填充SKU的购买数
        this.fillBuyCount(idsResponse.getGoodsInfos(), purchaseList);
        // 第三方平台填充状态
        thirdPlatformTradeService.fillCart(idsResponse.getGoodsInfos(), null);
        ...
        purchaseList.forEach(purchase -> {
            idsResponse.getGoodsInfos().forEach(info -> {
                if (purchase.getGoodsInfoId().equals(info.getGoodsInfoId())) {
                    ...
                    if (purchaseGoodsReponse.getPayMemberOwnFlag()) {
                        purchaseGoodsReponse.setSalePrice(purchaseGoodsReponse.getPayMemberPrice());
                    }
        ...
        miniPurchaseResponse.setGoodsList(purchaseGoodsReponseList);
        Integer count;
        if(request.getPluginTypeFlag() == BoolFlag.YES) {
            count = purchaseRepository.countByCustomerIdAndPluginTypeNotExist(customer.getCustomerId(),PluginType.O2O, request.getInviteeId()) - cycleIds.size();
        } else {
            count = countGoods(customer.getCustomerId(), request.getInviteeId()) - cycleIds.size();
        }
        Long num;
        if (CollectionUtils.isNotEmpty(cycleIds)) {
            num = purchaseRepository.queryGoodsNum(customer.getCustomerId(), request.getInviteeId(), cycleIds);
        } else {
            num = purchaseRepository.queryGoodsNum(customer.getCustomerId(), request.getInviteeId());;
        }
        miniPurchaseResponse.setPurchaseCount(count);
        miniPurchaseResponse.setNum(num);
...
//        }).collect(Collectors.toList());
    ...
    private GoodsInfoViewByIdsResponse getPurchaseGoodsResponse(GoodsInfoRequest goodsInfoRequest) throws SbcRuntimeException {
        ...
        if (!CommonErrorCodeEnum.K000000.getCode().equals(response.getCode())) {
            if (GoodsErrorCodeEnum.K030035.getCode().equals(response.getCode())) {
                return null;
            } else {
                throw new SbcRuntimeException();
            }
...
//                            goodsInfo.getGoodsId(), goodsInfo.getGoodsInfoId());
     ...
     * @return
     */
    public List<Purchase> queryPurchase(String customerId, List<String> goodsInfoIds, String inviteeId,boolean isO2O) {
        //分页查询SKU信息列表
        PurchaseRequest request = PurchaseRequest.builder()
                .customerId(customerId)
                .goodsInfoIds(goodsInfoIds)
                .inviteeId(inviteeId)
                .pluginTypeFlag(BoolFlag.YES)
                .build();
        if(isO2O) {
            request.setPluginTypes(Collections.singletonList(PluginType.O2O));
        }else{
            List<PluginType> pluginTypes = Arrays.stream(PluginType.values()).filter(p -> !p.equals(PluginType.O2O)).collect(Collectors.toList());
            request.setPluginTypes(pluginTypes);
        }
        Map<String, String> map = new LinkedHashMap<>();
        map.put("updateTime", "desc");
        map.put("orderSort", "asc");
        request.setSortMap(map);
        Sort sort = request.getSort();
        return Objects.nonNull(sort)
                ? purchaseRepository.findAll(request.getWhereCriteria(), sort)
                : purchaseRepository.findAll(request.getWhereCriteria());
    }
     ...
     * @return
     */
    public List<Purchase> queryPurchase(PurchaseInfoRequest request) {
        // 分页查询SKU信息列表
        PurchaseRequest dbRequest =
                PurchaseRequest.builder()
                        ...
                        .pluginTypeFlag(BoolFlag.YES)
                        .pluginTypes(request.getPluginTypes())
                        .build();
        ...
        if (StringUtils.isNotBlank(dbRequest.getCustomerId())) {
            wheresql.append(" and p.customer_id = :customerId");
        }
        if (CollectionUtils.isNotEmpty(dbRequest.getGoodsInfoIds())) {
            wheresql.append(" and p.goods_info_id in (:goodsInfoIds)");
        }
        if (StringUtils.isNotBlank(dbRequest.getInviteeId())) {
            wheresql.append(" and p.invitee_id = :inviteeId");
        }
        if (CollectionUtils.isNotEmpty(dbRequest.getPluginTypes())) {
            wheresql.append(" and plugin_type in (:pluginTypes)");
        }

        wheresql.append(" order by p.update_time DESC,order_sort ASC");
        Query query = entityManager.createNativeQuery(sql.append(wheresql).toString());
        //组装查询参数
        if (StringUtils.isNotBlank(dbRequest.getCustomerId())) {
            query.setParameter("customerId", dbRequest.getCustomerId());
        }
        if (CollectionUtils.isNotEmpty(dbRequest.getGoodsInfoIds())) {
            query.setParameter("goodsInfoIds", dbRequest.getGoodsInfoIds());
        }
        if (StringUtils.isNotBlank(dbRequest.getInviteeId())) {
            query.setParameter("inviteeId", dbRequest.getInviteeId());
        }
        if (CollectionUtils.isNotEmpty(dbRequest.getPluginTypes())) {
            List<Integer> types = dbRequest.getPluginTypes().stream().map(t->t.toValue()).collect(Collectors.toList());
            query.setParameter("pluginTypes", types);
        }

        query.unwrap(NativeQuery.class).addEntity("p", Purchase.class);
...
//            purchase.setBrandId(goodsInfo.getBrandId());
     ...
     */
    @Transactional
    public int delete(PurchaseRequest request) {
        int result = purchaseRepository.deleteByGoodsInfoids(request.getGoodsInfoIds(), request.getCustomerId(),
                request.getInviteeId());
    ...
    public void addFollow(PurchaseRequest queryRequest) {
        ...
        if (CollectionUtils.isNotEmpty(existSku)) {
            ...
            if (CollectionUtils.isNotEmpty(newSkuIds) && existSkuIds.size() + newSkuIds.size() > Constants.FOLLOW_MAX_SIZE) {
                throw new SbcRuntimeException(GoodsErrorCodeEnum.K030147, new Object[]{Constants.FOLLOW_MAX_SIZE});
            }
        } else {
        ...
        GoodsInfoListByIdsResponse goodsInfoListByIdsResponse = new GoodsInfoListByIdsResponse();
        Map<String, GoodsInfoVO> isBuyCycleMap = new HashMap<>();

        if(CollectionUtils.isNotEmpty(newSkuIdsBack)) {
            goodsInfoListByIdsResponse = goodsInfoQueryProvider.getGoodsInfoByIds(
                    GoodsInfoListByIdsRequest.builder().goodsInfoIds(newSkuIdsBack).build()
            ).getContext();
        }
        if(CollectionUtils.isNotEmpty(goodsInfoListByIdsResponse.getGoodsInfos())){
            isBuyCycleMap = goodsInfoListByIdsResponse.getGoodsInfos().stream().collect(Collectors.toMap(
                    GoodsInfoVO::getGoodsInfoId, Function.identity()
            ));
        }

        Map<String, GoodsInfoVO> finalIsBuyCycleMap = isBuyCycleMap;
        purchaseList.stream().forEach(info -> {
            ...
            follow.setIsBuyCycle(finalIsBuyCycleMap.getOrDefault(info.getGoodsInfoId(), new GoodsInfoVO()).getIsBuyCycle());
     ...
     */
    @Transactional
    public void clearLoseGoods(CustomerVO customerVO, DistributeChannel distributeChannel,Long storeId) {
...
//        customerCommonService.
        ...
        thirdPlatformTradeService.fillCart(goodsInfoList, null);
        List<Long> storeIds;
        if (Objects.nonNull(storeId)) {
            if (CollectionUtils.isNotEmpty(goodsInfoList)) {
                goodsInfoList = goodsInfoList.parallelStream().filter(goodsInfoVO -> goodsInfoVO.getPluginType() == PluginType.O2O).map(goodsInfoVO -> {
                    goodsInfoVO.setStoreId(storeId);
                    return stockService.setStock(goodsInfoVO);
                }).collect(Collectors.toList());
            }
        ...
        List<GoodsInfoVO> finalGoodsInfoList = goodsInfoList;
        purchaseList.forEach(item -> {
            GoodsInfoVO goodsInfo = finalGoodsInfoList.stream().filter(goods -> goods.getGoodsInfoId().equals(item
            ...
            if (Objects.nonNull(goodsInfo)) {
                ...
                if (Objects.nonNull(store)) {
                    ...
                    if (goodsInfo.getAddedFlag() == 0 || goodsInfo.getDelFlag() == DeleteFlag.YES || goodsInfo.getAuditStatus() == CheckStatus.FORBADE || goodsInfo.getGoodsStatus() == GoodsStatus.INVALID
                            || store.getStoreState() == StoreState.CLOSED || duration.toMinutes() >= 0 || buildGoodsInfoVendibility(goodsInfo).equals(Constants.no)) {
     ...
     * @return
     */
    protected List<GoodsInfoVO> setRestrictedNum(List<GoodsInfoVO> goodsInfoVOS,
    ...
    public PurchaseMarketingCalcResponse calcMarketingByMarketingIdBase(Long marketingId,
        ...
        // 判断会员等级是否满足要求
        //校验营销活动
        if (marketingResponse.getIsPause() == BoolFlag.YES || marketingResponse.getDelFlag() == DeleteFlag.YES
                || marketingResponse.getBeginTime().isAfter(LocalDateTime.now())
        ...
        if (Objects.nonNull(customer)) {
            ...
            //兼容满返
            if (Constants.BOSS_DEFAULT_STORE_ID.equals(marketingResponse.getStoreId())){
                levelMap =
                        customerLevelQueryProvider.listCustomerLevelMapByCustomerId(
                                CustomerLevelMapByCustomerIdAndStoreIdsRequest.builder()
                                        .customerId(customer.getCustomerId())
                                        .storeIds(Collections.singletonList(marketingResponse.getStoreId()))
                                        .build()).getContext().getCommonLevelVOMap();
            } else {
                levelMap = this.getLevelMapByStoreIds(Collections.singletonList(marketingResponse.getStoreId()), customer);
            }
            //校验用户级别
        ...
        if (!valid) {
            throw new SbcRuntimeException(CommonErrorCodeEnum.K999999, marketingResponse.getMarketingName() + "无效！");
        }
        List<GoodsInfoVO> goodsInfoList = Collections.EMPTY_LIST;
        long lackCount;
        BigDecimal lackAmount;
        BigDecimal discount = BigDecimal.ZERO;
        // 采购单勾选的商品中没有参与该营销的情况，取营销的最低等级
        if (isPurchase && (goodsInfoIds == null || goodsInfoIds.isEmpty())) {
            // 满金额
            if (marketingResponse.getSubType() == MarketingSubType.REDUCTION_FULL_AMOUNT
                    || marketingResponse.getSubType() == MarketingSubType.DISCOUNT_FULL_AMOUNT
                    || marketingResponse.getSubType() == MarketingSubType.GIFT_FULL_AMOUNT
                    || marketingResponse.getSubType() == MarketingSubType.PREFERENTIAL_FULL_AMOUNT
                ) {
                ...
                switch (marketingResponse.getMarketingType()) {
                    ...
                    case GIFT:
                        ...
                        break;
                    case RETURN:
                        levelAmount = marketingResponse.getFullReturnLevelList().get(0).getFullAmount();
                        response.setFullReturnLevel(marketingResponse.getFullReturnLevelList().get(0));
                        response.setFullReturnLevelList(marketingResponse.getFullReturnLevelList());
                        break;
                    case PREFERENTIAL:
                        levelAmount = marketingResponse.getPreferentialLevelList().get(0).getFullAmount();
                        response.setPreferentialLevel(marketingResponse.getPreferentialLevelList().get(0));
                        response.setPreferentialLevelList(marketingResponse.getPreferentialLevelList());
                        break;
                    default:
                ...
                response.setLack(levelAmount);
                    ...
                    || marketingResponse.getSubType() == MarketingSubType.PREFERENTIAL_FULL_COUNT
                ) {
                // 满数量
                // 计算达到营销级别的数量
                Long levelCount;
                switch (marketingResponse.getMarketingType()) {
                    ...
                    case HALF_PRICE_SECOND_PIECE:
                        ...
                        break;
                    case PREFERENTIAL:
                        levelCount = marketingResponse.getPreferentialLevelList().get(0).getFullCount();
                        response.setPreferentialLevel(marketingResponse.getPreferentialLevelList().get(0));
                        response.setPreferentialLevelList(marketingResponse.getPreferentialLevelList());
                        break;
                    default:
                ...
                if (goodsInfoResponse.getGoodsInfos() != null) {
                    goodsInfoResponse.getGoodsInfos().forEach((goodsInfo) -> {
                        ...
                        List<PurchaseGoodsInfoDTO> dtoList =
                                frontReq.getGoodsInfoDTOList().stream()
                                        .filter((purchase) -> goodsInfo.getGoodsInfoId().equals(purchase.getGoodsInfoId()))
                                        .collect(Collectors.toList());
                        if (CollectionUtils.isNotEmpty(dtoList)) {
                            goodsInfo.setBuyCount((dtoList.get(0)).getGoodsNum());
                        }
                ...
                if (!isPurchase) {
                    if (customer != null) {
                        ...
                        goodsMarketingList = frontReq.getGoodsMarketingDTOList().stream().map((dto) ->
                                GoodsMarketingVO.builder()
                                        .marketingId(dto.getMarketingId())
                                        .goodsInfoId(dto.getGoodsInfoId())
                                        .build()
                        ).collect(Collectors.toList());
                    }
                    ...
                    // 凑单页，过滤出参加营销的商品
                    Stream<GoodsInfoVO> goodsInfoVOStream = goodsInfoResponse.getGoodsInfos().stream()
                            .filter(goodsInfo -> {
                                if (goodsInfo.getGoodsStatus() != GoodsStatus.OK) {
                                    return false;
                                }
                                if (marketingResponse.getScopeType() == MarketingScopeType.SCOPE_TYPE_CUSTOM) {
                                    return marketingResponse.getMarketingScopeList().stream().
                                            anyMatch(scope -> scope.getScopeId().equals(goodsInfo.getGoodsInfoId()));
                                }
                                return true;
                            });
                    if (MarketingType.PREFERENTIAL.equals(marketingResponse.getMarketingType())){
                        MarketingVO marketingVO =
                                marketingQueryProvider.getById(marketingGetByIdRequest).getContext().getMarketingVO();
                        MarketingScopeByMarketingIdResponse marketingScopeByMarketingIdResponse =
                                marketingScopeQueryProvider.listByMarketingId(new MarketingScopeByMarketingIdRequest(marketingId)).getContext();
                        List<String> scopeIds =
                                marketingScopeByMarketingIdResponse.getMarketingScopeVOList().stream()
                                        .map(MarketingScopeVO::getScopeId).collect(Collectors.toList());
                        switch (marketingVO.getScopeType()){
                            case SCOPE_TYPE_ALL:
                                goodsInfoList = goodsInfoVOStream.peek(goodsInfoVO -> {
                                    boolean flag = !Objects.equals(goodsInfoVO.getStoreId(), marketingResponse.getStoreId());
                                    if (flag){
                                        goodsInfoVO.setBuyCount(0L);
                                    }
                                }).collect(Collectors.toList());
                                break;
                            case SCOPE_TYPE_CUSTOM:
                                goodsInfoList = goodsInfoVOStream.peek(goodsInfoVO -> {
                                    boolean flag = !scopeIds.contains(goodsInfoVO.getGoodsInfoId());
                                    if (flag){
                                        goodsInfoVO.setBuyCount(0L);
                                    }
                                }).collect(Collectors.toList());
                            ...
                            case SCOPE_TYPE_BRAND:
                                goodsInfoList = goodsInfoVOStream.peek(goodsInfoVO -> {
                                    boolean flag = !scopeIds.contains(goodsInfoVO.getBrandId().toString());
                                    if (flag){
                                        goodsInfoVO.setBuyCount(0L);
                                    }
                                }).collect(Collectors.toList());
                            ...
                            case SCOPE_TYPE_STORE_CATE:
                                List<String> goodsIds =
                                        goodsInfoResponse.getGoodsInfos().stream().map(GoodsInfoVO::getGoodsId).collect(Collectors.toList());
                                ...
                                List<StoreCateGoodsRelaVO> storeCateGoodsRelaVOList =
                                        storeCateGoodsRelaQueryProvider.listByGoodsIds(storeCateGoodsRelaListByGoodsIdsRequest).getContext().getStoreCateGoodsRelaVOList();
                                Map<String, List<Long>> goodsIdToStoreCateIdMap =
                                        storeCateGoodsRelaVOList.stream().collect(Collectors.groupingBy(StoreCateGoodsRelaVO::getGoodsId,
                                                Collectors.mapping(StoreCateGoodsRelaVO::getStoreCateId, Collectors.toList())));
                                goodsInfoList = goodsInfoVOStream.peek(goodsInfoVO -> {
                                    List<Long> storeCateId = goodsIdToStoreCateIdMap.get(goodsInfoVO.getGoodsId());
                                    boolean flag = false;
                                    if (CollectionUtils.isNotEmpty(storeCateId)){
                                        flag = storeCateId.stream().anyMatch(g -> scopeIds.contains(g.toString()));
                                    }
                                    if (!flag){
                                        goodsInfoVO.setBuyCount(0L);
                                    }
                        ...
                        goodsInfoList = goodsInfoVOStream.peek(goodsInfo -> {
            ...
            goodsInfoList.forEach(goodsInfo -> {
                ...
                if (Objects.nonNull(customer) && customer.getEnterpriseCheckState() == EnterpriseCheckState.CHECKED
                    ...
                    VASEntity vasEntity = VASList.stream()
                            .filter(f -> StringUtils.equals(f.getServiceName().toValue(), VASConstants.VAS_IEP_SETTING.toValue())
                                    && f.isServiceStatus())
                ...
                if (IEPFlag) {
                    ...
                    Optional<GoodsIntervalPriceVO> optional = goodsIntervalPrices.stream().filter(goodsIntervalPrice
                            ...
                            .filter(goodsIntervalPrice -> goodsInfo.getBuyCount().compareTo(goodsIntervalPrice.getCount()) >= 0)
                ...
                } else if (Boolean.TRUE.equals(goodsInfo.getPayMemberOwnFlag())) {
            ...
            BigDecimal totalAmount = goodsInfoList.stream()
                    .map(goodsInfo -> skuSalePriceMap.getOrDefault(goodsInfo.getGoodsInfoId(), BigDecimal.ZERO)
                            .multiply(BigDecimal.valueOf(goodsInfo.getBuyCount())))
            ...
            if (marketingResponse.getSubType() == MarketingSubType.REDUCTION_FULL_AMOUNT
                    || marketingResponse.getSubType() == MarketingSubType.DISCOUNT_FULL_AMOUNT
                    || marketingResponse.getSubType() == MarketingSubType.GIFT_FULL_AMOUNT
                    || MarketingSubType.RETURN == marketingResponse.getSubType()
                    || MarketingSubType.PREFERENTIAL_FULL_AMOUNT.equals(marketingResponse.getSubType())
            ) {
                // 计算达到营销级别的金额
                ...
                switch (marketingResponse.getMarketingType()) {
                    case REDUCTION:
                        Optional<MarketingFullReductionLevelVO> fullReductionLevelOptional =
                                marketingResponse.getFullReductionLevelList().stream()
                                        .filter(level -> level.getFullAmount().compareTo(totalAmount) <= 0)
                    ...
                    case DISCOUNT:
                        Optional<MarketingFullDiscountLevelVO> fullDiscountLevelOptional =
                                marketingResponse.getFullDiscountLevelList().stream()
                                        .filter(level -> level.getFullAmount().compareTo(totalAmount) <= 0)
                    ...
                    case GIFT:
                        List<MarketingFullGiftLevelVO> levels =
                                marketingResponse.getFullGiftLevelList().stream()
                                        .filter(level -> level.getFullAmount().compareTo(totalAmount) <= 0)
                                        .collect(Collectors.toList());
                    ...
                    case RETURN:
                        List<MarketingFullReturnLevelVO> returnLevels =
                                marketingResponse.getFullReturnLevelList().stream()
                                        .filter(level -> level.getFullAmount().compareTo(totalAmount) <= 0)
                                        .collect(Collectors.toList());
                        ...
                        if (fullReturnLevelOptional.isPresent()) {
                            levelAmount = fullReturnLevelOptional.get().getFullAmount();
                            response.setFullReturnLevel(fullReturnLevelOptional.get());
                        } else {
                            // 没有满足条件，默认取最低等级
                            levelAmount = marketingResponse.getFullReturnLevelList().get(0).getFullAmount();
                            response.setFullReturnLevel(marketingResponse.getFullReturnLevelList().get(0));
                        ...
                        break;
                    case PREFERENTIAL:
                        List<MarketingPreferentialLevelVO> preferentialLevels =
                                marketingResponse.getPreferentialLevelList().stream()
                                        .filter(level -> level.getFullAmount().compareTo(totalAmount) <= 0)
                                        .collect(Collectors.toList());
                        Optional<MarketingPreferentialLevelVO> fullPreferentialLevelOptional =
                                preferentialLevels.stream()
                                        .max(Comparator.comparing(MarketingPreferentialLevelVO::getFullAmount));
                        // 满足条件的最大的等级
                        if (fullPreferentialLevelOptional.isPresent()) {
                            levelAmount = fullPreferentialLevelOptional.get().getFullAmount();
                            response.setPreferentialLevel(fullPreferentialLevelOptional.get());
                        }  else {
                            // 没有满足条件，默认取最低等级
                            levelAmount = marketingResponse.getPreferentialLevelList().get(0).getFullAmount();
                            response.setPreferentialLevel(marketingResponse.getPreferentialLevelList().get(0));
                ...
                response.setLack(lackAmount);
                    ...
                    || marketingResponse.getSubType() == MarketingSubType.BUYOUT_PRICE
                    || marketingResponse.getSubType() == MarketingSubType.HALF_PRICE_SECOND_PIECE
                    || MarketingSubType.PREFERENTIAL_FULL_COUNT.equals(marketingResponse.getSubType())
            ) {
                // 满数量
                // 计算达到营销级别的数量
                Long levelCount;
                switch (marketingResponse.getMarketingType()) {
                    case REDUCTION:
                        Optional<MarketingFullReductionLevelVO> fullReductionLevelOptional =
                                marketingResponse.getFullReductionLevelList().stream()
                                        .filter(level -> level.getFullCount().compareTo(totalCount) <= 0)
                    ...
                    case BUYOUT_PRICE:
                        List<MarketingBuyoutPriceLevelVO> buyoutPriceLevels =
                                marketingResponse.getBuyoutPriceLevelList().stream()
                                        .filter(level -> level.getChoiceCount().compareTo(totalCount) <= 0)
                                        .collect(Collectors.toList());
                        MarketingBuyoutPriceLevelVO levelVO = buyoutPriceLevels.stream()
                                .max(Comparator.comparing(MarketingBuyoutPriceLevelVO::getChoiceCount))
                                .orElse(null);
                        if (Objects.nonNull(levelVO)) {
                            ...
                            List<GoodsInfoVO> marketInfoInfoList = goodsInfoList.stream()
                                    .sorted((v1, v2) -> skuSalePriceMap.getOrDefault(v2.getGoodsInfoId(), BigDecimal.ZERO)
                                            .compareTo(skuSalePriceMap.getOrDefault(v1.getGoodsInfoId(), BigDecimal.ZERO)))
                                    .collect(Collectors.toList());
                            ...
                            for (GoodsInfoVO info : marketInfoInfoList) {
                                ...
                                if (number == size) {
                                    noDiscountPrice = noDiscountPrice.add(
                                            skuSalePriceMap.getOrDefault(info.getGoodsInfoId(), BigDecimal.ZERO)
                                            .multiply(BigDecimal.valueOf((mod)))
                                    );
                                } else {
                                    number++;
                                }
                            ...
                            levelCount = marketingResponse.getBuyoutPriceLevelList().get(0).getChoiceCount();
                            response.setBuyoutPrice(marketingResponse.getBuyoutPriceLevelList().get(0).getFullAmount());
                            discount = totalAmount.compareTo(response.getBuyoutPrice()) < 0 ?
                                    BigDecimal.ZERO :
                                    totalAmount.subtract(response.getBuyoutPrice());
                            response.setBuyoutPriceLevel(marketingResponse.getBuyoutPriceLevelList().get(0));
                        }
                        break;
                    case HALF_PRICE_SECOND_PIECE:
                        ...
                        if (halfPriceSecondPieceLevel.getNumber().compareTo(totalCount) <= 0) {
                            //按价格倒序,以便优先以价格高的商品凑单计算优惠金额
                            List<GoodsInfoVO> marketInfoInfoList = goodsInfoList.stream()
                                    .sorted(Comparator.comparing(v -> skuSalePriceMap.getOrDefault(v.getGoodsInfoId(),
                                            BigDecimal.ZERO)))
                                    .collect(Collectors.toList());
                                ...
                                if (marketInfoInfoList.size() < grade) {
                                    if (marketInfoInfoList.size() == 1) {
                                        for (int i = 0; i < grade; i++) {
                                            if (halfPriceSecondPieceLevel.getDiscount().compareTo(BigDecimal.ZERO) == 0) {
                                                ...
                                                halfPriceSecondPiece = halfPriceSecondPiece.add(
                                                        skuSalePriceMap.getOrDefault(
                                                                marketInfoInfoList.get(0).getGoodsInfoId(),
                                                                BigDecimal.ZERO)
                                                );
                                            } else {
                                                //第N件以最小的商品金额计算折扣
                                                halfPriceSecondPiece = halfPriceSecondPiece.add(
                                                        skuSalePriceMap.getOrDefault(
                                                                ...
                                                                (BigDecimal.valueOf(10)
                                                                        .subtract(halfPriceSecondPieceLevel.getDiscount())
                                        ...
                                        for (GoodsInfoVO gradeGoods : marketInfoInfoList) {
                                            if (halfPriceSecondPieceLevel.getDiscount().compareTo(BigDecimal.ZERO) == 0) {
                                                //第N件0折，第N件是买N件送一件，以价格最低的送
                                                halfPriceSecondPiece = halfPriceSecondPiece
                                                        .add(
                                                                skuSalePriceMap.getOrDefault(
                                                                        gradeGoods.getGoodsInfoId(),
                                                                        BigDecimal.ZERO
                                                                )
                                                        );
                                            } else {
                                        ...
                                        for (int i = 0; i < grade - marketInfoInfoList.size(); i++) {
                                            if (halfPriceSecondPieceLevel.getDiscount().compareTo(BigDecimal.ZERO) == 0) {
                                                ...
                                                halfPriceSecondPiece =
                                                        halfPriceSecondPiece.add(
                                                                skuSalePriceMap.getOrDefault(
                                                                        marketInfoInfoList.get(marketInfoInfoList.size() - 1)
                                                                                .getGoodsInfoId(), BigDecimal.ZERO
                                                                )
                                                        );
                                            } else {
                                                //第N件以最小的商品金额计算折扣
                                                halfPriceSecondPiece =
                                                        halfPriceSecondPiece.add(
                                                                skuSalePriceMap.getOrDefault(marketInfoInfoList.get(marketInfoInfoList.size() - 1).getGoodsInfoId(), BigDecimal.ZERO)
                                                                        .multiply((BigDecimal.valueOf(10).subtract(halfPriceSecondPieceLevel.getDiscount()).multiply(BigDecimal.valueOf(0.1))))
                                                                        .setScale(2, RoundingMode.HALF_UP)
                                                        );
                                            }
                                        }
                                    ...
                                    for (GoodsInfoVO gradeGoods : grademarketInfoInfoList) {
                                        if (halfPriceSecondPieceLevel.getDiscount().compareTo(BigDecimal.ZERO) == 0) {
                                            ...
                                            halfPriceSecondPiece =
                                                    halfPriceSecondPiece.add(
                                                            skuSalePriceMap.getOrDefault(gradeGoods.getGoodsInfoId(), BigDecimal.ZERO)
                                                                    .multiply((BigDecimal.valueOf(10).subtract(halfPriceSecondPieceLevel.getDiscount()).multiply(BigDecimal.valueOf(0.1))))
                                                                    .setScale(2, RoundingMode.HALF_UP)
                                                    );
                                        }
                    ...
                    case PREFERENTIAL:
                        List<MarketingPreferentialLevelVO> preferentialLevels =
                                marketingResponse.getPreferentialLevelList().stream()
                                        ...
                                        .collect(Collectors.toList());
                        Optional<MarketingPreferentialLevelVO> fullPreferentialLevelOptional =
                                preferentialLevels.stream().max(Comparator.comparing(MarketingPreferentialLevelVO::getFullCount));
                        // 满足条件的最大的等级
                        if (fullPreferentialLevelOptional.isPresent()) {
                            levelCount = fullPreferentialLevelOptional.get().getFullCount();
                            response.setPreferentialLevel(fullPreferentialLevelOptional.get());
                        } else {
                            // 没有满足条件，默认取最低等级
                            levelCount = marketingResponse.getPreferentialLevelList().get(0).getFullCount();
        ...
        if (response.getFullGiftLevelList() != null && !response.getFullGiftLevelList().isEmpty()) {
            List<String> skuIds =
                    response.getFullGiftLevelList().stream()
        ...
        // 如果有加价购商品，则查询商品的详细信息
        if (CollectionUtils.isNotEmpty(response.getPreferentialLevelList())) {
            List<String> skuIds =
                    response.getPreferentialLevelList().stream()
                            .flatMap(marketingPreferentialLevel ->
                                    marketingPreferentialLevel.getPreferentialDetailList().stream()
                                            .map(MarketingPreferentialGoodsDetailVO::getGoodsInfoId))
                            .distinct().collect(Collectors.toList());
            GoodsInfoViewByIdsRequest goodsInfoRequest = GoodsInfoViewByIdsRequest.builder()
                    .goodsInfoIds(skuIds)
                    .isHavSpecText(Constants.yes)
                    .build();
            response.setPreferentialGoodsInfoResponse(
                    KsBeanUtil.convert(
                            goodsInfoQueryProvider.listViewByIds(goodsInfoRequest).getContext(),
                            PurchaseGoodsViewVO.class
                    )
            );
        }
     ...
     * @return
     */
    protected Map<Long, CommonLevelVO> getLevelMapByStoreIds(List<Long> storeIds, CustomerVO customer) {
    ...
    public Map<Long, List<PurchaseMarketingCalcResponse>> getStoreMarketingBase(List<GoodsMarketingDTO> goodsMarketings,
        ...
        marketingGoodsesMap.entrySet().stream().forEach(set -> {
            ...
            if (customer != null) {
                response = this.calcMarketingByMarketingIdBase(set.getKey(), customer, null,
                        set.getValue(), true);
            } else {
                response = this.calcMarketingByMarketingIdBase(set.getKey(), customer, frontReq,
                        set.getValue(), true);
            }
    ...
    public Map<Long, Boolean> getStoreCouponExist(List<GoodsInfoVO> goodsInfoList, CustomerVO customer,
        ...
        CouponCacheListForGoodsGoodInfoListRequest request = CouponCacheListForGoodsGoodInfoListRequest.builder()
                                                                .customer(customer)
                                                                .goodsInfoList(goodsInfoList)
                                                                .storeId(storeId)
                                                                .pluginType(pluginType).build();

        BaseResponse<CouponCacheListForGoodsDetailResponse> cacheResponse = couponCacheProvider.listCouponForGoodsList(request);

        List<CouponCacheVO> couponCaches = cacheResponse.getContext().getCouponCacheVOList();

        boolean hasPlatformCoupon = couponCaches.stream().anyMatch(marketInfo -> marketInfo.getCouponInfo().getCouponType().equals(CouponType.GENERAL_VOUCHERS));
        ...
        goodsInfoList.stream().map(GoodsInfoVO::getStoreId).forEach(marketInfo -> {
            if (hasPlatformCoupon) {
                result.put(marketInfo, Boolean.TRUE);
            } else {
    ...
    public PurchaseListResponse calcAmount(PurchaseListResponse response, List<String> goodsInfoIds,
        ...
        if (CollectionUtils.isNotEmpty(appointGoodsInfoIds)) {
            ...
            if (Objects.nonNull(appointmentSaleInProcessResponse) && CollectionUtils.isNotEmpty(appointmentSaleInProcessResponse.getAppointmentSaleVOList())) {
        ...
        BigDecimal discountTotalPrice =
                response.getStoreMarketingMap().values().parallelStream().flatMap(Collection::parallelStream).map(storeMarketing -> {
        ...
        Long sumBuyPoint = response.getGoodsInfos().stream()
                ...
                .sum();
        response.setBuyPointValid(Boolean.TRUE);
        if (response.getTotalBuyPoint() != null && response.getTotalBuyPoint() > 0
            ...
            response.setBuyPointValid(Boolean.FALSE);
        }
     ...
     */
    @Transactional
    public void quickOrderAddFollow(PurchaseRequest queryRequest) {
        //获取所有收藏商品
        GoodsCustomerFollowQueryRequest followQueryRequest = GoodsCustomerFollowQueryRequest.builder()
                .customerId(queryRequest.getCustomerId())
                .build();
        ...
        Map<String, GoodsInfoVO> finalIsBuyCycleMap = isBuyCycleMap;
        newSkuIds.stream().forEach(info -> {
            GoodsCustomerFollow follow = new GoodsCustomerFollow();
            follow.setCateId(finalIsBuyCycleMap.getOrDefault(info, new GoodsInfoVO()).getCateId());
            follow.setFollowTime(LocalDateTime.now());
            follow.setFollowFlag(FollowFlag.FOLLOW);
            follow.setCreateTime(LocalDateTime.now());
            follow.setBrandId(finalIsBuyCycleMap.getOrDefault(info, new GoodsInfoVO()).getBrandId());
            follow.setCateTopId(finalIsBuyCycleMap.getOrDefault(info, new GoodsInfoVO()).getCateTopId());
            follow.setCompanyInfoId(finalIsBuyCycleMap.getOrDefault(info, new GoodsInfoVO()).getCompanyInfoId());
            follow.setCustomerId(queryRequest.getCustomerId());
            follow.setGoodsInfoId(finalIsBuyCycleMap.getOrDefault(info, new GoodsInfoVO()).getGoodsInfoId());
            follow.setGoodsId(finalIsBuyCycleMap.getOrDefault(info, new GoodsInfoVO()).getGoodsId());
            follow.setGoodsNum(1L);
            follow.setStoreId(finalIsBuyCycleMap.getOrDefault(info, new GoodsInfoVO()).getStoreId());
            follow.setTerminalSource(TerminalSource.PC);
            follow.setFollowTime(LocalDateTime.now());
            follow.setIsBuyCycle(finalIsBuyCycleMap.getOrDefault(info, new GoodsInfoVO()).getIsBuyCycle());
            goodsCustomerFollowRepository.save(follow);
        });
    }
