 ...
 */
@Service(value = "quickOrderBuy")
@Slf4j
public class TradeQuickOrderConfirmServiceImpl implements TradeConfirmInterface {
    ...
    public void getParams(TradeConfirmParam param) {
        ...
        param.getMarketingMap().entrySet().stream().forEach(m ->{
            m.getValue().stream().forEach(mg -> {
                if(Objects.nonNull(mg.getMarketingId()) &&
                        Arrays.asList(MarketingPluginType.REDUCTION.getId(),
                                MarketingPluginType.DISCOUNT.getId(),
                                MarketingPluginType.GIFT.getId(),
                                MarketingPluginType.BUYOUT_PRICE.getId(),
                                MarketingPluginType.HALF_PRICE_SECOND_PIECE.getId()).contains(mg.getMarketingType())){
                    map.put(m.getKey(),  Long.valueOf(mg.getMarketingId().toString()));
                }
            });
        });
        // 获取购物车中商品选中的营销
        param.setGoodsMarketingMap(map);
        // 获取预售商品
