...
@Service
@Slf4j
public class WxPayAndRefundCallBackService implements PayAndRefundCallBackBaseService {
    ...
    public BaseResponse payCallBack(TradePayOnlineCallBackRequest tradePayOnlineCallBackRequest) {
        ...
        try {
            ...
            //判断当前回调是否是合并支付
            outTradeNo = wxPayResultResponse.getOut_trade_no();
            // 如果是付费会员 回调
            if (outTradeNo.startsWith(GeneratorService._PREFIX_PAY_MEMBER_RECORD_ID)) {
                ...
                try {
                    ...
                    if (wxPayResultResponse.getReturn_code().equals(WXPayConstants.SUCCESS) &&
                        ...
                        PayGatewayConfigResponse payGatewayConfig = payCallBackUtil.getGatewayConfigByGateway(WECHAT_PAY_GET_WAY_INFO_KEY, PayGatewayEnum.WECHAT);
                        ...
                        if (WXPayUtil.isSignatureValid(params, apiKey)) {
                            PayingMemberPayRecord payingMemberPayRecord = payingMemberPayRecordService.findByBusinessId(outTradeNo);
                            ...
                            if (NumberUtils.INTEGER_ONE.equals(payState)) {
                                ...
                                payChannelItemVOList.forEach(payChannelItemVO -> {
                                    if (channelItemSaveRequest.getCode().equals(payChannelItemVO.getCode())) {
                                        ...
                                        payingMemberPayRecord.setChannelItemId(payChannelItemVO.getId().intValue());
                                    }
                                });
                    ...
                    if("JSAPI".equals(wxPayResultResponse.getTrade_type())){
                        try {
                            wxPayUploadShippingInfoService.wxPayUploadShippingInfo(null, wxPayResultResponse, outTradeNo, false, false);
                        }  catch (Exception e) {
            ...
            //执行回调
            try {
                payCallBackEvent(tradePayOnlineCallBackRequest,wxPayResultResponse,businessId,isMergePay,isCreditRepay,payTimeSeries);
                log.info("微信支付异步通知回调end---------");
                // 改成订单发货时调用
                if(wxPayResultResponse.getTrade_type().equals("JSAPI")){
                    //是否发送小程序发货
                    ...
                    if(uploadFlag) {
                        wxPayUploadShippingInfoService.wxPayUploadShippingInfo(trade, wxPayResultResponse, businessId, isMergePay, isCreditRepay);
                    }
                }
            } catch (Exception e) {
    ...
    @Transactional
    @GlobalTransactional
    public void payCallBackEvent(TradePayOnlineCallBackRequest tradePayOnlineCallBackRequest,WxPayResultResponse wxPayResultResponse,
                                 String businessId,boolean isMergePay,boolean isCreditRepay,PayTimeSeries payTimeSeries) throws Exception {
        PayGatewayConfigResponse payGatewayConfig = payCallBackUtil.getGatewayConfigByGateway(WECHAT_PAY_GET_WAY_INFO_KEY, PayGatewayEnum.WECHAT);
        String apiKey = payGatewayConfig.getApiKey();
        //支付回调事件成功
        if (wxPayResultResponse.getReturn_code().equals(WXPayConstants.SUCCESS) &&
            ...
            try{
                ...
                if (WXPayUtil.isSignatureValid(params, apiKey)) {
                    ...
                    List<PayCallBackResult> payCallBackResultList =
                            payCallBackResultService.list(PayCallBackResultQueryRequest.builder().businessId(payTimeSeries.getPayNo()).build());
...
//                                wxRefundHandle(wxPayResultResponse, businessId, -1L);
                            log.error("订单重复支付或过期作废,不直接退款wxPayResultResponse:{}.businessId:{}",wxPayResultResponse,
                                    businessId);
                            this.modifyPayTimeSeries(wxPayResultResponse,payTimeSeries);
                            //throw new SbcRuntimeException(CommonErrorCodeEnum.K000001);
                        } else if (payCallBackResultList.size() == 0 ||
                                (payCallBackResultList.size()>0 && payCallBackResult.getResultStatus() != PayCallBackResultStatus.SUCCESS)) {
                            wxPayCallbackHandle(payGatewayConfig, wxPayResultResponse, businessId, trades, true, false, null,payTimeSeries);
                        }
                    } else if (isCreditRepay) {
                        creditWxPayCallbackHandle(payGatewayConfig, wxPayResultResponse, -1L,payTimeSeries);
                    } else {
                        //单笔支付
                        Trade trade = new Trade();
                        if (payCallBackUtil.isTailPayOrder(businessId)) {
                        ...
                        if (trade.getTradeState().getFlowState() == FlowState.VOID || (trade.getTradeState()
                            ...
                            //同一批订单重复支付或过期作废，直接退款
//                                wxRefundHandle(wxPayResultResponse, businessId, -1L);
                            log.error("订单重复支付或过期作废,不直接退款wxPayResultResponse:{}.businessId:{},storeId:{}",wxPayResultResponse,
                                    businessId);
                            this.modifyPayTimeSeries(wxPayResultResponse,payTimeSeries);
                          //  throw new SbcRuntimeException(CommonErrorCodeEnum.K000001);
                        } else if (payCallBackResultList.size() == 0 ||
                                (payCallBackResultList.size()>0 && payCallBackResult.getResultStatus() != PayCallBackResultStatus.SUCCESS)) {
                            trades.add(trade);
                            wxPayCallbackHandle(payGatewayConfig, wxPayResultResponse, businessId, trades, false, false, null,payTimeSeries);
                        }
                    }
                    //支付回调处理成功
    ...
    public BaseResponse refundCallBack(TradeRefundOnlineCallBackRequest request) {
        ...
        String resultStr = request.getWxRefundCallBackResultStr();
        try {
            log.info("refund:解密后的字符串:{}", resultStr);
            Map<String, String> aesMap = WXPayUtil.xmlToMap(resultStr);
            WxPayRefundCallBackDataResponse dataResponse = (WxPayRefundCallBackDataResponse) WXPayUtil.mapToObject(aesMap, WxPayRefundCallBackDataResponse.class);
            ...
            if (out_refund_no.startsWith(Constants.STR_RT)) {
                returnOrder = returnOrderService.findByCondition(ReturnQueryRequest.builder()
                        .businessTailId(payTradeRecord.getBusinessId()).build()).get(0);
            } else {
            ...
            Operator operator = Operator.builder().ip(HttpUtil.getIpAddr()).adminId("-1").name("WECHAT")
            ...
            payTradeRecordRequest.setPracticalPrice(new BigDecimal(dataResponse.getTotal_fee()).
                    divide(new BigDecimal(100)).setScale(2, RoundingMode.DOWN));
            // 如果payTradeRecord为空将在301或303行报错，此代码将永远不会被执行
...
//        }
        ...
        if (isCreditRepay) {
            ...
            payCallBackUtil.creditRepayCallbackOnline(
                    CreditCallBackRequest.builder()
                            ...
                            .repayType(CreditRepayTypeEnum.WECHAT).build()
            );
        } else {
    ...
    private void creditWxPayCallbackHandle(
        ...
        if (cancel || paid || !correctAmount || returning || finish) {
            //直接退款
            wxRefundHandle(wxPayResultResponse, payTimeSeries.getPayNo(), storeId);
        } else {
            ...
            wxPayCallbackHandle(
                    ...
                    true,
                    true,
                    creditRepayAndOrders.getCustomerCreditRepayVO(),
                    payTimeSeries);
        }
    }
