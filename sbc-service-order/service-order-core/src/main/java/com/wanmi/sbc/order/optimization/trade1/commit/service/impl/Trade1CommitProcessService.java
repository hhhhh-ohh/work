...
import com.wanmi.sbc.customer.api.response.ledgerreceiverrel.LedgerReceiverRelBindStateResponse;
...
import com.wanmi.sbc.order.util.LaKaLaUtils;
...
@Service
@Slf4j
public class Trade1CommitProcessService implements Trade1CommitProcessInterface {
        ...
        param.getSnapshot()
                ...
                .forEach(
                        group -> {
                            ...
                            // 18、构建预售
                            build.buildBooking(trade, tradeCommitRequest, group);
                            ...
                            //21.1 给订单明细设置校徽
                            setBadgeForItems(trade,tradeCommitRequest.getExtendMap());
                            // 22、设置第三方类型
                            ...
                            // 23、设置开店礼包
                            trade.setStoreBagsFlag(group.getStoreBagsFlag());
                            // 24、设置开店礼包分享人
                            trade.setStoreBagsInviteeId(
                                    tradeCommitRequest.getDistributeChannel().getInviteeId());
                            // 25、设置组合购标识
                            trade.setSuitMarketingFlag(group.getSuitMarketingFlag());
                            // 26、设置拼团
                            trade.setTradeGroupon(
                                    build.buildTradeGroupon(
                                            group.getGrouponForm(), group.getTradeItems()));
                            ...
                            // 30 处理视频号订单
                            if (tradeCommitRequest.getIsChannelsFlag()) {
                                // 只可以是先款后货
                            ...
                            // 32、处理砍价订单标识
                            trade.setBargain(snapshot.getBargain());
                            trade.setBargainId(snapshot.getBargainId());
                            ...
                            // 34、周期购业务
                            trade.setTradeBuyCycle(group.getTradeBuyCycleDTO());
                            trades.add(trade);
                        });
        return trades;
    }

    private void setBadgeForItems(Trade trade, Map<String, String> extendMap) {
        if (extendMap == null || extendMap.isEmpty()) {
            return;
        }
        List<TradeItem> tradeItems = trade.getTradeItems();
        for (TradeItem tradeItem : tradeItems) {
            boolean hasKey = extendMap.containsKey(tradeItem.getSkuNo());
            if (hasKey) {
                String badge = extendMap.get(tradeItem.getSkuNo());
                tradeItem.setSchoolBadgeCode(badge);
            }
        }
    }
    ...
    public void priceProcess(List<Trade> tradeList) {
        tradeList.forEach(trade -> Trade1CommitPriceUtil.calc(trade));
    }
    ...
    public void marketingProcess(
        ...
        for (Trade trade : tradeList) {
            ...
            // 开店礼包
            if (trade.getStoreBagsFlag().equals(DefaultFlag.YES)) {
                this.storeBagsProcess(trade);
                break;
            }

            // 组合购
            if (trade.getSuitMarketingFlag()) {
                this.suitMarketingProcess(trade, param.getCustomerVO().getCustomerId());
                break;
            }

            // 设置价格
            this.setPluginPrice(
                    trade, param.getGoodsTradePluginResponse().getSkuMarketingLabelMap());
            ...
            // 预售
            if (trade.getTradeItems().get(0).getIsBookingSaleGoods()) {
                this.bookingProcess(trade);
                break;
            }

            // 预约
            if (trade.getTradeItems().get(0).getIsAppointmentSaleGoods()) {
                this.appointmentProcess(trade);
                break;
            }
        }
        // 其他营销
     ...
     * @param trade
     */
    private void storeBagsProcess(Trade trade) {
        if (DefaultFlag.YES.equals(trade.getStoreBagsFlag())) {

            // 开店礼包商品，使用市场价，且不计算营销
            trade.getTradeItems()
                    .forEach(
                            item -> {
                                if (Objects.isNull(item.getIsAppointmentSaleGoods())
                                        || !item.getIsAppointmentSaleGoods()
                                        || !(Objects.nonNull(item.getIsBookingSaleGoods())
     ...
     * @param customerId
     */
    private void suitMarketingProcess(Trade trade, String customerId) {

        // 开店礼包、组合购不需要走营销插件
        if (trade.getSuitMarketingFlag()) {
            // 获取并校验组合购活动信息
            MarketingSuitsValidRequest marketingSuitsValidRequest =
                    new MarketingSuitsValidRequest();
            marketingSuitsValidRequest.setMarketingId(
                    trade.getTradeItems().get(NumberUtils.INTEGER_ZERO).getMarketingIds().get(0));
            marketingSuitsValidRequest.setUserId(customerId);
            BaseResponse<MarketingSuitsValidResponse> marketingSuits =
                    marketingSuitsQueryProvider.validSuitOrderBeforeCommit(
                            marketingSuitsValidRequest);
            List<MarketingSuitsSkuVO> marketingSuitsSkuVOList =
                    marketingSuits.getContext().getMarketingSuitsSkuVOList();
            if (CollectionUtils.isNotEmpty(marketingSuitsSkuVOList)) {
                if (trade.getTradeItems().size() != marketingSuitsSkuVOList.size()) {
                    throw new SbcRuntimeException(MarketingErrorCodeEnum.K080001);
                }
                trade.setSuitsId(marketingSuitsSkuVOList.get(0).getMarketingId());
                Map<String, MarketingSuitsSkuVO> suitsSkuVOMap =
                        marketingSuitsSkuVOList.stream()
                                .collect(
                                        Collectors.toMap(
                        ...
                        .forEach(
                                item -> {
                                    MarketingSuitsSkuVO suitsSku =
                                            suitsSkuVOMap.get(item.getSkuId());
                                    if (suitsSku == null) {}
                                    ...
                                    TradeItem.MarketingSettlement marketingSettlement =
                                            TradeItem.MarketingSettlement.builder()
                                                    ...
                                                    .marketingId(suitsSku.getMarketingId())
                                                    .splitPrice(splitPrice)
                                                    .discountsAmount(item.getOriginalPrice().subtract(discountPrice).multiply(BigDecimal.valueOf(item.getNum())))
                                                    .build();
                                    settlements.add(marketingSettlement);
                                    item.setMarketingSettlements(settlements);
                                });
                List<TradeMarketingVO> tradeMarketings = new ArrayList<>();
                BigDecimal discountsAmount =
                        marketingSuitsSkuVOList.stream().map(marketingSuitsSkuVO -> {
                    return marketingSuitsSkuVO.getDiscountPrice().multiply(BigDecimal.valueOf(marketingSuitsSkuVO.getNum()));
                }).reduce(BigDecimal.ZERO, BigDecimal::add);
                BigDecimal originalPrice =
                        trade.getTradeItems().stream().map(tradeItem -> {
                            return tradeItem.getOriginalPrice().multiply(BigDecimal.valueOf(tradeItem.getNum()));
                        }).reduce(BigDecimal.ZERO, BigDecimal::add);
                TradeMarketingVO tradeMarketingVO =
                        TradeMarketingVO.builder()
                                .marketingId(marketingSuitsSkuVOList.get(0).getMarketingId())
                                .marketingLevelId(marketingSuitsSkuVOList.get(0).getSuitsId())
                                .skuIds(trade.getTradeItems().stream().map(TradeItem::getSkuId).collect(Collectors.toList()))
                                .marketingType(MarketingType.SUITS)
                                .discountsAmount(originalPrice.subtract(discountsAmount))
                                .realPayAmount(discountsAmount)
                                .build();
                tradeMarketings.add(tradeMarketingVO);
                trade.setTradeMarketings(tradeMarketings);
            } else {
                throw new SbcRuntimeException(MarketingErrorCodeEnum.K080001);
            }
        }
    ...
    private void setPluginPrice(
            Trade trade, Map<String, List<MarketingPluginLabelDetailVO>> marketingMap) {
    ...
    /** 预售 */
    private void bookingProcess(Trade trade) {
        ...
        trade.getTradeItems()
                .forEach(
                        i -> {
                            if (i.getBookingType().equals(BookingType.EARNEST_MONEY)) {
                                BigDecimal tempPrice =
                                        i.getPrice()
                                                .multiply(new BigDecimal(i.getNum()))
                                                .subtract(i.getSwellPrice());
                                BigDecimal tailPrice =
                                        tempPrice.compareTo(BigDecimal.ZERO) < 0
                                                ? BigDecimal.ZERO
                                                : tempPrice;
                                i.setSplitPrice(tailPrice.add(i.getEarnestPrice()));
                                i.setTailPrice(tailPrice);
                                tradePrice.setTailPrice(tailPrice);
                                tradePrice.setSwellPrice(i.getSwellPrice());
                                tradePrice.setEarnestPrice(i.getEarnestPrice());
                            }
                        });
    }

    /** 预约 */
    private void appointmentProcess(Trade trade) {}

    /** 拼团 */
    private void grouponProcess(
            Trade trade, Map<String, List<MarketingPluginLabelDetailVO>> marketingMap) {

        if (trade.getTradeGroupon().getLeader()) {
            String skuId = trade.getTradeItems().get(0).getSkuId();
            List<MarketingPluginLabelDetailVO> marketingPluginLabelDetailVOS =
                    marketingMap.get(skuId);
            if (CollectionUtils.isNotEmpty(marketingPluginLabelDetailVOS)) {
                MarketingPluginLabelDetailVO marketing =
                        marketingPluginLabelDetailVOS.stream()
                                .filter(
                                        m ->
                                                m.getMarketingType()
                                                        .equals(
                                ...
                                .findFirst()
                                .orElse(null);
                if (marketing != null && marketing.getMarketingId() != null) {
                    String activityId = marketing.getMarketingId();

                    GrouponActivityVO grouponActivityVO =
                            grouponActivityQueryProvider
                                    .getSimpleById(
                                            GrouponActivityByIdRequest.builder()
                                                    .grouponActivityId(activityId)
                                                    .build())
                                    .getContext();
                    if (grouponActivityVO != null) {
                        trade.getTradeGroupon().setGrouponNo(generatorService.generateGrouponNo());
                        trade.getTradeGroupon()
                                .setGrouponActivityId(grouponActivityVO.getGrouponActivityId());
                        trade.getTradeGroupon().setFreeDelivery(grouponActivityVO.isFreeDelivery());
                    }
                }
            }
        } else {
            String grouponNo = trade.getTradeGroupon().getGrouponNo();
            GrouponInstance grouponInstance =
                    grouponInstanceRepository.findTopByGrouponNo(grouponNo);
            ...
            if (CollectionUtils.isNotEmpty(marketingPluginLabelDetailVOS)) {
                ...
                if (marketing != null && marketing.getMarketingId() != null) {
                    String activityId = marketing.getMarketingId();
                    if (!grouponInstance.getGrouponActivityId().equals(activityId)) {
                        throw new SbcRuntimeException(
                                OrderErrorCodeEnum.K050100);
                    }
                    GrouponActivityVO grouponActivityVO =
                            grouponActivityQueryProvider
                                    ...
                                    .getContext();
                    trade.getTradeGroupon()
                            .setGrouponActivityId(grouponInstance.getGrouponActivityId());
                    trade.getTradeGroupon().setFreeDelivery(grouponActivityVO.isFreeDelivery());
                }
            }
        }
    ...
    public void pointProcess(
        ...
        if (!buyCycleFlag && EnableStatus.ENABLE.equals(pointsConfig.getStatus())
            ...
            List<TradeItem> items =
                    tradeList.stream()
                            .flatMap(trade -> trade.getTradeItems().stream())
                            .sorted(Comparator.comparing(TradeItem::getSplitPrice))
                            .collect(Collectors.toList());
    ...
    public void freightProcess(List<Trade> tradeList, TradeCommitRequest request, Trade1CommitParam param) {
        ...
        tradeList.forEach(
                trade -> {
                    ...
                    if (Objects.nonNull(trade.getTradeGroupon()) && trade.getGrouponFlag()) {
                        // 秒杀商品是否包邮
                        // 获取秒杀抢购活动详情
                        boolean isFreeDelivery = trade.getTradeGroupon().isFreeDelivery();
                        if (isFreeDelivery) {
                            postage = true;
                    ...
                    if (Objects.equals(Boolean.TRUE, trade.getBargain())) {
                        ...
                        if (Objects.isNull(bargainVO)
                                || Objects.isNull(bargainVO.getBargainGoodsVO())) {
                    ...
                    // 是否是虚拟订单或者卡券订单
                    boolean isVirtual =
                            Objects.nonNull(trade.getOrderTag())
                                    && (trade.getOrderTag().getVirtualFlag()
                                            || trade.getOrderTag().getElectronicCouponFlag());
                    // 虚拟订单或者卡券订单没有运费
                    if (isVirtual) {
                        deliveryPrice = BigDecimal.ZERO;
                        trade.setFreight(Freight.builder().freight(BigDecimal.ZERO).build());
                    }
    ...
    public void giftCardProcess(List<Trade> tradeList, TradeCommitRequest request, Trade1CommitParam param) {
        ...
        for (UserGiftCardInfoVO giftCardInfoVO : gorpList) {
            ...
            try {
                ...
                log.error("giftCardProcess下单礼品卡任务执行-线程池异常", e.getMessage());
                Thread.currentThread().interrupt();
                throw new SbcRuntimeException(CommonErrorCodeEnum.K000001);
            } catch (ExecutionException e) {
                log.error("giftCardProcess下单礼品卡任务执行出错:{}", e.getStackTrace());
                throw new SbcRuntimeException(CommonErrorCodeEnum.K000001);
            }
        }
    }
    ...
    public void distributionProcess(
        ...
        if (Objects.nonNull(payGatewayVO)){
            isopen = IsOpen.YES.equals(payGatewayVO.getIsOpen());
        }
        LedgerReceiverRelBindStateResponse ledgerReceiverRelBindStateResponse = null;
        if (isopen){
            List<Long> storeIds = tradeList.stream().map(trade -> trade.getSupplier().getStoreId()).collect(Collectors.toList());
            ledgerReceiverRelBindStateResponse =
                    ledgerReceiverRelQueryProvider.findUnBindStoresByDistribution(DistributionBindStateRequest.builder()
                            .customerId(tradeList.get(0).getBuyer().getId()).storeIds(storeIds).build()).getContext();
        }

        // 是不是分销员
        DistributionCustomerEnableByCustomerIdResponse distributionCustomerResponse =
                distributionCustomerQueryProvider
                        .checkEnableByCustomerId(
                                DistributionCustomerEnableByCustomerIdRequest.builder()
                                        .customerId(request.getOperator().getUserId())
                                        .build())
                        .getContext();
        request.setIsDistributor(
                distributionCustomerResponse.getDistributionEnable() == true
                        ? DefaultFlag.YES
                        : DefaultFlag.NO);
        ...
        // 2.1.查询佣金受益人列表
        DistributionCustomerListForOrderCommitRequest distributionRequest =
                new DistributionCustomerListForOrderCommitRequest();
        distributionRequest.setBuyerId(tradeList.get(0).getBuyer().getId());
        distributionRequest.setCommissionPriorityType(
                CommissionPriorityType.fromValue(
                        multistageSetting.getCommissionPriorityType().toValue()));
        distributionRequest.setCommissionUnhookType(
                CommissionUnhookType.fromValue(
                        multistageSetting.getCommissionUnhookType().toValue()));
        distributionRequest.setDistributorLevels(multistageSetting.getDistributorLevels());
        distributionRequest.setInviteeId(channel.getInviteeId());
        distributionRequest.setIsDistributor(request.getIsDistributor());
        List<DistributionCustomerSimVO> inviteeCustomers =
                distributionCustomerQueryProvider
                        .listDistributorsForOrderCommit(distributionRequest)
                        .getContext()
                        .getDistributorList();
        Map<String, List<MarketingPluginLabelDetailVO>> marketingMap =
                param.getGoodsTradePluginResponse().getSkuMarketingLabelMap() == null
                        ? new HashMap<>()
                        : param.getGoodsTradePluginResponse().getSkuMarketingLabelMap();
        LedgerReceiverRelBindStateResponse finalLedgerReceiverRelBindStateResponse = ledgerReceiverRelBindStateResponse;
        boolean finalIsopen = isopen;
        tradeList.forEach(
                trade -> {
                    if (DefaultFlag.YES.equals(request.getOpenFlag())) {
                        Boolean isGoodsPoint =
                                param.getSystemPointsConfigQueryResponse()
                                        .getPointsUsageFlag()
                                        .equals(PointsUsageFlag.GOODS);
                        trade.getTradeItems()
                                .forEach(
                                        item -> {
                                            // 不是商品抵扣模式 || 非积分价商品
                                            Boolean isPointPriceGoods =
                                                    !isGoodsPoint.booleanValue()
                                                            || (Boolean.TRUE.equals(
                                                                    item.getBuyPoint() == null
                                                                            || item.getBuyPoint()
                                                                                    == 0));
                                            // 非预售、预约商品
                                            ...
                                            List<MarketingPluginLabelDetailVO> marketingList =
                                                    marketingMap.get(item.getSkuId()) == null
                                                            ? new ArrayList<>()
                                                            : marketingMap.get(item.getSkuId());
                                            // 判断营销插件是不是分销商品
                                            ...
                                            // 拉卡拉开启时。分销员需要判断是否和商家已绑定分账关系。
                                            if (finalIsopen && finalLedgerReceiverRelBindStateResponse.getUnBindStores()
                                                    .contains(trade.getSupplier().getStoreId())){
                                                isDistribution = Boolean.FALSE;
                                            }
                                            if (DistributionGoodsAudit.CHECKED
                                                            == item.getDistributionGoodsAudit()
                                                    //
                                                    //      && DefaultFlag.YES.equals(
                                                    //
                                                    //              trade.getStoreBagsFlag())
                                                    && ChannelType.PC_MALL
                                                            != channel.getChannelType()
                                                    && isPointPriceGoods
                                                    && isAppointOrBookingGoods
                                                    && isDistribution) {
                                                ...
                                                item.setDistributionCommission(BigDecimal.ZERO);
                                            }
                                        });
                        List<TradeItem> distributionTradeItems =
                                trade.getTradeItems().stream()
                                        .filter(
                                                item ->
                        ...
                        if (distributionTradeItems.size() != 0) {
                            ...
                            distributionTradeItems.forEach(
                                    item ->
                                            skuBaseCommissionMap.put(
                                                    item.getSkuId(),
                                                    item.getDistributionCommission()));
                            // 2.2.根据受益人列表设置分销相关字段
                            ...
                            if (CollectionUtils.isNotEmpty(inviteeCustomers)) {
                                for (int idx = 0; idx < inviteeCustomers.size(); idx++) {
                                    ...
                                    DistributorLevelVO level =
                                            multistageSetting.getDistributorLevels().stream()
                                                    .filter(
                                                            l ->
                                                                    l.getDistributorLevelId()
                                                                            .equals(
                                                                                    customer
                                        ...
                                        if (idx == 0) {
                                            ...
                                            distributionTradeItems.forEach(
                                                    item -> {
                                                        ...
                                                        TradeDistributeItem distributeItem =
                                                                new TradeDistributeItem();
                                                        distributeItem.setGoodsInfoId(
                                                                item.getSkuId());
                                                        distributeItem.setNum(item.getNum());
                                                        distributeItem.setActualPaidPrice(
                                                                item.getSplitPrice());
                                                        distributeItem.setCommissionRate(
                                                                item.getCommissionRate());
                                                        distributeItem.setCommission(
                                                                item.getDistributionCommission());
                                            ...
                                            trade.setCommission(
                                                    distributeItems.stream()
                                                            .map(TradeDistributeItem::getCommission)
                                                            .reduce(
                                                                    BigDecimal.ZERO,
                                                                    BigDecimal::add));
                                            // 累加返利人佣金至总佣金
                                            totalCommission =
                                                    totalCommission.add(trade.getCommission());
                                            ...
                                            for (int i = 0; i < distributeItems.size(); i++) {
                                                ...
                                                TradeDistributeItemCommission itemCommission =
                                                        new TradeDistributeItemCommission();
                                                itemCommission.setCustomerId(
                                                        customer.getCustomerId());
                                                itemCommission.setDistributorId(
                                                        customer.getDistributionId());
                                                itemCommission.setCommission(
                                                        skuBaseCommissionMap
                                                                .get(item.getGoodsInfoId())
                                                                .multiply(level.getPercentageRate())
                                                                .setScale(
                                                                        2, RoundingMode.DOWN));
                                                item.getCommissions().add(itemCommission);
                                                percentageTotal =
                                                        percentageTotal.add(
                                                                itemCommission.getCommission());
                                            }
                                            ...
                                            tradeCommission.setDistributorId(
                                                    customer.getDistributionId());
                                            tradeCommission.setCustomerName(
                                            ...
                                            totalCommission =
                                                    totalCommission.add(
                                                            tradeCommission.getCommission());
                                        }
                                    }
                                }

                                // 求和分销商品总佣金 trade.distributeItems.totalCommission
                                distributeItems.forEach(
                                        item -> {
                                            // 追加返利人佣金
                                            ...
                                            item.getCommissions()
                                                    .forEach(
                                                            i ->
                                                                    item.setTotalCommission(
                                                                            item.getTotalCommission()
                                                                                    .add(
                                                                                            i
                                                                                                    .getCommission())));
                                        });
                    ...
                    // 设置渠道信息、小店名称、小B-会员ID
                    trade.setChannelType(request.getDistributeChannel().getChannelType());
                    trade.setShopName(request.getShopName());
                    trade.setDistributionShareCustomerId(
                            request.getDistributeChannel().getInviteeId());
                });
    }
    ...
    public void auditStateProcess(List<Trade> tradeList, TradeCommitRequest request) {
        ...
        tradeList.forEach(
                trade -> {
                    // 初始化订单提交状态
                    FlowState flowState;
                    com.wanmi.sbc.order.bean.enums.AuditState auditState;
                    ...
                    if (Objects.nonNull(trade.getIsBookingSaleGoods())
                        ...
                        auditState = com.wanmi.sbc.order.bean.enums.AuditState.CHECKED;
                    }
                    // 如果是拼团订单商品不需要审
                        ...
                        if (CollectionUtils.isNotEmpty(giftIds)) {
                            List<GoodsInfoTradeVO> goodsInfoList =
                                    goodsInfoQueryProvider
                                            ...
                                            .getContext();
                            //查询营销详情信息
                            TradeMarketingVO tradeMarketingVO = trade.getTradeMarketings().stream()
                                    .filter(item -> item.getMarketingType().equals(MarketingType.GIFT))
                                    .findFirst().orElse(null);

                            FullGiftDetailListByMarketingIdRequest request =
                                    FullGiftDetailListByMarketingIdRequest.builder().build();
                            request.setMarketingId(tradeMarketingVO.getMarketingId());

                            List<MarketingFullGiftDetailVO> marketingFullGiftDetailVOS =
                                    fullGiftQueryProvider.listDetailByMarketingId(request).getContext().getFullGiftDetailVOList();
                            ...
                            if (!isForceCommit) {
                                ...
                                goodsInfoList.forEach(
                                        goodsInfo -> {
                                            MarketingFullGiftDetailVO marketingFullGiftDetailVO = marketingFullGiftDetailVOS.stream()
                                                    .filter(gift -> gift.getProductId().equals(goodsInfo.getGoodsInfoId()))
                                                    .findFirst().orElse(null);
                                            if (!goodsInfo.getGoodsStatus().equals(GoodsStatus.OK)
                                                    || goodsInfo.getStock() < 1 ||
                                                    (marketingFullGiftDetailVO.getProductStock() !=null && marketingFullGiftDetailVO.getProductStock() < 1)) {
                                                // 无效赠品提示
                                                String tmp = "赠品%s";
                                                String errMsg =
                                                        String.format(
...
//                                                    tradeItem.setSupplyPrice(BigDecimal.ZERO);
                                ...
                                //赠品库存-1
                                FullGiftReduceStockRequest fullGiftReduceStockRequest = new FullGiftReduceStockRequest();
                                fullGiftReduceStockRequest.setMarketingId(tradeMarketingVO.getMarketingId());
                                List<FullGiftDetailDTO> fullGiftDetailDTOList = trade.getGifts()
                                        .stream().map(
                                                tradeItem -> {
                                                    FullGiftDetailDTO fullGiftDetailDTO = new FullGiftDetailDTO();
                                                    fullGiftDetailDTO.setProductId(tradeItem.getSkuId());
                                                    fullGiftDetailDTO.setProductStock(tradeItem.getNum() == null? 0l: tradeItem.getNum());
                                                    return fullGiftDetailDTO;
                                                }).collect(Collectors.toList());
                                fullGiftReduceStockRequest.setFullGiftDetailList(fullGiftDetailDTOList);
                                fullGiftProvider.reduceStock(fullGiftReduceStockRequest);
                            }
                        }
                    }
                });
    ...
    public void fullReturnProcess(List<Trade> tradeList, Trade1CommitParam param) {
        ...
        MarketingMapGetByGoodsIdRequest marketingMapGetByGoodsIdRequest = MarketingMapGetByGoodsIdRequest.builder()
                ...
                .excludeStatus(MarketingStatus.PAUSED)
            ...
            if (MapUtils.isNotEmpty(marketingMap)){
                ...
                if (MapUtils.isNotEmpty(newMarketingMap)){
                    ...
                    for (Trade trade:tradeList){
                        // 符合条件的券id
                        // <营销id, 券id>
                        Map<Long, List<FullReturnCoupon>> fullReturnCoupons = new HashMap<>();
                        List<TradeItem> tradeItems = trade.getTradeItems();
                        ...
                        //将券信息记录到订单中
                        if (CollectionUtils.isNotEmpty(fullReturnCoupons.values())){
                            trade.setFullReturnCoupons(fullReturnCoupons.values().stream().flatMap(Collection::stream).collect(Collectors.toList()));
                        }
                    }
                }
            }
     ...
     */
    private void dealFullReturnLevel(Map<Long, List<FullReturnCoupon>> fullReturnCoupons, MarketingForEndVO marketingForEndVO,
                                     List<MarketingFullReturnLevelVO> returnLevels, int num) {
        List<FullReturnCoupon> coupons = fullReturnCoupons.getOrDefault(marketingForEndVO.getMarketingId(), Lists.newArrayList());
        ...
        if (CollectionUtils.isNotEmpty(returnLevels) && num++ < Constants.FIVE){
            ...
            if (fullReturnLevelOptional.isPresent()) {
                ...
                dealReturnDetail(coupons, marketingForEndVO, marketingFullReturnDetailVOS);
                fullReturnCoupons.put(marketingForEndVO.getMarketingId(), coupons);
