...
public class OrderPerformanceDetailService {
     ...
     */
    public void createPurchaseDetails(OrderPerformance orderPerformance, List<TradeItem> tradeItems) {
        ...
        List<OrderPerformanceDetail> details = tradeItems.stream()
                ...
                .map(tradeItem -> convertTradeItemToDetail(orderPerformance, tradeItem))
     ...
     * @return 订单业绩明细
     */
    public OrderPerformanceDetail convertTradeItemToDetail(OrderPerformance orderPerformance, TradeItem tradeItem) {
        ...
        BigDecimal unitPrice = tradeItemAnalyzer.calculateItemPrice(season.orElse(""), clothingTypes, schoolLevel);
        ...
        // 设置地区
        detail.setProvinceId(orderPerformance.getProvinceId());
        detail.setCityId(orderPerformance.getCityId());
        detail.setAreaId(orderPerformance.getAreaId());
        detail.setProvinceName(orderPerformance.getProvinceName());
        detail.setCityName(orderPerformance.getCityName());
        detail.setAreaName(orderPerformance.getAreaName());
        detail.setShopName(orderPerformance.getShopName());
    ...
    private List<GetSchooluUniformSalesDataResponse> convertToResponseListStream(Map<String, Map<String, SeasonSummary>> resultMap) {
        return resultMap.entrySet().stream()
                .map(entry -> {
                    ...
                    // 初始化所有字段
                    response.setSummerNum(0l);
                    response.setAutumnNum(0l);
                    response.setWinterNum(0l);
                    ...
                    seasonMap.forEach((season, summary) -> {
                        if ("夏装".equals(season)) {
                            response.setSummerNum(summary.getQuantity().longValue());
                            ...
                            response.setAutumnNum(summary.getQuantity().longValue());
                            ...
                            response.setWinterNum(summary.getQuantity().longValue());
    ...
    public BaseResponse<GetSchooluUniformSalesDataResponse> getSchooluUniformSalesDataNew(OrderPerformanceByUniqueCodesRequest request) {
        List<SeasonSummaryDto> seasonSummaryDto = orderPerformanceDetailRepository.findSeasonSummary(request);
        //设置响应参数
        GetSchooluUniformSalesDataResponse response = new GetSchooluUniformSalesDataResponse();
        Long totalNum = 0l;
        BigDecimal totalCommission = BigDecimal.ZERO;
        Long summerNum = 0l;
        BigDecimal summerCommission = BigDecimal.ZERO;
        Long autumnNum = 0l;
        BigDecimal autumnCommission = BigDecimal.ZERO;
        Long winterNum = 0l;
        BigDecimal winterCommission = BigDecimal.ZERO;
        for (SeasonSummaryDto summaryDto : seasonSummaryDto) {
            String season = summaryDto.getSeason();
            if (summaryDto.getDetailType() == 2){
                summaryDto.setQuantity(summaryDto.getQuantity() *-1);
            }
            if ("夏装".equals(season)) {
                summerNum+=summaryDto.getQuantity();
                summerCommission = summerCommission.add(summaryDto.getUniformTotalAmount());
            } else if ("春秋装".equals(season)) {
                autumnNum+=summaryDto.getQuantity();
                autumnCommission = autumnCommission.add(summaryDto.getUniformTotalAmount());
            } else if ("冬装".equals(season)) {
                winterNum+=summaryDto.getQuantity();
                winterCommission = winterCommission.add(summaryDto.getUniformTotalAmount());
            }
            totalNum+=summaryDto.getQuantity();
            totalCommission = totalCommission.add(summaryDto.getUniformTotalAmount());
        }
        response.setSummerNum(summerNum);
        response.setSummerCommission(summerCommission);
        response.setAutumnNum(autumnNum);
        response.setAutumnCommission(autumnCommission);
        response.setWinterNum(winterNum);
        response.setWinterCommission(winterCommission);
        response.setTotalNum(totalNum);
        response.setTotalCommission(totalCommission);
        //判断是省级数据还是区级数据
        if (request.getAreaId() == null) {
            List<AreaSummaryDto> areaSummary = orderPerformanceDetailRepository.findAreaSummary(request);
            List<GetSchooluUniformSalesDataResponse> citySummaryList = summarizeAreaClothingQuantities(areaSummary);
            response.setChildList(citySummaryList);
        }else{
            List<ShopSummaryDto> shopSummary = orderPerformanceDetailRepository.findShopSummary(request);
            List<GetSchooluUniformSalesDataResponse> getSchooluUniformSalesDataResponses = summarizeShopClothingQuantities(shopSummary);
            response.setChildList(getSchooluUniformSalesDataResponses);
        }

        return BaseResponse.success(response);
    }

    public  List<GetSchooluUniformSalesDataResponse> summarizeAreaClothingQuantities(List<AreaSummaryDto> areaSummaryDtos) {
        // 按 cityId 分组
        Map<Long, GetSchooluUniformSalesDataResponse> cityMap = new HashMap<>();

        for (AreaSummaryDto dto : areaSummaryDtos) {
            if (dto.getDetailType() == 2){
                dto.setQuantity(-dto.getQuantity());
            }
            // 获取或创建 CitySummary
            GetSchooluUniformSalesDataResponse citySummary = cityMap.computeIfAbsent(dto.getCityId(),
                    k ->   new GetSchooluUniformSalesDataResponse()
            );
            citySummary.setCityId(dto.getCityId());
            citySummary.setCityName(dto.getCityName());
            // 更新市级数量
            citySummary.add(dto.getSeason(), dto.getQuantity(), dto.getCommissionAmount());

            // 查找或创建 AreaSummary
            Optional<GetSchooluUniformSalesDataResponse> existingArea = citySummary.getChildList().stream()
                    .filter(area -> area.getAreaId().equals(dto.getAreaId()))
                    .findFirst();

            GetSchooluUniformSalesDataResponse areaSummary;
            if (existingArea.isPresent()) {
                areaSummary = existingArea.get();
            } else {
                areaSummary = new GetSchooluUniformSalesDataResponse();
                areaSummary.setAreaId(dto.getAreaId());
                areaSummary.setAreaName(dto.getAreaName());
                areaSummary.setCityId(dto.getCityId());
                areaSummary.setCityName(dto.getCityName());
                citySummary.getChildList().add(areaSummary);
            }

            // 更新区级数量
            areaSummary.add(dto.getSeason(), dto.getQuantity(), dto.getCommissionAmount());
        }
        // 返回市级汇总列表
        return new ArrayList<>(cityMap.values());
    }

    public  List<GetSchooluUniformSalesDataResponse> summarizeShopClothingQuantities(List<ShopSummaryDto> shopSummaryDtos) {
        // 按 cityId 分组
        Map<String, GetSchooluUniformSalesDataResponse> shopMap = new HashMap<>();

        for (ShopSummaryDto dto : shopSummaryDtos) {
            if (dto.getDetailType() == 2){
                dto.setQuantity(-dto.getQuantity());
            }
            // 获取或创建 CitySummary
            GetSchooluUniformSalesDataResponse shopSummary = shopMap.computeIfAbsent(dto.getShopName(),
                    k ->   new GetSchooluUniformSalesDataResponse()
            );
            shopSummary.setAreaId(dto.getAreaId());
            shopSummary.setAreaName(dto.getAreaName());
            shopSummary.setShopName(dto.getShopName());
            // 更新市级数量
            shopSummary.add(dto.getSeason(), dto.getQuantity(), dto.getCommissionAmount());
        }
        // 返回市级汇总列表
        return new ArrayList<>(shopMap.values());
    }

    public BaseResponse<GetSchooluUniformSalesDataResponse> getAllSchooluUniformSalesDataAndZero(OrderPerformanceByUniqueCodesRequest request) {
        List<SeasonSummaryDto> seasonSummaryDto = orderPerformanceDetailRepository.findSeasonSummaryByUniqueCode(request);
        //设置响应参数
        GetSchooluUniformSalesDataResponse response = new GetSchooluUniformSalesDataResponse();
        Long totalNum = 0l;
        BigDecimal totalCommission = BigDecimal.ZERO;
        ...
        //判断是省级数据还是区级数据
        if (request.getAreaId() == null) {
            List<AreaSummaryDto> areaSummary = orderPerformanceDetailRepository.findAreaSummaryByUniqueCode(request);
            List<GetSchooluUniformSalesDataResponse> citySummaryList = summarizeAreaClothingQuantities(areaSummary);
            response.setChildList(citySummaryList);
        }else{
            List<ShopSummaryDto> shopSummary = orderPerformanceDetailRepository.findShopSummaryByUniqueCode(request);
            List<GetSchooluUniformSalesDataResponse> getSchooluUniformSalesDataResponses = summarizeShopClothingQuantities(shopSummary);
            response.setChildList(getSchooluUniformSalesDataResponses);
        }
