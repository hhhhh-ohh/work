...
public class ThirdPlatformTradeQueryRequest extends BaseQueryRequest {
     ...
     * 查询空的第三方平台订单
     */
    @Schema(description = "查询空的第三方平台订单")
    private Boolean emptyThirdPlatformOrderId;
    ...
    private List<Criteria> getCommonCriteria() {
        ...
        // 第三方平台订单
        if(StringUtils.isNotBlank(thirdPlatformOrderId)) {
            criterias.add(Criteria.where("thirdPlatformOrderIds").is(thirdPlatformOrderId));
        }
...
//            criterias.add(new Criteria().orOperator(andCriteria,Criteria.where("storeBagsFlag").is(DefaultFlag.YES)));
        ...
        // 是否出现错误
        if (Objects.nonNull(thirdPlatformPayErrorFlag)) {
            criterias.add(Criteria.where("thirdPlatformPayErrorFlag").is(thirdPlatformPayErrorFlag));
        }

        // 空第三方渠道订单id
        if(Boolean.TRUE.equals(emptyThirdPlatformOrderId)) {
            Criteria orCriteria = new Criteria();
            orCriteria.orOperator(
                    Criteria.where("thirdPlatformOrderIds").exists(false),
                    Criteria.where("thirdPlatformOrderIds").is(new ArrayList<>()));
            criterias.add(orCriteria);
        }
