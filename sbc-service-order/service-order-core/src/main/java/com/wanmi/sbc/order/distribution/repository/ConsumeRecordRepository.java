...
public interface ConsumeRecordRepository extends JpaRepository<ConsumeRecord, String>,
     ...
     * @return
     */
    @Query(value = "SELECT r.customer_id ,COUNT(*) ,SUM(r.valid_consume_sum) ,min(r.customer_name) customer_name  " +
            "FROM consume_record r WHERE r.customer_id IN(:ids) and r.flow_state ='AVAILABLE_ORDER' GROUP BY r.customer_id", nativeQuery = true)
     ...
     * @return
     */
    @Query(value = "SELECT r.customer_id ,COUNT(*) ,SUM(r.consume_sum)  ,min(r.customer_name) customer_name  " +
    ...
    @Query(value = "SELECT r.customer_id ,COUNT(*) ,SUM(r.consume_sum), min(r.customer_name) customer_name ," +
            "min(r.head_img) head_img, min(r.order_create_time) order_create_time" +
            " FROM consume_record r WHERE r.distribution_customer_id=?1 and r" +
            ".distribution_customer_id != r.customer_id " +
            "GROUP BY r.customer_id ORDER BY order_create_time DESC limit ?2,?3", nativeQuery = true)
