...
import com.wanmi.sbc.order.purchase.GoodsMarketingServiceInterface;
...
public class PurchaseQueryController implements PurchaseQueryProvider {
    ...
    @Autowired
    private GoodsMarketingServiceInterface goodsMarketingServiceInterface;
    ...
    public BaseResponse<PurchaseQueryResponse> query(@RequestBody @Valid PurchaseQueryRequest request) {
        ...
        List<Purchase> purchaseList = purchaseService.queryPurchase(request.getCustomerId(), request.getGoodsInfoIds
                (), request.getInviteeId(),request.getIsO2O());
     ...
     */
    @Override
    public BaseResponse<PurchaseQueryResponse> listByCustomerId(@RequestBody @Valid PurchaseQueryRequest request) {
        List<Purchase> purchaseList = purchaseService.queryPurchase(request.getCustomerId(), request.getGoodsInfoIds
                (), request.getInviteeId(),request.getIsO2O());
        PurchaseQueryResponse purchaseQueryResponse = new PurchaseQueryResponse();
        purchaseQueryResponse.setPurchaseList(KsBeanUtil.convertList(purchaseList, PurchaseVO.class));
    ...
    public BaseResponse<PurchaseGetStoreCouponExistResponse> getStoreCouponExist(@RequestBody @Valid
        ...
        // 0.判断是否O2O模式
        Boolean isO2o = PluginType.O2O.equals(request.getPluginType());
        List<Purchase> purchases = purchaseService.queryPurchase(request.getCustomer().getCustomerId(), null, request.getInviteeId(),isO2o);
        ...
        List<GoodsInfoVO> goodsInfos = goodsInfoQueryProvider.listByIds(GoodsInfoListByIdsRequest.builder()
                .goodsInfoIds(purchases.stream().map(Purchase::getGoodsInfoId).collect(Collectors.toList()))
                .build())
                .getContext()
                .getGoodsInfos();
        ...
        Map<Long, Boolean> storeCouponExist = purchaseService.getStoreCouponExist(KsBeanUtil.convertList(goodsInfos, GoodsInfoVO.class),
                KsBeanUtil.convert(request.getCustomer(), CustomerVO.class),request.getStoreId(), request.getPluginType());
    ...
    public BaseResponse<PurchaseGetGoodsMarketingResponse> getGoodsMarketing(@RequestBody @Valid
        ...
        if (Objects.isNull(request.getStoreId())) {
            return BaseResponse.success(goodsMarketingServiceInterface.getGoodsMarketing(
                            goodsInfos, request.getCustomer(), null, PluginType.NORMAL));
        } else {
            return BaseResponse.success(goodsMarketingServiceInterface.getGoodsMarketing(
                    goodsInfos, request.getCustomer(), request.getStoreId(), PluginType.O2O));
        }
    }
    ...
    public BaseResponse checkAdd(PurchaseSaveRequest request) {
        ...
        if (Objects.nonNull(goodsInfo)) {
            ...
            goodsListByIdsRequest.setGoodsIds(Collections.singletonList(goodsInfo.getGoodsId()));
            List<GoodsVO> providerGoodsVOList = goodsQueryProvider.listByIds(goodsListByIdsRequest).getContext().getGoodsVOList();
            //校验goods是否支持加入购物车
            boolean checkResult = true;
            if (CollectionUtils.isNotEmpty(providerGoodsVOList)) {
                for (GoodsVO goods : providerGoodsVOList) {
                    String goodsBuyTypes = goods.getGoodsBuyTypes();
                    if (!goodsBuyTypes.contains("0")) {
                        checkResult = false;
                    }
                }
            }
