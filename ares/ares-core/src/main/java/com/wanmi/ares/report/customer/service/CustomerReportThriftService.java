...
public class CustomerReportThriftService implements CustomerReportQueryService.Iface {
    ...
    public static final String ERR_MSG = "时间参数有错误";
    ...
    public CustomerOrderPageView queryCustomerOrders(CustomerOrderQueryRequest request) {
        ...
        if (CustomerQueryType.CUSTOMER == request.getQueryType()) {
            return queryCustomerReport(request);
        }
        if (CustomerQueryType.CUSTOMER_LEVEL == request.getQueryType()) {
            return queryCustomerLevelReport(request);
        }

        if (CustomerQueryType.CUSTOMER_AREA == request.getQueryType()) {
    ...
    private CustomerOrderPageView queryCustomerReport(CustomerOrderQueryRequest request) {
        ...
        if (QueryDateCycle.YESTERDAY == request.getDateCycle() || QueryDateCycle.TODAY == request.getDateCycle()) {
            tableName = CustomerReportTable.CUSTOMER_DAY.toString();
            daylyDate = QueryDateCycle.YESTERDAY == request.getDateCycle() ?
                    DateUtil.format(LocalDateTime.now().minusDays(1), DateUtil.FMT_DATE_1) :
                    DateUtil.format(LocalDateTime.now(), DateUtil.FMT_DATE_1);
        }
        if (QueryDateCycle.LATEST_7_DAYS == request.getDateCycle()) {
            tableName = CustomerReportTable.CUSTOMER_RECENT_SEVEN.toString();
        }

        if (QueryDateCycle.LATEST_30_DAYS == request.getDateCycle()) {
        ...
        if (StringUtils.isEmpty(tableName)) {
            throw new AresRuntimeException(ERR_MSG);
        }
...
//        if (request.getCompanyId().equals(Constants.BOSS_ID) || (store.getCompanyType() == 0 && store.getStoreType() != 2)) {
//            request.setCompanyId(Constants.BOSS_ID);
//        }

        //按照店铺或者门店查询，统计的companyId 是-12 和-13
        if (Constants.BOSS_ID.equals(request.getCompanyId()) && (StoreSelectType.SUPPLIER.equals(request.getStoreSelectType()) || StoreSelectType.O2O.equals(request.getStoreSelectType()))) {
            request.setCompanyId(request.getStoreSelectType().getMockCompanyInfoId());
        }
        ...
        List<CustomerOrderView> customerOrderViewList = customerReportList.stream().map(customerReport -> {
            ...
            customerOrderView.setAmount(customerReport.getAmount().doubleValue())
                    ...
                    .setCustomerId(customerReport.getCustomerId())
        ...
        customerOrderPageView.setTotal(total);
    ...
    private CustomerOrderPageView queryCustomerLevelReport(CustomerOrderQueryRequest request) {
        List<CustomerLevelReport> customerLevelReports;
        CustomerOrderPageView customerOrderPageView = new CustomerOrderPageView();
...
//        }
    ...
    private CustomerOrderPageView queryCustomerAreaReport(CustomerOrderQueryRequest request) {
        List<CustomerAreaReport> customerAreaReports;
        CustomerOrderPageView customerOrderPageView = new CustomerOrderPageView();
        ...
        if (StringUtils.isEmpty(request.getQueryText())) {
            request.setCityList(null);
        } else {
            request.setCityList(Lists.newArrayList(request.getQueryText().split(",")));
        }

//        Store store = replayStoreMapper.queryByCompanyInfoId(Long.valueOf(request.getCompanyId()));
//        if (request.getCompanyId().equals(Constants.BOSS_ID) || store.getCompanyType() == 0) {
//            request.setCompanyId(Constants.BOSS_ID);
//        }
