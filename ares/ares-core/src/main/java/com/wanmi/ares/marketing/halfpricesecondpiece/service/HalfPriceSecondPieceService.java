package com.wanmi.ares.marketing.halfpricesecondpiece.service;
...
@Service
@Slf4j
public class HalfPriceSecondPieceService implements ExportBaseService {
    ...
    public BaseResponse exportReport(ExportQuery exportQuery) throws Exception {
        ...
        request.setMarketingType(MarketingType.HALF_PRICE_SECOND_PIECE);
    ...
    protected void uploadFileOfFullMoneyBookingSale(List<MarketingReduceDiscountGiftReport> records, List<String> locations, ExportQuery query)
        ...
        String fileName = String.format("marketing/halfpricesecondpiece/%s/%s第二件半价活动效果统计_%s_%s-%s.xls",
                query.getCompanyId(),
        ...
        if (CollectionUtils.isNotEmpty(query.getMarketingIds())) {
            fileName = String.format("marketing/halfpricesecondpiece/%s/%s/%s第二件半价活动效果统计_%s_%s-%s.xls",
                    query.getCompanyId(),
