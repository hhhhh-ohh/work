...
public class CustomerLevelAndAreaStatisticsService {
    ...
    @Resource
    private CustomerLevelAndAreaStatisticsService customerLevelAndAreaStatisticsService;

    public void generateData(String type){
        LocalDate targetDate = LocalDate.now();
        //判断是否是执行当天的定时任务
        if("1".equals(type)){//今天数据
            customerLevelAndAreaStatisticsService.generateThirdShopCustomerAreaDistribute(targetDate);
            customerLevelAndAreaStatisticsService.generateThirdShopCustomerLevelDistribute(targetDate);
        } else {//昨天统计数据
