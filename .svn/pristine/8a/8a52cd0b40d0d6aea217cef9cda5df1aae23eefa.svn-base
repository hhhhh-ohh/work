...
public class EmployeeQueryServiceImpl implements EmployeeQueryService.Iface {
    ...
    public List<EmployeeClientView> exportViewByClient(ExportQuery query) {
        if (!"0".equals(query.getCompanyId())) {
            query.setStoreSelectType(StoreSelectType.ALL);
        }

        List<EmployeeClientReport> reports = clientMapper.export(query, query.getStoreSelectType() == null ? 0 : query.getStoreSelectType().toValue());
        if (reports.isEmpty()) {
    ...
    List<EmployeePerformanceView> exportViewByPerformance(ExportQuery query) {
        ...
        int storeType = query.getStoreSelectType() == null ? 0 : query.getStoreSelectType().toValue();
        List<EmployeePerformanceReport> reports;

        reports = performanceMapper.collectBossAndThirdEmployeeTrade(TradeCollect.builder()
        ...
        if (!"0".equals(companyId)) {
            ...
            reports = reports.stream().filter(i -> i.getStoreType() == storeType).collect(Collectors.toList());
        }
    ...
    private void wraperPerformanceViewList(List<Map<String, Object>> employees, List<EmployeePerformanceReport> reports,
        ...
        reports.forEach(
                r -> {
                    ...
                    if (employee != null && employee.get(EMPLOYEE_NAME) != null &&  StringUtils.isNotBlank(String.valueOf(employee.get(EMPLOYEE_NAME)))) {
