 ...
 **/
@Component
public class TradeDataForYesterdayStrategy extends TradeDataStrategy {
    ...
    public List<TradeView> getTradeData(FlowDataListRequest flowDataListRequest) {
        flowDataListRequest.setBeginDate(LocalDate.now().minusDays(1));
        ...
        return tradeDays.stream().map(this::buildTitleForDayList).collect(Collectors.toList());
    }
    ...
    public TradeView getTradeView(FlowDataListRequest flowDataListRequest) {
        ...
        return ParseData.parseData(tradeDay);
    }
