...
public interface CustomerDrawCashQueryProvider {
	 ...
	 * @author xuyunpeng
	 * @param customerDrawCashExportRequest 导出请求参数和筛选对象 {@link CustomerDrawCashExportRequest}
	 * @return 会员提现管理导出信息 {@link Long}
	 */
	@PostMapping("/account/${application.account.version}/customerdrawcash/export/count")
	BaseResponse<Long> countForExport(@RequestBody @Valid CustomerDrawCashExportRequest customerDrawCashExportRequest);
    ...
    BaseResponse<CustomerDrawCashStatusResponse> countDrawCashTabNum(@RequestBody CustomerDrawCashStatusQueryRequest request);
