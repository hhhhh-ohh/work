...
public class TradeItemAnalyzer {
    ...
    @Value("${noBadgeAreas:上海市}")
    private String noBadgeAreas;
    ...
    static {
        ...
        SEASON_MAPPING.put("春装", "春秋装");
        SEASON_MAPPING.put("秋装", "春秋装");
     ...
     * 判断是否需要送校徽,是否包含 "上衣"
     */
    public boolean isNeedSchoolBadge(String skuName ,String specDetails) {
        List<String> types = extractClothingTypes(specDetails);
        boolean hasSY = types.contains("上衣");
        List<String> areas = Arrays.asList(noBadgeAreas.split(","));
        boolean isNeedBadgeArea = Boolean.TRUE;
        for (String area : areas) {
            boolean contains = skuName.contains(area);
            if (contains) {
                isNeedBadgeArea = false;
                break;
            }
        }

        return hasSY && isNeedBadgeArea;
    }
