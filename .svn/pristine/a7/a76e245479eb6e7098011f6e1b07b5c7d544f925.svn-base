...
@Component
@Slf4j
public class CouponAndIntegralMessageJobHandler {
    ...
    @XxlJob(value = "CouponAndIntegralMessageJobHandler")
...
//        handleIntegralExpire();
        ...
        try {
            ...
            handleGrowthValueAndIntegralExpireAndBalanceChange(customerDetailPageRequest, startTime, endTime,
                    pointsConfig);
        } catch (Exception e) {
            log.error("成长值到账 - 积分过期 - 余额账户变更任务异常", e);
            exceptionList.add(e);
        }
        //余额账户变更
//        handleBalanceChange(startTime, endTime);
    ...
    private void handleIntegralRecept(LocalDateTime startTime, LocalDateTime endTime, AccoutAssetsType nodeCode) {
        ...
        do {
            ...
            assert page != null;
            if (Objects.nonNull(page) && page.getNumberOfElements() > 0) {
                List<CustomerStatisticsPointsVO> list = page.getContent();
                for (CustomerStatisticsPointsVO vo : list) {
                    this.sendMessage(NodeType.ACCOUNT_ASSETS, nodeCode, Collections.singletonList(vo.getPoints().toString()), null, vo.getCustomerId(), vo.getCustomerAccount());
                }
            }
        } while (flag);
    }
...
//                    BigDecimal sum = couponInfoVOS.stream().map(CouponInfoVO::getDenomination)
    ...
    private void handleGrowthValueAndIntegralExpireAndBalanceChange(CustomerDetailPageRequest customerDetailPageRequest, LocalDateTime startTime,
        ...
        customerIds.parallelStream().forEach(customerId -> {
            ...
            try {
                ...
                if (Objects.nonNull(growthValueSum) && growthValueSum > 0) {
                ...
                log.error("{}成长值到账异常", customerId, e);
            }
            //积分过期
            try {
                if (pointsExpireMonth.equals(LocalDate.now().getMonthValue()) && pointsExpireDay.equals(LocalDate.now().plusDays(Constants.SEVEN).getDayOfMonth())) {
            ...
            try {
                ...
                if (customerFundsTodayResponse.getPaymentAmount().compareTo(BigDecimal.ZERO) > 0 || customerFundsTodayResponse.getReceiptAmount().compareTo(BigDecimal.ZERO) > 0) {
                ...
                log.error("{}余额账户变更异常", customerId, e);
            }
        });
