...
public class AcpService {
	...
	public static String send(String url, Map<String, String> dataMap) {
		...
		try {
			...
			entity = new UrlEncodedFormEntity(formpair, StandardCharsets.UTF_8.name());
			if (entity != null) {
				post.setEntity(entity);
			}
			resp = client.execute(post);
			if (resp.getStatusLine().getStatusCode() == Constants.NUM_200) {
				InputStream is = null;
				ByteArrayOutputStream baos = new ByteArrayOutputStream();
				try {
					byte[] buffer = new byte[Constants.NUM_1024];
					is = resp.getEntity().getContent();
					int count = is.read(buffer);
					while (count != Constants.MINUS_ONE) {
	 ...
	 * @param encoding 上送请求报文域encoding字段的值<br>
	 * @return　签名后的map对象<br>for(Iterator<String> it = customerInfoMap.keySet().iterator(); it.hasNext();){
	 * 			String key = it.next();
	...
	public static Map<String,String> post(
		...
		//发送后台请求数据
		HttpClient hc = new HttpClient(reqUrl, Constants.NUM_30000, Constants.NUM_30000);//连接超时时间，读超时时间（可自行判断，修改）
		try {
	...
	public static String get(String reqUrl,String encoding) {
		...
		LogUtil.writeLog("请求银联地址:" + reqUrl);
		//发送后台请求数据
		HttpClient hc = new HttpClient(reqUrl, Constants.NUM_30000, Constants.NUM_30000);
		try {
	...
	public static String createAutoFormHtml(String reqUrl, Map<String, String> hiddens,String encoding) {
		...
		sb.append("<form id = \"pay_form\" action=\"");
		sb.append(reqUrl);
	...
	public static String getFileContent(String fileContent,String encoding){
		String fc = "";
		try {
			fc = new String(SDKUtil.inflater(SecureUtil.base64Decode(fileContent.getBytes(StandardCharsets.UTF_8))),encoding);
		} catch (UnsupportedEncodingException e) {
			LogUtil.writeErrorLog(e.getMessage(), e);
		} catch (IOException e) {
			LogUtil.writeErrorLog(e.getMessage(), e);
	...
	public static String getCustomerInfo(Map<String,String> customerInfoMap,String accNo,String encoding) {
		...
		StringBuilder sb = new StringBuilder("{");
		Integer index = 0;
		Integer size = customerInfoMap.size();
		for (Entry<String, String> entry : customerInfoMap.entrySet()) {
			...
			sb.append(key).append(SDKConstants.EQUAL).append(value);
			if(index != size - 1)
				sb.append(SDKConstants.AMPERSAND);
			index++;
		}
		String customerInfo = sb.append('}').toString();
		LogUtil.writeLog("组装的customerInfo明文："+customerInfo);
		try {
			return new String(SecureUtil.base64Encode(sb.toString().getBytes(
					encoding)),encoding);
		} catch (UnsupportedEncodingException e) {
			LogUtil.writeErrorLog(e.getMessage(), e);
		} catch (IOException e) {
	...
	public static String getCustomerInfoWithEncrypt(Map<String,String> customerInfoMap,String accNo,String encoding) {
		...
		if(!encryptedInfoSb.toString().equals("")){
			...
			LogUtil.writeLog(encryptedInfoSb.toString());
			sb.append("encryptedInfo").append(SDKConstants.EQUAL).append(encryptData(encryptedInfoSb.toString(), encoding));
		}else{
		...
		LogUtil.writeLog("组装的customerInfo明文："+customerInfo);
		try {
			return new String(SecureUtil.base64Encode(sb.toString().getBytes(encoding)),encoding);
		} catch (UnsupportedEncodingException e) {
			LogUtil.writeErrorLog(e.getMessage(), e);
		} catch (IOException e) {
			LogUtil.writeErrorLog(e.getMessage(), e);
	...
	public static Map<String,String> parseCustomerInfo(String customerInfo,String encoding){
		...
		try {
				...
				String encInfoStr = customerInfoMap.remove("encryptedInfo");
				if(encInfoStr != null){
	...
	public static String getCardTransData(Map<String, String> cardTransDataMap,
		...
		StringBuilder cardTransDataBuilder = new StringBuilder();

		String track2Data = cardTransDataMap.get("track2Data");
		if(track2Data != null){
			StringBuilder track2Builder = new StringBuilder();
			track2Builder.append(requestData.get("merId"))
			...
			cardTransDataMap.put("track2Data",
					AcpService.encryptData(track2Builder.toString(),	encoding));
		}
		String track3Data = cardTransDataMap.get("track3Data");
		if(track3Data != null){
			StringBuilder track3Builder = new StringBuilder();
			track3Builder.append(requestData.get("merId"))
				.append(SDKConstants.COLON).append(requestData.get("orderId"))
				.append(SDKConstants.COLON).append(requestData.get("txnTime"))
				.append(SDKConstants.COLON).append(requestData.get("txnAmt")==null?0:requestData.get("txnAmt"))
				.append(SDKConstants.COLON).append(track3Data);
			cardTransDataMap.put("track3Data",
					AcpService.encryptData(track3Builder.toString(),	encoding));
		}

		return cardTransDataBuilder.append(SDKConstants.LEFT_BRACE)
