...
public interface CustomerDrawCashRepository extends JpaRepository<CustomerDrawCash, String>,
    ...
    @Query("update CustomerDrawCash set auditStatus = ?2,rejectReason = ?3, finishStatus=?4,drawCashStatus=?5," +
            "drawCashFailedReason=?6,accountBalance = ?7 , finishTime = now(),batchId = ?8,detailId = ?9 where drawCashId in (?1)")
    ...
    @Query("SELECT sum(drawCashSum) from CustomerDrawCash where customerId = ?1 " +
            "AND applyTime >= ?2  AND applyTime <= ?3 and ((auditStatus = com.wanmi.sbc.account.bean.enums.AuditStatus.WAIT " +
            "and customerOperateStatus = com.wanmi.sbc.account.bean.enums.CustomerOperateStatus.APPLY) or drawCashStatus = com.wanmi.sbc.account.bean.enums.DrawCashStatus.SUCCESS)")
    BigDecimal countDrawCashSum(@Param("customerId") String customerId,
                                @Param("beginTime") LocalDateTime beginTime,
                                @Param("endTime") LocalDateTime endTime);


    @Query("SELECT count(drawCashId) from CustomerDrawCash where auditStatus=?1 AND drawCashStatus=?2 " +
            "AND customerOperateStatus=?3 AND finishStatus=?4 " +
            "AND (COALESCE(?6, null) is null or customerId in (?6)) " +
            "AND delFlag=?5 ")
