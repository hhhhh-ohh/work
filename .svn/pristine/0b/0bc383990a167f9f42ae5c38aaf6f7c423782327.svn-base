...
public class MarketingController implements MarketingProvider {
    ...
    @Override
    public BaseResponse close(@Valid MarketingCloseRequest request) {
        marketingService.closeActivity(request.getMarketingId(), request.getStoreId(),request.getPlatform());
        return BaseResponse.SUCCESSFUL();
    }

    @Override
    public BaseResponse pauseModify(@RequestBody @Valid PauseModifyRequest request) {
        marketingService.pauseModify(request);
        return BaseResponse.SUCCESSFUL();
    }
}
