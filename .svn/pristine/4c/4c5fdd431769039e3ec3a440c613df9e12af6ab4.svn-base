...
public class StandardGoodsQueryController implements StandardGoodsQueryProvider {
    ...
    public BaseResponse<StandardGoodsPageResponse> page(@RequestBody @Valid StandardGoodsPageRequest request) {
        ...
        if (CollectionUtils.isNotEmpty(queryResponse.getGoodsCateList())) {
            pageResponse.setGoodsCateList(queryResponse.getGoodsCateList());
        }
        if (CollectionUtils.isNotEmpty(queryResponse.getGoodsBrandList())) {
            pageResponse.setGoodsBrandList(queryResponse.getGoodsBrandList());
        }
        if (CollectionUtils.isNotEmpty(queryResponse.getStandardSkuList())) {
            pageResponse.setStandardSkuList(queryResponse.getStandardSkuList());
        }
        if (CollectionUtils.isNotEmpty(queryResponse.getStandardSkuSpecDetails())) {
            pageResponse.setStandardSkuSpecDetails(queryResponse.getStandardSkuSpecDetails());
        }
     ...
     */
    @Override
    public BaseResponse<List<StandardGoodsVO>> listStandardGoods(@RequestBody StandardGoodsPageRequest request) {
        StandardQueryRequest standardQueryRequest = KsBeanUtil.convert(request, StandardQueryRequest.class);
        return BaseResponse.success(KsBeanUtil.convert(standardGoodsService.listStandardGoods(standardQueryRequest), StandardGoodsVO.class));
    }
     ...
     */
    @Override
    public BaseResponse<List<StandardSkuVO>> listStandardSku(StandardSkuListQueryRequest request) {
        StandardSkuQueryRequest standardSkuQueryRequest = KsBeanUtil.convert(request, StandardSkuQueryRequest.class);
        return BaseResponse.success(KsBeanUtil.convert(standardGoodsService.listStandardSku(standardSkuQueryRequest), StandardSkuVO.class));
    }
    ...
    public BaseResponse<StandardGoodsByIdResponse> getById(@RequestBody @Valid StandardGoodsByIdRequest request) {
        ...
        StandardGoodsVO standardGoods = standardEditResponse.getGoods();
     ...
     */
    @Override
    public BaseResponse<StandardGoodsByIdResponse> getByProviderId(@RequestBody @Valid StandardGoodsByIdRequest request) {
        StandardEditResponse standardEditResponse = standardGoodsService.findInfoByProviderGoodsId(request.getGoodsId());
        StandardGoodsByIdResponse response = KsBeanUtil.convert(standardEditResponse, StandardGoodsByIdResponse.class);
        StandardGoodsVO standardGoods = standardEditResponse.getGoods();
        if (Objects.nonNull(standardGoods) && Objects.nonNull(standardGoods.getCateId())){
            GoodsCate goodsCate = goodsCateService.findById(standardGoods.getCateId());
