...
import com.wanmi.sbc.customer.api.request.storelevel.StoreLevelByStoreIdAndLevelNameRequest;
import com.wanmi.sbc.customer.api.request.storelevel.StoreLevelDelByIdRequest;
import com.wanmi.sbc.customer.api.request.storelevel.StoreLevelListModifyRequest;
import com.wanmi.sbc.customer.api.request.storelevel.StoreLevelListRequest;
import com.wanmi.sbc.customer.api.request.storelevel.StoreLevelModifyRequest;
 ...
 * @since 2019/2/28
 */
@Tag(name =  "商铺会员等级", description =  "StoreLevelController")
@RestController
@Validated
...
public class StoreLevelController {
     ...
     * @return
     */
    @Operation(summary = "删除商铺会员等级")
    @Parameter(name = "storeLevelId", description = "店铺等级Id",
            required = true)
    ...
    private String checkLevelName(Long storeLevelId, String levelName, BigDecimal discountRate, BigDecimal amountConditions, Integer orderConditions) {
        ...
        if (!(length >= 1 && length <= Constants.TEN)) {
        ...
        if (storeLevelId == null) {
            if (storeLevelVOS.size() >= Constants.TEN) {
                return "最多支持10个客户等级";
            }
        }
        ...
        BigDecimal result = discountRate.setScale(2, RoundingMode.HALF_UP);
