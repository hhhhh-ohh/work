...
public class DistributionCustomerWhereCriteriaBuilder {
    public static Specification<DistributionCustomer> build(DistributionCustomerQueryRequest queryRequest) {
        return (root, cquery, cbuild) -> {
            ...
            // 邀新人数-从
            if (Objects.nonNull(queryRequest.getInviteCountStart()) && queryRequest.getInviteCountStart().compareTo(0) >= 0) {
                predicates.add(cbuild.ge(root.get("inviteCount"), queryRequest.getInviteCountStart()));
            }

            // 邀新人数-至
            if (Objects.nonNull(queryRequest.getInviteCountEnd()) && queryRequest.getInviteCountEnd().compareTo(0) >= 0) {
                predicates.add(cbuild.le(root.get("inviteCount"), queryRequest.getInviteCountEnd()));
            }

            // 有效邀新人数-从
            if (Objects.nonNull(queryRequest.getInviteAvailableCountStart()) && queryRequest.getInviteAvailableCountStart().compareTo(0) >= 0) {
                predicates.add(cbuild.ge(root.get("inviteAvailableCount"), queryRequest.getInviteAvailableCountStart()));
            }

            // 有效邀新人数-至
            if (Objects.nonNull(queryRequest.getInviteAvailableCountEnd()) && queryRequest.getInviteAvailableCountEnd().compareTo(0) >= 0) {
                predicates.add(cbuild.le(root.get("inviteAvailableCount"), queryRequest.getInviteAvailableCountEnd()));
            }

            // 邀新奖金(元)-从
            if (Objects.nonNull(queryRequest.getRewardCashStart()) && queryRequest.getRewardCashStart().compareTo(BigDecimal.ZERO) >= 0) {
                predicates.add(cbuild.ge(root.get("rewardCash"), queryRequest.getRewardCashStart()));
            }

            // 邀新奖金(元)-至
            if (Objects.nonNull(queryRequest.getRewardCashEnd()) && queryRequest.getRewardCashEnd().compareTo(BigDecimal.ZERO) >= 0) {
                predicates.add(cbuild.le(root.get("rewardCash"), queryRequest.getRewardCashEnd()));
            }

            // 分销订单(笔)-从
            if (Objects.nonNull(queryRequest.getDistributionTradeCountStart()) && queryRequest.getDistributionTradeCountStart().compareTo(0) >= 0) {
                predicates.add(cbuild.ge(root.get("distributionTradeCount"), queryRequest.getDistributionTradeCountStart()));
            }

            // 分销订单(笔)-至
            if (Objects.nonNull(queryRequest.getDistributionTradeCountEnd()) && queryRequest.getDistributionTradeCountEnd().compareTo(0) >= 0) {
                predicates.add(cbuild.le(root.get("distributionTradeCount"), queryRequest.getDistributionTradeCountEnd()));
            }

            // 销售额(元)-从
            if (Objects.nonNull(queryRequest.getSalesStart()) && queryRequest.getSalesStart().compareTo(BigDecimal.ZERO) >= 0) {
                predicates.add(cbuild.ge(root.get("sales"), queryRequest.getSalesStart()));
            }

            // 销售额(元)-至
            if (Objects.nonNull(queryRequest.getSalesEnd()) && queryRequest.getSalesEnd().compareTo(BigDecimal.ZERO) >= 0) {
                predicates.add(cbuild.le(root.get("sales"), queryRequest.getSalesEnd()));
            }

            // 分销佣金(元)-从
            if (Objects.nonNull(queryRequest.getCommissionStart()) && queryRequest.getCommissionStart().compareTo(BigDecimal.ZERO) >= 0) {
                predicates.add(cbuild.ge(root.get("commission"), queryRequest.getCommissionStart()));
            }

            // 分销佣金(元)-至
            if (Objects.nonNull(queryRequest.getCommissionEnd()) && queryRequest.getCommissionEnd().compareTo(BigDecimal.ZERO) >= 0) {
