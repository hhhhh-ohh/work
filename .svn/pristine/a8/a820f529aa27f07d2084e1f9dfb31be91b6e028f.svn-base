...
public class StandardExcelService {
    ...
    @Value("classpath:/download/standard_template.xls")
    private Resource templateFile;

    @Value("classpath:/download/virtual_standard_template.xls")
    private Resource virtualTemplateFile;
     ...
     * @return base64位文件字符串
     */
    public String exportTemplate(Integer goodsType) {
        Resource resource;
        if(NumberUtils.INTEGER_ZERO.equals(goodsType)) {
            ...
            if (virtualTemplateFile == null || !virtualTemplateFile.exists()) {
                throw new SbcRuntimeException(GoodsErrorCodeEnum.K030147);
            }
            resource = virtualTemplateFile;
        }
        //根据店铺获取品牌
        ...
        if(NumberUtils.INTEGER_ZERO.equals(goodsType)){
            cates = goodsCateRepository.queryLeaf();
        } else {
            cates = goodsCateRepository.queryLeaf(BoolFlag.YES);
        }
