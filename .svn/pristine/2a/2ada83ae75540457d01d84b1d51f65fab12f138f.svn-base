...
@Tag(name =  "管理员登录/登出API", description =  "LoginController")
public class LoginController {
     ...
     */
    @Operation(summary = "管理员登录接口，会员登录")
    @RequestMapping(value = "/login", method = RequestMethod.POST)
    public BaseResponse<LoginResponse> login(@Valid @RequestBody
                                             @Parameter(name = "loginRequest", description = "管理员登录信息", required = true)
        ...
        // 禁止使用绑定的手机号码去登录
        return loginByAccount(account, passwordChars, response);
    }
     ...
     * @return
     */
    private BaseResponse<LoginResponse> loginByAccount(String account, char[] password, HttpServletResponse response) {
    ...
    public SbcRuntimeException validateNull(String account) {
        ...
        if (error < PASS_WRONG_MAX_COUNTS) {
            //用户名或密码错误，还有几次机会
            throw new SbcRuntimeException(CustomerErrorCodeEnum.K010078, new
    ...
    private BaseResponse<LoginResponse> customerValidate(EmployeeVO employee, char[] password, HttpServletResponse
        ...
        if (AccountState.DISABLE.equals(employee.getAccountState())) {
            ...
            // 账户被锁定
            throw new SbcRuntimeException(CustomerErrorCodeEnum.K010087, new Object[]{""});
        }
        //输错密码，账号被锁定
        if (Objects.nonNull(employee.getLoginLockTime())) {
            if (LocalDateTime.now().isAfter(employee.getLoginLockTime().plus(LOCK_MINUTES, ChronoUnit.MINUTES))) {
                ...
                if (!unlockResponse.getCode().equals(CommonErrorCodeEnum.K000000.getCode())) {
                    throw new SbcRuntimeException(CommonErrorCodeEnum.K999999, unlockResponse.getMessage());
                }
                currentErrCount = 0;
            }
        }
        ...
        if (employee.getAccountPassword().equals(encodePwd)) {
            ...
            if (Objects.nonNull(employee.getLoginLockTime()) && LocalDateTime.now().isBefore(employee
                ...
                //请{0}分钟后重试
                throw new SbcRuntimeException(CustomerErrorCodeEnum.K010092, new Object[]{LocalDateTime.now().until
            ...
            Boolean vasRecommendFlag = false;
            ...
            String token = Jwts.builder().setSubject(employee.getAccountName())
                    ...
                    .setIssuedAt(now)
                    .setClaims(claims)
                    .setExpiration(Date.from(Instant.now().plus(12, ChronoUnit.HOURS))) // 有效期一个星期
                    .compact();
            ...
            return BaseResponse.success(LoginResponse.builder()
                    ...
                    .token(token)
                    .vasRecommendFlag(vasRecommendFlag)
                    .build());
        } else {
            ...
            if (!idResponse.getCode().equals(CommonErrorCodeEnum.K000000.getCode())) {
                throw new SbcRuntimeException(CommonErrorCodeEnum.K999999, idResponse.getMessage());
            }
            ...
            if (currentErrCount < PASS_WRONG_MAX_COUNTS) {
                ...
                long retryMinutes = LocalDateTime.now()
