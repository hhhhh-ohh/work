...
public class DistributionGoodsMatterController implements DistributionGoodsMatterProvider {
     ...
     */
    @Override
    public BaseResponse<String> add(@RequestBody @Valid DistributionGoodsMatteAddRequest distributionGoodsMatteAddRequest) {
        ...
        //营销素材配置图片链接的，预生成小程序码
        if(distributionGoodsMatteAddRequest.getMatterType() == MatterType.MARKETING){
           List<MarketingMaterialDTO> marketingMaterialDTOS =
                   JSONArray.parseArray(distributionGoodsMatteAddRequest.getMatter(),MarketingMaterialDTO.class);
        ...
        String id = distributionGoodsMatterService.addDistributionGoodsMatter(distributionGoodsMatter);
    ...
    public BaseResponse<DistributionGoodsMatterModifyRequest> updataQrcode(@RequestBody @Valid DistributionGoodsMatterModifyRequest distributionGoodsMatterModifyRequest){
        if(distributionGoodsMatterModifyRequest.getMatterType() == MatterType.MARKETING){
            List<MarketingMaterialDTO> marketingMaterialDTOS =
                    JSONArray.parseArray(distributionGoodsMatterModifyRequest.getMatter(),MarketingMaterialDTO.class);
            marketingMaterialDTOS.forEach(item->{
                //把链接加密，生成redisKey，作为参数传递，用来生成小程序码
                if(StringUtils.isNotBlank(item.getLink()) && item.getLink()!=null){
                    String redisKey = RedisKeyConstant.QR_CODE_LINK.concat((MD5Util.md5Hex(item.getLink(), "utf-8")).toUpperCase().substring(16));
     ...
     */
    @Override
    public BaseResponse modifyByStoreId(@RequestBody @Valid DistributionGoodsMatterModifyByStoreIdRequest distributionGoodsMatterModifyRequest) {
        DistributionGoodsMatter distributionGoodsMatter = KsBeanUtil.copyPropertiesThird(distributionGoodsMatterModifyRequest, DistributionGoodsMatter.class);
        if (Objects.nonNull(distributionGoodsMatter.getStoreState())){
            distributionGoodsMatterService.updateStoreStateByStoreId(distributionGoodsMatter);
        }else if (Objects.nonNull(distributionGoodsMatter.getOpenFlag())){
            distributionGoodsMatterService.updateOpenFlagByStoreId(distributionGoodsMatter);
        } else if (Objects.nonNull(distributionGoodsMatter.getContractEndDate())){
            distributionGoodsMatterService.updateContractEndDateByStoreId(distributionGoodsMatter);
        }
        return BaseResponse.SUCCESSFUL();
    }
    ...
    public BaseResponse updataRecomendNumById(@RequestBody @Valid UpdateRecommendNumRequest updateRecommendNumRequest) {
        ...
        if(!Objects.isNull(distributionGoodsMatter)){
            ...
            throw new SbcRuntimeException(CommonErrorCodeEnum.K000001, "查找不到分销素材");
        }
    }
}
