...
public class TradeQueryController implements TradeQueryProvider {
    ...
    public BaseResponse<TradePageCriteriaResponse> supplierPageCriteria(@RequestBody @Valid TradePageCriteriaRequest tradePageCriteriaRequest) {
        // 根据providerTradeId模糊查询ProviderTrade,获取tradeId
        TradeQueryDTO tradeQuery = tradePageCriteriaRequest.getTradePageDTO();
        if (StringUtils.isNotBlank(tradeQuery.getProviderTradeId()) || StringUtils.isNotBlank(tradeQuery.getProviderName())) {
            List<ProviderTrade> providerTrades = providerTradeService.queryAll(ProviderTradeQueryRequest.builder()
                    .providerName(tradeQuery.getProviderName())
                    .id(tradeQuery.getProviderTradeId())
                    .build());

            if (CollectionUtils.isEmpty(providerTrades)) {
                return BaseResponse.success(TradePageCriteriaResponse.builder().tradePage(new MicroServicePage<>()).build());
            }

            List<String> tradeIds = providerTrades.stream().map(ProviderTrade::getParentId).toList();
            String[] ids = tradeIds.toArray(new String[0]);
            tradeQuery.setIds(ids);

        }

        tradeQuery.setProviderTradeId(null);
        tradeQuery.setProviderName(null);
        TradeQueryRequest tradeQueryRequest = KsBeanUtil.convert(tradeQuery, TradeQueryRequest.class);
        if (StringUtils.isNotBlank(tradeQuery.getDeliverNo())) {
            List<String> logisticNos = Arrays.stream(tradeQuery.getDeliverNo().split(",")).toList();
            tradeQueryRequest.setLogisticNos(logisticNos);
        }
        Criteria criteria;

        if (Objects.nonNull(tradeQuery.getQueryOrderType())) {
            switch (tradeQuery.getQueryOrderType()) {
                case GROUPON:
                    tradeQueryRequest.setGrouponFlag(Boolean.TRUE);
                    break;
                case FLASH_SALE:
    ...
    public BaseResponse<TradePageCriteriaResponse> pageBossCriteria(@RequestBody @Valid TradePageCriteriaRequest tradePageCriteriaRequest) {
        // 根据 providerTradeId 模糊查询 ProviderTrade,获取 tradeId
        TradeQueryDTO tradeQuery = tradePageCriteriaRequest.getTradePageDTO();
        if (StringUtils.isNotBlank(tradeQuery.getProviderTradeId()) || StringUtils.isNotBlank(tradeQuery.getProviderName()) || StringUtils.isNotEmpty(tradeQuery.getProviderCode())) {
            List<ProviderTrade> providerTrades = providerTradeService.queryAll(ProviderTradeQueryRequest.builder()
                    .providerName(tradeQuery.getProviderName())
                    .providerCode(tradeQuery.getProviderCode())
                    .id(tradeQuery.getProviderTradeId())
                    .build()
            );
        ...
        tradeQuery.setProviderTradeId(null);
        tradeQuery.setProviderName(null);
        tradeQuery.setProviderCode(null);
        TradeQueryRequest tradeQueryRequest = KsBeanUtil.convert(tradeQuery, TradeQueryRequest.class);
        if (StringUtils.isNotBlank(tradeQuery.getDeliverNo())) {
            List<String> logisticNos = Arrays.stream(tradeQuery.getDeliverNo().split(",")).toList();
            tradeQueryRequest.setLogisticNos(logisticNos);
        ...
        if (tradeQuery.getQueryOrderType() != null) {
            switch (tradeQuery.getQueryOrderType()) {
                case GROUPON:
                    tradeQueryRequest.setGrouponFlag(Boolean.TRUE);
                    break;
        ...
        if (StringUtils.isNotEmpty(tradeQuery.getBuyerId())) {
        ...
        if (CollectionUtils.isNotEmpty(tradeVOS.getContent())) {
            //填充团长名称
            if (Boolean.TRUE.equals(tradeQuery.getFillLeaderNameFlag())) {
                this.fillLeaderName(tradeVOList);
            }
