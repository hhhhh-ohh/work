...
import com.wanmi.sbc.marketing.api.request.appointmentsale.AppointmentSaleCloseRequest;
...
public class AppointmentSaleController implements AppointmentSaleProvider {
    ...
    @Override
    public BaseResponse close(@RequestBody @Valid AppointmentSaleCloseRequest request) {
        appointmentSaleService.closeActivity(request.getId(), request.getStoreId());
        return BaseResponse.SUCCESSFUL();
    }

    @Override
    public BaseResponse sycCache(){
        appointmentSaleService.sycCache();
        return BaseResponse.SUCCESSFUL();
    }
}
