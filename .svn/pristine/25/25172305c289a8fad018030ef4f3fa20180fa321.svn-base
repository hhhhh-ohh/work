...
import com.wanmi.sbc.setting.api.request.searchterms.AssociationLongTailWordDeleteByIdRequest;
import com.wanmi.sbc.setting.api.request.searchterms.AssociationLongTailWordModifyRequest;
import com.wanmi.sbc.setting.api.request.searchterms.AssociationLongTailWordRequest;
import com.wanmi.sbc.setting.api.request.searchterms.AssociationLongTailWordSortRequest;
import com.wanmi.sbc.setting.api.request.searchterms.SearchAssociationalWordDeleteByIdRequest;
import com.wanmi.sbc.setting.api.request.searchterms.SearchAssociationalWordModifyRequest;
import com.wanmi.sbc.setting.api.request.searchterms.SearchAssociationalWordRequest;
...
public class SearchAssociationalWordController implements SearchAssociationalWordProvider {
    ...
    public BaseResponse addAssociationalWord(@Valid AssociationLongTailWordRequest request) {
        ...
        if (CollectionUtils.isNotEmpty(associationWordList)){
            // 一个搜索词下面最多20个联想词
            if (associationWordList.size() >= Constants.NUM_20) {
                throw new SbcRuntimeException(SettingErrorCodeEnum.K070076);
            }
            // 同一搜索词的联想词名称不能重复
            if (associationWordList.stream().anyMatch(word -> StringUtils.equals(request.getAssociationalWord(),
                    word.getAssociationalWord()))) {
                throw new SbcRuntimeException(SettingErrorCodeEnum.K070013);
            }
        }
    ...
    public BaseResponse modifyAssociationalWord(@Valid AssociationLongTailWordModifyRequest request) {
        ...
        if (Objects.isNull(oldAssociationWord)){
            throw new SbcRuntimeException(SettingErrorCodeEnum.K070016);
        }
        // 同一搜索词的联想词名称不能重复
        Integer repeatName =
    ...
    public void setLongTailWord(AssociationLongTailWord associationLongTailWord, List<String> longTails){
        ...
        if (CollectionUtils.isNotEmpty(longTails)) {
            // 一个联想词最多可添加3个长尾词
            if (longTails.size() > Constants.THREE){
                throw new SbcRuntimeException(SettingErrorCodeEnum.K070014);
            }
            // 一个联想词的长尾词不能重复
            List<String> newTails = longTails.stream().distinct().collect(Collectors.toList());
            if (longTails.size() != newTails.size()){
                throw new SbcRuntimeException(SettingErrorCodeEnum.K070015);
            }
            ...
            for (String longTailWord : longTails) {
                ...
                if (longTailWord.length() > 5) {
                    throw new SbcRuntimeException(SettingErrorCodeEnum.K070077);
                }
