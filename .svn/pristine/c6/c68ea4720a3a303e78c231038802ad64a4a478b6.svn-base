...
public interface OfflineRepository extends JpaRepository<OfflineAccount, Long> {
     ...
     * @return 线下账户列表
     */
    @Query("from OfflineAccount off where off.accountId in (:offlineAccountIds)")
    List<OfflineAccount> findOfflineAccountByIds(@Param("offlineAccountIds") List<Long> offlineAccountIds);
}
