...
@Component
@Slf4j
public class LinkedMallOrderJobHandler {
     ...
     * linkedMall订单定时任务
     */
    @XxlJob(value = "linkedMallOrderJobHandler")
    public void execute() throws Exception {
        boolean flag = commonUtil.findVASBuyOrNot(VASConstants.THIRD_PLATFORM_LINKED_MALL);
        if (!flag) {
            return;
        }
        XxlJobHelper.log("linkedmall订单定时任务执行 " + LocalDateTime.now());
            ...
            tradeVOList.stream()
                    ...
                    .forEach(tradeVO -> {
                        if (Objects.nonNull(tradeVO.getBuyer()) && StringUtils.isNotBlank(tradeVO.getBuyer().getId())) {
                            ...
                            List<TradeVO> tradeVOS = map.get(customerId);
            ...
            map.forEach((customerId, tradeVOs) -> {
                ...
                if (Objects.nonNull(response) && CollectionUtils.isNotEmpty(response.getLmOrderList())) {
                    response.getLmOrderList().forEach(lmOrderListItem -> {
                        ...
                        if (Objects.nonNull(tradeVOMap.get(lmOrderId))) {
    ...
    private void fillTradeStatusByLinkedMallOrderStatus(List<ThirdPlatformTradeUpdateStateDTO> tradeUpdateStates, TradeVO tradeVO, Integer orderStatus,
        ...
        if (Integer.valueOf(Constants.SIX).equals(orderStatus) || Integer.valueOf(Constants.THREE).equals(logisticsStatus)) {
            flowState = FlowState.COMPLETED;
        } else if (Integer.valueOf(Constants.FOUR).equals(orderStatus)) {
            flowState = FlowState.VOID;
        } else if (Integer.valueOf(Constants.EIGHT).equals(orderStatus)) {
            flowState = FlowState.VOID;
        } else if (Integer.valueOf(Constants.NUM_12).equals(orderStatus)) {
            flowState = FlowState.AUDIT;
        } else if (Integer.valueOf(Constants.TWO).equals(orderStatus) && (Integer.valueOf(Constants.TWO).equals(logisticsStatus) ||
                Integer.valueOf(Constants.FIVE).equals(logisticsStatus) || Integer.valueOf(Constants.SIX).equals(logisticsStatus))) {
            flowState = FlowState.DELIVERED;
        } else if (Integer.valueOf(Constants.TWO).equals(orderStatus) &&
                (Integer.valueOf(Constants.ONE).equals(logisticsStatus) || Integer.valueOf(Constants.EIGHT).equals(logisticsStatus))) {
            flowState = FlowState.AUDIT;
        } else if (Integer.valueOf(Constants.FOUR).equals(logisticsStatus)) {
            flowState = FlowState.VOID;
        }

        if (Integer.valueOf(Constants.ONE).equals(logisticsStatus) ||
                (Integer.valueOf(Constants.EIGHT).equals(logisticsStatus) && Integer.valueOf(Constants.NUM_12).equals(orderStatus))) {
            deliverStatus = DeliverStatus.NOT_YET_SHIPPED;
        } else if (Integer.valueOf(Constants.TWO).equals(logisticsStatus) || Integer.valueOf(Constants.THREE).equals(logisticsStatus)) {
            deliverStatus = DeliverStatus.SHIPPED;
        } else if (Integer.valueOf(Constants.FIVE).equals(logisticsStatus) || Integer.valueOf(Constants.SIX).equals(logisticsStatus)) {
            deliverStatus = DeliverStatus.PART_SHIPPED;
        } else if (Integer.valueOf(Constants.FOUR).equals(logisticsStatus) || Integer.valueOf(Constants.FOUR).equals(orderStatus) ||
                (Integer.valueOf(Constants.EIGHT).equals(logisticsStatus) && Integer.valueOf(Constants.EIGHT).equals(orderStatus))) {
        ...
        if (PayState.UNCONFIRMED.equals(tradeVO.getTradeState().getPayState()) && Integer.valueOf(Constants.TWO).equals(orderStatus)) {
            tradeVO.getTradeState().setPayState(PayState.PAID);
            updateFlag = true;
        }

        if (PayState.UNCONFIRMED.equals(tradeVO.getTradeState().getPayState()) && Integer.valueOf(Constants.NUM_12).equals(orderStatus)) {
