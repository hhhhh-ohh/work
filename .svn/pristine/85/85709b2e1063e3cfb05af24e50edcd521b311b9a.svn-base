...
/**
 * @author lvzhenwei
 * @className DrawCashModifyCustomerAccountService
 * @description 会员账户信息修改,触发账户模块-会员提现管理的会员账户修改
 * @date 2021/8/13 4:44 下午
 */
@Slf4j
@Service
public class ModifyDrawCashCustomerAccountService {

    @Autowired private AccountMqConsumerProvider accountMqConsumerProvider;

    @Bean
    public Consumer<Message<String>> mqModifyDrawCashCustomerAccountService() {
        return message->{
            ...
            accountMqConsumerProvider.modifyDrawCashCustomerAccount(accountMqConsumerRequest);
        };
    }
}
