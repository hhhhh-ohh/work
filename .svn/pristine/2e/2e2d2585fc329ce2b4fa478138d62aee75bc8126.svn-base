...
public class CouponActivityQueryController implements CouponActivityQueryProvider {
    ...
    @Override
    public BaseResponse<CouponActivityByStartTimeResponse> getActivityByStartTime(@RequestBody @Valid CouponActivityByStartTimeRequest request) {
        List<CouponActivityVO> vos = couponActivityService.getActivityByStartTime(request.getMinute());
        return BaseResponse.success(CouponActivityByStartTimeResponse.builder().couponActivityVOList(vos).build());
    }

    @Override
    public BaseResponse<CouponActivityConfigBySceneResponse> getCouponActivityConfigByScene(CouponActivityConfigBySceneRequest request) {
        CouponActivityConfig config = couponActivityService.getCouponActivityConfigByScene(request.getScene());
        if (Objects.isNull(config)) {
            throw new SbcRuntimeException(MarketingErrorCodeEnum.K080053);
        }
        CouponActivityConfigBySceneResponse response = KsBeanUtil.convert(config,
                CouponActivityConfigBySceneResponse.class);
        return BaseResponse.success(response);
    }

    @Override
    public BaseResponse<CouponActivitySceneResponse> getSceneByCouponInfo(MagicCouponInfoRequest request) {
        CouponActivityConfig config = couponActivityConfigService.queryByActivityIdAndCouponId(request.getActivityId(), request.getCouponId());
        if (Objects.isNull(config)) {
            throw new SbcRuntimeException(MarketingErrorCodeEnum.K080053);
        }
        return BaseResponse.success(new CouponActivitySceneResponse(config.getScene()));
    }

    @Override
    public BaseResponse<CouponActivityGetByIdResponse> getByDrawActivityId(@RequestBody @Valid CouponActivityByDrawIdRequest request) {
        CouponActivityGetByIdResponse response = couponActivityService.getByDrawActivityId(request.getDrawActivityId());
        return BaseResponse.success(response);
    }
}
