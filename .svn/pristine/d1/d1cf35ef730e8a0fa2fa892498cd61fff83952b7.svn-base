...
public interface RestrictedRecordRepository extends JpaRepository<RestrictedRecord, Long>,
     ...
     * @return
     */
    @Query("from RestrictedRecord r where r.customerId = ?1 and r.goodsInfoId in ?2 and r.storeId = ?3")
    List<RestrictedRecord> findByCustomerIdAndGoodsInfoIdsAndStoreId(String customerId, List<String> goodsInfoIds,
                                                                     Long storeId);
