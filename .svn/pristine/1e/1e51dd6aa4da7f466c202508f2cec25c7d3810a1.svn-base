...
public interface SettlementRepository extends JpaRepository<Settlement, Long>, JpaSpecificationExecutor<Settlement> {
    ...
    @Modifying
    @Query("update LakalaSettlement set lakalaLedgerStatus = :settleStatus where settleUuid in (:uuids)")
    void updateLakalaSettleStatusBatch(@Param("settleStatus") LakalaLedgerStatus settleStatus,
    ...
    @Query(value = " SELECT * FROM settlement WHERE  store_id = :storeId and date_format(create_time, '%Y-%m-%d') != " +
            "CURRENT_DATE order by end_time desc LIMIT 1  ",nativeQuery = true)
