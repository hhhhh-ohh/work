...
public interface PointsGoodsSaveProvider {
     ...
     * 根据店铺id更新状态
     *
     * @param request 修改状态参数结构 {@link PointsGoodsModifyStatusByStoreIdRequest}
     * @return 状态结果 {@link BaseResponse}
     * @author dyt
     */
    @PostMapping("/goods/${application.goods.version}/pointsgoods/modify-status-by-store-id")
    BaseResponse modifyStatusByStoreId(@RequestBody @Valid PointsGoodsModifyStatusByStoreIdRequest request);

    /**
     * 关闭积分商品API
     *
     * @param request 单个删除参数结构 {@link PointsGoodsCloseRequest}
     * @return 删除结果 {@link BaseResponse}
     * @author yang
     */
    @PostMapping("/goods/${application.goods.version}/pointsgoods/close")
    BaseResponse closeById(@RequestBody @Valid PointsGoodsCloseRequest request);
}
