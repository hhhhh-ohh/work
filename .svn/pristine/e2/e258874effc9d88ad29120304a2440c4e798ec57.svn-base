...
public interface PurchaseQueryProvider {
    ...
    /**
     * 根据用户Id查询采购单信息  允许skuIdList 为空
     * @param request 查询采购单请求结构 {@link PurchaseQueryRequest}
     * @return {@link PurchaseQueryResponse}
     */
    @PostMapping("/order/${application.order.version}/purchase/list-by-customer-id")
    BaseResponse<PurchaseQueryResponse> listByCustomerId(@RequestBody @Valid PurchaseQueryRequest request);
