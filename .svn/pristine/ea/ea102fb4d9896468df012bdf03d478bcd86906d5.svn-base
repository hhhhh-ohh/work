...
public class SettlementQueryController implements SettlementQueryProvider {
    ...
    @Override
    public BaseResponse<SettlementPageResponse> lakalaPage(@RequestBody LakalaSettlementPageRequest request) {
        LakalaSettlementQueryRequest queryRequest = new LakalaSettlementQueryRequest();
        KsBeanUtil.copyPropertiesThird(request, queryRequest);
        Page<LakalaSettlementView> page = settlementService.queryLakalaSettlementPage(queryRequest);
        List<LakalaSettlementViewVO> vos = page.getContent().stream().map(view -> {
            LakalaSettlementViewVO vo = new LakalaSettlementViewVO();
            KsBeanUtil.copyPropertiesThird(view, vo);
            vo.setSettlementCode(String.format("js%07d", view.getSettleId()));
            return vo;
        }).collect(Collectors.toList());
        SettlementPageResponse response = SettlementPageResponse.builder()
                .lakalaSettlementViewVOPage(new MicroServicePage<>(vos, request.getPageRequest(), page.getTotalElements())).build();
        return BaseResponse.success(response);
    }
    ...
    @Override
    public BaseResponse<LakalaSettlementGetViewResponse> getLakalaView(@RequestBody @Valid LakalaSettlementGetViewRequest request) {
        LakalaSettlement settlement = KsBeanUtil.convert(request, LakalaSettlement.class);
        LakalaSettlementGetViewResponse response = KsBeanUtil.convert(settlementService.getLakalaSettlementById(settlement),
                LakalaSettlementGetViewResponse.class);
        return BaseResponse.success(response);
    }
    ...
    public BaseResponse<SettlementGetByIdResponse> getById(@RequestBody @Valid SettlementGetByIdRequest request) {
        ...
        if (Objects.isNull(settlement)) {
            throw new SbcRuntimeException(settlementService.getDeleteIndex(EsConstants.DOC_SETTLEMENT, request.getSettleId()), AccountErrorCodeEnum.K020010);
        }
        return BaseResponse.success(KsBeanUtil.convert(settlement, SettlementGetByIdResponse.class));
    }

    @Override
    public BaseResponse<LakalaSettlementGetByIdResponse> getLakalaById(@RequestBody @Valid SettlementGetByIdRequest request) {
        LakalaSettlement settlement = settlementService.queryLakalaSettlementById(request.getSettleId());
        if (Objects.isNull(settlement)) {
            throw new SbcRuntimeException(settlementService.getDeleteIndex(EsConstants.DOC_LAKALA_SETTLEMENT, request.getSettleId()),AccountErrorCodeEnum.K020010);
        }
        return BaseResponse.success(KsBeanUtil.convert(settlement, LakalaSettlementGetByIdResponse.class));
    }
    ...
    @Override
    public BaseResponse<LakalaSettlementGetByIdResponse> getLastLakalaSettlementByStoreId(@RequestBody @Valid SettlementLastByStoreIdRequest request){
        LakalaSettlement settlement = settlementService.getLastLakalaSettlementByStoreId(request.getStoreId());
        LakalaSettlementGetByIdResponse response = new LakalaSettlementGetByIdResponse();
        if (Objects.nonNull(settlement)) {
            response = KsBeanUtil.convert(settlement, LakalaSettlementGetByIdResponse.class);
        }else {
            response = null;
        }
        return BaseResponse.success(response);
    ...
    public BaseResponse<SettlementListResponse> initEsPage(@RequestBody @Valid SettlementPageRequest request){
        ...
        KsBeanUtil.copyPropertiesThird(request, queryRequest);
        return BaseResponse.success(
                SettlementListResponse.builder().lists(settlementService.initEsPage(queryRequest)).build());
    }

    @Override
    public BaseResponse<SettlementListResponse> initEsLakalaPage(@RequestBody @Valid SettlementPageRequest request){
        SettlementQueryRequest queryRequest = new SettlementQueryRequest();
        KsBeanUtil.copyPropertiesThird(request, queryRequest);
        return BaseResponse.success(
                SettlementListResponse.builder().lakalaSettlementViewVOS(settlementService.initEsLakalaPage(queryRequest)).build());
    }

    @Override
    public BaseResponse<GetBySettlementUuidResponse> getBySettlementUuid(
            @RequestBody @Valid GetBySettlementUuidRequest request) {
        return BaseResponse.success(
                GetBySettlementUuidResponse
                        .builder()
                        .lakalaSettlementVOS(settlementService.getBySettlementUuid(request))
                        .build());
    }
}
