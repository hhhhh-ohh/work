...
public interface PointsCouponRepository extends JpaRepository<PointsCoupon, Long>,
     ...
     */
    @Modifying
    @Query("update PointsCoupon set delFlag = com.wanmi.sbc.common.enums.DeleteFlag.YES where pointsCouponId = ?1")
     ...
     */
    @Modifying
    @Query("update PointsCoupon p set p.sellOutFlag = com.wanmi.sbc.common.enums.BoolFlag.YES where p.pointsCouponId = " +
            "?1 and p.exchangeCount = p.totalCount")
     ...
     * @return
     */
    @Query("select c from PointsCoupon c where c.endTime< now() and c.delFlag = com.wanmi.sbc.common.enums.DeleteFlag" +
            ".NO and status = 1")
    List<PointsCoupon> queryOverdueList();

    @Modifying
    @Query("update PointsCoupon set endTime = now() where pointsCouponId = ?1")
    void close(Long pointsCouponId);
}
