...
@Tag(name =  "S2B的拼团商品服务", description =  "GrouponGoodsController")
            ...
            if(CollectionUtils.isNotEmpty(activityVOList)) {
                ...
                List<GrouponGoodsVO> list = response.getGrouponGoodsVOS().getContent().stream()
                        ...
                        .map(g -> {
                            ...
                            List<GrouponActivityVO> activityVOS = activityVOList.stream().filter(a -> a.getGrouponActivityId().equals(g.getGrouponActivityId())).collect(Collectors.toList());
                            if(CollectionUtils.isNotEmpty(activityVOS)){
                                g.setStartTime(activityVOS.get(Constants.ZERO).getStartTime());
                                g.setEndTime(activityVOS.get(Constants.ZERO).getEndTime());
                            }
                    return g;
