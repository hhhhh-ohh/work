...
public interface StoreCateRepository extends JpaRepository<StoreCate, Long>, JpaSpecificationExecutor<StoreCate>{
     ...
     * @return
     */
    @Query("select count(1) from StoreCate s where s.cateName=:#{#storeCate.cateName} and s.storeId=:#{#storeCate.storeId} " +
     ...
     * @return 子分类结果
     */
    @Query("select new com.wanmi.sbc.goods.storecate.entity.StoreCateBase(g.storeCateId , g.cateName,g.cateParentId ) from StoreCate g where g.storeCateId in ?1 and g.delFlag = com.wanmi.sbc.common.enums.DeleteFlag.NO ")
    List<StoreCateBase> findAllByStoreCateIdIn(List<Long> storeCateIds);

    List<StoreCate> findByStoreIdAndDelFlag(Long storeId, DeleteFlag deleteFlag);
}
