package com.wanmi.sbc.order.trade.fsm.action;

import com.wanmi.sbc.common.base.Operator;
import com.wanmi.sbc.common.enums.CommonErrorCodeEnum;
import com.wanmi.sbc.common.exception.SbcRuntimeException;
import com.wanmi.sbc.common.util.Constants;
import com.wanmi.sbc.order.bean.dto.CycleDeliveryPlanDTO;
import com.wanmi.sbc.order.bean.dto.TradeBuyCycleDTO;
import com.wanmi.sbc.order.bean.enums.CycleDeliveryState;
import com.wanmi.sbc.order.bean.enums.DeliverStatus;
import com.wanmi.sbc.order.bean.enums.FlowState;
import com.wanmi.sbc.order.trade.fsm.TradeAction;
import com.wanmi.sbc.order.trade.fsm.TradeStateContext;
import com.wanmi.sbc.order.trade.fsm.event.TradeEvent;
import com.wanmi.sbc.order.trade.fsm.params.StateRequest;
import com.wanmi.sbc.order.trade.model.entity.TradeDeliver;
import com.wanmi.sbc.order.trade.model.entity.TradeItem;
import com.wanmi.sbc.order.trade.model.entity.TradeState;
import com.wanmi.sbc.order.trade.model.entity.value.ShippingItem;
import com.wanmi.sbc.order.trade.model.entity.value.TradeEventLog;
import com.wanmi.sbc.order.trade.model.root.OrderTag;
import com.wanmi.sbc.order.trade.model.root.ProviderTrade;
import com.wanmi.sbc.order.trade.model.root.Trade;
import org.apache.commons.collections4.CollectionUtils;
import org.apache.commons.lang3.StringUtils;
import org.springframework.stereotype.Component;

import java.math.BigDecimal;
import java.time.LocalDateTime;
import java.util.*;
import java.util.concurrent.ConcurrentHashMap;
import java.util.function.Function;
import java.util.stream.Collectors;

/**
 * 作废发货
 * Created by chenpeng on 2017/6/1.
 */
@Component
public class ObsoleteDeliverAction extends TradeAction {

    @Override
    protected void evaluateInternal(Trade trade, StateRequest request, TradeStateContext tsc) {
        obsoleteDeliveryRecords(trade, request.getOperator(), tsc.getRequestData());
    }

    /**
     * 是否全部未发货
     *
     * @param trade
     * @return
     */
    private boolean isAllNotShipped(Trade trade) {
        OrderTag orderTag = trade.getOrderTag();
        if (Objects.nonNull(orderTag) && Objects.equals(orderTag.getBuyCycleFlag(),Boolean.TRUE)) {
            TradeBuyCycleDTO tradeBuyCycle = trade.getTradeBuyCycle();
            List<CycleDeliveryPlanDTO> deliveryPlanS = tradeBuyCycle.getDeliveryPlanS();
            //已发货的计划为0，代表全部未发货
            long count = deliveryPlanS.parallelStream().filter(cycleDeliveryPlanDTO ->
                    Objects.equals(CycleDeliveryState.SHIPPED, cycleDeliveryPlanDTO.getCycleDeliveryState())).count();
            return Objects.equals(count, BigDecimal.ZERO.longValue());
        }
        List<TradeItem> allItems = new ArrayList<>();
        allItems.addAll(trade.getTradeItems());
        allItems.addAll(trade.getGifts());
        allItems.addAll(trade.getPreferential());
        List<TradeItem> collect = allItems.stream()
                .filter(tradeItem -> !tradeItem.getDeliveredNum().equals(0L))
                .collect(Collectors.toList());
        return collect.isEmpty();
    }
    /**
     * 是否全部未发货
     *
     * @param trade
     * @return
     */
    private boolean isAllNotShipped(ProviderTrade trade) {
        OrderTag orderTag = trade.getOrderTag();
        if (Objects.nonNull(orderTag) && Objects.equals(orderTag.getBuyCycleFlag(),Boolean.TRUE)) {
            TradeBuyCycleDTO tradeBuyCycle = trade.getTradeBuyCycle();
            List<CycleDeliveryPlanDTO> deliveryPlanS = tradeBuyCycle.getDeliveryPlanS();
            //已发货的计划为0，代表全部未发货
            long count = deliveryPlanS.parallelStream().filter(cycleDeliveryPlanDTO ->
                    Objects.equals(CycleDeliveryState.SHIPPED, cycleDeliveryPlanDTO.getCycleDeliveryState())).count();
            return Objects.equals(count, BigDecimal.ZERO.longValue());
        }
        List<TradeItem> allItems = new ArrayList<>();
        allItems.addAll(trade.getTradeItems());
        allItems.addAll(trade.getGifts());
        allItems.addAll(trade.getPreferential());
        List<TradeItem> collect = allItems.stream()
                .filter(tradeItem -> !tradeItem.getDeliveredNum().equals(0L))
                .collect(Collectors.toList());
        return collect.isEmpty();
    }

    /**
     * 作废发货记录
     *
     * @param trade
     * @param operator
     * @param deliverId
     */
    private void obsoleteDeliveryRecords(Trade trade, Operator operator, String deliverId) {

        List<TradeDeliver> tradeDelivers = trade.getTradeDelivers();
        //查询发货记录
        Optional<TradeDeliver> tradeDeliverOptional = tradeDelivers
                .stream()
                .filter(tradeDeliver -> StringUtils.equals(deliverId, tradeDeliver.getDeliverId()))
                .findFirst();

        if (tradeDeliverOptional.isPresent()) {
            StringBuilder stringBuilder = new StringBuilder(200);

            TradeDeliver tradeDeliver = tradeDeliverOptional.get();

            //处理商品
            handleShippingItems(trade, tradeDeliver.getShippingItems(), stringBuilder);

            //处理赠品
            handleGiftShippingItems(trade, tradeDeliver.getGiftItemList(),
                    stringBuilder);

            //处理加价购
            handlePreferentialShippingItems(trade, tradeDeliver.getPreferentialItemList(), stringBuilder);

            //周期购处理
            buyCycleObsoleteDeliveryRecords(trade,tradeDeliver);

            //订单状态更新
            TradeState tradeState = trade.getTradeState();
            if (isAllNotShipped(trade)) {
                tradeState.setFlowState(FlowState.AUDIT);
                tradeState.setDeliverStatus(DeliverStatus.NOT_YET_SHIPPED);
            } else {
                tradeState.setFlowState(FlowState.DELIVERED_PART);
                tradeState.setDeliverStatus(DeliverStatus.PART_SHIPPED);
            }

            //添加操作日志
            stringBuilder.trimToSize();
            trade.appendTradeEventLog(TradeEventLog
                    .builder()
                    .operator(operator)
                    .eventType(TradeEvent.OBSOLETE_DELIVER.getDescription())
                    .eventDetail(stringBuilder.toString())
                    .eventTime(LocalDateTime.now())
                    .build());

            //删除发货单
            tradeDelivers.remove(tradeDeliver);

            //保存
            save(trade);

            //处理子单
            List<ProviderTrade> listByParentId = providerTradeService.findListByParentId(trade.getId());
            if(CollectionUtils.isNotEmpty(listByParentId)){
                for (ProviderTrade providerTrade : listByParentId) {
                    List<String> deliverIds = providerTrade.getTradeDelivers().stream().map(TradeDeliver::getDeliverId).collect(Collectors.toList());
                    if(CollectionUtils.isNotEmpty(deliverIds) && deliverIds.contains(tradeDeliver.getSunDeliverId())){
                        providerObsoleteDeliveryRecords(providerTrade, operator, tradeDeliver.getSunDeliverId());
                    }
                }
            }
            super.operationLogMq.convertAndSend(operator, TradeEvent.OBSOLETE_DELIVER.getDescription(), stringBuilder.toString());
        }


    }

    private void providerObsoleteDeliveryRecords(ProviderTrade providerTrade, Operator operator, String deliverId) {
        List<TradeDeliver> tradeDelivers = providerTrade.getTradeDelivers();
        //查询发货记录
        Optional<TradeDeliver> tradeDeliverOptional = tradeDelivers
                .stream()
                .filter(tradeDeliver -> StringUtils.equals(deliverId, tradeDeliver.getDeliverId()))
                .findFirst();
        if (tradeDeliverOptional.isPresent()) {
            StringBuilder stringBuilder = new StringBuilder(200);

            TradeDeliver tradeDeliver = tradeDeliverOptional.get();

            //处理商品
            providerHandleShippingItems(providerTrade, tradeDeliver.getShippingItems(), stringBuilder);

            //处理赠品
            providerHandleGiftShippingItems(providerTrade,
                    tradeDeliver.getGiftItemList(), stringBuilder);

            //处理加价购
            providerHandlePreferentialShippingItems(providerTrade, tradeDeliver.getPreferentialItemList(), stringBuilder);

            //周期购
            buyCycleObsoleteDeliveryRecords(providerTrade,tradeDeliver);
            //订单状态更新
            TradeState tradeState = providerTrade.getTradeState();
            if (isAllNotShipped(providerTrade)) {
                tradeState.setFlowState(FlowState.AUDIT);
                tradeState.setDeliverStatus(DeliverStatus.NOT_YET_SHIPPED);
            } else {
                tradeState.setFlowState(FlowState.DELIVERED_PART);
                tradeState.setDeliverStatus(DeliverStatus.PART_SHIPPED);
            }

            //添加操作日志
            stringBuilder.trimToSize();
            providerTrade.appendTradeEventLog(TradeEventLog
                    .builder()
                    .operator(operator)
                    .eventType(TradeEvent.OBSOLETE_DELIVER.getDescription())
                    .eventDetail(stringBuilder.toString())
                    .eventTime(LocalDateTime.now())
                    .build());

            //删除发货单
            tradeDelivers.remove(tradeDeliver);

            //保存
            saveProviderTrade(providerTrade);

            super.operationLogMq.convertAndSend(operator, TradeEvent.OBSOLETE_DELIVER.getDescription(), stringBuilder.toString());
        }

    }

    /**
     * 供应商周期购作废
     * @param trade
     * @param tradeDeliver
     */
    private void buyCycleObsoleteDeliveryRecords(ProviderTrade trade,TradeDeliver tradeDeliver) {
        OrderTag orderTag = trade.getOrderTag();
        if (Objects.nonNull(orderTag) && Objects.equals(orderTag.getBuyCycleFlag(),Boolean.TRUE)) {
            TradeBuyCycleDTO tradeBuyCycle = trade.getTradeBuyCycle();
            CycleDeliveryPlanDTO planDTO = tradeDeliver.getCycleDeliveryPlan();
            List<CycleDeliveryPlanDTO> deliveryPlanS = tradeBuyCycle.getDeliveryPlanS();
            Integer deliveryNum = planDTO.getDeliveryNum();
            Integer nextNum = deliveryNum + Constants.ONE;
            CycleDeliveryPlanDTO nextPlan = new CycleDeliveryPlanDTO();
            for (CycleDeliveryPlanDTO plan: deliveryPlanS) {
                if (Objects.equals(deliveryNum,plan.getDeliveryNum())) {
                    CycleDeliveryState cycleDeliveryState = plan.getCycleDeliveryState();
                    if (!Objects.equals(cycleDeliveryState,CycleDeliveryState.SHIPPED)) {
                        throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
                    }
                    plan.setCycleDeliveryState(CycleDeliveryState.NOT_SHIP);
                }
                if (Objects.equals(nextNum,plan.getDeliveryNum())) {
                    nextPlan = plan;
                    break;
                }
            }
            CycleDeliveryState cycleDeliveryState = nextPlan.getCycleDeliveryState();
            //下一个计划是已配送，证明目前作废这个计划不是最近发货
            if (Objects.equals(cycleDeliveryState,CycleDeliveryState.SHIPPED)) {
                throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
            }
            trade.getTradeBuyCycle().setBuyCycleNextPlanDate(planDTO.getDeliveryDate());
            trade.getTradeBuyCycle().setDeliveryPlanS(deliveryPlanS);
            trade.getTradeItems().forEach(tradeItem -> {
                if (isAllNotShipped(trade)) {
                    tradeItem.setDeliverStatus(DeliverStatus.NOT_YET_SHIPPED);
                } else {
                    tradeItem.setDeliverStatus(DeliverStatus.PART_SHIPPED);
                }
            });
        }
    }

    /**
     * 周期购作废
     * @param trade
     * @param tradeDeliver
     */
    private void buyCycleObsoleteDeliveryRecords(Trade trade,TradeDeliver tradeDeliver) {
        OrderTag orderTag = trade.getOrderTag();
        if (Objects.nonNull(orderTag) && Objects.equals(orderTag.getBuyCycleFlag(),Boolean.TRUE)) {
            TradeBuyCycleDTO tradeBuyCycle = trade.getTradeBuyCycle();
            CycleDeliveryPlanDTO planDTO = tradeDeliver.getCycleDeliveryPlan();
            List<CycleDeliveryPlanDTO> deliveryPlanS = tradeBuyCycle.getDeliveryPlanS();
            Integer deliveryNum = planDTO.getDeliveryNum();
            Integer nextNum = deliveryNum + Constants.ONE;
            CycleDeliveryPlanDTO nextPlan = new CycleDeliveryPlanDTO();
            for (CycleDeliveryPlanDTO plan: deliveryPlanS) {
                if (Objects.equals(deliveryNum,plan.getDeliveryNum())) {
                    CycleDeliveryState cycleDeliveryState = plan.getCycleDeliveryState();
                    if (!Objects.equals(cycleDeliveryState,CycleDeliveryState.SHIPPED)) {
                        throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
                    }
                    plan.setCycleDeliveryState(CycleDeliveryState.NOT_SHIP);
                }
                if (Objects.equals(nextNum,plan.getDeliveryNum())) {
                    nextPlan = plan;
                    break;
                }
            }
            CycleDeliveryState cycleDeliveryState = nextPlan.getCycleDeliveryState();
            //下一个计划是已配送，证明目前作废这个计划不是最近发货
            if (Objects.equals(cycleDeliveryState,CycleDeliveryState.SHIPPED)) {
                throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
            }
            trade.getTradeBuyCycle().setBuyCycleNextPlanDate(planDTO.getDeliveryDate());
            trade.getTradeBuyCycle().setDeliveryPlanS(deliveryPlanS);
            trade.getTradeItems().forEach(tradeItem -> {
                if (isAllNotShipped(trade)) {
                    tradeItem.setDeliverStatus(DeliverStatus.NOT_YET_SHIPPED);
                } else {
                    tradeItem.setDeliverStatus(DeliverStatus.PART_SHIPPED);
                }
            });
        }
    }

    /**
     * 普通
     * @param trade
     * @param shippingItems
     * @param stringBuilder
     */
    private void handleShippingItems(Trade trade, List<ShippingItem> shippingItems, StringBuilder stringBuilder) {
        ConcurrentHashMap<String, TradeItem> skuItemMap ;
        skuItemMap = trade.skuItemMap();

        //订单商品更新
        shippingItems.forEach(shippingItem -> {
            TradeItem tradeItem = skuItemMap.get(shippingItem.getSkuId());

            Long shippedNum = tradeItem.getDeliveredNum();
            shippedNum -= shippingItem.getItemNum();
            tradeItem.setDeliveredNum(shippedNum);

            if (shippedNum.equals(0L)) {
                tradeItem.setDeliverStatus(DeliverStatus.NOT_YET_SHIPPED);
            } else if (shippedNum < tradeItem.getNum()) {
                tradeItem.setDeliverStatus(DeliverStatus.PART_SHIPPED);
            }

            stringBuilder.append(String.format("订单[%s],商品[%s], 作废发货[%s], 目前状态:[%s]\r\n",
                    trade.getId(),
                    tradeItem.getSkuName(),
                    shippingItem.getItemNum().toString(),
                    tradeItem.getDeliverStatus().getDescription())
            );

        });
    }

    /**
     * 加价购商品发货作废
     * @param trade
     * @param shippingItems
     * @param stringBuilder
     */
    private void handlePreferentialShippingItems(Trade trade, List<ShippingItem> shippingItems, StringBuilder stringBuilder) {
        ConcurrentHashMap<Long, List<TradeItem>> skuItemMap = trade.preferentialItemMap();

        //订单商品更新
        shippingItems.forEach(shippingItem -> {
            List<TradeItem> tradeItems = skuItemMap.get(shippingItem.getMarketingId());
            Map<String, TradeItem> skuIdToTradeItemMap =
                    tradeItems.stream().collect(Collectors.toMap(TradeItem::getSkuId,
                            Function.identity()));
            TradeItem tradeItem = skuIdToTradeItemMap.get(shippingItem.getSkuId());

            Long shippedNum = tradeItem.getDeliveredNum();
            shippedNum -= shippingItem.getItemNum();
            tradeItem.setDeliveredNum(shippedNum);

            if (shippedNum.equals(0L)) {
                tradeItem.setDeliverStatus(DeliverStatus.NOT_YET_SHIPPED);
            } else if (shippedNum < tradeItem.getNum()) {
                tradeItem.setDeliverStatus(DeliverStatus.PART_SHIPPED);
            }

            stringBuilder.append(String.format("订单[%s],商品[%s], 作废发货[%s], 目前状态:[%s]\r\n",
                    trade.getId(),
                    ( "【加价购商品】") + tradeItem.getSkuName(),
                    shippingItem.getItemNum().toString(),
                    tradeItem.getDeliverStatus().getDescription())
            );

        });
    }

    /**
     * 处理子订单 普通
     * @param providerTrade
     * @param shippingItems
     * @param stringBuilder
     */
    private void providerHandleShippingItems(ProviderTrade providerTrade, List<ShippingItem> shippingItems, StringBuilder stringBuilder) {
        ConcurrentHashMap<String, TradeItem> skuItemMap ;
        skuItemMap = providerTrade.skuItemMap();

        //订单商品更新
        shippingItems.forEach(shippingItem -> {
            TradeItem tradeItem = skuItemMap.get(shippingItem.getSkuId());

            Long shippedNum = tradeItem.getDeliveredNum();
            shippedNum -= shippingItem.getItemNum();
            tradeItem.setDeliveredNum(shippedNum);

            if (shippedNum.equals(0L)) {
                tradeItem.setDeliverStatus(DeliverStatus.NOT_YET_SHIPPED);
            } else if (shippedNum < tradeItem.getNum()) {
                tradeItem.setDeliverStatus(DeliverStatus.PART_SHIPPED);
            }

            stringBuilder.append(String.format("订单[%s],商品[%s], 作废发货[%s], 目前状态:[%s]\r\n",
                    providerTrade.getId(),
                    tradeItem.getSkuName(),
                    shippingItem.getItemNum().toString(),
                    tradeItem.getDeliverStatus().getDescription())
            );

        });
    }


    /**
     * 处理加价购
     * @param providerTrade
     * @param shippingItems
     * @param stringBuilder
     */
    private void providerHandlePreferentialShippingItems(ProviderTrade providerTrade, List<ShippingItem> shippingItems, StringBuilder stringBuilder) {
        ConcurrentHashMap<Long, List<TradeItem>> skuItemMap = providerTrade.preferentialItemMap();

        //订单商品更新
        shippingItems.forEach(shippingItem -> {
            List<TradeItem> tradeItems = skuItemMap.get(shippingItem.getMarketingId());
            Map<String, TradeItem> skuIdToTradeItemMap =
                    tradeItems.stream().collect(Collectors.toMap(TradeItem::getSkuId,
                            Function.identity()));
            TradeItem tradeItem = skuIdToTradeItemMap.get(shippingItem.getSkuId());

            Long shippedNum = tradeItem.getDeliveredNum();
            shippedNum -= shippingItem.getItemNum();
            tradeItem.setDeliveredNum(shippedNum);

            if (shippedNum.equals(0L)) {
                tradeItem.setDeliverStatus(DeliverStatus.NOT_YET_SHIPPED);
            } else if (shippedNum < tradeItem.getNum()) {
                tradeItem.setDeliverStatus(DeliverStatus.PART_SHIPPED);
            }

            stringBuilder.append(String.format("订单[%s],商品[%s], 作废发货[%s], 目前状态:[%s]\r\n",
                    providerTrade.getId(),
                    ( "【加价购商品】") + tradeItem.getSkuName(),
                    shippingItem.getItemNum().toString(),
                    tradeItem.getDeliverStatus().getDescription())
            );

        });
    }

    /**
     * 赠品发货作废
     * @param trade
     * @param shippingItems
     * @param stringBuilder
     */
    private void handleGiftShippingItems(Trade trade,
                                      List<ShippingItem> shippingItems, StringBuilder stringBuilder) {
        ConcurrentHashMap<Long, List<TradeItem>> skuItemMap = trade.giftSkuItemMap();

        //订单商品更新
        shippingItems.forEach(shippingItem -> {
            List<TradeItem> tradeItems = skuItemMap.get(shippingItem.getMarketingId());
            Map<String, TradeItem> skuIdToTradeItemMap =
                    tradeItems.stream().collect(Collectors.toMap(TradeItem::getSkuId,
                            Function.identity()));
            TradeItem tradeItem = skuIdToTradeItemMap.get(shippingItem.getSkuId());

            Long shippedNum = tradeItem.getDeliveredNum();
            shippedNum -= shippingItem.getItemNum();
            tradeItem.setDeliveredNum(shippedNum);

            if (shippedNum.equals(0L)) {
                tradeItem.setDeliverStatus(DeliverStatus.NOT_YET_SHIPPED);
            } else if (shippedNum < tradeItem.getNum()) {
                tradeItem.setDeliverStatus(DeliverStatus.PART_SHIPPED);
            }

            stringBuilder.append(String.format("订单[%s],商品[%s], 作废发货[%s], 目前状态:[%s]\r\n",
                    trade.getId(),
                    ( "【赠品】") + tradeItem.getSkuName(),
                    shippingItem.getItemNum().toString(),
                    tradeItem.getDeliverStatus().getDescription())
            );

        });
    }

    /**
     * 处理赠品
     * @param providerTrade
     * @param shippingItems
     * @param stringBuilder
     */
    private void providerHandleGiftShippingItems(ProviderTrade providerTrade,
                                              List<ShippingItem> shippingItems, StringBuilder stringBuilder) {
        ConcurrentHashMap<Long, List<TradeItem>> skuItemMap = providerTrade.giftSkuItemMap();

        //订单商品更新
        shippingItems.forEach(shippingItem -> {
            List<TradeItem> tradeItems = skuItemMap.get(shippingItem.getMarketingId());
            Map<String, TradeItem> skuIdToTradeItemMap =
                    tradeItems.stream().collect(Collectors.toMap(TradeItem::getSkuId,
                            Function.identity()));
            TradeItem tradeItem = skuIdToTradeItemMap.get(shippingItem.getSkuId());

            Long shippedNum = tradeItem.getDeliveredNum();
            shippedNum -= shippingItem.getItemNum();
            tradeItem.setDeliveredNum(shippedNum);

            if (shippedNum.equals(0L)) {
                tradeItem.setDeliverStatus(DeliverStatus.NOT_YET_SHIPPED);
            } else if (shippedNum < tradeItem.getNum()) {
                tradeItem.setDeliverStatus(DeliverStatus.PART_SHIPPED);
            }

            stringBuilder.append(String.format("订单[%s],商品[%s], 作废发货[%s], 目前状态:[%s]\r\n",
                    providerTrade.getId(),
                    ( "【赠品】") + tradeItem.getSkuName(),
                    shippingItem.getItemNum().toString(),
                    tradeItem.getDeliverStatus().getDescription())
            );

        });
    }


}
