...
public class DistributionCustomerInviteInfoWhereCriteriaBuilder {
    public static Specification<DistributionCustomerInviteInfo> build(DistributionCustomerInviteInfoQueryRequest queryRequest) {
        return (root, cquery, cbuild) -> {
            ...
            // 已发放邀新奖励现金人数-至
            if (Objects.nonNull(queryRequest.getRewardCashCountEnd()) && queryRequest.getRewardCashCountEnd().compareTo(0) >= 0) {
                predicates.add(cbuild.lt(root.get("rewardCashCount"), queryRequest.getRewardCashCountEnd()));
            }

            // 达到上限未发放奖励现金人数-从
            if (Objects.nonNull(queryRequest.getRewardCashLimitCountStart()) && queryRequest.getRewardCashLimitCountStart().compareTo(0) >= 0) {
                predicates.add(cbuild.gt(root.get("rewardCashLimitCount"), queryRequest.getRewardCashLimitCountStart()));
            }

            // 达到上限未发放奖励现金有效邀新人数-从
            if (Objects.nonNull(queryRequest.getRewardCashAvailableLimitCountStart()) && queryRequest.getRewardCashAvailableLimitCountStart().compareTo(0) >= 0) {
                predicates.add(cbuild.gt(root.get("rewardCashAvailableLimitCount"), queryRequest.getRewardCashAvailableLimitCountStart()));
            }

            // 已发放邀新奖励优惠券人数-至
            if (Objects.nonNull(queryRequest.getRewardCouponCountEnd()) && queryRequest.getRewardCouponCountEnd().compareTo(0) >= 0) {
                predicates.add(cbuild.lt(root.get("rewardCouponCount"), queryRequest.getRewardCouponCountEnd()));
            }

            // 达到上限未发放奖励优惠券人数-从
            if (Objects.nonNull(queryRequest.getRewardCouponLimitCountStart()) && queryRequest.getRewardCouponLimitCountStart().compareTo(0) >= 0) {
                predicates.add(cbuild.gt(root.get("rewardCouponLimitCount"), queryRequest.getRewardCouponLimitCountStart()));
            }

            // 达到上限未发放奖励优惠券有效邀新人数-从
            if (Objects.nonNull(queryRequest.getRewardCouponAvailableLimitCountStart()) && queryRequest.getRewardCouponAvailableLimitCountStart().compareTo(0) >= 0) {
