 ...
 * @description <p>授信待还款提醒</p>
 */
@Slf4j
@Component
public class CreditNoRepayNoticeJobHandler {
    ...
    @XxlJob(value = "CreditNoRepayNoticeJobHandler")
    public void execute() throws Exception {
        log.info("CreditNoRepayNoticeJobHandler start");
        // 三天后需要还款的用户
        CreditAccountPageRequest request =
                CreditAccountPageRequest.builder()
                        .endTime(LocalDateTime.now().plusDays(3L))
                        .build();
        // 每次拉100条
        ...
        while (true) {
            if (Objects.nonNull(response)
                ...
                log.info("CreditNoRepayNoticeJobHandler accountList,currentPage = {} , size = {}",currentPage,accountList.size());

                currentPage++;
                for (CreditAccountPageResponse account : accountList) {
                    if (Objects.nonNull(account.getUsableAmount())
                            && Objects.nonNull(account.getRepayAmount())
                            && (account.getRepayAmount().compareTo(BigDecimal.ZERO) != 0)) {
                        // 发送还款提醒
                        noticeService.sendMessage(
                                NodeType.ACCOUNT_ASSETS.toValue(),
                                AccoutAssetsType.CREDIT_REPAY_NOTICE.toValue(),
                                AccoutAssetsType.CREDIT_REPAY_NOTICE.getType(),
                                "",
                ...
                log.info(
                        "CreditNoRepayNoticeJobHandler run over! query page result empty,request:{}",
