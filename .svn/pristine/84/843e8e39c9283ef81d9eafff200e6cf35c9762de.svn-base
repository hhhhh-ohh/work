...
public class DiscountPlugin implements IGoodsListPlugin, IGoodsDetailPlugin, ITradeCommitPlugin {
    ...
    public void goodsListFilter(List<GoodsInfoVO> goodsInfos, MarketingPluginRequest request) {
        ...
        List<MarketingResponse> marketingList = request.getMarketingMap().values().stream()
                ...
                .filter(marketing -> MarketingType.DISCOUNT == marketing.getMarketingType())
    ...
    public void goodsDetailFilter(GoodsInfoDetailByGoodsInfoResponse detailResponse, MarketingPluginRequest request) {
        ...
        if (CollectionUtils.isEmpty(marketingList)
                || marketingList.stream().filter(marketing -> MarketingType.DISCOUNT == marketing.getMarketingType()).count() < 1) {
            return;
        }

        MarketingResponse marketingObj = marketingList.stream().filter(marketing ->
                MarketingType.DISCOUNT == marketing.getMarketingType()).findFirst().orElse(new MarketingResponse());
        //填充营销描述<营销编号,描述>
        Map<Long, MarketingResponse> params = new HashMap<>();
    ...
    public TradeMarketingResponse wraperMarketingFullInfo(TradeMarketingPluginRequest request) {
        ...
        BigDecimal amount = price.multiply(level.getDiscount()).setScale(2, RoundingMode.HALF_UP);

        TradeMarketingResponse response = new TradeMarketingResponse();
        response.setTradeMarketing(TradeMarketing.builder()
                .fullDiscountLevel(KsBeanUtil.convert(level, MarketingFullDiscountLevelVO.class))
    ...
    private Map<Long, String> getLabelMap(List<Long> marketingIds, Map<Long, MarketingResponse> marketingMap) {
        ...
        levelsMap.forEach((marketingId, levels) -> {
            List<String> descs = levels.stream().map(level -> {
                if (MarketingSubType.DISCOUNT_FULL_AMOUNT == marketingMap.get(marketingId).getSubType()) {
                    return String.format("满%s元享%s折", fmt.format(level.getFullAmount()), fmt.format(level.getDiscount().multiply(new BigDecimal(10))));
                }
