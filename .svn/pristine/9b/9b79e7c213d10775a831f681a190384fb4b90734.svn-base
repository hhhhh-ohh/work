...
public class RoleInfoController implements RoleInfoProvider {
    ...
    @Override
    public BaseResponse delete(@RequestBody @Valid RoleInfoDeleteRequest request) {
        List<String> roleList  = new ArrayList<>();
        EmployeeListRequest employeeListRequest = new EmployeeListRequest();
        employeeListRequest.setCompanyInfoId(request.getCompanyInfoId());
        roleList.add(request.getRoleInfoId().toString());
        ...
        if (!CollectionUtils.isEmpty(employeeList)){
            throw new SbcRuntimeException(CustomerErrorCodeEnum.K010100);
        }
