...
public class BossApiIntercepter implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws
        ...
        if ((!Objects.equals(operator.getVersionNo(), employee.getVersionNo()))) {
            //如果不一致，则清理失效token的redis
            commonUtil.deleteLoginRedis(commonUtil.getToken(request));
            loginInvalid(response);
            return false;
        }
        ...
        ids = this.filterRoleId(ids, platform == Platform.STOREFRONT ? O2O_COMPANYINFOID : employee.getCompanyInfoId());
    ...
    private void notAllowed(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        response.getWriter().write(JSONObject.toJSONString(new BaseResponse(CommonErrorCodeEnum.K999998)));
    ...
    private void disable(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        response.addHeader("Access-Control-Allow-Headers", "authorization,content-type,x-requested-with,systemId," +
                "Platform,reqId");
        response.setContentType("application/json;charset=utf-8");
        response.getWriter().write(JSONObject.toJSONString(new BaseResponse(CommonErrorCodeEnum.K000005)));
        response.getWriter().flush();
        response.getWriter().close();
    }
     ...
     * @throws Exception
     */
    private void loginInvalid(HttpServletResponse response) throws Exception {
        response.setStatus(HttpStatus.OK.value());
        response.addHeader("Access-Control-Allow-Headers", "authorization,content-type,x-requested-with,systemId," +
                "Platform,reqId");
        response.setContentType("application/json;charset=utf-8");
        response.getWriter().write(JSONObject.toJSONString(new BaseResponse(CommonErrorCodeEnum.K000015)));
        response.getWriter().flush();
        response.getWriter().close();
    }
