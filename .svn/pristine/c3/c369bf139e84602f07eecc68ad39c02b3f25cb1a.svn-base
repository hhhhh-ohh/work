...
import com.wanmi.perseus.param.*;
...
public class StatisticsController {
        ...
        for(FollowParam param: params) {
            ...
            if (sendFlag) {
                ...
                if (StringUtils.isNotBlank(param.getId())) {
                    ...
                    if(CollectionUtils.isNotEmpty(param.getMarketingList())){
                        customerFollowInfoParam.setMarketingRequestList(param.getMarketingList());
                    }
            ...
            if (StringUtils.isNoneEmpty(param.getSkuId(), param.getShopId())) {//skuId,shopId都不为空,表示商品详情页,既计算商品的pv/uv , 也计算平台中所有商品的pv/uv , 也计算店铺所有商品的pv/uv , 也计算店铺总的pv/uv
                ...
                if (CollectionUtils.isNotEmpty(param.getMarketingList())) {
                    setDateMarketingSkuPv(param.getSkuId(), param.getId(), param.getClientType(), dateStr, expireTime, param.getMarketingList(),param.getShopId());
                }
            }
    ...
    private void setDateSkuPv(String skuId, String shopId, String clientType, String dateStr, Date expireTime){
        ...
        //1.统计某Sku当日pv
        String pvDateKey = String.format(StaticParam.SKU_PV_KEY,clientType,dateStr,skuId,shopId);//三端某sku当日pv的key,形如pc-pv-20170921-sku-123asd2asdf
        setPv(pvDateKey,expireTime);
        //2.统计平台商品页pv
    ...
    private void setDateSkuUv(String id, String skuId, String shopId, String clientType, String dateStr, Date expireTime){
        ...
        String uvDateKey = String.format(StaticParam.SKU_UV_KEY,clientType,dateStr,skuId,shopId);//三端某sku当日uv的key,形如pc-uv-20170921-sku-fsd2ffaakl32fas
        setUv(id,uvDateKey,expireTime);
        uvDateKey = String.format(StaticParam.SKU_UV_KEY,StaticParam.END_ALL,dateStr,skuId,shopId);//某sku当日汇总uv的key,形如ALL-uv-20170921-sku-fsd2ffaakl32fas(包括所有端的总量)
        setUv(id,uvDateKey,expireTime);
    ...
    private Date getExpireTime(){
        //明天凌晨2点，随机秒数，防止同一时间大量数据过期造成redis不可用
        LocalDateTime localDateTime = LocalDateTime.of(LocalDate.now().plusDays(1), LocalTime.of(StaticParam.EXPIRE_HOUR, StaticParam.EXPIRE_MINUTE,new Random().nextInt(59)));
        //转换成Date类型
        return Date.from(localDateTime.atZone(ZoneId.systemDefault()).toInstant());
    }

    /**
     * 分端获取某营销活动的Sku当日pv 营销活动pv 营销活动sku当日的uv
     *
     */
    private void setDateMarketingSkuPv(String skuId, String id, String clientType, String dateStr, Date expireTime,List<MarketingRequest> marketingList){
        if(StringUtils.isEmpty(skuId)){
            return;
        }

        for(MarketingRequest request : marketingList) {
            log.info("营销埋单数据：MarketingRequest===={}", request.toString());
            //营销活动对应的pv
            String pvDateKey = String.format(StaticParam.MARKETING_PV_KEY,
                   // clientType,
                    dateStr,
                    request.getMarketingId(),
                    request.getMarketingType().getId());
            setPv(pvDateKey, expireTime);

            //营销活动sku对应的pv
            pvDateKey = String.format(StaticParam.MARKETING_SKU_PV_KEY,
                //    clientType,
                    dateStr,
                    request.getMarketingId(),
                    request.getMarketingType().getId(),
            ...
            //营销活动sku对应的uv
            String uvDateKey = String.format(StaticParam.MARKETING_SKU_UV_KEY,
                 //   clientType,
                    dateStr,
                    request.getMarketingId(),
                    request.getMarketingType().getId(),
     ...
     *
     */
    private void setDateMarketingSkuPv(String skuId, String id, String clientType, String dateStr, Date expireTime,List<MarketingRequest> marketingList,String shopId){
        if(StringUtils.isEmpty(skuId)){
            return;
        }
 ...
 /*   private String getSkuIdFromUrl(String url, String clientType){
    ...
    private void customerFollowInfo(CustomerFollowInfoParam customerFollowInfoParam){
        log.info("会员浏览行为数据信息2===={}",
                customerFollowInfoParam.getGoods_info_id());
        ProducerRecord<String, String> producerRecord = new ProducerRecord<>("customer_follow",
                JSON.toJSONString(customerFollowInfoParam,"yyyy-MM-dd HH:mm:ss"));
