...
@Schema
@Data
public class GetCommissionRequest extends BaseRequest {

    @Schema(description = "订单商品信息")
    private List<GetCommissionTradeParams> tradeParams;
    ...
    public void checkParam() {
        ...
        for (GetCommissionTradeParams param : this.tradeParams) {
            if (CollectionUtils.isEmpty(param.getOldTradeItems())
                    || Objects.isNull(param.getSupplier())
                    || Objects.isNull(param.getSupplier().getStoreId())) {
                throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
            }
            for (TradeItemDTO tradeItemDTO : param.getOldTradeItems()) {
                if (Objects.isNull(tradeItemDTO.getSplitPrice())
                        || Objects.isNull(tradeItemDTO.getSkuId())) {
                    throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
                }
            }
            if (CollectionUtils.isNotEmpty(param.getOldPreferential())) {
                for (TradeItemDTO tradeItemDTO : param.getOldPreferential()) {
                    if (Objects.isNull(tradeItemDTO.getSplitPrice())
                            || Objects.isNull(tradeItemDTO.getSkuId())) {
                        throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
                    }
    ...
    public void handlePointsPrice() {
        ...
        for(GetCommissionTradeParams tradeParams : this.tradeParams) {
            ...
            if (CollectionUtils.isNotEmpty(tradeParams.getOldPreferential())) {
