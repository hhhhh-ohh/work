package com.wanmi.sbc.payingmemberlevel.excel.impl;
 ...
 * @description <p> 第二件半价 </p>
 */
@Slf4j
@Service
public class DiscountExcelServiceImpl implements PayingMemberExcelService {
    @Autowired
    ...
    public void downloadTemplate(Integer activityType) {
        ...
        this.write(file, "付费会员折扣商品导入模板.xls");
    }

    @Override
    public void downloadErrorFile(String fileExt) {
        String userId = commonUtil.getOperatorId();
        String resourceKey = Constants.PAYING_MEMBER_DISCOUNT_ERROR_EXCEL.concat(userId);
        goodsExcelService.getErrExcel(resourceKey,fileExt);
    }

    @Override
    public String uploadFile(MultipartFile file, PayingMemberTemplateRequest payingMemberTemplateRequest) {
        ...
        String userId = commonUtil.getOperatorId();
        String resourceKey = Constants.PAYING_MEMBER_DISCOUNT_EXCEL.concat(userId);
        try (Workbook workbook = WorkbookFactory.create(new ByteArrayInputStream(file.getBytes()))) {
            ...
            if(lastRowNum > Constants.NUM_1000){
                int emptyRowNum = ExcelHelper.getEmptyRowNum(sheet);
                if ((lastRowNum - emptyRowNum) > Constants.NUM_1000) {
                    throw new SbcRuntimeException(CommonErrorCodeEnum.K999999, "文件数据超过1000条，请修改");
                }
            }
            int maxCell = 2;
            ...
            for (int rowNum = 1; rowNum <= lastRowNum; rowNum++) {
                ...
                //校验折扣
                String discount = ExcelHelper.getValue(getCell(cells, 1));
                checkDiscount(discount,workbook,cells,1,isError);
            }
            ...
            //校验sku是否存在，或者重复
            this.checkGoodsInfoNo(skuNoMap,goodsInfoList,workbook,isError,payingMemberTemplateRequest);
            if (isError.get()) {
                String newFileName = userId.concat(".").concat(fileExt);
                resourceKey = Constants.PAYING_MEMBER_DISCOUNT_ERROR_EXCEL.concat(userId);
    ...
    public List getGoodsInfoList() {
        ...
        List<PayingMemberDiscountRelVO> payingMemberDiscountRelVOS = Lists.newArrayList();
        try (Workbook workbook = WorkbookFactory.create(new ByteArrayInputStream(content))) {
            ...
            for (int rowNum = 1; rowNum <= lastRowNum; rowNum++) {
                ...
                if (StringUtils.isEmpty(discount)) {
                    discount =  String.valueOf(Constants.TEN);
                }
                PayingMemberDiscountRelVO payingMemberDiscountRelVO = new PayingMemberDiscountRelVO();
                payingMemberDiscountRelVO.setGoodsInfoNo(skuNo);
                payingMemberDiscountRelVO.setDiscount(new BigDecimal(discount));
                payingMemberDiscountRelVOS.add(payingMemberDiscountRelVO);
            }
            ...
            Map<String, GoodsInfoVO> goodsInfoMap = goodsInfoList.parallelStream()
                    .collect(Collectors.toMap(GoodsInfoVO::getGoodsInfoNo, Function.identity()));
            return payingMemberDiscountRelVOS.parallelStream().peek(payingMemberDiscountRelVO -> {
                String goodsInfoNo = payingMemberDiscountRelVO.getGoodsInfoNo();
                GoodsInfoVO goodsInfoVO = goodsInfoMap.get(goodsInfoNo);
