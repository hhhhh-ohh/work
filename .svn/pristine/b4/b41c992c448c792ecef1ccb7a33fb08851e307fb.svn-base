...
/**
 * @author lvzhenwei
 * @className EsCustomerModifyBaseInfoService
 * @description 修改会员基本信息成功，发送订单支付MQ消息，同步ES
 * @date 2021/8/12 10:36 上午
 */
@Slf4j
@Service
public class EsCustomerModifyBaseInfoService {

    @Autowired private EsMqConsumerProvider esMqConsumerProvider;

    @Bean
    public Consumer<Message<String>> mqEsCustomerModifyBaseInfoService() {
        return message->{
            String json = message.getPayload();
            EsMqConsumerRequest esMqConsumerRequest = new EsMqConsumerRequest();
            esMqConsumerRequest.setMqContentJson(json);
            esMqConsumerProvider.customerModifyBaseInfo(esMqConsumerRequest);
        };
    }
}
