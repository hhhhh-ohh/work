...
import com.wanmi.sbc.setting.api.request.expresscompany.*;
...
public class ExpressCompanyQueryController implements ExpressCompanyQueryProvider {
	...
	public BaseResponse<ExpressCompanyListResponse> list() {
		...
		try{
			...
			String expressCom = CharStreams.toString(new InputStreamReader(inputStream, StandardCharsets.UTF_8));
	...
	@Override
	public BaseResponse<ExpressCompanyByIdResponse> getByCode(@Valid ExpressCompanyByCodeRequest expressCompanyByCodeRequest) {
		ExpressCompany expressCompany = expressCompanyService.getByCode(expressCompanyByCodeRequest.getExpressCode());
		return BaseResponse.success(new ExpressCompanyByIdResponse(expressCompanyService.wrapperVo(expressCompany)));
	}
