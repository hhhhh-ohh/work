...
@Component
@Slf4j
public class DistributionMonthlyPerformanceJobHandler {
    ...
    @XxlJob("distributionMonthlyPerformanceJobHandler")
    public void execute() {
        String args = XxlJobHelper.getJobParam();
        XxlJobHelper.log("========= Monthly distribution performance generation start... =========");
        // 清理过期的分销数据
        clear();
        // 录入月分销业绩数据
        int total = generate(args);
        XxlJobHelper.log("This task generates {} performance records", total);
        XxlJobHelper.log("========= Monthly distribution performance has been generation completed... =========");
    }
    ...
    private void clear() {
        ...
        try {
            ...
            XxlJobHelper.log("This task clean performance data by date completed,request={}", request);
        } catch (Exception e) {
            //清理失败不中断任务，确保数据生成不受影响
            XxlJobHelper.log("This task clean performance data by date error,request={},", request, e);
        }
    }
