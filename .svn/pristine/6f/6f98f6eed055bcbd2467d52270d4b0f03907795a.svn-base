...
//        boolean hasKey = redisService.hasKey(key);
     ...
     * @param skus
     */
    public void clearBuyPointsForSkusDetail(List<GoodsInfoSimpleVO> skus){
        if(skus == null){
            return;
        }
        Long sum = skus.stream().filter(g -> g.getBuyPoint() != null && g.getBuyPoint() > 0)
                .mapToLong(GoodsInfoSimpleVO::getBuyPoint).sum();
        if(sum>0 && (!isGoodsPoint())){
            skus.forEach(g -> g.setBuyPoint(0L));
        }
    }
     ...
     * @param sku
     */
    public void clearBuyPointsForSkusDetail(GoodsInfoSimpleVO sku){
        if(sku!= null && sku.getBuyPoint() != null && sku.getBuyPoint()>0 && (!isGoodsPoint())){
            sku.setBuyPoint(0L);
        }
    }
