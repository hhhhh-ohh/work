package com.wanmi.sbc.marketing.provider.impl.bookingsale;
...
import com.wanmi.sbc.marketing.api.request.bookingsale.BookingSaleCloseRequest;
...
public class BookingSaleController implements BookingSaleProvider {
    ...
    @Override
    public BaseResponse close(@RequestBody @Valid BookingSaleCloseRequest request) {
        bookingSaleService.close(request.getId(), request.getStoreId());
        return BaseResponse.SUCCESSFUL();
    }

    @Override
    public BaseResponse sycCache(){
        bookingSaleService.sycCache();
        return BaseResponse.SUCCESSFUL();
    }
}
