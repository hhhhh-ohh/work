...
public class DistributionCustomerService {
     ...
     */
    @Transactional
    public DistributionCustomer modify(DistributionCustomerModifyRequest request) {
        //验证分销员
        DistributionCustomer distributionCustomer = getByCustomerId(request.getCustomerId());
        if (Objects.nonNull(distributionCustomer)) {
            KsBeanUtil.copyPropertiesThird(request, distributionCustomer);
    ...
    public DistributionCustomer getById(String id) {
        DistributionCustomer distributionCustomer = distributionCustomerRepository.findById(id).orElseThrow(() ->
                new SbcRuntimeException(getDeleteIndex(id), CustomerErrorCodeEnum.K010001)
        );
        if (DeleteFlag.YES.equals(distributionCustomer.getDelFlag())) {
    ...
    public List<DistributionCustomer> listDistributorsForOrderCommit(
        ...
        if (StringUtils.isEmpty(request.getInviteeId())
            ...
            // 非分销员自购
            if (CommissionPriorityType.INVITOR == request.getCommissionPriorityType()) {
                // 邀请人优先
                if (buyerDistributors.size() >= Constants.TWO &&
                ...
                // 店铺优先
                if (storeDistributors.size() >= Constants.ONE) {
                    // 如果在店内购买，受益人为店主及其邀请人
                    inviteeCustomers = storeDistributors;
                } else if (buyerDistributors.size() >= Constants.TWO &&
                        isDistributor(buyerDistributors.get(Constants.ONE))) {
                    // 否则如果下单人有邀请人且是分销员，受益人为邀请人及其邀请人
                    inviteeCustomers = buyerDistributors.subList(Constants.ONE, buyerDistributors.size());
                }
        ...
        if (CollectionUtils.isNotEmpty(inviteeCustomers)) {
            ...
            for (int idx = 0; idx < inviteeCustomers.size(); idx++) {
                ...
                if (idx != 0) {
                    ...
                    Optional<DistributorLevelVO> preLevelOptional = request.getDistributorLevels().stream()
                            .filter(l -> l.getDistributorLevelId().equals(preCustomer.getDistributorLevelId())).findFirst();
                    Integer preLevelSort = 0;
                    if (preLevelOptional.isPresent()) {
                        preLevelSort = preLevelOptional.get().getSort();
                    }
                    // 根据脱钩状态，判断邀请人是否脱钩
                    if (CommissionUnhookType.EQUAL == request.getCommissionUnhookType()) {
                        // 当为平级脱钩，如果分销员等级大于等于邀请人则脱钩
                        if (preLevelSort >= level.getSort()) {
                            inviteeCustomers = inviteeCustomers.subList(0, idx);
                            break;
                        }
                    } else if (CommissionUnhookType.HIGHER == request.getCommissionUnhookType()
                            && preLevelSort > level.getSort()) {
        ...
        if (inviteeCustomers.size() > Constants.TWO) {
            inviteeCustomers = inviteeCustomers.subList(Constants.ZERO, Constants.TWO);
        }
    ...
    public DistributionCustomerShowPhoneVO addForBoss(@RequestBody @Valid DistributionCustomerAddForBossRequest distributionCustomerAddForBossRequest) {
        ...
        if (Objects.nonNull(customer)) {
            ...
            customerDetail = customerDetailRepository.findByCustomerId(customer.getCustomerId());
        } else {
        ...
        if (Objects.isNull(distributionCustomer)) {
            ...
            distributionCustomer.setInviteCode(CodeGenUtil.toSerialCode(Long.parseLong(customer.getCustomerAccount()), 8).toUpperCase());
        }
    ...
    public DistributionCustomerVO add(DistributionCustomerAddRequest distributionCustomerAddRequest) {
        ...
        distributionCustomer.setInviteCode(CodeGenUtil.toSerialCode(Long.parseLong(distributionCustomer.getCustomerAccount())
                , 8).toUpperCase());
    ...
    public DistributionCustomerVO wrapperVo(DistributionCustomer distributionCustomer) {
        if (distributionCustomer != null) {
            ...
            if (StringUtils.isNotBlank(distributionCustomer.getCustomerId())) {
                ...
                if (Objects.nonNull(response) && Objects.nonNull(response.getValidCount())) {
    ...
    public void upgrade(String customerId, List<DistributorLevelVO> distributorLevelVOList, DefaultFlag baseLimitType, Integer inviteNum) {
        ...
        if (Objects.isNull(distributionCustomer)) {
            ...
            distributionCustomer.setCreateTime(LocalDateTime.now());
            distributionCustomer.setDistributorFlag(DefaultFlag.YES);
            distributionCustomer.setInviteCode(CodeGenUtil.toSerialCode(Long.parseLong(distributionCustomer.getCustomerAccount()), 8).toUpperCase());
    ...
    public String calDistributorLevel(DistributionCustomer distributionCustomer, List<DistributorLevelVO> distributorLevelVOList, DefaultFlag baseLimitType) {
        ...
        DistributorLevelVO defaultDistributorLevel =
                distributorLevelVOList.stream().findFirst().orElseGet(DistributorLevelVO::new);

        Map<String, DistributorLevelVO> map =
                distributorLevelVOList.stream().collect(Collectors.toMap(DistributorLevelVO::getDistributorLevelId, Function.identity()));
        ...
        for (DistributorLevelVO distributorLevelVO : distributorLevelVOList) {
            ...
            boolean salesThresholdFlag = Objects.isNull(salesThreshold) || sales.compareTo(salesThreshold) < 0;

            boolean recordThresholdFlag = Objects.isNull(recordThreshold) || commission.compareTo(recordThreshold) < 0;

            boolean inviteThresholdFlag = Objects.isNull(inviteThreshold) || inviteCount.compareTo(inviteThreshold) < 0;
    ...
    private CustomerAddRequest getCustomer(DistributionCustomerAddForBossRequest request) {
        ...
        customerAddRequest.setCustomerType(request.getCustomerType());
    ...
    public Boolean isDistributor(DistributionCustomerCheckRequest request) {
        ...
        if (Objects.isNull(customer) || CheckState.CHECKED != customer.getCheckState()) {
            return Boolean.FALSE;
        }
        //查询会员detail表
        CustomerDetail customerDetail = customerDetailRepository.findAnyByCustomerId(customerId);
        //如果会员detail表信息是null 或者不是启用状态
        if (Objects.isNull(customerDetail) || CustomerStatus.ENABLE != customerDetail.getCustomerStatus()) {
            return Boolean.FALSE;
        }
     ...
     */
    @Transactional
    public void updateForbiddenFlagByCustomerId(String customerId) {
        distributionCustomerRepository.updateForbiddenFlagByCustomerId(customerId);
    }
    ...
    private Object getDeleteIndex(String id) {
        return String.format(EsConstants.DELETE_SPLIT_CHAR, EsConstants.DISTRIBUTION_CUSTOMER, id);
    }
}
