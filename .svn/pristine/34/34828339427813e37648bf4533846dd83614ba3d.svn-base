...
public class GoodsEvaluateService {
    ...
    @GlobalTransactional
    @Transactional
    public BaseResponse<EsEvaluateAddResponse> addGoodsEvaluate(@RequestBody EvaluateAddRequest evaluateAddRequest, TradeVO tradeVO) {
        ...
        // 设置商品评价内的店铺信息
        if(Objects.nonNull(goodsEvaluateAddRequest)) {
            goodsEvaluateAddRequest.setStoreId(supplierVO.getStoreId());
            goodsEvaluateAddRequest.setStoreName(supplierVO.getStoreName());
        }

        //获取customer 信息
        CustomerDetailGetWithNotDeleteByCustomerIdResponse context =
                customerDetailQueryProvider.getCustomerDetailWithNotDeleteByCustomerId(
                        CustomerDetailWithNotDeleteByCustomerIdRequest.builder().customerId(commonUtil.getOperatorId())
                                .build()
                ).getContext();

        // 设置店铺评价内的店铺信息
        if(Objects.nonNull(storeEvaluateAddRequest)) {
            storeEvaluateAddRequest.setStoreId(supplierVO.getStoreId());
            storeEvaluateAddRequest.setStoreName(supplierVO.getStoreName());
        }
        EsEvaluateAddResponse res = new EsEvaluateAddResponse();
        //商品评价
        if (Objects.nonNull(goodsEvaluateAddRequest) && ((Objects.nonNull(goodsEvaluateAddRequest.getEvaluateContent())
            ...
            Map<String, TradeItemVO> skuMap =
                    tradeVO.getTradeItems().stream().collect(Collectors.toMap(TradeItemVO::getSkuId, Function.identity()));
            ...
            goodsEvaluateAddRequest.setCustomerAccount(context.getCustomerVO().getCustomerAccount());
            goodsEvaluateAddRequest.setCustomerId(context.getCustomerVO().getCustomerId());
            goodsEvaluateAddRequest.setCustomerName(context.getCustomerName());
            ...
            goodsEvaluateAddRequest.setStoreType(supplierVO.getStoreType());
            boolean flag = (Objects.nonNull(imageAddRequestList) && !imageAddRequestList.isEmpty());
            if (flag) {
                goodsEvaluateAddRequest.setIsUpload(1);
            }
            if (Objects.isNull(goodsEvaluateAddRequest.getIsAnonymous())) {
                goodsEvaluateAddRequest.setIsAnonymous(NumberUtils.INTEGER_ZERO);
            }
            ...
            if (Objects.nonNull(goodsEvaluateVO) && Objects.nonNull(imageAddRequestList) && imageAddRequestList.size() > 0) {
                List<GoodsEvaluateImageVO> imageList = new ArrayList<>();
                //晒单
                if (flag) {
                    for (GoodsEvaluateImageAddRequest imageAddRequest : imageAddRequestList) {
                        ...
                        GoodsEvaluateImageAddResponse imageAddResponse = imageSaveProvider.add(imageAddRequest).getContext();
                        GoodsEvaluateImageVO goodsEvaluateImageVO = imageAddResponse.getGoodsEvaluateImageVO();
                        imageList.add(goodsEvaluateImageVO);
                    }
                }
                goodsEvaluateVO.setEvaluateImageList(imageList);
            }
            res.setGoodsEvaluateVO(goodsEvaluateVO);
            //删除待评价记录
            ...
            if (goodsEvaluateAddRequest.getEvaluateScore() >= Constants.FOUR
                    && goodsEvaluateAddRequest.getEvaluateContent().trim().length() >= Constants.NUM_30) {
                // 增加成长值
        ...
        if (Objects.nonNull(storeEvaluateAddRequest) && (Objects.nonNull(storeEvaluateAddRequest.getServerScore())
            ...
            if (Objects.isNull(evaluateRatioVO)) {
                ...
                redisService.setObj(CacheKeyConstant.EVALUATE_RATIO, evaluateRatioVO, Duration.ofHours(24).getSeconds());
            }
            ...
            storeEvaluateAddRequest.setCustomerId(commonUtil.getOperatorId());
            ...
            res.setStoreEvaluateVO(storeEvaluateVO);
