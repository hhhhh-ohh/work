...
public class StoreCateQueryController implements StoreCateQueryProvider {
    ...
    /**
     * 根据父分类ID查询商品分类
     * @param storeCateListByParentIdRequest {@link StoreCateListByParentIdRequest }
     * @return
     */
    @Override
    public BaseResponse<StoreCateListByStoreIdResponse> listByCateParentId(StoreCateListByParentIdRequest storeCateListByParentIdRequest) {

        List<StoreCateResponse> storeCateResponses = storeCateService.listByCateParentId(storeCateListByParentIdRequest);
        if (CollectionUtils.isEmpty(storeCateResponses)){
            return BaseResponse.success(new StoreCateListByStoreIdResponse(Collections.EMPTY_LIST));
        }
        List<StoreCateResponseVO> storeCateResponseVOList = StoreCateConvert.toStoreCateResponseVOList(storeCateResponses);
        return BaseResponse.success(new StoreCateListByStoreIdResponse(storeCateResponseVOList));
    }
    ...
    public BaseResponse<StoreCateListByGoodsResponse> listByGoods(@RequestBody @Valid StoreCateListByGoodsRequest storeCateListByGoodsRequest){
        ...
        return BaseResponse.success(new StoreCateListByGoodsResponse(WmCollectionUtils
                .convert(storeCateGoodsRelaList, s-> KsBeanUtil.copyPropertiesThird(s,StoreCateGoodsRelaVO.class))));
    }
    ...
    @Override
    public BaseResponse<GoodsIdByStoreCateIdAndIsHaveSelfResponse> listGoodsIdByStoreCateIdAndIsHaveSelf(@Valid StoreCateListGoodsRelByStoreCateIdAndIsHaveSelfRequest storeCateListGoodsRelByStoreCateIdAndIsHaveSelfRequest) {
        List<String> goodsIdList = storeCateService.findGoodsIdByStoreCate(storeCateListGoodsRelByStoreCateIdAndIsHaveSelfRequest.getStoreCateId(),storeCateListGoodsRelByStoreCateIdAndIsHaveSelfRequest.isHaveSelf());
        if (CollectionUtils.isEmpty(goodsIdList)){
            return BaseResponse.success(new GoodsIdByStoreCateIdAndIsHaveSelfResponse(Collections.EMPTY_LIST));
        }
        return BaseResponse.success(new GoodsIdByStoreCateIdAndIsHaveSelfResponse(goodsIdList));
    }

    @Override
    public BaseResponse<StoreCateCoincideResponse> coincide(@RequestBody @Valid StoreCateCoincideRequest request) {
        return BaseResponse.success(new StoreCateCoincideResponse(storeCateService.coincideStoreCate(request.getStoreCateIds(), request.getStoreCateSecIds())));
    }
}
