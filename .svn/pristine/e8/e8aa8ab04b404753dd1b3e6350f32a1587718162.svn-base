...
public class PointsGoodsQueryController implements PointsGoodsQueryProvider {
    ...
    public BaseResponse<PointsGoodsPageResponse> page(@RequestBody @Valid PointsGoodsPageRequest pointsGoodsPageReq) {
        ...
        if (CollectionUtils.isNotEmpty(newPage.getContent())) {
            newPage.getContent().stream()
                    .filter(
                            g ->
                                    Objects.nonNull(g.getGoodsInfo())
                                            && (GoodsStatus.INVALID != g.getGoodsInfo().getGoodsStatus()))
                    .forEach(
                            g ->
                                    g.getGoodsInfo()
                                            .setGoodsStatus(
                                                    g.getGoodsInfo().getStock() > 0
                                                            ? GoodsStatus.OK
    ...
    @Override
    public BaseResponse<PointsGoodsListResponse> listForImport(@RequestBody @Valid PointsGoodsListRequest pointsGoodsListReq) {
        PointsGoodsQueryRequest queryReq = new PointsGoodsQueryRequest();
        KsBeanUtil.copyPropertiesThird(pointsGoodsListReq, queryReq);
        List<SimplePointsGoods> pointsGoodsList = pointsGoodsService.simplelist(queryReq);
        List<PointsGoodsVO> newList = KsBeanUtil.convertList(pointsGoodsList, PointsGoodsVO.class);
        return BaseResponse.success(new PointsGoodsListResponse(newList));
    }
    ...
    @Override
    public BaseResponse<PointsGoodsPageResponse> simplePage(@RequestBody PointsGoodsSimplePageRequest request) {
        return BaseResponse.success(new PointsGoodsPageResponse(pointsGoodsService.simplePage(request)));
    }

    @Override
    public BaseResponse<PointsGoodsListResponse> getByGoodsInfoId(@RequestBody @Valid PointsGoodsByGoodsInfoIdRequest pointsGoodsByGoodsInfoIdRequest) {
        List<PointsGoods> pointsGoodsList = pointsGoodsService.getByGoodsInfoId(pointsGoodsByGoodsInfoIdRequest.getGoodsInfoId());
        List<PointsGoodsVO> pointsGoodsVOS = pointsGoodsList.stream().map(entity -> pointsGoodsService.wrapperVo(entity)).collect(Collectors.toList());
        return BaseResponse.success(new PointsGoodsListResponse(pointsGoodsVOS));
    }
}
