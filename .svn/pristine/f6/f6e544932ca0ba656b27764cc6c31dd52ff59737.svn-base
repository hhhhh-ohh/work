 ...
 **/
@Tag(name = "AliyunSmsHttpPush", description = "短信事件回调")
@RestController
@Validated
...
public class AliyunSmsHttpPush {
    ...
    public void signSmsReport(HttpServletRequest request, HttpServletResponse response) {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(request.getInputStream()))){
            ...
            for (int i = 0; i < jsonArray.size(); i++) {
                ...
                SignSmsReportResponse signSmsReportResponse = jsonObject.toJavaObject(SignSmsReportResponse.class);
    ...
    @Operation(summary = "短信模板审核状态消息回调")
    ...
    public void templateSmsReport(HttpServletRequest request, HttpServletResponse response) {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(request.getInputStream()))){
            ...
            for (int i = 0; i < jsonArray.size(); i++) {
                JSONObject jsonObject = jsonArray.getJSONObject(i);
                TemplateSmsReportResponse templateSmsReportResponse = jsonObject.toJavaObject(TemplateSmsReportResponse.class);
