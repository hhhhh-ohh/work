...
@Slf4j
@Service
public class FullDiscountExcelServiceImpl implements MarketingExcelService {
    @Autowired
    ...
    public void downloadTemplate(Integer activityType) {
        ...
        this.write(file, "满折活动商品导入模板.xls");
    }

    @Override
    public void downloadErrorFile(String fileExt) {
        String userId = commonUtil.getOperatorId();
        String resourceKey = Constants.FULL_DISCOUNT_SALE_GOODS_ERR_EXCEL.concat(userId);
    ...
    public String uploadFile(MultipartFile file) {
        ...
        String userId = commonUtil.getOperatorId();
        String resourceKey = Constants.FULL_DISCOUNT_SALE_GOODS_EXCEL.concat(userId);
        try (Workbook workbook = WorkbookFactory.create(new ByteArrayInputStream(file.getBytes()))) {
            ...
            this.checkGoodsInfoNo(skuNoMap,goodsInfoQueryProvider,workbook,isError,null,
                    Boolean.FALSE,commonUtil.getStoreId(),Boolean.FALSE,Boolean.FALSE,Boolean.FALSE, Boolean.FALSE);
            if (isError.get()) {
                String newFileName = userId.concat(".").concat(fileExt);
                resourceKey = Constants.FULL_DISCOUNT_SALE_GOODS_ERR_EXCEL.concat(userId);
