...
@Tag(name =  "rfm参数配置管理API", description =  "RfmSettingController")
@RestController
@Validated
...
public class RfmSettingController {
    ...
    @Operation(summary = "根据id查询rfm参数配置")
    @GetMapping("/{id}")
    ...
    @Operation(summary = "根据id删除rfm参数配置")
    @DeleteMapping("/{id}")
    ...
    @Operation(summary = "根据idList批量删除rfm参数配置")
    ...
    public void exportData(@PathVariable String encrypted, HttpServletResponse response) {
        ...
        try {
            ...
            String fileName = URLEncoder.encode(String.format("rfm参数配置列表_%s.xls", nowStr), StandardCharsets.UTF_8.name());
