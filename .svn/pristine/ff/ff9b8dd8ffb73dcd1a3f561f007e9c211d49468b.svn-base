...
public class MarketingSuitsService {
    ...
    public void modify(MarketingSuitsSaveRequest marketingSuitsSaveRequest) {
        ...
        if (DefaultFlag.YES.equals(marketingSuitsSaveRequest.getIsPause()) && LocalDateTime.now().isAfter(marketingSuitsSaveRequest.getBeginTime())) {
            //已开始且暂停的活动可编辑
            marketingService.pauseModify(PauseModifyRequest.builder()
                    .marketingId(marketingSuitsSaveRequest.getMarketingId())
                    .endTime(marketingSuitsSaveRequest.getEndTime())
                    .joinLevel(marketingSuitsSaveRequest.getJoinLevel())
                    .updatePerson(marketingSuitsSaveRequest.getUserId())
                    .build());
            ...
            //组合商品主表数据
            MarketingSuits entity = new MarketingSuits();
            entity.setMarketingId(marketingSuitsSaveRequest.getMarketingId());
            entity.setMainImage(marketingSuitsSaveRequest.getSuitsPictureUrl());
            entity.setSuitsPrice(suitPrice);
            //删除原有数据
            deleteByMarketingId(marketingSuitsSaveRequest.getMarketingId());
            marketingSuitsRepository.save(entity);

            List<MarketingSuitsSku> list = marketingSuitsSkuService.list(MarketingSuitsSkuQueryRequest.builder()
                    .marketingId(marketingSuitsSaveRequest.getMarketingId())
                    .build());
            for (MarketingSuitsSku suit : list) {

                GoodsSuitsDTO dto = marketingSuitsSaveRequest.getGoodsSuitsDTOList().stream()
                        .filter(v -> v.getGoodsInfoId().equals(suit.getSkuId())).findFirst().orElse(null);
                if (Objects.nonNull(dto) && dto.getGoodsSkuNum() < suit.getNum()) {
                    throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
                } else if (Objects.nonNull(dto) && dto.getGoodsSkuNum() > suit.getNum()) {
                    suit.setNum(dto.getGoodsSkuNum());
                    marketingSuitsSkuService.modify(suit);
                }
            }
    ...
    private List<GoodsSuitsDTO> check(MarketingSuitsSaveRequest marketingSuitsSaveRequest) {
        ...
        //1、校验商品数量
        if (goodsSuitsDTOList.size() < Constants.TWO) {
            throw new SbcRuntimeException(MarketingErrorCodeEnum.K080104);
        } else if (goodsSuitsDTOList.size() > Constants.NUM_20) {
            throw new SbcRuntimeException(MarketingErrorCodeEnum.K080105);
        }
		...
		for (GoodsSuitsDTO goodsSuitsDTO : goodsSuitsDTOList) {
			...
			if(goodsInfo != null){
				if(AddedFlag.YES.toValue() == (goodsInfo.getAddedFlag())){
					//3、校验商品折扣价
					if(goodsSuitsDTO.getDiscountPrice().compareTo(goodsInfo.getMarketPrice()) > 0){
						throw new SbcRuntimeException(MarketingErrorCodeEnum.K080106);
					}
				}else{
					throw new SbcRuntimeException(MarketingErrorCodeEnum.K080107, new Object[]{goodsInfo.getGoodsInfoName()});
				}
			}else{
				throw new SbcRuntimeException(MarketingErrorCodeEnum.K080110);
			}
        ...
        if (existsSkuNum > NumberUtils.INTEGER_ZERO) {
            throw new SbcRuntimeException(MarketingErrorCodeEnum.K080004, new Object[]{existsSkuNum});
        }
        ...
        if (CollectionUtils.isNotEmpty(marketingList)) {
            ...
            marketingList.forEach((marketing) -> {
                ...
                List<MarketingSuitsSku> marketingSuitsSkuList =
                        marketingSuitsSkuService.getByMarketingIds(Collections.singletonList(marketing.getMarketingId()));
                //1、先判断组合购包含商品数是否一致
                if (marketingSuitsSkuList.size() == goodsSuitsDTOList.size()) {
                    ...
                    List<MarketingSuitsSku> marketingSuitsSkus = marketingSuitsSkuList.stream().filter(
                            item -> goodsSuitsMap.get(item.getSkuId()) != null &&
                                    goodsSuitsMap.get(item.getSkuId()).equals(item.getNum()))
                            .collect(Collectors.toList());
                    //过滤后结果如果和新增保持一致
                        ...
                        if (marketingSuitsSkus.size() == marketingSuitsSkuList.size()) {
                            ...
                            marketingSuitsSkuList.forEach(marketingSuitsSku -> {
                                ...
                                if ((marketingSuitsSaveRequest.getBeginTime().isAfter(marketingPo.getBeginTime())
                                    ...
                                    throw new SbcRuntimeException(MarketingErrorCodeEnum.K080109);
                                }
...
//	}
    ...
    public MarketingSuitsByMarketingIdResponse getByMarketingId(Long marketingId) {
        ...
        // 营销活动信息
        MarketingVO marketing = marketingMapper.marketingToMarketingVO(marketingService.queryById(marketingId));
        if (Objects.isNull(marketing) || !Objects.equals(MarketingSubType.SUITS_GOODS, marketing.getSubType())) {
    ...
    public MarketingSuitsByMarketingIdResponse getByMarketingIdForSupplier(Long marketingId) {
        ...
        List<GoodsInfoVO> goodsInfos = goodsInfoQueryProvider.listViewByIds(GoodsInfoViewByIdsRequest.builder()
                .goodsInfoIds(skuIds).isHavSpecText(1).goodsSuitsFlag(true).build()).getContext().getGoodsInfos();

        this.populateProviderName(goodsInfos);
        ...
        if (CollectionUtils.isNotEmpty(goodsInfos)) {
            ...
            response.getMarketingSuitsSkuVOList().stream().forEach(marketingSuitsSkuVO -> {
                goodsInfos.stream().filter(goodsInfoVO ->
                    ...
                    marketingSuitsSkuVO.setGoodsType(goodsInfoVO.getGoodsType());
                    marketingSuitsSkuVO.setSupplyPrice(goodsInfoVO.getSupplyPrice());
                    marketingSuitsSkuVO.setProviderName(goodsInfoVO.getProviderName());
     ...
     * @description 填充供应商名称
     * @Author qiyuanzhao
     * @Date 2022/7/30 13:44
     * @param
     * @return
     **/
    public void populateProviderName(List<GoodsInfoVO> goodsInfoVOList) {
        if (org.apache.commons.collections.CollectionUtils.isEmpty(goodsInfoVOList)){
            return;
        }
        ...
        if (org.apache.commons.collections.CollectionUtils.isEmpty(storeVOS)){
            return;
        }
    ...
    public List<MarketingMoreGoodsInfoResponse> getMarketingSuitsBySkuId(String skuId, Long storeId, String customerId) {
        ...
        //组装数据
        List<MarketingMoreGoodsInfoResponse> responseList = new ArrayList<>(10);
    ...
    public MarketingSuits getOne(Long id) {
        return marketingSuitsRepository.findById(id)
                .orElseThrow(() -> new SbcRuntimeException(CommonErrorCodeEnum.K999999, "组合商品主表不存在"));
    }
