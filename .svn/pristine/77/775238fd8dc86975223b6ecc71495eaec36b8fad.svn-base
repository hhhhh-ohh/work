...
@Component
@Slf4j
public class GoodsEvaluateRemindJobHandler {
    ...
    @XxlJob(value = "goodsEvaluateRemindJobHandler")
    ...
    private void handleEvaluate(TradeQueryDTO dto) {
        TradeListAllRequest tradeListAllRequest = TradeListAllRequest.builder()
                .tradeQueryDTO(dto).build();
        log.info("开始分页查询订单，这是第{}页", dto.getPageNum() + 1);
        List<TradeVO> tradeVOList = tradeQueryProvider.pageAll(tradeListAllRequest).getContext().getTradeVOList();
