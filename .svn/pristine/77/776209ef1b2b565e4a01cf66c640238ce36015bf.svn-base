...
public interface GrouponActivityQueryProvider {
     ...
     * @return
     */
    @PostMapping("/marketing/${application.marketing.version}/groupon/activity/get-simple-by-id")
    BaseResponse<GrouponActivityVO> getSimpleById(@RequestBody @Valid GrouponActivityByIdRequest request);

    /**
     * 互斥验证
     *
     * @author dyt
     * @param request 互斥请求参数 {@link GrouponGoodsValidateRequest}
     */
    @PostMapping("/marketing/${application.goods.version}/groupon/validate")
    BaseResponse validate(@RequestBody @Valid GrouponGoodsValidateRequest request);
}
