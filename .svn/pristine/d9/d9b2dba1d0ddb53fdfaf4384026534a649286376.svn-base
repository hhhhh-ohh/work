...
@Slf4j
@Service
public class FullReturnExcelServiceImpl implements MarketingExcelService {
    @Autowired
    ...
    public void downloadTemplate(Integer activityType) {
        ...
        this.write(file, "满返活动商品导入模板.xls");
    }

    @Override
    public void downloadErrorFile(String fileExt) {
        String userId = commonUtil.getOperatorId();
        String resourceKey = Constants.FULL_RETURN_GOODS_ERR_EXCEL.concat(userId);
    ...
    public String uploadFile(MultipartFile file) {
        ...
        String userId = commonUtil.getOperatorId();
        String resourceKey = Constants.FULL_RETURN_GOODS_EXCEL.concat(userId);
        try (Workbook workbook = WorkbookFactory.create(new ByteArrayInputStream(file.getBytes()))) {
            ...
            if (isError.get()) {
                ...
                resourceKey = Constants.FULL_RETURN_GOODS_ERR_EXCEL.concat(userId);
