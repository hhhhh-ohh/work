...
public class LoginController {
    ...
    @Value("${channel.env.prod.open}")
    private boolean open;

    @Value("${channel.env.prod.type}")
    private String type;
    ...
    public BaseResponse<LoginResponse> login(@Valid @RequestBody EmployeeLoginRequest loginRequest,
        ...
        String account = new String(Base64.getUrlDecoder().decode(loginRequest.getAccount()), StandardCharsets.UTF_8);
        ...
        //供应商登录
        return loginByAccount(account, passwordChars, AccountType.s2bProvider.toValue(),httpServletResponse);
    }
     ...
     * @return
     */
    @Operation(summary = "会员自动免密登录登录")
    @RequestMapping(value = "/login-auto", method = RequestMethod.POST)
    public BaseResponse<LoginResponse> loginAuto(@Valid @RequestBody EmployeeAutoLoginRequest loginRequest,
                                             HttpServletResponse httpServletResponse) {
        String account = new String(Base64.getUrlDecoder().decode(loginRequest.getAccount()), StandardCharsets.UTF_8);
        //供应商登录
        return loginByAccountWithOutPassword(account, AccountType.s2bProvider.toValue(),httpServletResponse);
    }
     ...
     * @return
     */
    private BaseResponse<LoginResponse> loginByAccount(String account, char[] password,int type ,
                                                       HttpServletResponse httpServletResponse) {
     ...
     * @return
     */
    private BaseResponse<LoginResponse> loginByAccountWithOutPassword(String account,int type ,
                                                       HttpServletResponse httpServletResponse) {
        return Optional.ofNullable(employeeQueryProvider.getByAccountName(
                        EmployeeByAccountNameRequest.builder()
                                .accountName(account)
                ...
                .map(response -> {
                    EmployeeVO employee = new EmployeeVO();
                    KsBeanUtil.copyPropertiesThird(response, employee);
                    return customerValidateSuccess(employee,httpServletResponse);
                })
                .orElseThrow(() -> validateNull(account));
    }
    ...
    public BaseResponse<LoginResponse> customerValidateSuccess(EmployeeVO employee,HttpServletResponse
            httpServletResponse){
        ...
        BoolFlag companyType = null;
        CheckState auditState = null;
//            Store store = storeService.queryStoreByCompanyInfoId(companyInfoId);
        ...
        if (open
                ...
                .anyMatch(
                        t ->
                                store.getCompanySourceType().toValue()
                                        == Integer.parseInt(t))) {
            throw new SbcRuntimeException(CustomerErrorCodeEnum.K010089,
                    new Object[]{"如有疑问请联系平台"});
        }

        Long storeId = null;
        Integer storeType = StoreType.PROVIDER.toValue();
        if (Objects.nonNull(store)) {
            storeId = store.getStoreId();
            companyType = store.getCompanyType();
            auditState = store.getAuditState();
            storeType = store.getStoreType().toValue();
        }
        ...
        claims.put("storeType", storeType);
        //员工版本，不一致表示失效
        claims.put("versionNo", employee.getVersionNo());
        if (Objects.nonNull(companyType)){
            claims.put("companyType",companyType.toValue());
        }
        ...
        if (StringUtils.isEmpty(employee.getRoleIds())) {
            ...
            StringBuilder roleNames = new StringBuilder(org.apache.commons.lang3.StringUtils.EMPTY);
            for (String id : ids) {
                ...
                if (Objects.nonNull(roleInfoQueryResponse) && Objects.nonNull(roleInfoQueryResponse.getRoleInfoVO())
                    ...
                    roleNames.append(roleInfoQueryResponse.getRoleInfoVO().getRoleName()).append(";");
                }
            }
            claims.put("roleName", roleNames.toString());
        ...
        return BaseResponse.success(LoginResponse.builder()
                ...
                .auditState(Objects.isNull(auditState) ? -1 : auditState.toValue())
                ...
                .companySourceType(Objects.nonNull(store) ? store.getCompanySourceType() : null)
                .token(token)
                .build());
    }
     ...
     * @return
     */
    @Operation(summary = "登录后验证供应商店铺的各种状态")
