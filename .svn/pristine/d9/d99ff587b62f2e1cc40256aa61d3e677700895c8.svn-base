...
@Tag(name =  "标签维度管理API", description =  "TagDimensionController")
@RestController
@Validated
                ...
                switch (TagParamColumn.valueOf(_item.getColumnName().toUpperCase())){
                    ...
                    case  TERMINAL_SOURCE:
                        if(_item.getColumnType()!=0){
                            _item.setDefaultValue(Stream.of(
                                    TerminalSourceType.WEIXIN.toValue(),
                                    TerminalSourceType.APP.toValue(),
                                    TerminalSourceType.PC.toValue(),
                                    TerminalSourceType.H5.toValue()
                            ).map(str->{
                                Map<String,String> rsMap = new HashMap<String,String>();
                                rsMap.put("id",str);
                                rsMap.put("value",str);
                    ...
                    case DATE:
                        if(_item.getColumnType()!=0){
                            _item.setDefaultValue(Stream.of(
                                    DateType.MONDAY.toValue(),
                                    DateType.TUESDAY.toValue(),
                                    DateType.WEDNESDAY.toValue(),
                                    DateType.THURSDAY.toValue(),
                                    DateType.FRIDAY.toValue(),
                                    DateType.SATURDAY.toValue(),
                                    DateType.SUNDAY.toValue()
                            ).map(str->{
                                Map<String,String> rsMap = new HashMap<String,String>();
                                rsMap.put("id",str);
                                rsMap.put("value",str);
