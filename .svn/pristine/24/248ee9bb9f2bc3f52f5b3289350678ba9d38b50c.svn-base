...
import com.wanmi.sbc.common.util.HttpServletRequestCopier;
import com.wanmi.sbc.common.util.HttpServletResponseCopier;
...
public class RequestLogFilter extends GenericFilterBean {
    ...
    protected void initFilterBean() throws ServletException {
        ...
        if (filterConfig != null) {
            ...
            String maxResultLengthStr =
                    filterConfig.getInitParameter("MAX_RESULT_LENGTH");
            ...
            String logTypeStr = filterConfig.getInitParameter("LOG_TYPE");
            logType = Integer.parseInt(logTypeStr);

            String initExcludePatterns =
                    filterConfig.getInitParameter("EXCLUDE_PATTERNS");
            if (StringUtils.isNotEmpty(initExcludePatterns)) {
            ...
            if (StringUtils.isNotEmpty(initExcludeMethods)) {
                excludeMethods = initExcludeMethods.split(SEPARATOR);
            }

            String initExcludeHeaders = filterConfig.getInitParameter("EXCLUDE_HEADERS");
            if (StringUtils.isNotEmpty(initExcludeHeaders)) {
                excludeHeaders = initExcludeHeaders.split(SEPARATOR);
            }

            String initIncludeHeaders = filterConfig.getInitParameter("INCLUDE_HEADERS");
            if (StringUtils.isNotEmpty(initIncludeHeaders)) {
                includeHeaders = initIncludeHeaders.split(SEPARATOR);
            }

            String passwordFieldRegex =
                    filterConfig.getInitParameter("PASSWORD_FIELD_REGEX");
            if (StringUtils.isNotEmpty(passwordFieldRegex)) {
                this.passwordPattern = Pattern.compile(passwordFieldRegex);
            }

            String needBlurPasswordForParamStr =
                    filterConfig.getInitParameter("NEED_BLUR_PASSWORD_FOR_PARAM");
            needBlurPasswordForParam = Boolean.parseBoolean(needBlurPasswordForParamStr);

            objectMapper.configure(SerializationFeature.FAIL_ON_EMPTY_BEANS, false);
            objectMapper.configure(SerializationFeature.ORDER_MAP_ENTRIES_BY_KEYS, true);

            String siteOwnerKeyParam = filterConfig.getInitParameter("SITE_OWNER_KEY");
            siteOwnerKey =
                    StringUtils.isNotEmpty(siteOwnerKeyParam)
                            ? siteOwnerKeyParam
                            : DEFAULT_SITE_OWNER_KEY;

            String requestIdKeyParam = filterConfig.getInitParameter("REQUEST_ID_KEY");
            requestIdKey =
                    StringUtils.isNotEmpty(requestIdKeyParam)
                            ? requestIdKeyParam
                            : DEFAULT_REQUEST_ID_KEY;

            String sessionIdKeyParam = filterConfig.getInitParameter("SESSION_ID_KEY");
            sessionIdKey =
                    StringUtils.isNotEmpty(sessionIdKeyParam)
                            ? sessionIdKeyParam
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (request instanceof HttpServletRequest) {
            ...
            if (isExcludePath(requestUri) || isExcludeMethod(method)) {
                ...
                String uuid = StringUtils.isNotEmpty(request.getParameter(requestIdKey)) ? request.getParameter(requestIdKey)
                        : httpRequest.getHeader(requestIdKey); // reqId
                uuid = StringUtils.isNotEmpty(uuid) ? uuid : UUID.randomUUID().toString();

                String sessionId = StringUtils.isNotEmpty(request.getParameter(sessionIdKey))
                        ? request.getParameter(sessionIdKey)
                        : httpRequest.getHeader(sessionIdKey); // sessionId
                sessionId = StringUtils.isNotEmpty(sessionId) ? sessionId : httpRequest.getSession().getId(); // sessionId
                ...
                try {
                    ...
                    try {
                        ...
                        // 耗时
                        logBean.setCost(System.currentTimeMillis() - startTimeMills);
                        // uri
                        logBean.setUri(String.format("%s %s", method, requestUri));
                        // fromIp
                        logBean.setFromIp(HttpUtil.getRealRemoteIp(requestCopier));
                        // sessionId
                        ...
                        // 清除mapping
                        MDC.remove("reqLog_mapping");
                        // 清除clzMethod
                        MDC.remove("reqLog_clzMethod");
                        // 清除exMsg
                        MDC.remove("reqLog_exMsg");
                        // 清除mappingName
                        MDC.remove("reqLog_mappingName");

                        Object siteOwner = httpRequest.getAttribute(this.siteOwnerKey);
                        if (siteOwner != null) {
                            // 设置站点所有者, 需要业务通过request.setAttribute设置
                        ...
                        // 根据异常消息判断日志级别
                        recordLog(logBean, StringUtils.isNotEmpty(exMsg));
