...
public class CustomerSignRecordService {
    ...
    public CustomerSignRecord add(CustomerSignRecord entity) {
        //查询昨天是否签到
        Optional<Customer> customerOptional = customerRepository.findById(entity.getCustomerId());
        if (customerOptional.isPresent()) {
            Customer customer = customerOptional.get();
            Integer signContinuousDays = customer.getSignContinuousDays() == null ? Integer.valueOf(0) :
                    customer.getSignContinuousDays();
