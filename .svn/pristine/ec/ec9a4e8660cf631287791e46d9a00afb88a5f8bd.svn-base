...
public class OrderPerformanceService {
    ...
    public void createOrderPerformance(Trade trade) {
        try {
            ...
            // 获取会员信息
            BaseResponse<CustomerDetailGetCustomerIdResponse> customerDetail = customerDetailQueryProvider.getCustomerDetailByCustomerId(new CustomerDetailByCustomerIdRequest(customerId));
            if (customerDetail.getContext() == null || !customerDetail.isSuccess()) {
                log.info("订单业绩创建失败，会员信息未查询到，会员ID：{}，会员ID为空", customerId);
                return;
            }
            // 判断是否是一户一码会员过来的
            if (StringUtils.equals(customerDetail.getContext().getAgentUniqueCode(), "-1") || StringUtils.equals(customerDetail.getContext().getAgentId(), "-1")) {
            ...
            String agentUniqueCode = customerDetail.getContext().getAgentUniqueCode();
            ...
            // 获取到代理信息
            AgentGetByUniqueCodeResponse agentBase = responseBaseResponse.getContext();

            // 组装订单业绩
            OrderPerformance orderPerformance = assembleOrderPerformance(trade, agentBase);
            ...
            log.error("订单{},创建订单业绩异常: {}", trade.getId(), e, e);
        }
    }
     ...
     * 组装订单业绩
     */
    public OrderPerformance assembleOrderPerformance(Trade trade, AgentGetByUniqueCodeResponse agentBase) {
        OrderPerformance orderPerformance = new OrderPerformance();

        orderPerformance.setProvinceId(agentBase.getProvinceId());
        orderPerformance.setCityId(agentBase.getCityId());
        orderPerformance.setAreaId(agentBase.getAreaId());
        orderPerformance.setProvinceName(agentBase.getProvinceName());
        orderPerformance.setCityName(agentBase.getCityName());
        orderPerformance.setAreaName(agentBase.getAreaName());
        orderPerformance.setShopName(agentBase.getShopName());
        orderPerformance.setAgentUniqueCode(agentBase.getAgentUniqueCode());
        orderPerformance.setAgentId(agentBase.getAgentId());
