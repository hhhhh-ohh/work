...
import com.wanmi.sbc.customer.api.request.customer.DistributionInviteNewUpdateRequest;
...
public class CustomerDistributionInviteNewService {
    ...
    public DistributionInviteNewVo addInviteNewRecord(InviteNewRecord inviteNewRecord) {
        ...
        return this.wrapperVo(newRecord);
    }
    ...
    public DistributionInviteNewVo update(DistributionInviteNewUpdateRequest inviteNewRecord) {
        ...
        if (InvalidFlag.NO == newRecord.getRewardRecorded()
                && inviteNewRecord.getOverLimit() == DefaultFlag.NO) {
     ...
     */
    @Transactional
    public DistributionInviteNewRecordVO updateBySupplyAgain(DistributionInviteNewSupplyAgainRequest inviteNewRecord) {
        InviteNewRecord newRecord = this.queryById(inviteNewRecord.getRecordId());
        DistributionInviteNewRecordVO inviteNewVo = new DistributionInviteNewRecordVO();
        if (null == newRecord) {
            return inviteNewVo;
        }
        newRecord.setRewardRecorded(InvalidFlag.YES);
    ...
    public InviteNewRecord queryById(String id) {
        InviteNewRecord inviteNewRecord = customerDistributionInviteNewRepository.findById(id).orElse(null);
        if (inviteNewRecord == null) {
            throw new SbcRuntimeException(getDeleteIndex(id), CustomerErrorCodeEnum.K010001);
        }
     ...
     */
    @Transactional
    public DistributionInviteNewSaveResponse addDistributionInviteNewRecord(DistributionInviteNewAddRegisterRequest addRegisterRequest) {
        ...
        DistributionInviteNewSaveResponse distributionInviteNewSaveResponse = new DistributionInviteNewSaveResponse();
        if (Objects.isNull(customer) || StringUtils.isBlank(requestCustomerId) || StringUtils.isBlank(customerId)) {
            ...
            return distributionInviteNewSaveResponse;
        }
        // 判断邀请人是否存在分销员表中
        ...
        if (CollectionUtils.isNotEmpty(saveResult)) {
            List<DistributionInviteNewRecordVO> inviteNewRecordVOList = KsBeanUtil.convertList(saveResult, DistributionInviteNewRecordVO.class);
            distributionInviteNewSaveResponse.setInviteNewRecordVOList(inviteNewRecordVOList);
...
//            Integer invitedCount = NumberUtils.INTEGER_ZERO;
            if (Objects.nonNull(distributionCustomer)) {
                ...
                if(Objects.nonNull(rewardCashResult) || Objects.nonNull(rewardCouponResult)){
            ...
            //返回分销员id
            if (Objects.nonNull(distributionCustomer)) {
                distributionInviteNewSaveResponse.setDistributionId(distributionCustomer.getDistributionId());
            }
            //好友注册成功通知触发
    ...
    public DistributionInviteNewUpdateResponse modifyDistributionInviteNewRecord(DistributionInviteNewModifyRegisterRequest modifyRegisterRequest) {
        ...
        if (CollectionUtils.isNotEmpty(resultList)) {
            ...
            List<DistributionInviteNewRecordVO> inviteNewRecordVOList = KsBeanUtil.convertList(resultList, DistributionInviteNewRecordVO.class);
            response.setInviteNewRecordVOList(inviteNewRecordVOList);
            // 判断邀请人是否存在分销员表中
            DistributionCustomer distributionCustomer = distributionCustomerService.getByCustomerIdAndDelFlag(requestCustomerId);
            ...
            if (Objects.nonNull(coupon) && Objects.isNull(failReasonFlag_coupon) && InvalidFlag.NO == rewardRecorded_coupon_old && InvalidFlag.YES == coupon.getRewardRecorded()) {
                ...
                List<DistributionRewardCouponDTO> list = JSON.parseArray(settingCouponIdsCounts, DistributionRewardCouponDTO.class);
    ...
    public InviteNewRecord addRewardCash(DistributionInviteNewAddRegisterRequest addRegisterRequest, Integer distributor, LocalDateTime now) {
        ...
        //0.判断邀新开关是否打开
        DefaultFlag inviteOpenFlag = addRegisterRequest.getInviteOpenFlag();

        //1.判断发放奖励奖金开关是否打开
        DefaultFlag rewardCashFlag = addRegisterRequest.getRewardCashFlag();

//        //2.邀新开关、发放奖励奖金-开关关闭
//        if (Objects.isNull(inviteOpenFlag) || DefaultFlag.NO == inviteOpenFlag || Objects.isNull(rewardCashFlag) || DefaultFlag.NO == rewardCashFlag) {
//            return inviteNewRecord;
//        }
...
//        //2.邀新开关、发放优惠券-开关关闭
//        if (Objects.isNull(inviteOpenFlag) || DefaultFlag.NO == inviteOpenFlag || Objects.isNull(rewardCouponFlag) ||  DefaultFlag.NO == rewardCouponFlag) {
//            return inviteNewRecord;
//        }
        if(rewardCouponFlag == null || DefaultFlag.NO.equals(rewardCouponFlag)) {
            return null;
        }
        ...
        if(CollectionUtils.isNotEmpty(addRegisterRequest.getCouponNameList())){
            inviteNewRecord.setSettingCoupons(Joiner.on(separator).join(addRegisterRequest.getCouponNameList()));
        }
        inviteNewRecord.setTerminalSource(addRegisterRequest.getTerminalSource());
        inviteNewRecord.setSettingCouponIdsCounts(JSON.toJSONString(addRegisterRequest.getDistributionRewardCouponDTOList()));
    ...
    public FailReasonFlag getSendRewardFailReason(DistributionInviteNewAddRegisterRequest addRegisterRequest) {
        ...
        //2：奖励未开启
        if (DefaultFlag.NO == openFlag || DefaultFlag.NO == inviteOpenFlag || DefaultFlag.NO == inviteFlag) {
            return FailReasonFlag.UNOPENED;
        }
        ...
        if (request.getAvailableDistribution() != null) {
            inviteNewRecords = customerDistributionInviteNewRepository
                    .findInviteNewRecordPageByAvailableDistribution(request.getRequestCustomerId(), request
                            .getAvailableDistribution().toValue(), request
                            .getPageRequest());
        } else {
            inviteNewRecords = customerDistributionInviteNewRepository
    ...
    private void dealMessage(DistributionInviteNewAddRegisterRequest addRegisterRequest, InviteNewRecord rewardCashResult, InviteNewRecord rewardCouponResult, String customerName) {
        ...
        if (Objects.nonNull(rewardCouponResult)) {
            if(Objects.nonNull(addRegisterRequest.getDenominationSum())){
    ...
    private Object getDeleteIndex(String id) {
        return String.format(EsConstants.DELETE_SPLIT_CHAR, EsConstants.INVITE_NEW_RECORD, id);
    }

    /**
     * @description 查询记录数量
     * @author  xuyunpeng
     * @date 2021/6/16 2:09 下午
     * @param request
     * @return
     */
    public Long count(DistributionInviteNewPageRequest request) {
        return customerDistributionInviteNewRepository.count(QueryConditionsUtil.getWhereCriteria(request));
    }
}
