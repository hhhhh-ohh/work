...
public class StoreController implements StoreProvider {
    ...
    public BaseResponse<StoreSwitchResponse> switchStore(@RequestBody @Valid StoreSwitchRequest storeSwitchRequest) {
        ...
        StoreVO storeVO = KsBeanUtil.convert(store, StoreVO.class);
        storeVO.setCompanyCode(store.getCompanyInfo().getCompanyCode());
        return BaseResponse.success(new StoreSwitchResponse(storeVO));
    }
    ...
    @Override
    public BaseResponse modifyPickupState(@RequestBody @Valid StorePickupStateModifyRequest request) {
        storeService.modifyPickupState(request);
        return BaseResponse.SUCCESSFUL();
    }
    ...
    public BaseResponse<EsStoreInfoResponse> queryEsStoreInfoByPage(@RequestBody @Valid EsStoreInfoQueryRequest request){
        List<EsStoreInfoVo> lists = storeService.queryEsStoreInfoByPage(request);
    ...
    @Override
    public BaseResponse closePickupSetting(@Valid ClosePickupSettingRequest request) {
        if (request.getStoreType() != null && request.getStoreType() == Constants.TWO){
            storeService.closePickupSettingForO2o();
        }else {
            storeService.closePickupSetting(request.getCompanyType());
        }
     ...
     */
    @Override
    public BaseResponse updateContractAuditState(StoreContractAuditRequest request) {
        storeService.updateContractAuditState(request);
        return BaseResponse.SUCCESSFUL();
    }
     ...
     */
    @Override
    public BaseResponse closeStoreForLaKaLa() {
        storeService.closeStoreForLaKaLa();
        return BaseResponse.SUCCESSFUL();
    }
     ...
     */
    @Override
    public BaseResponse openStoreForLaKaLa() {
        storeService.openStoreForLaKaLa();
        return BaseResponse.SUCCESSFUL();
    }
     ...
     */
    @Override
    public BaseResponse updateLaKaLaState(@RequestBody @Valid StoreByCompanyInfoIdRequest storeByCompanyInfoIdRequest) {
        storeService.updateLaKaLaState(storeByCompanyInfoIdRequest.getCompanyInfoId());
        return BaseResponse.SUCCESSFUL();
    }
}
