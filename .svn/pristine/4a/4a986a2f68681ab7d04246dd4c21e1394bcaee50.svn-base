...
@Slf4j
@Service
public class FullReductionExcelServiceImpl implements MarketingExcelService {
    @Autowired
    ...
    public void downloadTemplate(Integer activityType) {
        ...
        this.write(file, "满减活动商品导入模板.xls");
    }

    @Override
    public void downloadErrorFile(String fileExt) {
        String userId = commonUtil.getOperatorId();
        String resourceKey = Constants.FULL_REDUCTION_SALE_GOODS_ERR_EXCEL.concat(userId);
    ...
    public String uploadFile(MultipartFile file) {
        ...
        String userId = commonUtil.getOperatorId();
        String resourceKey = Constants.FULL_REDUCTION_SALE_GOODS_EXCEL.concat(userId);
        try (Workbook workbook = WorkbookFactory.create(new ByteArrayInputStream(file.getBytes()))) {
            ...
            this.checkGoodsInfoNo(skuNoMap,goodsInfoQueryProvider,workbook,isError,null,Boolean.FALSE,
            ...
            if (isError.get()) {
                ...
                resourceKey = Constants.FULL_REDUCTION_SALE_GOODS_ERR_EXCEL.concat(userId);
