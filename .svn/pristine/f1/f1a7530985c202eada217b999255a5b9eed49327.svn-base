...
public class CompanyRequest extends BaseQueryRequest {
     ...
     * 商家编号
     */
    private String equalsCompanyCode;
    ...
    /**
     * 二次签约审核状态 -1全部 ,0:待审核,1:已审核,2:审核未通过
     */
    ...
    public Specification<CompanyInfo> getWhereCriteria() {
        return (root, cquery, cbuild) -> {
            ...
            if (StringUtils.isNotBlank(equalsCompanyCode)) {
                predicates.add(cbuild.equal(root.get("companyCode"), equalsCompanyCode));
            }
            ...
            if (Objects.nonNull(deleteFlag)) {
                ...
                predicates.add(cbuild.equal(companyInfoStoreJoin.get("delFlag"), deleteFlag));
            }
            ...
            if (StringUtils.isNotBlank(applyEnterTimeStart)) {
                DateTimeFormatter formatter = DateTimeFormatter.ofPattern(DateUtil.FMT_DATE_1);
            ...
            if (Objects.nonNull(auditState) && auditState >= 0) {
                Predicate auditPredicate = cbuild.equal(companyInfoStoreJoin.get("auditState"), auditState);
                if (Objects.nonNull(contractAuditState) && contractAuditState >= 0) {
                    Predicate predicate = cbuild.equal(companyInfoStoreJoin.get("contractAuditState"), contractAuditState);
                    predicates.add(cbuild.or(auditPredicate,predicate));
                }else{
