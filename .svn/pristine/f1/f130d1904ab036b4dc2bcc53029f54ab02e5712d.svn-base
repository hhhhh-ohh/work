...
import com.wanmi.sbc.goods.api.response.info.GoodsElectronicIdResponse;
...
//    private BookingSaleService bookingSaleService;
    ...
    public BaseResponse<GoodsPageResponse> page(@RequestBody @Valid GoodsPageRequest request) {
        ...
        Page<GoodsSaveVO> goodsPage = goodsQueryResponse.getGoodsPage();
        ...
        if (Objects.nonNull(goodsPage) && CollectionUtils.isNotEmpty(goodsPage.getContent())) {
            response.setGoodsBrandList(goodsQueryResponse.getGoodsBrandList());
            response.setGoodsCateList(goodsQueryResponse.getGoodsCateList());
            ...
            microServicePage.setContent(goodsMapper.goodsSaveListToGoodsVOList(goodsPage.getContent()));
    ...
    public BaseResponse<GoodsPageByConditionResponse> pageByCondition(@RequestBody @Valid GoodsPageByConditionRequest request){
        ...
        if (Objects.nonNull(goodsPage) && CollectionUtils.isNotEmpty(goodsPage.getContent())) {
            //es评价数实时查询评价表
            //全部重置 0
            goodsPage.getContent().stream().forEach(goods -> {
    ...
    public BaseResponse<GoodsViewByIdResponse> getCacheViewById(@RequestBody @Valid GoodsCacheInfoByIdRequest request) {
        ...
        if (Objects.isNull(goodsInfo) || DeleteFlag.YES.equals(goodsInfo.getDelFlag())) {
            throw new SbcRuntimeException(goodsInfoService.getDeleteIndex(request.getGoodsInfoId()), GoodsErrorCodeEnum.K030035);
        }
        /*if ((!Objects.equals(CheckStatus.CHECKED, goodsInfo.getAuditStatus()))) {
        ...
        if ((!Objects.equals(CheckStatus.CHECKED, goodsInfo.getAuditStatus()))) {
            goodsByIdResponse.setAvailable(NumberUtils.INTEGER_ZERO);
        }
...
//        }
    ...
    public BaseResponse<GoodsDetailProperResponse> getGoodsDetail(@RequestBody @Valid GoodsDetailProperBySkuIdRequest request) {
        return BaseResponse.success(KsBeanUtil.convert(goodsService.findGoodsDetail(request), GoodsDetailProperResponse.class));
    }

    @Override
    public BaseResponse<GoodsDetailProperResponse> getChannelGoodsDetail(@RequestBody @Valid GoodsDetailProperBySkuIdRequest request) {
        String goodsDetail = BaseResUtils.getResultFromRes(channelGoodsQueryProvider.queryDetail(
                                ChannelGoodsDetailRequest.builder()
                                        .thirdPlatformType(ThirdPlatformType.VOP)
                                        .sku(request.getSkuId())
                                        .goodsDetailType(request.getGoodsDetailType())
                                        .build()), detail -> detail.getGoodsDetail());

        return BaseResponse.success(GoodsDetailProperResponse.builder().goodsDetail(goodsDetail).build());
    }
        ...
        if (StringUtils.isNotBlank(goodsDetailInfo)) {
            ...
            if (Objects.nonNull(response)) {
                redisService.setString(RedisKeyConstant.GOODS_DETAIL_CACHE + goodsId,
                        JSONObject.toJSONString(response), 6L * 60L * 60L);
            }
    ...
    public BaseResponse<GoodsViewByIdResponse> getViewById(@RequestBody @Valid GoodsViewByIdRequest request) {
        ...
        GoodsEditResponse goodsEditResponse = goodsService.findInfoById(goodsId ,isEditProviderGoods);
                ...
                if (CollectionUtils.isNotEmpty(providerGoodsInfoList)) {
                    goodsByIdResponse.getGoodsInfos().forEach(goodsInfoVO -> {
                        ...
                        if (Objects.nonNull(providerGoodsInfo)) {
                            ...
                            goodsInfoVO.setProviderQuickOrderNo(providerGoodsInfo.getQuickOrderNo());
    ...
    public BaseResponse<GoodsListByIdsResponse> listByIds(@RequestBody @Valid GoodsListByIdsRequest request) {
        ...
        if(BoolFlag.YES == request.getGetGoodsInfoFlag()){
            goodsService.wrapGoodsInfoListByGoodsIdsAndNoDelFlag(voList);
        }
    ...
    @Override
    public BaseResponse<GoodsListNeedSynCountResponse> countNeedSyn(@RequestBody @Valid GoodsQueryNeedSynRequest request) {
        Long count = goodsService.countNeedSyn(request);
        return BaseResponse.success(GoodsListNeedSynCountResponse.builder().count(count).build());
    }

    @Override
    public BaseResponse<GoodsColsByConditionResponse> listColsByCondition(@Valid @RequestBody GoodColsByConditionRequest request) {
        GoodsQueryRequest queryRequest = KsBeanUtil.convert(request, GoodsQueryRequest.class);
        List<Goods> goodsList = goodsService.listCols(queryRequest, request.getCols());
        if (CollectionUtils.isEmpty(goodsList)) {
            return BaseResponse.success(new GoodsColsByConditionResponse(Collections.emptyList()));
        }
        List<GoodsVO> goodsVOList = KsBeanUtil.convert(goodsList, GoodsVO.class);
        return BaseResponse.success(new GoodsColsByConditionResponse(goodsVOList));
    }

    /**
     * 检查电子卡券是否被商品绑定
     * @param goodsCheckBindRequest
     * @return
     */
    @Override
    public BaseResponse<String> checkBindElectronicCoupons(@Valid @RequestBody GoodsCheckBindRequest goodsCheckBindRequest) {
        goodsService.checkBindElectronicCoupons(goodsCheckBindRequest.getElectronicCouponsId(),goodsCheckBindRequest.getGoodsInfoId());
        return BaseResponse.SUCCESSFUL();
    }

    /**
     * 根据商品类型查询SKU
     * @param goodsByTypeRequest
     * @return
     */
    @Override
    public BaseResponse<GoodsInfoListByGoodsTypeResponse> findByGoodsType(@Valid @RequestBody GoodsByTypeRequest goodsByTypeRequest) {
        Page<GoodsInfo> goodsInfos = goodsService.findByGoodsType(goodsByTypeRequest.getGoodsType(), goodsByTypeRequest.getPageable());
        return BaseResponse.success(GoodsInfoListByGoodsTypeResponse.builder()
                .goodsInfoVOList(KsBeanUtil.convert(goodsInfos.getContent(), GoodsInfoVO.class))
                .build());
    }
     ...
     */
    @Override
    public BaseResponse<GoodsInfoListByIdResponse> findByElectronicCouponsId(@Valid @RequestBody GoodsCheckBindRequest goodsCheckBindRequest) {
        GoodsInfo goodsInfo = goodsService.findByElectronicCouponsId(goodsCheckBindRequest.getElectronicCouponsId());
        return BaseResponse.success(GoodsInfoListByIdResponse.builder()
                .goodsInfoVO(KsBeanUtil.convert(goodsInfo, GoodsInfoVO.class))
                .build());
    }

    @Override
    public BaseResponse<GoodsElectronicIdResponse> findElectronicCouponIds(@Valid @RequestBody GoodsElectronicIdRequest goodsElectronicIdRequest) {
        List<Long> electronicCouponIds = goodsService.findElectronicCouponIds(goodsElectronicIdRequest.getGoodsIds());
        return BaseResponse.success(GoodsElectronicIdResponse.builder().electronicCouponIds(electronicCouponIds).build());
    }

    @Override
    public BaseResponse<GoodsSellResponse> findSellGoods(@Valid GoodsSellRequest goodsSellRequest) {
        List<GoodsVO> goodsVOList = new ArrayList<>();
        List<Goods> goodsList = goodsService.findElectronicCouponIds(goodsSellRequest.getStoreId(), goodsSellRequest.getProviderGoodsId());
        if(CollectionUtils.isNotEmpty(goodsList)) {
            goodsVOList = KsBeanUtil.convertList(goodsList, GoodsVO.class);
        }
        return BaseResponse.success(GoodsSellResponse.builder().goodsList(goodsVOList).build());
    }

    @Override
    public BaseResponse<GoodsImageResponse> findGoodsImageByGoodsId(@Valid GoodsImageRequest goodsImageRequest) {
        List<GoodsImageVO>  goodsImageVOList = goodsImageService.findByGoodsId(goodsImageRequest.getGoodsId());
        return BaseResponse.success(GoodsImageResponse.builder().goodsImageVOList(goodsImageVOList).build());
    }

    private List<GoodsInfoVO> providerStockSync(List<GoodsInfoVO> goodsInfoVOList){
        goodsInfoService.fillSupplyPriceAndStock(goodsInfoVOList);
        return goodsInfoVOList;
    }

    @Override
    public BaseResponse<List<String>> findGoodsNameByIds (@Valid List<String> joinGoodsIdList) {
        return BaseResponse.success(goodsService.findGoodsNameByIds(joinGoodsIdList));
    }

    @Override
    public BaseResponse<GoodsMainImageResponse> findGoodsMainImageByGoodsId(List<String> goodsIdList) {
        List<GoodsMainImageVO> goodsMainImageVOList = goodsService.findGoodsMainImageByGoodsId(goodsIdList);
        return BaseResponse.success(GoodsMainImageResponse.builder().goodsMainImageVOList(goodsMainImageVOList).build());
    }
}
