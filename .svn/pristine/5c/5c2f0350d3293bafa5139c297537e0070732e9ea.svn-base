...
import com.wanmi.sbc.bargain.vo.DecodeByTokenVO;
 ...
 * Time: 2017/01/09
 */
@Tag(name = "CheckAuthController", description = "检查是否有权限")
@RestController
@Validated
public class CheckAuthController {
     ...
     */
    @Operation(summary = "验证用户是否有某一个url/requestType的接口权限，true: 拥有该权限, false: 没有权限")
    ...
    public BaseResponse<Boolean> checkFunctionAuth(@RequestBody AuthorityRequest authorityRequest){
        ...
        if(employee != null) {
            ...
            ids = authBaseService.filterRoleId(ids);
    ...
    @Operation(summary = "鉴权用户信息")
    @PostMapping(value = "/decode-by-token")
    public BaseResponse<DecodeByTokenVO> decodeByToken(){
        var vo = new DecodeByTokenVO();
        String customerName = commonUtil.getAccountName();
        String customerAccount = commonUtil.getAccount();
        LocalDateTime expireTime = commonUtil.getExpireTime();
        vo.setCustomerName(customerName);
        vo.setCustomerAccount(customerAccount);
     ...
     */
    @Operation(summary = "判断登录用户是否具备这些功能，拥有权限的功能")
    @Parameter(name = "functions",
            description = "判断当前登录用户是否具有传入的这些功能权限", required = true)
    ...
    public ResponseEntity<List<String>> findTodoFunctionIds(@RequestBody List<String> functions) {
        List<String> todoFunctionIds = authBaseService.findTodoFunctionIds(functions);
        return ResponseEntity.ok(todoFunctionIds);
    }
