...
import com.wanmi.sbc.order.returnorder.service.ProviderReturnOrderService;
...
public class ReturnOrderController implements ReturnOrderProvider {
    ...
    @Autowired
    private ProviderReturnOrderService providerReturnOrderService;
    ...
    public BaseResponse onlineModifyPrice(@RequestBody @Valid ReturnOrderOnlineModifyPriceRequest request) {
        returnOrderService.onlineEditPrice(KsBeanUtil.convert(request.getReturnOrder(), ReturnOrder.class),
                ...
                request.getOperator(),request.getCustomerAccountId(),request.getCustomerAccount(),request.getFee());
        return BaseResponse.SUCCESSFUL();
    }
    ...
    public BaseResponse offlineRefundForSupplier(@RequestBody @Valid ReturnOrderOfflineRefundForSupplierRequest
        ...
        returnOrderService.supplierRefundOffline(request.getRid(), customerAccount, KsBeanUtil.convert(request
                .getRefundBill(), RefundBill.class), request.getOperator(),request.getFee());
        return BaseResponse.SUCCESSFUL();
    }
    ...
    public BaseResponse cancel(@RequestBody @Valid ReturnOrderCancelRequest request) {
        thirdPlatformReturnOrderService.cancel(request.getRid());
    ...
    @Override
    public BaseResponse<String> refundOnlineByTid(@RequestBody @Valid ReturnOrderOnlineRefundByTidRequest request){
        List<Object> result = returnOrderService.refundOnlineByTid(
                request.getReturnOrderCode(),
                request.getOperator(),
                request.getOfflineAccountId()
        );
        if (CollectionUtils.isEmpty(result)){
            return BaseResponse.success(null);
        }
        return BaseResponse.success(JSON.toJSONString(result.get(0), JSONWriter.Feature.ReferenceDetection));
    }
     ...
     */
    @Override
    public BaseResponse remedySellerRemark(@RequestBody @Valid ReturnOrderRemedySellerRemarkRequest request) {
        returnOrderService.remedySellerRemark(request.getRid(),
                request.getSellerRemark(),
                request.getOperator());
        return BaseResponse.SUCCESSFUL();
    }
     ...
     */
    @Override
    public BaseResponse checkBatchAuditReturnOrder(ReturnOrderCheckRequest request) {
        returnOrderService.checkBatchAuditReturnOrder(request.getRids());
        return BaseResponse.SUCCESSFUL();
    }

    @Override
    public BaseResponse autoReturnByProviderOrderId(@RequestBody @Valid ReturnOrderAutoReturnByProviderTradeIdRequest request) {
        providerReturnOrderService.autoReturnByProviderOrderId(request.getProviderOrderId(), request.getDescription());
        return BaseResponse.SUCCESSFUL();
    }

    @Override
    public BaseResponse sendFastRefundMessage(@RequestBody @Valid ReturnOrderOnlineRefundByTidRequest request) {
        returnOrderService.sendFastRefundMessage(request);
        return BaseResponse.SUCCESSFUL();
    }

    @Override
    public BaseResponse doFastRefund(@RequestBody @Valid ReturnOrderOnlineRefundByTidRequest request) {
        returnOrderService.doFastRefund(request);
        return BaseResponse.SUCCESSFUL();
    }
