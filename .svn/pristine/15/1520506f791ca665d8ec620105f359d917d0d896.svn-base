...
public class DistributorLevelService {
	...
	public DistributorLevelVO getByCustomerId(String customerId) {
		...
		DistributorLevelVO distributorLevelVO = KsBeanUtil.copyPropertiesThird(distributorLevel, DistributorLevelVO.class);
		distributorLevelVO.setForbiddenFlag(distributionCustomer.getForbiddenFlag());
		return distributorLevelVO;
	}
	...
	public void validDistributorLevels(List<DistributorLevel> distributorLevels) {
		...
		Integer inviteThreshold = Constants.ZERO;

		// 分销员等级数量不能超过5个
		if (distributorLevels.size() > Constants.FIVE) {
			throw new SbcRuntimeException(CustomerErrorCodeEnum.K010033);
		}
		...
		for(int i = 0; i < distributorLevels.size(); i++) {
			...
			if (Objects.nonNull(item.getCommissionRate())
					&& item.getCommissionRate().compareTo(commissionRateMax) > Constants.ZERO) {
				commissionRateMax = item.getCommissionRate();
			}

			// 计算佣金提成比例最大值
			if (Objects.nonNull(item.getPercentageRate())
					&& item.getPercentageRate().compareTo(percentageRateMax) > Constants.ZERO) {
				percentageRateMax = item.getPercentageRate();
			}

			// 下级销售额门槛要比上级的大
			if (DefaultFlag.YES.equals(item.getSalesFlag())) {
				if (item.getSalesThreshold().compareTo(salesThreshold) > Constants.ZERO) {
					salesThreshold = item.getSalesThreshold();
				} else {
					throw new SbcRuntimeException(CustomerErrorCodeEnum.K010034);
				}
			}

			// 下级当前到账收益额门槛要比上级的大
			if (DefaultFlag.YES.equals(item.getRecordFlag())) {
				if (item.getRecordThreshold().compareTo(recordThreshold) > Constants.ZERO) {
			...
			if (DefaultFlag.YES.equals(item.getInviteFlag())) {
				if (item.getInviteThreshold().compareTo(inviteThreshold) > Constants.ZERO) {
			...
			// 必须勾选一个升级规则
			if (item.getSort() != Constants.ONE
					&& DefaultFlag.NO.equals(item.getSalesFlag())
					&& DefaultFlag.NO.equals(item.getRecordFlag())
					&& DefaultFlag.NO.equals(item.getInviteFlag())) {
				throw new SbcRuntimeException(CustomerErrorCodeEnum.K010036);
			}

		}

		// 存在佣金比例+佣金提成比例相加大于100%的情况
		if (commissionRateMax.add(percentageRateMax).compareTo(new BigDecimal(Constants.ONE)) > Constants.ZERO) {
			throw new SbcRuntimeException(CustomerErrorCodeEnum.K010035);
		}
	}
	...
	public int delete(String distributorLevelId) {
		// 1.判断当前等级有没有关联分销员
		distributorLevelRepository.findById(distributorLevelId).
		...
		if (CollectionUtils.isNotEmpty(customers)) {
			// 等级关联了分销员，不能删除
			throw new SbcRuntimeException(CustomerErrorCodeEnum.K010146);
		}
