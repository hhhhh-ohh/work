...
public class GrouponInstanceQueryRequest extends BaseQueryRequest {
    ...
    private List<Criteria> getCommonCriteria() {
        ...
        if (Objects.nonNull(endTime)){
            criterias.add(Criteria.where("endTime").lt(endTime));
        }

        // 距离拼团结束还剩3小时,是否发送订阅消息
        if (Objects.nonNull(grouponExpiredSendFlag)) {
            criterias.add(Criteria.where("grouponExpiredSendFlag").is(grouponExpiredSendFlag));
        }
        return criterias;
    }
}
