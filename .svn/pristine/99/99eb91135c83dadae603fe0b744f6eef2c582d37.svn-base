 ...
 * @Description: TODO
 */
@Tag(name = "SsoServerController", description = "魔方建站单点登录服务 Api")
@RestController
@Validated
public class SsoServerController {
   ...
   /* @Autowired
     ...
     * @return
     */
    @Operation(summary = "x-site项目访问url接口时,都要先携带token访问此接口验证是否返回正确的结果,返回结果会存在x-site项目中",
            description = "userId: 用户Id, shopId: 店铺Id, shopName: 店铺名称, " +
...
//        BaseConfigRopResponse res = response.getSuccessResponse();
     ...
     * @return
     */
    @Operation(summary = "用于x-site建站项目获取当前建站的店铺信息",
            description = "data: (scene: 商城名称, systemCode: 系统编号, storeLogo: 店铺logo, storeName: 店铺名称, " +
...
//        BaseConfigRopResponse res = response.getSuccessResponse();
     ...
     * @return
     */
    @Operation(summary = "x-site页面发布pc端模板时调用此接口,获取发布之后的页面url相关信息,用于立即查看",
            description = "data: (adminId: 管理员Id, domainName: 网站名称, domainUrl: 网站地址, faviconUrl: logo, " +
     ...
     * @return
     */
    @Operation(summary = "x-site页面发布weixin端页面时调用此接口,获取发布之后的页面url相关信息,用于分享",
            description = "data: (adminId: 管理员Id, domainUrl: 网站地址, faviconUrl: logo, " +
