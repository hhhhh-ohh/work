...
/**
 * @author zhanggaolei
 * @type UnionB2BPayService.java
 * @desc
 * @date 2022/11/17 15:47
 */
@Slf4j
@PayPluginService(type = PayChannelType.UNION_B2B)
public class UnionB2BPayService extends ExternalPayService<UnionPayRequest> {
    ...
    public BaseResponse getResult(PayChannelRequest request) {
        ...
        RLock rLock = redissonClient.getFairLock(request.getId());
        rLock.lock();
        try {
            baseResponse = super.getResult(request);
        } catch (Exception e) {
            throw e;
        } finally {
            rLock.unlock();
    ...
    protected UnionPayRequest buildRequest(PayBaseBean bean, PayChannelRequest request) {
                ...
                paySettingQueryProvider
                        .getGatewayConfigByGateway(
                                new GatewayConfigByGatewayRequest(
                                        PayGatewayEnum.UNIONB2B, Constants.BOSS_DEFAULT_STORE_ID))
                        .getContext();
        // 4.组装消费接口数据
        log.info("payUnionB2B组装银联云闪付支付数据开始");
        ...
        // 前端回调地址 \(^o^)/~ h5  自己传值，
        // TODO 原来是后端写死，现在统一要求从前端进行传递，需要区分授信还款的回调和正常支付订单的回调页面的区别
        buildRequest.setFrontUrl(request.getSuccessUrl());
        // 后台回调地址
        buildRequest.setNotifyUrl(
                payGatewayConfigResponse.getBossBackUrl() + "/tradeCallback/UnionB2BCallBack");
        // 来源
    ...
    protected BasePayRequest buildPayRequest(UnionPayRequest t,PayBaseBean bean) {
        return BasePayRequest.builder().payType(PayType.UNIONB2BPAY).unionPayRequest(t).tradeId(bean.getId()).build();
    }
