...
public class CustomerDrawCashWhereCriteriaBuilder {
    public static Specification<CustomerDrawCash> build(CustomerDrawCashQueryRequest queryRequest) {
        return (root, cquery, cbuild) -> {
            List<Predicate> predicates = new ArrayList<>(25);
            // 批量查询-提现idList
            ...
            if (Boolean.FALSE.equals(queryRequest.getSourceFromPlatForm()) && AuditStatus.WAIT == queryRequest.getAuditStatus()) {
            ...
            // 批量查询-业务员关联的会员IDList
            if (CollectionUtils.isNotEmpty(queryRequest.getEmployeeCustomerIds())) {
                predicates.add(root.get("customerId").in(queryRequest.getEmployeeCustomerIds()));
            }
