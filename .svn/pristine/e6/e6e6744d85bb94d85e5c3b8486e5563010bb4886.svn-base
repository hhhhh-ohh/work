...
public class StoreQueryController implements StoreQueryProvider {
    ...
    public BaseResponse<ListStoreByNameResponse> listByName(@RequestBody @Valid ListStoreByNameRequest
        ...
        List<Store> storeList = storeService.queryStoreByName(listStoreByNameRequest.getStoreName(),listStoreByNameRequest.getStoreType());
    ...
    @Override
    public BaseResponse<ListStoreByNameResponse> listBySupplierName(@RequestBody @Valid ListStoreBySupplierNameRequest listStoreBySupplierNameRequest) {
        List<Store> storeList = storeService.queryStoreBySupplierName(listStoreBySupplierNameRequest.getSupplierName(),listStoreBySupplierNameRequest.getStoreType());
    ...
    @Override
    public BaseResponse<StoreListForDistributionResponse> listBySupplierName(@RequestBody @Valid ListStoreByNameRequest listStoreByNameRequest) {
        List<StoreSimpleInfo> storeSimpleInfos = new ArrayList<>();
        List<Store> storeList = storeService.queryStoreBySuuplierName(listStoreByNameRequest.getSupplierName());
    ...
    public BaseResponse<ListStoreResponse> listStore(@RequestBody @Valid ListStoreRequest listStoreRequest) {
        ...
        if(request.getStoreType() == StoreType.O2O){
            storeList = storeList.stream().filter(store -> !store.getCompanyInfo().getCompanyCode().equals("O0000001"))
    ...
    private StoreBaseResponse wraperBaseInfoResponseFromStore(Store store) {
        ...
        if (Objects.nonNull(store.getPickupState())){
            response.setPickupState(store.getPickupState().toValue());
        }
    ...
    @Override
    public BaseResponse<CacheNoDeleteStoreByIdResponse> getCacheNoDeleteStoreById(@Valid NoDeleteStoreByIdRequest noDeleteStoreByIdRequest) {
        return BaseResponse.success(CacheNoDeleteStoreByIdResponse.builder().storeVO(this.storeService.cacheFindOne(noDeleteStoreByIdRequest.getStoreId())).build());
    }

    @Override
    public BaseResponse<Map<Long, String>> storeNameDropdownBox(@RequestBody @Valid ListStoreByNameRequest listStoreByNameRequest) {

        return BaseResponse.success(storeService.storeNameDropdownBox(listStoreByNameRequest.getStoreName(),listStoreByNameRequest.getStoreType()));
    }

    @Override
    public BaseResponse<Long> count(@RequestBody @Valid StoreQueryRequest request) {
        Long count = storeService.countStoreNum(request);
