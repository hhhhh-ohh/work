...
public class GoodsBrandQueryRequest extends BaseQueryRequest {
     ...
     * and 模糊查询，品牌和昵称
     */
    private String likeNameAndNick;
    ...
    public Specification<GoodsBrand> getWhereCriteria(){
        return (root, cquery, cbuild) -> {
            ...
            //模糊查询名称和别名
            if(StringUtils.isNotEmpty(likeNameAndNick)){
                Predicate namePredicate = cbuild.like(root.get("brandName"), StringUtil.SQL_LIKE_CHAR.concat(XssUtils.replaceLikeWildcard(likeNameAndNick).trim()).concat(StringUtil.SQL_LIKE_CHAR));
                Predicate nickPredicate = cbuild.like(root.get("nickName"), StringUtil.SQL_LIKE_CHAR.concat(XssUtils.replaceLikeWildcard(likeNameAndNick).trim()).concat(StringUtil.SQL_LIKE_CHAR));
                Predicate nameOrNick = cbuild.or(namePredicate,nickPredicate);
                predicates.add(nameOrNick);
            }
            //查询昵称
            ...
            if(StringUtils.isNotBlank(keywords)){
                ...
                if(t_keywords.length > 0) {
                    List<Predicate> keywordPredicates = new ArrayList<>(10);
