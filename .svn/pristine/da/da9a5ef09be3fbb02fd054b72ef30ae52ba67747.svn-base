...
public class EsDistributionCustomerService {
     ...
     * @param request
     */
    public void init(EsDistributionCustomerInitRequest request) {
        boolean flg = true;
        ...
        try {
            while (flg) {
                if(CollectionUtils.isNotEmpty(request.getIdList())){
                    queryRequest.setDistributionIdList(request.getIdList());
                    pageNum = 0;
    ...
    private Iterable<EsDistributionCustomer> saveAll(List<EsDistributionCustomer> newInfos) {
        //手动删除索引时，重新设置mapping
        esBaseService.existsOrCreate(EsConstants.DISTRIBUTION_CUSTOMER, mapping);
    ...
    public void modify(String customerId, String customerName) {
        EsDistributionCustomer esDistributionCustomer = esDistributionCustomerRepository.findByCustomerId(customerId);
        if (Objects.nonNull(esDistributionCustomer)) {
            esDistributionCustomer.setCustomerName(customerName);
            saveAll(Collections.singletonList(esDistributionCustomer));
        }
    }
}
