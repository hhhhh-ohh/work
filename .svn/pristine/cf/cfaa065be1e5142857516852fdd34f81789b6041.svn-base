 ...
 */
@Service("GiftCardBatchService")
@Slf4j
public class GiftCardBatchService {
    ...
    public void batchMakeCreate(GiftCardBatchMakeCreateRequest request) {
        // 1. 校验并获取礼品卡信息(库存、过期)
        GiftCard giftCard = this.checkAndGetGiftCard(request);

        // 2.1 库存有限制时，扣减库存
		if (DefaultFlag.NO == giftCard.getStockType()) {
			int rows = giftCardRepository.subStock(giftCard.getGiftCardId(), request.getCreateNum());
			if (rows < 1) {
				// 库存不足
		...
		// 2.2 追加已制卡数量
		giftCardRepository.subMakeNum(giftCard.getGiftCardId(), -request.getCreateNum());

        // 3. 获取制卡审核配置
        boolean auditFlag = this.getAuditFlag(ConfigType.GIFT_CARD_MAKE_CARD_AUDIT);

        // 4. 构造制卡批次实体
        GiftCardBatch giftCardBatch = this.generateBasicGiftCardBatch(request, auditFlag);
        giftCardBatch.setBatchType(GiftCardBatchType.MAKE);
        giftCardBatch.setExchangeMode(GiftCardExchangeMode.SECRET_KEY);

        // 5. 无需审核时，生成并保存礼品卡详情列表，为批次赋值起始和结束卡号
        if (!auditFlag) {
            // 5.1 生成并保存卡详情列表，为批次赋值起始和结束卡号
			this.batchSaveGiftDetails4Make(request, giftCard, giftCardBatch);
        }
	...
	@Transactional(rollbackFor = Exception.class)
	public void batchSendCreate(GiftCardBatchSendCreateRequest request) {
		// 1. 校验并获取礼品卡信息(库存、过期)
		GiftCard giftCard = this.checkAndGetGiftCard(request);
		...
		// 2.2 追加已发卡数量
		giftCardRepository.subSendNum(giftCard.getGiftCardId(), -request.getCreateNum());
		...
		// 4. 构造发卡批次实体
		GiftCardBatch giftCardBatch = this.generateBasicGiftCardBatch(request, auditFlag);
		giftCardBatch.setExcelFilePath(request.getExcelFilePath());
		giftCardBatch.setBatchType(GiftCardBatchType.SEND);

		// 5.1 生成礼品卡详情列表
		List<GiftCardDetail> giftCardDetails = this.generateGiftDetails4Send(request, giftCard, giftCardBatch, auditFlag);
		// 5.2 为批次赋值起始和结束卡号
		...
		this.batchSaveCardDetailsAndCustomerCard4Send(auditStatus, giftCard, giftCardDetails, Collections.emptySet());
		// 5.4 分批保存卡详情
	 ...
	 * @param request 审核请求
	 * @param giftCardBatch 发卡批次
	 * @param giftCard 礼品卡
	 * @return void
	 */
	@Transactional(rollbackFor = Exception.class)
	public void batchSendAudit(GiftCardBatchAuditRequest request, GiftCardBatch giftCardBatch, GiftCard giftCard) {
		if (GiftCardBatchType.SEND != giftCardBatch.getBatchType()) {
			return;
		}
		AuditStatus auditStatus = request.getAuditStatus();
		Long giftCardId = giftCard.getGiftCardId();
		Long batchNum = giftCardBatch.getBatchNum();
		String batchNo = giftCardBatch.getBatchNo();

		if (AuditStatus.CHECKED == auditStatus) {
			// 1. 发卡，审核通过
			// 1.1 查询卡详情列表
			List<GiftCardDetail> giftCardDetails = giftCardDetailRepository.findByBatchNoAndDelFlag(batchNo, DeleteFlag.NO);
			// 1.2 筛选出其中不可用的会员id集合
			Set<String> notAvailableCustomerIds = this.getNotAvailableCustomerIds(giftCardDetails);
			// 1.3 更新卡详情列表，新增用户卡和交易记录
			this.batchSaveCardDetailsAndCustomerCard4Send(AuditStatus.CHECKED, giftCard, giftCardDetails, notAvailableCustomerIds);
			giftCardDetailRepository.saveAll(giftCardDetails);
		} else {
			// 2. 发卡，审核不通过
			// 2.1 还库存
			giftCardRepository.subStock(giftCardId, -batchNum);
			// 2.2 扣已发数量
			giftCardRepository.subSendNum(giftCardId, batchNum);
			// 2.3 删除卡详情
			giftCardDetailRepository.deleteByBatchNo(batchNo);
		}

		// 3. 批次更新
		giftCardBatch.setAuditStatus(auditStatus);
        giftCardBatch.setAuditReason(request.getAuditReason());
		giftCardBatch.setUpdatePerson(request.getUserId());
		giftCardBatch.setUpdateTime(LocalDateTime.now());
		giftCardBatchRepository.save(giftCardBatch);
	}
	 ...
	 * @date 2022/12/13 12:44
	 * @param request 审核请求
	 * @param giftCardBatch 制卡批次
	 * @param giftCard 礼品卡
	 * @return void
	 */
	@Transactional(rollbackFor = Exception.class)
	public void batchMakeAudit(GiftCardBatchAuditRequest request, GiftCardBatch giftCardBatch, GiftCard giftCard) {
		...
		if (AuditStatus.CHECKED == auditStatus) {
			// 1. 制卡，审核通过
			// 1.1 构造并保存卡详请求
			GiftCardBatchMakeCreateRequest createRequest = new GiftCardBatchMakeCreateRequest();
			createRequest.setGiftCardId(giftCardId);
			createRequest.setCreateNum(batchNum);
			createRequest.setCreatePerson(giftCardBatch.getCreatePerson());
			// 1.2 生成并保存，礼品卡详情列表，为批次赋值起始和结束卡号
			this.batchSaveGiftDetails4Make(createRequest, giftCard, giftCardBatch);
		} else {
			// 2. 制卡，不通过
			// 2.1 还库存
			giftCardRepository.subStock(giftCardId, -batchNum);
			// 2.2 扣已制数量
			giftCardRepository.subMakeNum(giftCardId, batchNum);
		}
	...
	/**
	 * @description 为制卡，生成礼品卡详情列表，为批次赋值起始和结束卡号
	 * @author malianfeng
	 ...
	 */
	private void batchSaveGiftDetails4Make(GiftCardBatchMakeCreateRequest request,
		...
		for (long i = 0; i < request.getCreateNum(); i++) {
			GiftCardDetail giftCardDetail = this.generateBasicGiftCardDetail(request, giftCard, giftCardBatch);
			// 生成兑换码
			giftCardDetail.setExchangeCode(CodeGenUtil.toSerialCode(RandomUtils.nextInt(1, 10000)).toUpperCase());
			// 填充来源类型为：制卡
			giftCardDetail.setSourceType(GiftCardSourceType.MAKE);
			// 填充卡状态为：未兑换
			giftCardDetail.setCardDetailStatus(GiftCardDetailStatus.NOT_EXCHANGE);
			giftCardDetail.setGiftCardType(giftCard.getGiftCardType());
			giftCardDetailList.add(giftCardDetail);
		}
		// 为批次赋值起始和结束卡号
		this.fillStartAndEndCardNo(giftCardBatch, giftCardDetailList);
		// 保存卡详情列表
		List<List<GiftCardDetail>> splitList = IterableUtils.splitList(giftCardDetailList, BATCH_INSERT_MAX_SIZE);
		for (List<GiftCardDetail> itemList : splitList) {
			giftCardDetailService.batchInsert(itemList);
		}
	}
		...
		giftCardDetails.stream()
				...
				.forEach(item -> {
					if (AuditStatus.WAIT_CHECK == auditStatus) {
						...
						// 构造用户礼品卡
						userGiftCardList.add(this.generateUserGiftCard(giftCard, item));
					}
				});
	 ...
	 **/
	public UserGiftCard generateUserGiftCard(GiftCard giftCard, GiftCardDetail giftCardDetail) {
	...
	/**
	 * @description 生成卡交易记录实体
	 * @author malianfeng
	 * @date 2022/12/10 14:32
	 * @param giftCard 礼品卡信息
	 * @param userGiftCard 用户卡信息
	 * @param operatorId 操作人id
	 * @return com.wanmi.sbc.marketing.giftcard.model.root.GiftCardBill
	 */
	public GiftCardBill generateGiftCardBill(GiftCard giftCard, UserGiftCard userGiftCard, String operatorId) {
		GiftCardBill giftCardBill = new GiftCardBill();
		giftCardBill.setUserGiftCardId(userGiftCard.getUserGiftCardId());
		giftCardBill.setGiftCardId(giftCard.getGiftCardId());
		...
		giftCardBill.setTradeBalance(BigDecimal.valueOf(giftCard.getParValue()));
		giftCardBill.setAfterBalance(BigDecimal.valueOf(giftCard.getParValue()));
		giftCardBill.setTradeTime(LocalDateTime.now());
		giftCardBill.setTradePersonType(DefaultFlag.YES);
		giftCardBill.setTradePerson(operatorId);
		return giftCardBill;
	}
	...
	/**
	 * @description 获取不可用客户ids
	 * @author malianfeng
	 * @date 2022/12/19 19:15
	 * @param giftCardDetails 卡详情列表
	 * @return java.util.Set<java.lang.String>
	 */
	private Set<String> getNotAvailableCustomerIds(List<GiftCardDetail> giftCardDetails) {
		if (CollectionUtils.isEmpty(giftCardDetails)) {
			return Collections.emptySet();
		}
		Set<String> existCustomerAccountSet = new HashSet<>();
		Set<String> notAvailableCustomerIds = new HashSet<>();
		List<String> customerIds = giftCardDetails.stream().map(GiftCardDetail::getBelongPerson).distinct().collect(Collectors.toList());
		// 构造客户详情列表请求
		EsCustomerDetailPageRequest customerPageRequest = new EsCustomerDetailPageRequest();
		customerPageRequest.setPageNum(0);
		customerPageRequest.setPageSize(customerIds.size());
		customerPageRequest.setCustomerIds(customerIds);
		...
		if (CollectionUtils.isNotEmpty(esCustomerList)) {
			for (EsCustomerDetailVO detail : esCustomerList) {
				...
				if (CustomerStatus.DISABLE == detail.getCustomerStatus()) {
					// 1. 客户已禁用
					notAvailableCustomerIds.add(customerId);
				} else if (!Long.valueOf(LogOutStatus.NORMAL.toValue()).equals(detail.getLogOutStatus())) {
					// 2. 客户已注销
					notAvailableCustomerIds.add(customerId);
				}
				// 记录已存在的客户账号集合，用于判断客户是否删除
				existCustomerAccountSet.add(customerId);
			}
		}
		// 3. 客户已删除
		customerIds.stream()
				.filter(item -> !existCustomerAccountSet.contains(item))
				.forEach(notAvailableCustomerIds::add);
		return notAvailableCustomerIds;
	}
	 ...
	 */
	@Transactional
	public GiftCardBatch add(GiftCardBatch entity) {
		giftCardBatchRepository.save(entity);
		return entity;
	}
	 ...
	 */
	@Transactional
	public GiftCardBatch modify(GiftCardBatch entity) {
		giftCardBatchRepository.save(entity);
		return entity;
	}
	 ...
	 */
	@Transactional
	public void deleteById(GiftCardBatch entity) {
		giftCardBatchRepository.save(entity);
	}
	...
	public void deleteByIdList(List<Long> ids) {
		giftCardBatchRepository.deleteByIdList(ids);
	}
	 ...
	 * @author 马连峰
	 */
	public GiftCardBatch getOne(Long id) {
		return giftCardBatchRepository.findByGiftCardBatchIdAndDelFlag(id, DeleteFlag.NO).orElse(null);
	}
	 ...
	 * @author 马连峰
	 */
	public Page<GiftCardBatch> page(GiftCardBatchQueryRequest queryReq){
		return giftCardBatchRepository.findAll(
				GiftCardBatchWhereCriteriaBuilder.build(queryReq),
	 ...
	 * @author 马连峰
	 */
	public List<GiftCardBatch> list(GiftCardBatchQueryRequest queryReq){
		return giftCardBatchRepository.findAll(GiftCardBatchWhereCriteriaBuilder.build(queryReq));
	}
	 ...
	 * @author 马连峰
	 */
	public GiftCardBatchVO wrapperVo(GiftCardBatch giftCardBatch) {
		if (giftCardBatch != null){
			GiftCardBatchVO giftCardBatchVO = KsBeanUtil.convert(giftCardBatch, GiftCardBatchVO.class);
			return giftCardBatchVO;
		}
		return null;
	}
	 ...
	 * @author 马连峰
	 */
	public Long count(GiftCardBatchQueryRequest queryReq) {
		return giftCardBatchRepository.count(GiftCardBatchWhereCriteriaBuilder.build(queryReq));
	}

	@Transactional(rollbackFor = Exception.class)
    public void sendPickUpCardToAccount(GiftCardGainRequest request) {
		log.info("买会员送校服提货卡发卡开始:"+request.toString());
		String season = "";
        if (request.getGiftCardType() == 1) {
            season = "夏装";
        } else if (request.getGiftCardType() == 2) {
            season = "秋装";
        } else if (request.getGiftCardType() == 3) {
            season = "冬装";
        }
		UniformsGiftCardBinding uniformsGiftCardBinding = uniformsGiftCardBindingRepository.findByCityAndSeason("空",season);
		GiftCard giftCard = giftCardService.checkAndGetForBatchSend(uniformsGiftCardBinding.getGiftCardId());
		// 校验并获取礼品卡信息
//		GiftCard giftCard = giftCardService.checkAndGetForBatchSendByName(giftCardInfoRequest.getName());

		try{
			// 客户账号和id映射map
			Map<String, String> customerAccountAndIdMap = new HashMap<>();
			// 校验客户账号，返回错误信息Map
			Map<String, String> customerAccountErrMap = this.checkCustomerAccountOne(request.getCustomerAccount(), customerAccountAndIdMap);
			...
			newSendCustomerDTO.setCustomerId(customerAccountAndIdMap.get(request.getCustomerAccount()));
			newSendCustomerDTO.setSendNum(Long.valueOf(1));
			...
			createRequest.setCreatePerson("-1");
			...
			try {
				// 执行发卡
				this.batchSendCreate(createRequest);
			}
			finally {
				rLock.unlock();
			}
