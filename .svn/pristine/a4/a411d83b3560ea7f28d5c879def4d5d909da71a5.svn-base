...
public class PointsGoodsCateService {
    ...
    public PointsGoodsCate add(PointsGoodsCate entity) {
        ...
        if (pointsGoodsCates.size() >= Constants.NUM_30) {
            throw new SbcRuntimeException(GoodsErrorCodeEnum.K030085);
        }
    ...
    public void editSort(PointsGoodsCateSortRequest request) {
        ...
        for (int i = 0; i < cateIdList.size(); i++) {
            Optional<PointsGoodsCate> cateOptional = pointsGoodsCateRepository.findById(cateIdList.get(i));
            if (cateOptional.isPresent()) {
                PointsGoodsCate pointsGoodsCate = cateOptional.get();
    ...
    public PointsGoodsCate getById(Integer id) {
        return pointsGoodsCateRepository.findById(id).orElse(null);
    }
