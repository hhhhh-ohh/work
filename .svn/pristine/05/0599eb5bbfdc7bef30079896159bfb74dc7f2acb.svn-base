...
/**
 * @author lvzhenwei
 * @className ModifyAccountFundsCustomerNameService
 * @description 会员名称修改,触发账户模块-会员资金的会员名称修改
 * @date 2021/8/13 3:46 下午
 */
@Slf4j
@Service
public class ModifyAccountFundsCustomerNameService {

    @Autowired private AccountMqConsumerProvider accountMqConsumerProvider;

    @Bean
    public Consumer<Message<String>> mqModifyAccountFundsCustomerNameService() {
        return message->{
            ...
            accountMqConsumerProvider.modifyAccountFundsCustomerName(accountMqConsumerRequest);
        };
    }
}
