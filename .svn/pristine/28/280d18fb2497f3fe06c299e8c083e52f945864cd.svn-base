...
public class MessageSendWhereCriteriaBuilder {
    public static Specification<MessageSend> build(MessageSendQueryRequest queryRequest) {
        return (root, cquery, cbuild) -> {
            ...
            //send message查询要发送的任务数据
            if (Objects.nonNull(queryRequest.getIsAppMessageSendFlag()) && queryRequest.getIsAppMessageSendFlag()) {
                // 发送状态(0:未开始,1:进行中,2:已结束，3：任务失败)
                predicates.add(cbuild.equal(root.get("status"), SendStatus.NO_BEGIN));
            }
