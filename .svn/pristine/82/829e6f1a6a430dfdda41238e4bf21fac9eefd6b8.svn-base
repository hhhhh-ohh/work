...
import com.wanmi.sbc.common.validation.HTMLEscapeUtil;
import com.wanmi.sbc.common.validation.ValidationException;
import com.wanmi.sbc.common.validation.ValidationPattern;
...
public class XssHttpServletRequestWrapper extends HttpServletRequestWrapper {
    ...
    public XssHttpServletRequestWrapper(HttpServletRequest servletRequest, String excludeFieldsName, Integer pns, Integer pvs) {
        super(servletRequest);
        if (StringUtils.isNotBlank(excludeFieldsName)) {
            this.excludeFields = StringUtils.split(excludeFieldsName, ',');
        }
        if (null != pns) {
        ...
        try {
            if (servletRequest.getContentType() != null
                    && servletRequest.getContentType().contains("multipart/form-data")) {

            }else {
                requestBody = StreamUtils.copyToByteArray(servletRequest.getInputStream());
            }
        } catch (IOException e) {
            logger.error("缓存请求body error ", e);
        }
    }
        ...
        for (Object o : map.entrySet()) {
            try {
                ...
                for (int j = 0; j < value.length; j++) {
                    ...
                    cleanValues[j] = StringEscapeUtils.unescapeHtml4(cleanValue);
                }
    ...
    public String[] getParameterValues(String name) {
        ...
        for (String value : values) {
            try {
                ...
                newValues.add(StringEscapeUtils.unescapeHtml4(cleanValue));
            } catch (ValidationException e) {
     ...
     * @return
     */
    private <T> Enumeration listToEnumeration(List<T> list) {
