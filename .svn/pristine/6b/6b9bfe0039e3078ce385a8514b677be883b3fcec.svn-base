...
@Slf4j
@Service
public class DiscountExcelServiceImpl implements MarketingExcelService {
    @Autowired
    ...
    public void downloadTemplate(Integer activityType) {
        ...
        this.write(file, "第二件半价商品导入模板.xls");
    }

    @Override
    public void downloadErrorFile(String fileExt) {
        String userId = commonUtil.getOperatorId();
        String resourceKey = Constants.DISCOUNT_GOODS_ERR_EXCEL.concat(userId);
    ...
    public String uploadFile(MultipartFile file) {
        ...
        String userId = commonUtil.getOperatorId();
        String resourceKey = Constants.DISCOUNT_GOODS_EXCEL.concat(userId);
        try (Workbook workbook = WorkbookFactory.create(new ByteArrayInputStream(file.getBytes()))) {
            ...
            if (isError.get()) {
                ...
                resourceKey = Constants.DISCOUNT_GOODS_ERR_EXCEL.concat(userId);
