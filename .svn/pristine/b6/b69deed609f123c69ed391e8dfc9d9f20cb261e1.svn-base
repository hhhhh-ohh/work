...
public class RefundReturnAction extends ReturnAction {
    ...
    private void delEvaluate(ReturnOrder returnOrder) {
        ...
        returnOrder.getReturnItems().forEach(returnItem -> {
            ...
            Integer num = returnItem.getNum();
            if (returnOrder.getReturnTag() != null && Boolean.TRUE.equals(returnOrder.getReturnTag().getBuyCycleFlag())) {
                num = returnItem.getNum() * returnItem.getBuyCycleNum();
            }
            if (num.equals(returnItem.getCanReturnNum())) {
