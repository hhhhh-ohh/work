...
public class CustomerLevelQueryController implements CustomerLevelQueryProvider {
    ...
    public BaseResponse<CustomerLevelWithRightsByCustomerIdResponse> getCustomerLevelRightsInfos(@RequestBody @Valid
        ...
        Optional<CustomerLevel> customerLevelOptional = customerLevelService.findById(customerLevelId);
        ...
        if (customerLevelOptional.isPresent()) {
            wrapperVo(customerLevelOptional.get(), response);
        }
        response.setCustomerGrowthValue(customer.getGrowthValue());
        response.setCustomerName(customerDetail.getCustomerName());
        response.setHeadImg(customer.getHeadImg());
        response.setMembershipExpiredTime(customer.getMembershipExpiredTime());
...
//        }
    ...
    /**
     * 提供给营销模块获取会员等级的接口, 包含校验会员信息是否存在
     *
     * @param request 会员Id和店铺Id参数 {@link CustomerLevelForMarketingRequest}
     *
     */
    @Override
    public BaseResponse validLevelForMarketing(@Valid CustomerLevelForMarketingRequest request) {
        customerLevelService.validLevelByCustomerForMarketing(request.getCustomerId(), request.getStoreId(),
                request.getJoinLevel());
        return BaseResponse.SUCCESSFUL();
    }


    @Override
    public BaseResponse<CustomerLevelMapGetResponse> listCustomerLevelMapByCustomerIdAndIdsByDefault(@RequestBody @Valid
                                                                                                    CustomerLevelMapByCustomerIdAndStoreIdsRequest request) {
        Map<Long, CommonLevel> commonLevelMap =
                customerLevelService.findLevelByCustomerAndDefault(request.getCustomerId(), request.getStoreIds());
        HashMap<Long, CommonLevelVO> customerLevelVOMap = new HashMap<>();
    ...
    @Override
    public BaseResponse<CustomerLevelMapGetResponse> listCustomerLevelMapByCustomerId(@RequestBody @Valid CustomerLevelMapByCustomerIdAndStoreIdsRequest request) {
        Map<Long, CommonLevel> commonLevelMap = customerLevelService.findPlatFormLevelByCustomerId(request.getCustomerId());
        //填充
        HashMap<Long, CommonLevelVO> customerLevelVOMap = new HashMap<>();
        if (MapUtils.isNotEmpty(commonLevelMap)) {
            commonLevelMap.forEach((storeId, commonLevel) -> {
