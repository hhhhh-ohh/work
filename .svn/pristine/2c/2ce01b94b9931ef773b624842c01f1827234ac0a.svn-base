...
public class SmsTemplateService {
    ...
    public SmsTemplate add(SmsTemplate entity) {
        ...
        if (smsBaseResponse.getCode().equals(SmsResponseCode.SUCCESS)) {
            ...
            entity.setSmsSettingId(smsBaseResponse.getSmsSettingId());
            smsTemplateRepository.save(entity);
        } else {
            throw new SbcRuntimeException(MessageErrorCodeEnum.K090010, new Object[]{"新增失败，调用短信平台报错：" + smsBaseResponse.getMessage()});
        }
        return entity;
    }
    ...
    public void upload(SmsTemplate entity) {
        ...
        if (smsBaseResponse.getCode().equals(SmsResponseCode.SUCCESS)) {
            ...
            throw new SbcRuntimeException(MessageErrorCodeEnum.K090010, new Object[]{"提交备案失败，调用短信平台报错：" + smsBaseResponse.getMessage()});
        }
    }
    ...
    public SmsTemplate modify(SmsTemplate entity) {
        ...
        if(Objects.isNull(smsTemplateInfo)){
            throw new SbcRuntimeException(MessageErrorCodeEnum.K090012);
        }
        //如果是待提交备案
        if(ReviewStatus.WAITSUBMIT == smsTemplateInfo.getReviewStatus()){
        ...
        if (smsBaseResponse.getCode().equals(SmsResponseCode.SUCCESS)) {
            ...
            throw new SbcRuntimeException(MessageErrorCodeEnum.K090010, new Object[]{"编辑失败，调用短信平台报错：" + smsBaseResponse.getMessage()});
        }
        return entity;
    }
    ...
    public void modifySmsTemplateReviewStatusByTemplateCode(SmsTemplate entity) {
        ...
        if (smsBaseResponse.getCode().equals(SmsResponseCode.SUCCESS)) {
            ReviewStatus reviewStatus = ReviewStatus.fromValue(Integer.parseInt(smsBaseResponse.getTemplateStatus()));
    ...
    public void deleteById(Long id) {
        ...
        if (Objects.isNull(smsTemplate) || smsTemplate.getDelFlag() == DeleteFlag.YES) {
            throw new SbcRuntimeException(MessageErrorCodeEnum.K090010, new Object[]{"删除失败，该模板删除"});
        }
        // 待提交状态可以直接删除
        ...
        if (num > 0) {
            throw new SbcRuntimeException(MessageErrorCodeEnum.K090010, new Object[]{"删除失败，该模板已被使用，无法删除"});
        }
        SmsBaseResponse smsBaseResponse = smsProxy.deleteSmsTemplate(smsTemplate);
        if (smsBaseResponse.getCode().equals(SmsResponseCode.SUCCESS)) {
            smsTemplateRepository.deleteByBeanId(id);
        } else {
            throw new SbcRuntimeException(MessageErrorCodeEnum.K090010, new Object[]{"删除失败，调用短信平台报错：" + smsBaseResponse.getMessage()});
        }
    }
        ...
        for (SmsTemplate smsTemplate : smsTemplateList) {
            if (Objects.nonNull(smsTemplate.getTemplateCode())) {
                ...
                if (smsBaseResponse.getCode().equals(SmsResponseCode.SMS_TEMPLATE_ILLEGAL)) {
                    ...
                    smsTemplate.setTemplateType(SmsType.fromValue(Integer.parseInt(smsBaseResponse.getTemplateType())));
                    ReviewStatus reviewStatus = ReviewStatus.fromValue(Integer.parseInt(smsBaseResponse.getTemplateStatus()));
                    smsTemplate.setReviewStatus(reviewStatus);
                    if (reviewStatus == ReviewStatus.REVIEWFAILED) {
                        smsTemplate.setReviewReason(smsBaseResponse.getReason());
                    }
                    if(smsTemplate.getTemplateType().equals(SmsType.VERIFICATIONCODE)){
                        smsTemplate.setBusinessType("VERIFICATION_CODE");
                    }
                    smsTemplateRepository.save(smsTemplate);
                }
            }
        ...
        for (String templateCode : request.getTemplateCodeList()) {
            ...
            if (Objects.isNull(smsTemplate)) {
                ...
                if (smsBaseResponse.getCode().equals(SmsResponseCode.SUCCESS)) {
                    ...
                    smsTemplateInfo.setSmsSettingId(smsBaseResponse.getSmsSettingId());
                    smsTemplateInfo.setTemplateType(SmsType.fromValue(Integer.parseInt(smsBaseResponse.getTemplateType())));
                    ReviewStatus reviewStatus = ReviewStatus.fromValue(Integer.parseInt(smsBaseResponse.getTemplateStatus()));
                    smsTemplateInfo.setReviewStatus(reviewStatus);
                    if (reviewStatus == ReviewStatus.REVIEWFAILED) {
                        smsTemplateInfo.setReviewReason(smsBaseResponse.getReason());
                    }
                    if(smsTemplateInfo.getTemplateType().equals(SmsType.VERIFICATIONCODE)){
                        smsTemplateInfo.setBusinessType("VERIFICATION_CODE");
                    }
    ...
    public Long getNoticeSmsCount() {
        int count = smsTemplateRepository.countAllByTemplateTypeAndDelFlag(SmsType.NOTICE, DeleteFlag.NO);
