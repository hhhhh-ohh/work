package com.wanmi.ares.marketing.buyoutprice.service;
...
@Service
@Slf4j
public class BuyOutPriceService implements ExportBaseService {
    ...
    public BaseResponse exportReport(ExportQuery exportQuery) throws Exception {
        ...
        request.setMarketingType(MarketingType.BUYOUT_PRICE);
    ...
    protected void uploadFileOfFullMoneyBookingSale(List<MarketingReduceDiscountGiftReport> records, List<String> locations, ExportQuery query)
        ...
        if (CollectionUtils.isNotEmpty(query.getMarketingIds())) {
            fileName = String.format("marketing/buyoutprice/%s/%s/%s打包一口价活动效果统计_%s_%s-%s.xls",
                    query.getCompanyId(),
