package com.wanmi.sbc.vas.api.response.recommend.goodscorrelationmodelsetting;

import com.wanmi.sbc.common.base.BasicResponse;
import com.wanmi.sbc.common.base.MicroServicePage;
import com.wanmi.sbc.vas.bean.vo.recommend.GoodsCorrelationModelSettingVO;

import io.swagger.v3.oas.annotations.media.Schema;
import lombok.AllArgsConstructor;
import lombok.Builder;
import lombok.Data;
import lombok.NoArgsConstructor;

import java.io.Serializable;

/**
 * <p>分页结果</p>
 * @author zhongjichuan
 * @date 2020-11-27 11:27:06
 */
@Schema
@Data
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class GoodsCorrelationModelSettingPageResponse extends BasicResponse {
    private static final long serialVersionUID = 1L;

    /**
     * 分页结果
     */
    @Schema(description = "分页结果")
    private MicroServicePage<GoodsCorrelationModelSettingVO> goodsCorrelationModelSettingVOPage;
}
