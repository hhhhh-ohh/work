...
public class DistributionCustomerInviteInfoService {
    ...
    public void revise(DistributorCustomerInviteInfoReviseRequest request) {
        ...
        //发放邀新奖金人数
        Long rewardCashCount =
                inviteNewRecords.stream().filter(vo->InvalidFlag.YES == vo.getRewardRecorded()).filter(vo -> RewardFlag.CASH == vo.getRewardFlag()).count();
        //未放邀新奖金人数
        Long rewardCashLimitCount =
                inviteNewRecords.stream().filter(vo->InvalidFlag.NO == vo.getRewardRecorded()).filter(vo->FailReasonFlag.LIMITED == vo.getFailReasonFlag()).filter(vo -> RewardFlag.CASH == vo.getRewardFlag()).count();
        //未放邀新奖金人数--有效邀新有几个
        Long rewardCashAvailableLimitCount =
                inviteNewRecords.stream().filter(vo->InvalidFlag.NO == vo.getRewardRecorded()).filter(vo->FailReasonFlag.LIMITED == vo.getFailReasonFlag()).filter(vo -> RewardFlag.CASH == vo.getRewardFlag() && InvalidFlag.YES == vo.getAvailableDistribution()).count();
         //发放邀新优惠券人数
        Long rewardCouponCount =
                inviteNewRecords.stream().filter(vo->InvalidFlag.YES == vo.getRewardRecorded()).filter(vo -> RewardFlag.COUPON == vo.getRewardFlag()).count();
        //未发放邀新优惠券人数
        Long rewardCouponLimitCount =
                inviteNewRecords.stream().filter(vo->InvalidFlag.NO == vo.getRewardRecorded()).filter(vo->FailReasonFlag.LIMITED == vo.getFailReasonFlag()).filter(vo -> RewardFlag.COUPON == vo.getRewardFlag()).count();
        //未发放邀新优惠券人数--有效邀新有几个
        Long rewardCouponAvailableLimitCount =
                inviteNewRecords.stream().filter(vo->InvalidFlag.NO == vo.getRewardRecorded()).filter(vo->FailReasonFlag.LIMITED == vo.getFailReasonFlag()).filter(vo -> RewardFlag.COUPON == vo.getRewardFlag() && InvalidFlag.YES == vo.getAvailableDistribution()).count();
