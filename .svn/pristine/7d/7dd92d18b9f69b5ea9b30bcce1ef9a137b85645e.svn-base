...
//    private CouponCacheService couponCacheService;
    ...
    public void deleteCouponCateByCouponCateId(CouponCateDeleteRequest request) {
        ...
        couponScopeCacheService.update(EsActivityCouponModifyRequest.builder()
                .couponCateId(request.getCouponCateId()).delete(true).build());
        //刷新优惠券缓存
//        couponCacheService.updateCateRelaCache(request.getCouponCateId(), true);
    ...
    public void addCouponCate(CouponCateAddRequest request) {
        ...
        if (CollectionUtils.isNotEmpty(cates)) {
            ...
            if (cates.size() >= Constant.MAX_CATE_COUNT) {
                throw new SbcRuntimeException(MarketingErrorCodeEnum.K080191);
            }
            // 优惠券名称不能已经存在
            if (cates.stream().anyMatch(cate ->
                    StringUtils.equals(request.getCouponCateName(), cate.getCouponCateName()))) {
                throw new SbcRuntimeException(MarketingErrorCodeEnum.K080044);
            }
        }
    ...
    public List<String> isOnlyPlatform(CouponCateIsOnlyPlatformRequest request) {
        if (StringUtils.isBlank(request.getCouponCateId())) {
            throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
        }
        CouponCate cate = couponCateRepository.getCouponCateByCouponCateId(request.getCouponCateId());
        ...
        List<CouponCateRela> couponCateRelaList = couponCateRelaRepository.findByCateIdAndPlatformFlag(request.getCouponCateId(), DefaultFlag.NO);
        List<String> couponIds = new ArrayList<>();
        if (CollectionUtils.isNotEmpty(couponCateRelaList)){
            couponIds = couponCateRelaList.stream().map(CouponCateRela::getCouponId).collect(Collectors.toList());
            couponCateRelaRepository.deleteByCateIdAndPlatformFlag(request.getCouponCateId(),DefaultFlag.NO);
        }

        //刷新es缓存
        couponScopeCacheService.update(EsActivityCouponModifyRequest.builder()
                .couponCateId(request.getCouponCateId()).delete(false).build());
        //刷新优惠券缓存
