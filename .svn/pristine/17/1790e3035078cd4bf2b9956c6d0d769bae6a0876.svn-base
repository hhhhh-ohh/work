...
@Slf4j
@Service
public class RecommendExcelServiceImpl implements PayingMemberExcelService {
    @Autowired
    ...
    public void downloadTemplate(Integer activityType) {
        ...
        this.write(file, "付费会员推荐商品导入模板.xls");
    }

    @Override
    public void downloadErrorFile(String fileExt) {
        String userId = commonUtil.getOperatorId();
        String resourceKey = Constants.PAYING_MEMBER_RECOMMEND_ERROR_EXCEL.concat(userId);
    ...
    public String uploadFile(MultipartFile file, PayingMemberTemplateRequest payingMemberTemplateRequest) {
        ...
        String userId = commonUtil.getOperatorId();
        String resourceKey = Constants.PAYING_MEMBER_RECOMMEND_EXCEL.concat(userId);
        try (Workbook workbook = WorkbookFactory.create(new ByteArrayInputStream(file.getBytes()))) {
            ...
            if (isError.get()) {
                ...
                resourceKey = Constants.PAYING_MEMBER_RECOMMEND_ERROR_EXCEL.concat(userId);
    ...
    public List getGoodsInfoList() {
        ...
        try (Workbook workbook = WorkbookFactory.create(new ByteArrayInputStream(content))) {
            ...
            return goodsInfoList.parallelStream().map(goodsInfoVO -> {
                PayingMemberRecommendRelVO payingMemberRecommendRelVO = new PayingMemberRecommendRelVO();
