...
public class HttpClient {
	...
	private String response(final HttpURLConnection connection, String encoding)
		...
		InputStream in = null;
		StringBuilder sb = new StringBuilder(Constants.NUM_1024);
		try {
			if (Constants.NUM_200 == connection.getResponseCode()) {
		...
		if (null != requestParam && 0 != requestParam.size()) {
			for (Entry<String, String> en : requestParam.entrySet()) {
				try {
					sb.append(en.getKey());
					sb.append('=');
					sb.append(null == en.getValue() || "".equals(en.getValue()) ? "" : URLEncoder.encode(en.getValue(), coder));
					sb.append('&');
