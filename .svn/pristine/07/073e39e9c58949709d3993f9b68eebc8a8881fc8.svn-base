...
public interface ThirdGoodsCateProvider {
	...
	/**
	 * 全量更新某渠道所有类目（每次同步仅针对未同步的分类做新增，已同步的分类不做删除）
	 * @return
	 */
	@PostMapping("/goods/${application.goods.version}/thirdgoodscate/updateAllNoDel")
	BaseResponse updateAllNoDel(@RequestBody UpdateAllRequest request);
	 ...
	 * @return
	 */
	@PostMapping("/goods/${application.goods.version}/thirdgoodscate/checkCateIdExist")
	BaseResponse<CheckCateIdResponse> checkCateIdExist(@RequestBody CheckCateIdRequest request);
	 ...
	 * @return
	 */
	@PostMapping("/goods/${application.goods.version}/thirdgoodscate/addAll")
	BaseResponse addAll(@RequestBody UpdateAllRequest request);
}
