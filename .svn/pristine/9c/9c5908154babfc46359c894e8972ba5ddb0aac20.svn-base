...
@Tag(name =  "短信模板管理API", description =  "SmsTemplateController")
@RestController
@Validated
...
public class SmsTemplateController {
    ...
    @Operation(summary = "查询是否可以新增通知类模板")
    @GetMapping("/btn/disabled")
    public BaseResponse<Boolean> getCreateBtnDisabled() {
        Long count = smsTemplateQueryProvider.getNoticeSmsCount().getContext();
        if(Objects.nonNull(count) && count > 0L) {
            int noticeSmsCount = count.intValue();
            int noticeTemplateCount = getAllSmsPurpose().size();
            if(noticeSmsCount>=noticeTemplateCount) {
    ...
    public BaseResponse<SmsTemplateAddResponse> add(@RequestBody @Valid SmsTemplateAddRequest addReq) {
        if (addReq.getTemplateType() == SmsType.VERIFICATIONCODE && StringUtils.isEmpty(addReq.getBusinessType())) {
            addReq.setBusinessType("VERIFICATION_CODE");
        }
        //填充用途
        ...
        if (addReq.getTemplateType() == SmsType.NOTICE) {
            // 仅通知类的短信模板，需验证用途
    ...
    public BaseResponse<SmsTemplateModifyResponse> modify(@RequestBody @Valid SmsTemplateModifyRequest modifyReq) {
        if (modifyReq.getTemplateType() == SmsType.VERIFICATIONCODE && StringUtils.isEmpty(modifyReq.getBusinessType())) {
            modifyReq.setBusinessType("VERIFICATION_CODE");
        }
        //填充用途
        ...
        if (modifyReq.getTemplateType() == SmsType.NOTICE) {
            // 仅通知类的短信模板，需验证用途
    ...
    @Operation(summary = "根据idList批量删除短信模板")
    ...
    @Operation(summary = "同步短信平台短信模板数据")
    ...
    public void exportData(@PathVariable String encrypted, HttpServletResponse response) {
        ...
        try {
            ...
            String fileName = URLEncoder.encode(String.format("短信模板列表_%s.xls", nowStr), StandardCharsets.UTF_8.name());
    ...
    private void checkTemplateName(SmsTemplateListRequest smsTemplateListReq) {
        ...
        if(CollectionUtils.isNotEmpty(smsTemplateVOList)){
            if (Objects.nonNull(smsTemplateListReq.getId()) && !smsTemplateVOList.get(0).getId().equals(smsTemplateListReq.getId())) {
                throw new SbcRuntimeException(MessageErrorCodeEnum.K090009);
    ...
    private void checkTemplateCode(SmsTemplateListRequest smsTemplateListReq) {
        if(StringUtils.isNotBlank(smsTemplateListReq.getBusinessType())) {
            ...
            if(CollectionUtils.isNotEmpty(smsTemplateVOList)){
                if (Objects.nonNull(smsTemplateListReq.getId()) && !smsTemplateVOList.get(0).getId().equals(smsTemplateListReq.getId())) {
                    throw new SbcRuntimeException(MessageErrorCodeEnum.K090011);
                }else if(Objects.isNull(smsTemplateListReq.getId())){
                    throw new SbcRuntimeException(MessageErrorCodeEnum.K090011);
                }
            }
    ...
    private List<SmsPurposeVO> listPurpose() {
        ...
        SmsTemplateListResponse listResponse =
                smsTemplateQueryProvider.list(SmsTemplateListRequest.builder()
                .delFlag(DeleteFlag.NO)
                .templateType(SmsType.VERIFICATIONCODE)
                .build()).getContext();
        return Arrays.stream(SmsTemplate.values())
                .filter(t -> !(t.equals(SmsTemplate.CUSTOMER_IMPORT_SUCCESS)
                        || t.equals(SmsTemplate.CUSTOMER_PASSWORD)
                        || t.equals(SmsTemplate.EMPLOYEE_PASSWORD))).map(t -> {
            SmsPurposeVO vo = new SmsPurposeVO();
            vo.setBusinessType(t.name());
            vo.setPurpose(fieldMap.get(t.name()));
            vo.setDisabled(Boolean.FALSE);
            if(CollectionUtils.isNotEmpty(listResponse.getSmsTemplateVOList())
                    && listResponse.getSmsTemplateVOList().stream()
                    .filter(s -> StringUtils.isNotBlank(s.getBusinessType())
                            && s.getBusinessType().equals(t.name())).findFirst().isPresent()){
    ...
    private List<SmsPurposeVO> listNode() {
        List<SmsPurposeVO> voList = getAllSmsPurpose();
        SmsTemplateListResponse listResponse =
                smsTemplateQueryProvider.list(SmsTemplateListRequest.builder()
                        .delFlag(DeleteFlag.NO)
                        .templateType(SmsType.NOTICE)
                        .build()).getContext();
        if(CollectionUtils.isNotEmpty(voList)){
            voList = voList.stream().map(t -> {
                t.setDisabled(Boolean.FALSE);
                if(CollectionUtils.isNotEmpty(listResponse.getSmsTemplateVOList())
                        && listResponse.getSmsTemplateVOList().stream()
                        .filter(s -> StringUtils.isNotBlank(s.getBusinessType())
                                && s.getBusinessType().equals(t.getBusinessType()))
                        .findFirst().isPresent()){
                    t.setDisabled(Boolean.TRUE);
                }
                return t;
            }).collect(Collectors.toList());
        }
    ...
    /**
     * @description 获取全部通知类节点
     * @author  songhanlin
     * @date: 2021/5/19 下午8:25
     * @return 通知类节点
     **/
    public List<SmsPurposeVO> getAllSmsPurpose() {
        commonUtil.getAllServices();
        boolean vopStatus = false;
        List<VASEntity> vops =
                commonUtil.getAllServices().stream().filter(c ->
                        c.isServiceStatus()
                                && (c.getServiceName().equals(VASConstants.THIRD_PLATFORM_VOP.toValue())
                                || c.getServiceName().equals(VASConstants.THIRD_PLATFORM_LINKED_MALL.toValue()))
                ).collect(Collectors.toList());
        if(CollectionUtils.isNotEmpty(vops)){
            vopStatus = true;
        }
        //账号安全
        ...
        //账户资产
        boolean finalVopStatus = vopStatus;
        voList.addAll(Arrays.stream(AccoutAssetsType.values()).filter(orderProcessType ->
                !finalVopStatus || !orderProcessType.equals(OrderProcessType.THIRD_PAY_ERROR_AUTO_REFUND)
        ).map(t -> {
            SmsPurposeVO vo = new SmsPurposeVO();
            vo.setBusinessType(t.getType());
            vo.setPurpose(t.getDescription());
