...
@Component
@Slf4j
public class StoreExpireHandler {
    ...
    @XxlJob(value = "StoreExpireHandler")
    public void execute() throws Exception {
        Integer total = NumberUtils.INTEGER_ZERO;
        XxlJobHelper.log("供应商店铺过期定时任务执行开始 " + LocalDateTime.now());
        ...
        if (listStoreResponse != null) {
            ...
            if (CollectionUtils.isNotEmpty(expiredStores)) {
                ...
                XxlJobHelper.log("处理店铺：" + storeIds);
            }
            total = expiredStores.size();

        }
        XxlJobHelper.log("供应商店铺过期定时任务执行结束 " + LocalDateTime.now() + ",处理总数为：" + total);
    }
}
