...
public class DistributionSettingService {
     ...
     */
    @Transactional
    public DistributionSettingGetResponse querySetting(Boolean isBoss) {
    ...
    public void saveOpenFlag(DefaultFlag openFlag) {
        ...
        distributionCacheService.saveSetting(this.querySetting(Boolean.FALSE));
    }
    ...
    private void appendConfigsToActivity(List<DistributionRewardCouponDTO> chooseCoupons) {
        ...
        if(Objects.isNull(activity)){
            ...
            newActivity.setPlatformFlag(DefaultFlag.YES);
            newActivity.setPauseFlag(DefaultFlag.NO);
            newActivity.setDelFlag(DeleteFlag.NO);
            newActivity.setPluginType(PluginType.NORMAL);
        ...
        List<String> couponIds = couponActivityConfigRepository.findByActivityId(activity.getActivityId())
                .stream().map(CouponActivityConfig::getCouponId).collect(Collectors.toList());

        List<CouponActivityConfig> couponActivityConfigs;
        // 2.过滤出活动还未关联的优惠券
        List<String> finalCouponIds = Objects.nonNull(couponIds) ? couponIds : Collections.emptyList();
    ...
    public DistributionSetting initSetting() {
        ...
        setting.setOpenFlag(DefaultFlag.YES);
     ...
     * @return
     */
    public Boolean isStoreBag(String goodsInfoId){
        Boolean isStoreBagFlag = false;
        // 1.查询分销设置
        DistributionSetting setting = settingRepository.findAll().get(0);
        if(setting.getOpenFlag().equals(DefaultFlag.YES)&&setting.getApplyType().equals(RecruitApplyType.BUY)){

            List<DistributionRecruitGift> list = recruitGiftRepository.findByGoodsInfoId(goodsInfoId);
            if(CollectionUtils.isNotEmpty(list)){
                isStoreBagFlag = true;
            }
        }
        return isStoreBagFlag;
    }
