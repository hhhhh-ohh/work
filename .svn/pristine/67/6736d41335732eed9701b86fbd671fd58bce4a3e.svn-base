    ...
    public Specification<Goods> getWhereCriteria() {
        return (root, cquery, cbuild) -> {
                ...
                predicates.add(
                        cbuild.like(
                                ...
                                StringUtil.SQL_LIKE_CHAR
                                        .concat(XssUtils.replaceLikeWildcard(likeGoodsNo.trim()))
            ...
            if (StringUtils.isNotEmpty(likeGoodsName)) {
                predicates.add(
                        cbuild.like(
                                root.get("goodsName"),
                                StringUtil.SQL_LIKE_CHAR
                                        .concat(XssUtils.replaceLikeWildcard(likeGoodsName.trim()))
                                        .concat(StringUtil.SQL_LIKE_CHAR)));
            }
            // 模糊查询商家名称
            ...
            if (StringUtils.isNotBlank(likeProviderName)) {
                predicates.add(
                        cbuild.like(
                                root.get("providerName"),
                                StringUtil.SQL_LIKE_CHAR
                                        .concat(
                                                XssUtils.replaceLikeWildcard(
                                                        likeProviderName.trim()))
                                        .concat(StringUtil.SQL_LIKE_CHAR)));
            }
            // 批量供应商商品编号
            ...
            if (StringUtils.isNotBlank(thirdPlatformSkuId)) {
                predicates.add(cbuild.equal(root.get("thirdPlatformSkuId"), thirdPlatformSkuId));
            }
            // 大于或等于 搜索条件:创建结束时间开始
            ...
            if (StringUtils.isNotBlank(keyword)) {
                String str =
                        StringUtil.SQL_LIKE_CHAR
                                .concat(XssUtils.replaceLikeWildcard(keyword.trim()))
                                .concat(StringUtil.SQL_LIKE_CHAR);
                predicates.add(
                        cbuild.or(
                                cbuild.like(root.get("goodsName"), str),
                                cbuild.like(root.get("goodsNo"), str)));
            }
            ...
            // 上下架状态
            if (addedFlag != null) {
                predicates.add(cbuild.equal(root.get("addedFlag"), AddedFlag.fromValue(addedFlag)));
            }

            // 多个上下架状态
            if (CollectionUtils.isNotEmpty(addedFlags)) {
                List<AddedFlag> addedFlagList = new ArrayList<>();
                addedFlags.forEach(a -> addedFlagList.add(AddedFlag.fromValue(a)));
                predicates.add(root.get("addedFlag").in(addedFlagList));
            }
            ...
            if (CollectionUtils.isNotEmpty(notGoodsIds)) {
                predicates.add(cbuild.not(root.get("goodsId").in(notGoodsIds)));
            }
            ...
            if (notThirdPlatformType != null && !notThirdPlatformType.isEmpty()) {
                ...
                predicates.add(
                        cbuild.or(
                                cbuild.not(root.get("thirdPlatformType").in(thirdPlatformTypes)),
                                root.get("thirdPlatformType").isNull()));
            }

            // 跨境商品标识
            if (Objects.nonNull(notShowCrossFlag) && notShowCrossFlag) {
                predicates.add(
                        cbuild.or(
                                cbuild.notEqual(root.get("pluginType"), PluginType.CROSS_BORDER),
                                root.get("pluginType").isNull()));
            }

            if (Objects.nonNull(isShowO2oFlag) && !isShowO2oFlag) {
                predicates.add(cbuild.notEqual(root.get("pluginType"), PluginType.O2O));
            }

            if (Objects.nonNull(onlyShowO2oFlag) && onlyShowO2oFlag) {
                predicates.add(cbuild.equal(root.get("pluginType"), PluginType.O2O));
            }

            // 商品状态筛选
            if (CollectionUtils.isNotEmpty(goodsSelectStatuses)) {
                List<Predicate> orPredicate = new ArrayList<>();
                goodsSelectStatuses.forEach(
                        goodsInfoSelectStatus -> {
            ...
            if (CollectionUtils.isNotEmpty(providerIds)) {
                predicates.add(root.get("providerId").in(providerIds));
            }
