...
@Service
@Slf4j
public class  VopGoodsExportService implements ExportBaseService {
    ...
    public BaseResponse exportReport(ExportData data) throws Exception {
        ...
        String fileName = String.format("批量导出京东VOP商品_%s.xls"
        ...
        Column[] colunms = this.getColunms(data.getPlatform() == Platform.SUPPLIER,
                isBoss);
        SXSSFSheet sheet = excelHelper.addSXSSFSheetHead("京东vop商品导出", colunms);
        //如果没有数据，直接生成空Excel
        ...
        for (int i = 0; i < fileSize; i++) {
            ...
            EsStandardPageResponse response = esStandardQueryProvider.page(esStandardPageRequest).getContext();
            List<EsStandardGoodsPageVO> esStandardGoodsPageVO = response.getStandardGoodsPage().getContent();
            List<GoodsBrandSimpleVO> goodsBrandList = response.getGoodsBrandList();
            Map<Long, String> goodsBrandNameMap = goodsBrandList.stream().collect(Collectors.toMap(GoodsBrandSimpleVO::getBrandId, GoodsBrandSimpleVO::getBrandName));
                ...
                if (CollectionUtils.isNotEmpty(esStandardGoodsPageVO)) {
                    ...
                    for (int n = 0; n < splitList.size(); n++) {
                        ...
                        FutureTask<List<GoodsExportVo>> futureTask = new FutureTask<>(
                                () -> this.getExportData(goodsBrandNameMap,goodsList, contractCateTask.get())
                        );
    ...
    public List<GoodsExportVo>  getExportData(Map<Long, String> goodsBrandNameMap,List<EsStandardGoodsPageVO> goodsList, List<GoodsCateVO> goodsCateVOS) {
            ...
            goodses.forEach(standardGoodsVO -> {
                ...
                if (io.seata.common.util.CollectionUtils.isNotEmpty(goodsInfoVOS)) {
                    goodsInfoVOS.forEach(goodsInfoVO -> {
                        Long brandId = standardGoodsVO.getBrandId();
                        ...
                        if(Objects.nonNull(brandId)){
                            goodsExportVo.setBrandName(goodsBrandNameMap.get(brandId));
                        }else{
                            goodsExportVo.setBrandName("-");
                        }
                        goodsExportVo.setImportState(standardGoodsVO.getImportState());

                        goodsExportVo.setProviderName("京东VOP");
