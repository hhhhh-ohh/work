...
public class GoodsIntervalPriceService {
     ...
     * @return 区间价结果
     */
    public List<GoodsIntervalPriceVO> putIntervalPrice(List<GoodsInfoSaveVO> goodsInfoList,
        ...
        List<GoodsIntervalPriceVO> goodsIntervalPrices = KsBeanUtil.convertList(intervalPrices, GoodsIntervalPriceVO.class);
        ...
        goodsInfoList.forEach(goodsInfo -> {
            ...
            //填充扁平化ids和范围值
            List<BigDecimal> intervalPriceList = goodsIntervalPrices.stream().filter(intervalPrice -> goodsInfo.getGoodsInfoId().equals(intervalPrice.getGoodsInfoId())).map(GoodsIntervalPriceVO::getPrice).collect(Collectors.toList());
            goodsInfo.setIntervalPriceIds(goodsIntervalPrices.stream().filter(intervalPrice -> goodsInfo.getGoodsInfoId().equals(intervalPrice.getGoodsInfoId())).map(GoodsIntervalPriceVO::getIntervalPriceId).collect(Collectors.toList()));
     ...
     * @return
     */
    public List<GoodsIntervalPriceVO> putGoodsIntervalPrice(List<GoodsSaveVO> goodsList, Map<Long, CommonLevelVO> levelMap){
    ...
    public BaseResponse<GoodsIntervalPriceByCustomerIdResponse> putByCustomerId(GoodsIntervalPriceByCustomerIdRequest request) {
        ...
        List<Long> storeIds = goodsInfoList.stream()
                .filter(goodsInfo -> Integer.valueOf(GoodsPriceType.STOCK.toValue()).equals(goodsInfo.getPriceType()))
                .map(GoodsInfoSaveVO::getStoreId)
                .filter(id -> id != null && id > 0)
                .distinct().collect(Collectors.toList());
        Map<Long, CommonLevelVO> levelMap = this.getLevelMap(request.getCustomerId(), storeIds);
        List<GoodsIntervalPriceVO> goodsIntervalPriceList =
                this.putIntervalPrice(goodsInfoList, levelMap);
