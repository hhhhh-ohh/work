...
public class OrderInvoiceController implements OrderInvoiceProvider {
    ...
    public BaseResponse<OrderInvoiceGenerateResponse> generateOrderInvoice(@RequestBody @Valid OrderInvoiceGenerateRequest request) {
        OrderInvoice orderInvoice = orderInvoiceService.generateOrderInvoice(KsBeanUtil.convert(request.getOrderInvoiceDTO(),
                OrderInvoiceSaveRequest.class), request.getEmployeeId(), request.getInvoiceState()).orElse(null);
        if (Objects.isNull(orderInvoice)){
            return BaseResponse.success(OrderInvoiceGenerateResponse.builder().build());
        }
        return BaseResponse.success(OrderInvoiceGenerateResponse.builder()
                .orderInvoiceVO(KsBeanUtil.convert(orderInvoice,OrderInvoiceVO.class ))
                .build());
    }
    ...
    public BaseResponse<OrderInvoiceModifyResponse> modify(@RequestBody @Valid OrderInvoiceModifyRequest modifyRequest) {
        OrderInvoice orderInvoice = orderInvoiceService.updateOrderInvoice(KsBeanUtil.convert(modifyRequest,
                OrderInvoiceSaveRequest.class)).orElse(null);
        if (Objects.isNull(orderInvoice)){
            return BaseResponse.success(OrderInvoiceModifyResponse.builder().build());
        }
