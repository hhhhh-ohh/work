...
public class GrouponOrderService {
    ...
    @Value("${groupon.test.enable:false}")
    private boolean grouponTestEnable = false;
    ...
    public Trade handleGrouponOrderPaySuccess(Trade trade) {
        ...
        if (leader) {
            if (checkStatus != GrouponOrderCheckStatus.PASS) {
                log.info("拼团活动未开始/已过期/不存在，具体失败原因信息：{}", checkStatus);
                return joinGrouponActivityFail(trade, checkStatus);
            }
        } else {
            if (checkStatus == GrouponOrderCheckStatus.ORDER_MARKETING_GROUPON_ACTIVITY_NOT_EXIST
                ...
                log.info("拼团活动未开始/不存在，具体失败原因信息：{}", checkStatus);
                return joinGrouponActivityFail(trade, checkStatus);
            }
        }
        ...
        if (checkStatus == GrouponOrderCheckStatus.PASS) {
            ...
            if (leader) {
                ...
                //测试模式
                if(grouponTestEnable){
                    endTime = currentTime.plusMinutes(4);
                    remindTime = endTime.minusMinutes(2);
                }
            ...
            if (grouponInstance.getGrouponStatus() == GrouponOrderStatus.COMPLETE) {
                ...
                //更新已成团人数
                orderProducerService.updateStatisticsNumByGrouponActivityId(grouponActivityId, grouponInstance.getJoinNum(), GrouponOrderStatus.COMPLETE);
                ...
                // 成团后，集体下单
                tradeList.forEach(t -> orderProducerService.sendMQForThirdPlatformSync(t.getId()));
                //拼团发送通知消息
                this.handleSendMessage(grouponNo, tradeList, NodeType.ORDER_PROGRESS_RATE,
                        OrderProcessType.JOIN_GROUP_SUCCESS, currentTime);
            } else {
                orderProducerService.updateStatisticsNumByGrouponActivityId(grouponActivityId, NumberUtils.INTEGER_ONE, GrouponOrderStatus.WAIT);
            }
            return trade;
     ...
     * @param nodeCode
     */
    public void handleSendMessage(String grouponNo, List<Trade> tradeList, NodeType nodeType,
                                  OrderProcessType nodeCode, LocalDateTime grouponSuccessTime) {
        for (Trade trade : tradeList) {
            this.sendNoticeMessage(nodeType, nodeCode, trade, grouponNo, null);
            // 拼团成功，发送订阅消息
            try {
                if (OrderProcessType.JOIN_GROUP_SUCCESS == nodeCode){
                    // 商品名称
                    ...
                    orderProducerService.sendMiniProgramSubscribeMessage(PlatformSendMiniMsgRequest.builder()
                            .triggerNodeId(TriggerNodeType.GROUPON_SUCCESS).skuName(skuName).price(String.valueOf(price))
                            .customerId(trade.getBuyer().getId()).grouponSuccessTime(DateUtil.format(grouponSuccessTime,
                                    DateUtil.FMT_TIME_1)).tradeId(trade.getId()).build());
                }
            }catch (Exception e){
                log.error("拼团成功，订阅消息发送失败，trade={}", JSON.toJSONString(trade));
            }
        }
    }
    ...
    /**
     * 根据团编号查询拼团订单信息（拼团订单 & 已成团 & 未退款）
     *
     * @param grouponNo
     * @return
     */
    public List<Trade> getCompletedTradeByGrouponNo(String grouponNo) {
        Criteria criteria = new Criteria();
        criteria.andOperator(
                Criteria.where("tradeState.payState").is(PayState.PAID.getStateId()),
                Criteria.where("grouponFlag").is(Boolean.TRUE),
                Criteria.where("tradeGroupon.grouponOrderStatus").is(GrouponOrderStatus.COMPLETE.toString()),
                Criteria.where("tradeGroupon.grouponNo").is(grouponNo),
                Criteria.where("tradeGroupon.grouponFailReason").exists(false)
        );
        return mongoTemplate.find(new Query(criteria), Trade.class);
...
//        }
    ...
    public GrouponGoodsInfoVO validGrouponOrderBeforeCommit(GrouponTradeValid valid) {
        ...
        if (openGroupon) {
            ...
            grouponGoodsInfo = grouponActivityQueryProvider.listActivityingByGoodsInfoIds(
                    new GrouponActivityingByGoodsInfoIdsRequest(Collections.singletonList(goodsInfoId))).getContext().getGrouponGoodsInfoMap().get(goodsInfoId);
        } else {
            GrouponInstance grouponInstance = getGrouponInstanceByActivityIdAndGroupon(grouponNo);
            if (Objects.isNull(grouponInstance) || StringUtils.isBlank(grouponInstance.getGrouponActivityId())) {
                ...
                throw new SbcRuntimeException(OrderErrorCodeEnum.K050100);
            }
        ...
        if (GrouponOrderCheckStatus.ORDER_MARKETING_GROUPON_ALREADY_JOINED == checkStatus) {
            ...
            throw new SbcRuntimeException(OrderErrorCodeEnum.K050095);
        }

        //已成团/团已作废，不可参团
        if (GrouponOrderCheckStatus.ORDER_MARKETING_GROUPON_ORDER_INST_ALREADY_OR_FAIL == checkStatus
                || GrouponOrderCheckStatus.ORDER_MARKETING_GROUPON_COUNT_DOWN_END == checkStatus) {
        ...
        if (Objects.nonNull(buyCount)) {
            ...
            if (Objects.nonNull(startSellingNum) && buyCount < startSellingNum || buyCount <= Constants.no) {
                throw new SbcRuntimeException(OrderErrorCodeEnum.K050097);
            }
            ...
            if (Objects.nonNull(record) && record.getLimitSellingNum() != 0) {
                if (Objects.nonNull(record) && Objects.nonNull(record.getLimitSellingNum())
                    ...
                    throw new SbcRuntimeException(OrderErrorCodeEnum.K050098);
                } else {
                    //第一次购买拼团商品
    ...
    public GrouponGoodsInfoVO validGrouponImmediately(GrouponTradeValid valid) {
        ...
        //存在优先级高的营销不能下拼团订单
        this.validMarketing(goodsInfo, valid);
        //获取供应商库存
        ...
        if(Objects.isNull(grouponGoodsInfo)){
            throw new SbcRuntimeException(OrderErrorCodeEnum.K050111);
        }
        if (Objects.isNull(grouponGoodsInfo.getGoodsInfo()) || grouponGoodsInfo.getGoodsInfo().getAddedFlag().equals(0)) {
        ...
        if (Objects.nonNull(buyCount)) {
            ...
            //限售数如果为0，则不限制拼团数量
            if (Objects.nonNull(record) && Objects.nonNull(record.getLimitSellingNum()) && record.getLimitSellingNum() != 0) {
                if ( (record.getBuyNum() + buyCount) > record.getLimitSellingNum()) {
                    throw new SbcRuntimeException(OrderErrorCodeEnum.K050098);
                } else {
                    //第一次购买拼团商品
                    if (Objects.nonNull(grouponGoodsInfo.getLimitSellingNum())
    ...
    public void validMarketing(
            GoodsInfoVO goodsInfoVO, GrouponTradeValid valid) {
        GoodsInfoSimpleVO t = KsBeanUtil.convert(goodsInfoVO,
                GoodsInfoSimpleVO.class);
        GoodsInfoPluginRequest goodsInfoPluginRequest = GoodsInfoPluginRequest.builder()
                .goodsInfoPluginRequest(t)
                .storeId(t.getStoreId())
                .build();
        String  customerId = valid.getCustomerId();
        goodsInfoPluginRequest.setTerminalSource(valid.getTerminalSource());
        ...
        GoodsInfoDetailPluginResponse pluginResponse =
                newMarketingPluginProvider
                        ...
                        .getContext();
        if (pluginResponse != null && CollectionUtils.isNotEmpty(pluginResponse.getMarketingLabels())) {
            boolean flag = pluginResponse.getMarketingLabels().stream()
                    .filter(marketingPluginSimpleLabelVO -> marketingPluginSimpleLabelVO.getMarketingType() == 111
                            || marketingPluginSimpleLabelVO.getMarketingType() == 5
                            || marketingPluginSimpleLabelVO.getMarketingType() == 6
                            || marketingPluginSimpleLabelVO.getMarketingType() == 102
                            || marketingPluginSimpleLabelVO.getMarketingType() == 103
                            || marketingPluginSimpleLabelVO.getMarketingType() == 109
                    ).count() > 0;
            if(flag){
                throw new SbcRuntimeException(OrderErrorCodeEnum.K050114);
            }
        }
    }
}
