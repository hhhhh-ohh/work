...
import com.wanmi.sbc.order.trade.service.VerifyServiceInterface;
...
public class VerifyQueryController implements VerifyQueryProvider {
    ...
    public BaseResponse<VerifyTradeMarketingResponse> verifyTradeMarketing(@RequestBody @Valid VerifyTradeMarketingRequest verifyTradeMarketingRequest) {
        verifyServiceInterface.verifyTradeMarketing(verifyTradeMarketingRequest.getTradeMarketingList(),
                KsBeanUtil.convertList(verifyTradeMarketingRequest.getOldGifts(), TradeItem.class),
                KsBeanUtil.convertList(verifyTradeMarketingRequest.getTradeItems(), TradeItem.class), verifyTradeMarketingRequest.getCustomerId(),
                verifyTradeMarketingRequest.getIsFoceCommit(),
                Nutils.defaultVal(verifyTradeMarketingRequest.getPluginType(), PluginType.NORMAL),
                verifyTradeMarketingRequest.getStoreId());
        return BaseResponse.success(new VerifyTradeMarketingResponse(verifyTradeMarketingRequest.getTradeMarketingList()));
    }

    @Override
    public BaseResponse verifyPurchase(@RequestBody @Valid TradeVerifyPurchaseRequest request){
        verifyService.verifyPurchase(request);
        return BaseResponse.SUCCESSFUL();
    }
}
