...
public class AccountRecordQueryController implements AccountRecordQueryProvider {
    ...
    /**
     * es初始化查询分页，不包含count查询
     * @param request
     * @return
     */
    @Override
    public BaseResponse<List<ReconciliationVO>> listByPage(@RequestBody @Valid BasePageRequest request) {
        return BaseResponse.success(accountRecordService.listByPage(request));
    }
    ...
    private Byte toType(AccountRecordType type){
        return Byte.valueOf(String.valueOf(type.toValue()));
    }

    public static void main(String[] args) {
        AccountDetails details = new AccountDetails();
        details.setPoints(10L);
        details.setAmount("100");

        AccountDetailsVO vo = new AccountDetailsVO();
        KsBeanUtil.copyPropertiesThird(details, vo);
