...
public class QueryConditionsUtil {
    ...
    public static Specification<CustomerDetail> getWhereCriteria(CustomerDetailPageRequest request) {
        ...
        LogOutStatus logOutStatus = request.getLogOutStatus();
        return (root, cquery, cbuild) -> {
            ...
            if (checkState != null) {
                predicates.add(cbuild.equal(customerDetailJoin.get("checkState"), CheckState.fromValue(checkState)));
                predicates.add(cbuild.notEqual(customerDetailJoin.get("logOutStatus"), LogOutStatus.LOGGED_OUT));
            }
...
//                predicates.add(root.get("customer").get("customerId").in(customerIds));
            ...
            if (LogOutStatus.LOGGED_OUT.equals(logOutStatus)){
                predicates.add(cbuild.equal(customerDetailJoin.get("logOutStatus"), logOutStatus));
            }
            //删除标记
            predicates.add(cbuild.equal(customerDetailJoin.get("delFlag"), DeleteFlag.fromValue(delFlag)));
     ...
     * @return
     */
    public static Specification<CustomerDetail> getWhereCriteria(CustomerDetailPageForSupplierRequest request) {
        String customerAccount = request.getCustomerAccount();
        String customerId = request.getCustomerId();
        String customerName = request.getCustomerName();
        String employeeId = request.getEmployeeId();
        ...
        return (root, cquery, cbuild) -> {
            ...
            predicates.add(cbuild.equal(customerDetailJoin.get("checkState"), CheckState.CHECKED));

            if (StringUtils.isNotEmpty(customerAccount)) {
                predicates.add(cbuild.like(customerDetailJoin.get("customerAccount"), new StringBuffer().append("%").append
                        (XssUtils.replaceLikeWildcard(customerAccount.trim())).append("%").toString()));
            }
            if (StringUtils.isNotEmpty(customerId)) {
                predicates.add(cbuild.equal(customerDetailJoin.get("customerId"), customerId.trim()));
            }

            if(CollectionUtils.isNotEmpty(customerIds)){
                predicates.add(customerDetailJoin.get("customerId").in(customerIds));
            }

            if (StringUtils.isNotEmpty(customerName)) {
                predicates.add(cbuild.like(root.get("customerName"), new StringBuffer().append("%")
    ...
    public static Specification<InviteNewRecord> getWhereCriteria(DistributionInviteNewPageRequest request) {
        ...
        return (root, cquery, cbuild) -> {
            ...
            if (null != request.getIsRewardRecorded()) {
                predicates.add(cbuild.equal(root.get("rewardRecorded"), InvalidFlag.fromValue(request.getIsRewardRecorded().toValue())));
            }
            //入账类型
            if (null != request.getRewardFlag()) {
                predicates.add(cbuild.equal(root.get("rewardFlag"), request.getRewardFlag()));
            }
            //未入账原因
            if (null != request.getFailReasonFlag()) {
                predicates.add(cbuild.equal(root.get("failReasonFlag"), request.getFailReasonFlag()));
            }
            // 是否有效邀新
            if (null != request.getAvailableDistribution()) {
                predicates.add(cbuild.equal(root.get("availableDistribution"), request.getAvailableDistribution()));
            }

            //是否分销员(空：全部，0：否，1：是 )
            if(StringUtils.isNotBlank(request.getIsDistributor())){
                predicates.add(cbuild.equal(root.get("distributor"), Integer.valueOf(request.getIsDistributor())));
            }
