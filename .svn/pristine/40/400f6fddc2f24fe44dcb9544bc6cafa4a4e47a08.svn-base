...
public class StoreEvaluateStatisticsService {
    ...
    public void statisticsData(List<StoreVO> storeList, int storeEvaluateStatisticsDay) {
        ...
        // 店铺名称可以更改，导致评价表冗余店铺名称不对。改为实时查询。
        List<Long> storeIds = storeList.stream().map(StoreVO::getStoreId).collect(Collectors.toList());
        Map<Long, Store> storeMap =
                storeService.findList(storeIds).stream().collect(Collectors.toMap(Store::getStoreId,
                Function.identity()));
        storeList.forEach(store -> {
            ...
            if (storeMap.containsKey(store.getStoreId())) {
                store.setStoreName(storeMap.get(store.getStoreId()).getStoreName());
            }
    ...
    public BaseResponse<StoreEvaluateSumAddResponse> addSumList(List<StoreEvaluateSumAddRequest> storeEvaluateSumAddRequestList) {
        ...
        goodsEvaluateMqService.sendStoreEvaluate(storeEvaluateSumAddRequestList.stream().map(storeEvaluateSumAddRequest -> {
    ...
    public List<StoreEvaluateVO> batchGetStoreEvaluateInfo(StoreEvaluatePageRequest storeEvaluatePageRequest) {
        ...
        MicroServicePage<StoreEvaluateVO> microPage = new MicroServicePage<>(storeEvaluatePage.map(entity ->
                storeEvaluateService.wrapperVo(entity)), storeEvaluatePageRequest.getPageable());
        if (CollectionUtils.isNotEmpty(microPage.getContent())) {
            ...
            storeEvaluateVOList.addAll(microPage.getContent());
    ...
    public StoreEvaluateSumAddRequest dealStoreEvaluates(List<StoreEvaluateVO> storeEvaluateList, EvaluateRatioVO evaluateRatioVO, StoreVO storeVO) {
        ...
        BigDecimal serverScore = serverScoreSum
                .divide(new BigDecimal(orderNum), 2, RoundingMode.DOWN);
        //商品质量综合评分
        BigDecimal goodsScore = goodsScoreSum.divide(new BigDecimal(orderNum), 2, RoundingMode.DOWN);
        //物流综合评分
        BigDecimal logisticsScoreScore = logisticsScoreSum.divide(new BigDecimal(orderNum), 2, RoundingMode.DOWN);
        ...
        BigDecimal sumCompositeScore = logisticsScoreScore.multiply(evaluateRatioVO.getLogisticsRatio()).setScale(2, RoundingMode.DOWN)
                .add(goodsScore.multiply(evaluateRatioVO.getGoodsRatio()).setScale(2, RoundingMode.DOWN))
                .add(serverScore.multiply(evaluateRatioVO.getServerRatio()).setScale(2, RoundingMode.DOWN));
    ...
    public BigDecimal getServerEvaluateCompositeScoreByScore(List<StoreEvaluateVO> storeEvaluateList) {
        ...
        Integer orderNum = storeEvaluateList.size();
        //服务质量综合评分
        return serverScoreSum.divide(new BigDecimal(orderNum), 2, RoundingMode.DOWN);
    }
    ...
    public BigDecimal getGoodsEvaluateCompositeScoreByScore(List<StoreEvaluateVO> storeEvaluateList) {
        ...
        Integer orderNum = storeEvaluateList.size();
        //商品质量综合评分
        return goodsScoreSum.divide(new BigDecimal(orderNum), 2, RoundingMode.DOWN);
    }
    ...
    public BigDecimal getLogisticsEvaluateCompositeScoreByScore(List<StoreEvaluateVO> storeEvaluateList) {
        ...
        Integer orderNum = storeEvaluateList.size();
        //物流质量综合评分
        return logisticsScoreSum.divide(new BigDecimal(orderNum), 2, RoundingMode.DOWN);
    }
