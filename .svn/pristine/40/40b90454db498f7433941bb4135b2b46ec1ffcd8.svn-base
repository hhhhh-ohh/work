 ...
 *
 * @author xufeng
 * @className MarketingFlashPromotionPlugin
 * @description TODO
 * @date 2022/02/16 17:08
 */
@Slf4j
@MarketingPluginService(type = MarketingPluginType.FLASH_PROMOTION)
public class MarketingFlashPromotionPlugin implements MarketingPluginInterface {
    ...
    public GoodsInfoDetailPluginResponse goodsDetail(GoodsInfoPluginRequest request) {
        ...
        log.debug(" MarketingFlashPromotionPlugin goodsDetail process");
    ...
    public MarketingPluginSimpleLabelVO check(GoodsInfoPluginRequest request) {
        ...
        if (MapUtils.isNotEmpty(map)) {
            ...
            List<GoodsInfoMarketingCacheDTO> flashSaleList =
                    map.get(MarketingPluginType.FLASH_PROMOTION);
            if (CollectionUtils.isNotEmpty(flashSaleList)) {
                ...
                labelVO.setMarketingType(MarketingPluginType.FLASH_PROMOTION.getId());
                labelVO.setMarketingDesc("限时购");
        ...
        for (GoodsInfoSimpleVO request : requestList) {
            if (MarketingContext.isNotNullSkuMarketingMap(request.getGoodsInfoId())
                    && MarketingContext.getBaseRequest()
                                    ...
                                    .get(MarketingPluginType.FLASH_PROMOTION)
                            != null) {
                ids =
                        MarketingContext.getBaseRequest()
                                .getSkuMarketingMap()
                        ...
                        if (Objects.nonNull(goodsVOMap)
                            ...
                            if (vo.getStock() >= vo.getMinNum()) {
                                ...
                                pluginLabelVO.setMarketingType(
                                        MarketingPluginType.FLASH_PROMOTION.getId());
                                pluginLabelVO.setMarketingDesc("限时购");
                                pluginLabelVO.setStartTime(vo.getStartTime());

                                pluginLabelVO.setEndTime(vo.getEndTime());
