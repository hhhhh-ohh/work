...
public class MarketingBuyoutPriceController {
    ...
    public BaseResponse add(@Valid @RequestBody MarketingBuyoutPriceAddRequest request) {
        ...
        //全局互斥
        marketingBaseService.mutexValidate(MarketingType.BUYOUT_PRICE, request.getBeginTime(), request.getEndTime(),
    ...
    public BaseResponse modify(@Valid @RequestBody MarketingBuyoutPriceAddRequest request) {
        ...
        if (DefaultFlag.YES.equals(request.getIsPause()) && LocalDateTime.now().isAfter(request.getBeginTime())) {
            //暂停中,一口价活动可以编辑
            marketingBuyoutPriceProvider.checkMarketingTimeAndRuleConflict(request);
    ...
    private void valid(String joinLevel, Long storeId, MarketingScopeType scopeType, List<String> scopeIds){
        ...
        } else if(MarketingScopeType.SCOPE_TYPE_CUSTOM.equals(scopeType)){
            List<GoodsInfoVO> goodsInfos = goodsInfoQueryProvider.listByIds(GoodsInfoListByIdsRequest.builder().goodsInfoIds(scopeIds).build()).getContext().getGoodsInfos();
            if (goodsInfos.stream().anyMatch(g -> !Objects.equals(storeId, g.getStoreId()))){
                throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
            }
        }
    }
