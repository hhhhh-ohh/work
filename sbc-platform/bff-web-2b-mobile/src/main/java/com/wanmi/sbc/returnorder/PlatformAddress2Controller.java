...
@Tag(name =  "平台地址信息管理API", description =  "PlatformAddress2Controller")
@RestController
@Validated
@RequestMapping(value = "/platformaddress2")
public class PlatformAddress2Controller {
    ...
    @Operation(summary = "根据地区查询编码")
    @Parameter(name = "addrName", description = "区县地址名称", required = true)
    @GetMapping("/address/{addrName}")
    public BaseResponse<PlatformAddressVO> getAddress(@PathVariable String addrName) {
        return platformAddressQueryProvider.findByAddrNameAndDelFlag(PlatformAddressListRequest.builder().delFlag(DeleteFlag.NO)
                .addrLevel(AddrLevel.STREET).addrName(addrName).build());
    }
}
