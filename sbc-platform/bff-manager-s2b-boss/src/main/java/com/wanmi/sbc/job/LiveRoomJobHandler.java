...
@Component
@Slf4j
public class LiveRoomJobHandler {
    @Autowired
    ...
    private static final String LIVE_ROOM_LIST_URL = "https://api.weixin.qq.com/wxa/business/getliveinfo?access_token=";

    @XxlJob(value = "LiveRoomJobHandler")
    ...
    public void getLiveInfo(Integer start, Integer limit) {
        ...
        JSONObject jsonObject = getLiveRoomList(start, accessToken);
        ...
        if (limit < total) {
            while (start < total) {
                start = start + limit + 1;
                //分页循环获取数据，存入数据库
                JSON.parseArray(getLiveRoomList(start, accessToken).getString("room_info"), LiveRoomByWeChatVO.class).stream().forEach(liveRoom -> {
      ...
      public JSONObject getLiveRoomList(Integer start, String accessToken){
          String url = LIVE_ROOM_LIST_URL + accessToken;
          ...
          if (0 != (Integer) jsonObject.get(Constants.ERRCODE)) {
              ...
              if (jsonObject.getString(Constants.ERRCODE).equals(Constants.STR_42001)
                  ...
                  return getLiveRoomList(start, miniProgramUtil.getTokenCleanRedis());
              } else {
