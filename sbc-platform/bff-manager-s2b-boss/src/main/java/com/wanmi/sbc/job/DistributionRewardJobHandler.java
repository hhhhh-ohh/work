...
@Component
@Slf4j
public class DistributionRewardJobHandler {
    ...
    private void dealRewardCash(DistributionSettingGetResponse distributionSettingGetResponse) {
        ...
        // 1.开启分销 2.开启邀新 3.开启邀新奖励 4.勾选奖励现金
        Boolean openFlag = DefaultFlag.YES.equals(distributionSetting.getOpenFlag())
                && DefaultFlag.YES.equals(distributionSetting.getInviteOpenFlag())
                && DefaultFlag.YES.equals(distributionSetting.getInviteFlag())
                && DefaultFlag.YES.equals(distributionSetting.getRewardCashFlag());
        XxlJobHelper.log("此次分销设置邀新奖励现金openFlag为:" + openFlag);
        // 处理邀新奖励现金
        if (openFlag) {
            while (true) {
    ...
    private boolean getDistributionCustomer4RewardCash(DistributionSettingVO distributionSetting) {
        ...
        if (Objects.isNull(distributionCustomerInviteInfoPage.getContext()) || CollectionUtils.isEmpty(distributionCustomerInviteInfoPage.getContext().getDistributionCustomerInviteInfoVOPage().getContent())) {
            XxlJobHelper.log("distributionRewardJobHandler补发奖励金:没有满足补发条件的用户");
            return false;
        }
        ...
        distributionCustomerInviteInfoVOList.forEach(vo -> {
            ...
            XxlJobHelper.log("distributionRewardJobHandler补发奖励金:1、用户".concat(JSONObject.toJSONString(vo)));
            XxlJobHelper.log("distributionRewardJobHandler补发奖励金:2、分销记录信息".concat(JSONObject.toJSONString(inviteNewListVOList)));
            if (CollectionUtils.isNotEmpty(inviteNewListVOList)) {
                ...
                XxlJobHelper.log("distributionRewardJobHandler补发奖奖金:校正数据".concat(vo.getCustomerId()));
    ...
    void updateInviteNew4RewardCash(DistributionInviteNewListVO inviteNewListVO, DistributionSettingVO distributionSetting) {
        ...
        BaseResponse<DistributionInviteNewRecordVO> supplyAgain = distributionInviteNewProvider.updateBySupplyAgain(request);

        DistributionInviteNewRecordVO inviteNewRecordVO = supplyAgain.getContext();
        //同步es
        this.addInviteNewRecordInfo(inviteNewRecordVO);
    }
     ...
     * @return
     */
    void updateInviteNew4RewardCoupon(DistributionInviteNewListVO inviteNewListVO) {
        DistributionInviteNewSupplyAgainRequest request = new DistributionInviteNewSupplyAgainRequest();
        request.setRecordId(inviteNewListVO.getRecordId());
        // 优惠券名称
     ...
     * 邀新数据同步es
     *
     * @param inviteNewRecordVO inviteNewRecordVO
     */
    private void addInviteNewRecordInfo(DistributionInviteNewRecordVO inviteNewRecordVO) {
        if (Objects.nonNull(inviteNewRecordVO)) {
            EsDistributionInviteNewSaveRequest saveRequest = EsDistributionInviteNewSaveRequest.builder()
                    .inviteNewRecordVOList(Collections.singletonList(inviteNewRecordVO))
                    .build();
    ...
    void updateDistributor4Reward(DistributionCustomerInviteInfoVO distributionCustomerInviteInfoVO, List<DistributionInviteNewListVO> inviteNewListVOList, DistributionSettingVO distributionSetting) {
        ...
        //未入账邀新奖金扣除预计奖励金额
        BigDecimal rewardCashNotRecorded = inviteNewListVOList.stream().filter(vo -> RewardFlag.CASH.equals(vo.getRewardFlag())).map(DistributionInviteNewForPageVO::getSettingAmount).reduce(BigDecimal.ZERO, BigDecimal::add).setScale(2, RoundingMode.HALF_UP);
        //预计发放邀新奖励金额
    ...
    private void dealRewardCoupon(DistributionSettingGetResponse distributionSettingGetResponse) {
        DistributionSettingVO distributionSetting = distributionSettingGetResponse.getDistributionSetting();
        // 1.开启分销 2.开启邀新 3.开启邀新奖励 4.勾选奖励优惠券
        Boolean openFlag = DefaultFlag.YES.equals(distributionSetting.getOpenFlag())
                && DefaultFlag.YES.equals(distributionSetting.getInviteOpenFlag())
                && DefaultFlag.YES.equals(distributionSetting.getInviteFlag())
                && DefaultFlag.YES.equals(distributionSetting.getRewardCouponFlag());
        XxlJobHelper.log("此次分销设置邀新奖励优惠券openFlag为:" + openFlag);
        //处理邀新奖励优惠券
        if (openFlag) {
            //优惠券活动信息
            ...
            if (CollectionUtils.isEmpty(couponInfos) || couponInfos.size() == 0) {
                XxlJobHelper.log("此次分销设置邀新奖励优惠券:没查到优惠券信息");
                return;
            }
            while (true) {
    ...
    private boolean getDistributionCustomer4RewardCoupon(List<GetCouponGroupResponse> couponInfos, DistributionSettingVO distributionSetting, String activityId) {
        ...
        BaseResponse<DistributionCustomerInviteInfoPageResponse> distributionCustomerInviteInfoPage = distributionCustomerInviteInfoQueryProvider.page(distributionCustomerInviteInfoPageReq);
        if (Objects.isNull(distributionCustomerInviteInfoPage.getContext()) || CollectionUtils.isEmpty(distributionCustomerInviteInfoPage.getContext().getDistributionCustomerInviteInfoVOPage().getContent())) {
            XxlJobHelper.log("distributionRewardJobHandler补发奖励优惠券:没有满足补发条件的用户");
            return false;
        }
        ...
        distributionCustomerInviteInfoVOList.forEach(vo -> {
            ...
            XxlJobHelper.log("distributionRewardJobHandler补发奖励优惠券:1、用户".concat(vo.getCustomerId()));
            XxlJobHelper.log("distributionRewardJobHandler补发奖励优惠券:2、分销记录信息".concat(JSONObject.toJSONString(inviteNewListVOList)));
            if (CollectionUtils.isNotEmpty(inviteNewListVOList)) {
                //开始补发
                //1.修改分销员信息
                updateDistributor4Reward(vo, inviteNewListVOList, distributionSetting);
                inviteNewListVOList.forEach(inviteNewListVO -> {
                    ...
                    //2:修改邀新记录
                    updateInviteNew4RewardCoupon(inviteNewListVO);
                    //3:发放优惠券
                    sendCoupon(vo, couponInfos, activityId);
                });
            } else {
                XxlJobHelper.log("distributionRewardJobHandler补发奖励优惠券:校正数据".concat(vo.getCustomerId()));
                DistributorCustomerInviteInfoReviseRequest request = new DistributorCustomerInviteInfoReviseRequest();
                request.setCustomerId(vo.getCustomerId());
                request.setDistributionId(vo.getDistributionId());
                // 校正
