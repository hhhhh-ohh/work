 ...
 */
@Tag(name =  "平台店铺评价API", description =  "BossStoreEvaluateController")
@RestController
@Validated
...
public class BossStoreEvaluateController {
    ...
    public BaseResponse<StoreEvaluatePageResponse> page(@RequestBody StoreEvaluatePageRequest pageRequest) {
        ...
        response.getContext().getStoreEvaluateVOPage().getContent().forEach(v->v.setLogOutStatus(map.get(v.getCustomerId())));
        return response;
    }
}
