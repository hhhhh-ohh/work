...
import com.wanmi.sbc.order.api.request.payingmemberrecord.PayingMemberRecordByIdRequest;
...
@Component
@Slf4j
public class OrderGrowthValueIncreaseJobHandler {
    ...
    @XxlJob(value = "orderGrowthValueIncreaseJobHandler")
    public void execute() {
        // 查询过退货期的订单
        ...
        for (OrderGrowthValueTempVO vo : orderGrowthValueTempVOList) {
            // 验证订单是否存在售后申请
            if (!tradeQueryProvider.verifyProcessing(TradeVerifyAfterProcessingRequest.builder().tid(vo.getOrderNo()).build())
                ...
                BigDecimal growthValueCount = trade.getTradeItems().stream()
                        .map(item -> {
                            ...
                            growthValueRate = growthValueRate == null ? BigDecimal.ZERO :
                                    growthValueRate.divide(new BigDecimal(100), 2, RoundingMode.HALF_UP);
                            Integer canReturnNum = item.getCanReturnNum();
                            BigDecimal decimalCanReturnNum = BigDecimal.ZERO;
                            if (Objects.nonNull(canReturnNum)) {
                                decimalCanReturnNum = new BigDecimal(canReturnNum);
                            }
                            Long num = item.getNum();
                            BigDecimal decimalNum = BigDecimal.ZERO;
                            if (Objects.nonNull(num)) {
                                decimalNum = new BigDecimal(num);
                            }
                            return item.getSplitPrice().multiply(growthValueRate).multiply(decimalCanReturnNum).divide(decimalNum, 0, RoundingMode.HALF_UP);
                        })
                ...
                BigDecimal pointsCount = trade.getTradeItems().stream()
                        .map(item -> {
                            ...
                            // 兼容老数据
                            if (Objects.isNull(item.getPointsRate())) {
                                pointsRate = goodsCateQueryProvider.getById(new GoodsCateByIdRequest(item.getCateId()))
                            ...
                            pointsRate = pointsRate == null ? BigDecimal.ZERO :
                                    pointsRate.divide(new BigDecimal(100), 2, RoundingMode.HALF_UP);
                            Integer canReturnNum = item.getCanReturnNum();
                            BigDecimal decimalCanReturnNum = BigDecimal.ZERO;
                            if (Objects.nonNull(canReturnNum)) {
                                decimalCanReturnNum = new BigDecimal(canReturnNum);
                            ...
                            BigDecimal splitPrice = item.getSplitPrice();
                            if(CollectionUtils.isNotEmpty(item.getGiftCardItemList())) {
                                splitPrice = splitPrice.add(item.getGiftCardItemList().stream().map(itemVO-> Objects.isNull(itemVO.getPrice())?BigDecimal.ZERO : itemVO.getPrice()).reduce(BigDecimal.ZERO, BigDecimal::add));
                            }
                            return splitPrice.multiply(pointsRate).multiply(decimalCanReturnNum).divide(decimalNum, 0, RoundingMode.HALF_UP);
                        })
                        .reduce(BigDecimal.ZERO, BigDecimal::add).setScale(2, RoundingMode.DOWN);

                //付费会员-积分倍率
                if (Objects.nonNull(trade.getPayingMemberInfo())) {
                    PayingMemberInfoVO payingMemberInfo = trade.getPayingMemberInfo();
                    //查询付费记录
                    PayingMemberRecordVO record = payingMemberRecordQueryProvider.findById(PayingMemberRecordByIdRequest.builder().recordId(payingMemberInfo.getRecordId()).build()).getContext().getPayingMemberRecordVO();
                    if (Objects.nonNull(record) && StringUtils.isNotBlank(record.getRightsIds())) {
                        //查询关联的积分倍率权益
                        List<Integer> rightsIds = Arrays.stream(record.getRightsIds().split(",")).map(Integer::parseInt).collect(Collectors.toList());
                        Optional<CustomerLevelRightsVO> optional = customerLevelRightsQueryProvider
                                .list(CustomerLevelRightsQueryRequest.builder().rightsIdList(rightsIds).rightsType(LevelRightsType.POINTS_TIMES).build())
                                .getContext().getCustomerLevelRightsVOList().stream().findFirst();
                        if (optional.isPresent()) {
                            BigDecimal pointsTimes = new BigDecimal(JSONObject.parseObject(optional.get().getRightsRule()).get("times").toString());
                            pointsCount = pointsCount.multiply(pointsTimes);
                        }
                    }
                }
                ...
                if (store.getCompanyType().toValue() == 1) {
                    ...
                    // 已累计金额 + 订单金额 - 退单总金额
                    BigDecimal totalPrice = Objects.nonNull(trade.getTradePrice().getTotalPrice()) ? trade.getTradePrice().getTotalPrice() : BigDecimal.ZERO;
                    modifyRequest.setTradePriceCount(Objects.nonNull(consumerStatistics) ?
                            consumerStatistics.getTradePriceCount().add(totalPrice).subtract(returnSumPrice)
                            : totalPrice.subtract(returnSumPrice));
