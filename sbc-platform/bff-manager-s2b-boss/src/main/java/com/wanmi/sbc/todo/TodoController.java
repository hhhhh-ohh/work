...
import com.wanmi.sbc.customer.api.response.company.CompanyWaitCheckCountResponse;
 ...
 */
@Tag(name="TodoController", description = "S2B平台管理员待处理事项todo")
@RestController
@Validated
public class TodoController {
     ...
     * @return
     */
    @Operation(summary = "S2B平台管理员todo")
    @RequestMapping(value = "todo", method = RequestMethod.GET)
    public BaseResponse<BossTodoResponse> bossTodo() {
        BossTodoResponse bossTodoResponse = new BossTodoResponse();
        CompanyWaitCheckCountResponse companyWaitCheckCountResponse = companyInfoQueryProvider.countCompanyByWaitCheck().getContext();
        bossTodoResponse.setWaitSupplier(companyWaitCheckCountResponse.getCount());
        bossTodoResponse.setWaitProvider(companyWaitCheckCountResponse.getProviderCount());
        //待审核商品
        if(auditQueryProvider.isBossGoodsAudit().getContext().isAudit()
            ...
            bossTodoResponse.setWaitGoods(
                    goodsQueryProvider
                            .countUnAudit(
                                    GoodsUnAuditCountRequest.builder()
                                            .auditStatus(CheckStatus.WAIT_CHECK)
                                            .build())
                            .getContext()
                            .getUnAuditCount());
        }
