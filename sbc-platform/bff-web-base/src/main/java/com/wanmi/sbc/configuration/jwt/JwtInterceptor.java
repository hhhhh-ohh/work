...
public class JwtInterceptor implements WebRequestInterceptor {
    ...
    public static final String LOG_OUT_STATUS = "LOG_OUT_STATUS:";

    public static final String LOGOUT_CANCEL_URI = "/logout/cancel";
    ...
    /** 注销排除redis校验 */
    private Boolean logoutExcludedRedis;
    ...
    public JwtInterceptor(
        ...
        this.logoutExcludedRedis = logoutExcludedRedis;
    }
    ...
    public void preHandle(WebRequest request) {
        if (request instanceof ServletWebRequest) {
            ...
            try {
                ...
                log.debug(
                        "JwtInterceptor preHandle out logoutExcludedRedis:{}", logoutExcludedRedis);
                // 开关，校验是否存在待注销、已注销数据,撤销注销接口，不做待注销已注销校验
                if (Boolean.FALSE.equals(logoutExcludedRedis)
                        && !LOGOUT_CANCEL_URI.equals(requestURI)) {
                    if (Objects.nonNull(claims.get("customerId"))) {
                        String customerId = claims.get("customerId").toString();
                        if (redisTemplate.hasKey(LOG_OUT_STATUS.concat(customerId))) {
                            throw new Exception();
                        }
                    }
                }
