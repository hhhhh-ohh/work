...
import com.wanmi.sbc.third.login.response.GetWeAppOpenIdResponse;
import com.wanmi.sbc.third.login.response.GetWeChatUserInfoResponse;
import com.wanmi.sbc.third.login.response.ThirdLoginResponse;
import com.wanmi.sbc.third.login.response.WechatBaseInfoResponse;
import com.wanmi.sbc.third.login.response.WechatSetDetailResponse;
...
@Slf4j
@Tag(name = "WechatLoginController", description = "第三方登录-微信 API")
@RestController
@Validated
...
public class WechatLoginController {
     ...
     * @return
     */
    @Operation(summary = "登录页（不需要token）微信授权，获得微信用户信息(h5,其他端待确定)")
     ...
     * 小程序登录接口
     */
    @Operation(summary = "登录页（不需要token）微信授权，获得微信用户信息(小程序端)")
    ...
    public BaseResponse<ThirdLoginResponse> weChatLogin(@RequestBody @Valid WechatQuickLoginRequest request) {
        ...
        if (StringUtils.equals(CommonErrorCodeEnum.K000000.getCode(),miniProgramSetByTypeResponseBaseResponse.getCode())) {
            ...
            weChatClient = new WeChatClient(miniProgramSetVO.getAppId(), miniProgramSetVO.getAppSecret());
        } else {
        ...
        if (StringUtils.isNotBlank(request.getIv()) && StringUtils.isNotBlank(request.getEncryptedData())) {
            WxDataDecryptUtils wxDataDecryptUtils = new WxDataDecryptUtils();
            JSONObject jsonObject = wxDataDecryptUtils.getUserInfo(request.getIv(), weChatSession.getSessionKey(),
                    request.getEncryptedData());
            String phoneNumber = jsonObject.get("phoneNumber") != null ? (String) jsonObject.get("phoneNumber") : null;
    ...
    private BaseResponse<ThirdLoginResponse> bind(WechatQuickLoginRequest request) {
        ...
        if (TerminalStringType.APP == request.getChannel()) {
            ...
            GetWeChatUserInfoResponse getWeChatUserInfoResponse = redisService.getObj(RedisKeyConstant.WX_USER_INFO.concat(request.getOpenId()),
...
//                    throw new SbcRuntimeException(CommonErrorCodeEnum.K999999, "未知错误，请重新进入小程序");
            ...
            if (Objects.isNull(thirdLoginRelation) || Objects.isNull(thirdLoginRelation.getCustomerId())) {
                ...
                if (StringUtils.isNotBlank(request.getNickName()) && StringUtils.isNotBlank(request.getHeadimgurl())) {
                    redisService.setObj(USER_INFO_KEY + getWeChatUserInfoResponse.getUnionid(),
                            getWeChatUserInfoResponse, 1800L);
                }
                ...
                if (registerLimitType == RegisterLimitType.INVITE && defaultFlag == DefaultFlag.YES && inviteOpenFlag == DefaultFlag.YES) {
                    if (StringUtils.isNotBlank(request.getInviteCode())) {
                        wechatBindForLoginRequest.setInviteCode(request.getInviteCode());
                    } else if (StringUtils.isBlank(request.getInviteeId())) {
                        ...
                        CustomerVO res =
                                customerSiteQueryProvider
                                        .getCustomerByCustomerAccount(accountRequest)
                                        .getContext();
                ...
                //不存在邀请码且存在邀请人ID，将邀请人ID塞入绑定微信账号入参中
                if (StringUtils.isBlank(request.getInviteCode()) && StringUtils.isNotBlank(request.getInviteeId())){
                    wechatBindForLoginRequest.setInviteeId(request.getInviteeId());
                }
                ...
                if (StringUtils.isBlank(thirdLoginRelation.getHeadimgurl()) || !Objects.equals(thirdLoginRelation.getHeadimgurl(), request.getHeadimgurl())) {
            ...
            redisService.delete(RedisKeyConstant.WX_USER_INFO.concat(request.getOpenId()));
            return BaseResponse.success(response);
        } else {
        ...
        //不存在，将授权信息存放在redis里，
        if (Objects.isNull(thirdLoginRelation) || Objects.isNull(thirdLoginRelation.getCustomerId())) {
            redisService.setObj(USER_INFO_KEY + weChatUserInfoResponse.getUnionid(), weChatUserInfoResponse, 1800L);
            response.setLoginFlag(Boolean.FALSE);
            WechatBaseInfoResponse info = new WechatBaseInfoResponse();
    ...
    public BaseResponse<LoginResponse> weChatBind(@Validated @RequestBody WechatBindForLoginRequest request) {
        ...
        if (!request.getChannel().equals(TerminalStringType.WEAPP)) {
            // 累计验证错误次数
            String errKey = CacheKeyConstant.WX_BINDING_LOGIN_NUM.concat(request.getPhone());
            // 校验及封装校验后请求参数
            String smsKey = CacheKeyConstant.WX_BINDING_LOGIN.concat(request.getPhone());
            // 校验短信验证码
...
//        CustomerVO res = customerSiteQueryProvider.getCustomerByCustomerAccount(accountRequest).getContext();
        ...
        if (StringUtils.isNotBlank(request.getInviteCode()) || StringUtils.isNotBlank(request.getInviteeId())) {
            //验证邀请ID、邀请码是否正确
        ...
        String token = Jwts.builder().setSubject(thirdLoginAndBindResponse.getCustomerAccount())
                ...
                .setExpiration(DateUtils.addDays(date, 30))
        ...
        redisService.setObj(CacheKeyConstant.WEAPP_LOGIN_INFO + response.getCustomerId(), response, 1800L);
        return BaseResponse.success(response);
    }
     ...
     * @return
     */
    @Operation(summary = "绑定微信账号(个人中心入口)")
     ...
     * @return
     */
    @Operation(summary = "小程序绑定微信账号(个人中心入口)")
    ...
    public BaseResponse miniprogramBind(@Validated @RequestBody WechatAuthRequest wechatAuthRequest) {
        ...
        String unionId = Objects.nonNull(jsonObject.get("unionId")) ? jsonObject.get("unionId").toString() : "";
        if(StringUtils.isBlank(unionId) && StringUtils.isNotBlank(weChatSession.getUnionId())){
        ...
        thirdLoginRelation.setThirdLoginUid(unionId);
        thirdLoginRelation.setCustomerId(commonUtil.getOperatorId());
     ...
     * @return
     */
    @Operation(summary = "获取微信授信配置，channel: PC,MOBILE,APP")
    ...
    @Operation(summary = "根据终端类型，获取授信开关状态，channel: PC,MOBILE,APP")
    @Parameter(name = "channel", description = "类型终端", required = true)
    ...
    @Operation(summary = "根据customerId获取从小程序端登录的登录信息")
    ...
    @Operation(summary = "小程序code换取openId和sessionKey,并将当前环境的小程序配置返回,无需登录")
    ...
    public BaseResponse<GetWeAppOpenIdResponse> authorize(@RequestBody WechatAuthRequest request) {
        ...
        if (StringUtils.equals(CommonErrorCodeEnum.K000000.getCode(),miniProgramSetByTypeResponseBaseResponse.getCode())) {
            ...
            weChatClient = new WeChatClient(appId, appSecret);
        } else {
            throw new SbcRuntimeException(CommonErrorCodeEnum.K000001);
        }
        ...
        getWeChatUserInfoResponse.setUnionid(unionId);
        ...
        redisService.setObj(RedisKeyConstant.WX_USER_INFO.concat(getWeChatUserInfoResponse.getOpenid()), getWeChatUserInfoResponse,
    ...
    @Operation(summary = "提现页面，查询当前登录用户的基础信息")
    ...
    public static void main(String[] args) {
        String key = "wmtest12boss41efaca1e051ccc610be62acdcf24c09f011f343583c41cfb941b";
        String token = "eyJhbGciOiJIUzI1NiJ9..MTVq_ZTSKZzc6xEeSBycUXSMrwsM-FwDVVi8TMTlKfs";
