 ...
 */
@Tag(name = "StoreHistoryBaseController", description = " 用户店铺搜索记录 API")
@RestController
...
public class StoreHistoryBaseController {
    ...
    private final static String REDIS_KEY = CacheKeyConstant.STORE_SEARCH_HISTORY_KEY;
    ...
    public BaseResponse add(@RequestBody @Valid SearchHistoryRequest request) {
        ...
        while(it.hasNext()){
            String x = it.next();
            if(Objects.nonNull(x) && x.equals(request.getKeyword())){
                it.remove();
                break;
            }
        }
