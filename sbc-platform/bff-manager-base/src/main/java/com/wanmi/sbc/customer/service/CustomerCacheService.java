...
public class CustomerCacheService {
	...
	public Integer sendMobileCode(String redisKey, String mobile, SmsTemplate smsTemplate) {
		...
		smsSendUtil.sendByMq(smsTemplate, new String[]{mobile}, verifyCode);
	 ...
	 * @return
	 */
	public LogOutStatus getCustomerLogOutStatus(String customer){
		String s = redisService.getString(CacheKeyConstant.LOG_OUT_STATUS + customer);
		if (StringUtils.isNotBlank(s)){
			String[] split = s.split(":");
			return LogOutStatus.fromValue(Integer.parseInt(split[1]));
		}else {
			return LogOutStatus.NORMAL;
		}
	}
