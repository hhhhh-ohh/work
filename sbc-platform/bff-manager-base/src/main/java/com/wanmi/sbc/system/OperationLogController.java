...
public class OperationLogController {
     ...
     * @return
     */
    @Operation(summary = "运营端和商家端查询最近10次登录信息")
     ...
     * @return
     */
    @Operation(summary = "根据条件查询操作日志 运营端和商家端")
    ...
    public BaseResponse<EsOperationLogPageResponse> queryByCriteria(@RequestBody EsOperationLogListRequest queryRequest,
        ...
        if (Platform.SUPPLIER.toValue().equals(platform) || Platform.PROVIDER.toValue().equals(platform) || Platform.STOREFRONT.toValue().equals(platform)) {
            queryRequest.setStoreId(Long.valueOf(Objects.toString(claims.get("storeId"), "")));
            queryRequest.setCompanyInfoId(Long.valueOf(Objects.toString(claims.get("adminId"), "")));
        }
    ...
    private void validTimeRange(String beginTime, String endTime) {
        ...
        if (!endTimeLocal.isBefore(beginTimeLocal)) {
            throw new SbcRuntimeException(CommonErrorCodeEnum.K000009, "开始时间和结束时间需在三个月之内");
        }
    }
    ...
    public BaseResponse exportOperationLogByParams(@PathVariable String encrypted) {
        ...
        if (Platform.PLATFORM.equals(operator.getPlatform())) {
            ...
            operationLogListRequest.setCompanyInfoId(0L);
                ...
                || Platform.PROVIDER.equals(operator.getPlatform())
                || Platform.STOREFRONT.equals(operator.getPlatform())) {
        ...
        EsOperationLogListRequest esOperationLogListRequest = KsBeanUtil.convert(operationLogListRequest, EsOperationLogListRequest.class);

        ExportDataRequest exportDataRequest = new ExportDataRequest();
        exportDataRequest.setParam(JSONObject.toJSONString(esOperationLogListRequest));
        exportDataRequest.setTypeCd(ReportType.BUSINESS_OPERATION_LOG);
