...
public class EmployeeEditValidator implements Validator{
    ...
    public void validate(Object target, Errors errors) {
        if (target instanceof EmployeeModifyRequest) {
            EmployeeModifyRequest saveRequest = (EmployeeModifyRequest)target;
            //验证员工名称
            if(StringUtils.isBlank(saveRequest.getEmployeeName())){
                throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
            }
            //验证手机
            if (StringUtils.isBlank(saveRequest.getEmployeeMobile()) ||
                    !ValidateUtil.isPhone(saveRequest.getEmployeeMobile())) {
                throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
            }
            //验证是否是员工
