...
public class VerifyCodeService {
    ...
    public Boolean validateCaptcha(String uuid, String enterCaptcha, long certTimeout, TimeUnit certTimeUnit) {
        ...
        Optional.ofNullable(stringRedisTemplate.opsForValue().get(String.format("%s_%s", CAPTCHA, uuid))).ifPresent(captcha -> {
    ...
    public boolean validateSmsVerifyCode(String phone, String verifyCode, VerifyType verifyType, long certTimeout, TimeUnit certTimeUnit) {
        Map<String, Boolean> res = new HashMap<>();
        res.put("res", Boolean.FALSE);
        Optional.ofNullable(stringRedisTemplate.opsForValue().get(String.format("%s_%s", verifyType, phone))).ifPresent(cacheCode -> {
    ...
    public void addVerifyCodeErrorCount(String phone) {
        ...
        if (count != null) {
            //如果输错5次
            if(Long.parseLong(count) >= Constants.FIVE) {
                //移除短信验证码
    ...
    public boolean validateSmsVerifyCodeAgain(String phone, String verifyCode, VerifyType verifyType) {
        ...
        if(cacheCode == null) {
            throw new SbcRuntimeException(CommonErrorCodeEnum.K000022);
        }
        if (cacheCode.equals(verifyCode)) {
            return true;
        } else {
