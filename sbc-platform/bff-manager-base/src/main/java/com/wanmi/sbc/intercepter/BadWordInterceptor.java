...
public class BadWordInterceptor implements HandlerInterceptor {
    ...
    public BadWordInterceptor( String apiBadWordsAddPathUrls, Boolean filterAllFlag) {
        this.apiBadWordsAddPathUrlsMap = JSONObject.parseObject(apiBadWordsAddPathUrls);
        this.filterAllFlag = filterAllFlag;
    }
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws
        ...
        if (Constants.ERROR_URI.equalsIgnoreCase(uri)) {
            return true;
        }

        String requestType = request.getMethod();
        if (AddPathUrlFilter(this.apiBadWordsAddPathUrlsMap, uri, requestType) || Objects.equals(Boolean.TRUE, this.filterAllFlag)) {
    ...
    private void notAllowed(HttpServletResponse response,HttpServletRequest request, String message) throws Exception {
        ...
        response.getWriter().write(JSONObject.toJSONString(BaseResponse.info(CommonErrorCodeEnum.K999996.getCode(), message)));
