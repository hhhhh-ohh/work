...
public class CustomerDrawCashSaveController implements CustomerDrawCashSaveProvider {
	...
	public BaseResponse<CustomerDrawCashAddResponse> add(@RequestBody @Valid CustomerDrawCashAddRequest customerDrawCashAddRequest) {
		...
		//判断提现金额是否大于会员账户可提现金额
		if(preDrawCash.compareTo(customerFunds.getWithdrawAmount()) > 0){
			throw new SbcRuntimeException(AccountErrorCodeEnum.K020019);
		}
		...
		if (preDrawCash.compareTo(MAX_DRAW_CASH) > 0) {
			throw new SbcRuntimeException(AccountErrorCodeEnum.K020020);
		}
