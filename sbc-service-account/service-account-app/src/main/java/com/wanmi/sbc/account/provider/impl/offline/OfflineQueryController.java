...
public class OfflineQueryController implements OfflineQueryProvider {
    ...
    public BaseResponse<OfflineAccountGetByIdResponse> getById(@RequestBody @Valid OfflineAccountGetByIdRequest request) {
        ...
        KsBeanUtil.copyPropertiesThird(optional.orElseThrow(() -> new SbcRuntimeException(CommonErrorCodeEnum.K000003)), response);
    ...
    @Override
    public BaseResponse<OfflineAccountListResponse> findOfflineAccountByIds(@RequestBody @Valid OfflineAccountGetByIdsRequest request) {
        List<OfflineAccount> list = offlineService.findOfflineAccountByIds(request.getOfflineAccountIds());
        return BaseResponse.success(new OfflineAccountListResponse(convertToVO(list)));
    }
