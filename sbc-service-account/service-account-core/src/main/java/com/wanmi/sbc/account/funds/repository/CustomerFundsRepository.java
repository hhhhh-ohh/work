...
public interface CustomerFundsRepository extends JpaRepository<CustomerFunds, String>, JpaSpecificationExecutor<CustomerFunds> {
     ...
     *
     * @param customerId  会员ID
     * @param communityLeader 是否社区团长
     * @param updateTime  更新时间
     * @return
     */
    @Modifying
    @Query("update CustomerFunds cf set cf.communityLeader = :communityLeader ,cf.updateTime = :updateTime where cf.customerId = :customerId")
    int updateIsCommunityLeaderByCustomerId(@Param("customerId") String customerId, @Param("communityLeader") Integer communityLeader, @Param("updateTime") LocalDateTime updateTime);
