...
public class BaseConfigQueryController implements BaseConfigQueryProvider {
	...
	public BaseResponse<BossLogoResponse> queryBossLogo(){
		...
		if(baseConfigs.size()>0){
			logo  = baseConfigs.get(0).getPcLogo();
		}
		return BaseResponse.success(new BossLogoResponse(logo));
	}

	@Override
	public BaseResponse<PayingMemberAgreementResponse> getPayingMemberAgreement() {
		PayingMemberAgreementResponse response = new PayingMemberAgreementResponse();
		List<BaseConfig> baseConfigs = baseConfigService.list(new BaseConfigQueryRequest());
		if(baseConfigs.size()>0){
			response.setAgreement(baseConfigs.get(0).getPayingMemberContent());
		}
		return BaseResponse.success(response);
	}
}
