...
public class SystemConfigQueryController implements SystemConfigQueryProvider {
    ...
    @Override
    public BaseResponse<SystemConfigResponse> findByConfigKeyAndDelFlagNew(@RequestBody @Valid ConfigQueryRequest request) {
        List<Config> configList = configService.findByConfigKeyAndDelFlagNew(request.getConfigKey(), DeleteFlag.NO);
        if (CollectionUtils.isEmpty(configList)) {
            return BaseResponse.success(new SystemConfigResponse());
        }
        List<ConfigVO> configVOList = KsBeanUtil.convert(configList, ConfigVO.class);
    ...
    public BaseResponse<OfflinePaySettingResponse> getOfflinePaySetting() {
        OfflinePaySettingResponse response = configService.getOfflinePaySetting();
        return BaseResponse.success(response);
    }

    @Override
    @Cacheable(value = CacheConstants.GLOBAL_CACHE_NAME, key = "'esQueryBoost'")
    public BaseResponse<SystemConfigContextReponse> findContextByConfigType() {
        String contextByConfigType = systemConfigService.findContextByConfigType();
        return BaseResponse.success(new SystemConfigContextReponse(contextByConfigType));
    }

    @Override
    public BaseResponse<SystemConfigContextReponse> findContextByConfigTypeAndConfigKey() {
        String contextByConfigType = systemConfigService.findContextByConfigTypeAndConfigKey();
