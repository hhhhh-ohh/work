...
public class SensitiveWordsSaveController implements SensitiveWordsSaveProvider {
    ...
    public BaseResponse<SensitiveWordsSaveResponse> add(@RequestBody @Valid SensitiveWordsAddRequest sensitiveWordsAddRequest) {
        ...
        if (wordArr.length > Constants.NUM_100) {
            throw new SbcRuntimeException(SettingErrorCodeEnum.K070002);
        }
        // 去重
        List<String> wordList = Stream.of(wordArr).distinct().collect(Collectors.toList());
        List<SensitiveWords> wordsList = new ArrayList<>(100);
        ...
        for (String word : wordList) {
            if (word.length() == 0) {
                ...
                throw new SbcRuntimeException(SettingErrorCodeEnum.K070003, new Object[]{word});
            }
		...
		badWordService.addBadWordToHashMap(Boolean.TRUE);
	...
	public BaseResponse deleteByIdList(@RequestBody @Valid SensitiveWordsDelByIdListRequest sensitiveWordsDelByIdListRequest) {
        SensitiveWordsDeleteResponse sensitiveWordsDeleteResponse =
                sensitiveWordsService.deleteByIdList(sensitiveWordsDelByIdListRequest);
        badWordService.addBadWordToHashMap(Boolean.TRUE);
		return BaseResponse.success(sensitiveWordsDeleteResponse);
	}
