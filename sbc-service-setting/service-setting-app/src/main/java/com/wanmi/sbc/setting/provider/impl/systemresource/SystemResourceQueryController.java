...
import com.wanmi.sbc.setting.api.response.systemresource.SystemResourceMaxSortResponse;
...
public class SystemResourceQueryController implements SystemResourceQueryProvider {
    ...
    @Override
    public BaseResponse<SystemResourceMaxSortResponse> getMaxSort() {
        Integer maxSort = systemResourceService.getMaxSort();
        if(Objects.nonNull(maxSort)){
            return BaseResponse.success(SystemResourceMaxSortResponse.builder().sort(maxSort).build());
        } else {
            return BaseResponse.success(SystemResourceMaxSortResponse.builder().sort(0).build());
        }
    }

    @Override
    public BaseResponse<SystemResourcePageResponse> initEsPage(@RequestBody @Valid SystemResourcePageRequest systemResourcePageReq) {
        SystemResourceQueryRequest queryReq = new SystemResourceQueryRequest();
        KsBeanUtil.copyPropertiesThird(systemResourcePageReq, queryReq);
        queryReq.setDelFlag(DeleteFlag.NO);
        queryReq.putSort("createTime", SortType.DESC.toValue());
        queryReq.putSort("resourceId", SortType.ASC.toValue());
        Page<SystemResource> systemResourcePage = systemResourceService.initEsPage(queryReq);
        Page<SystemResourceVO> newPage = systemResourcePage.map(entity -> systemResourceService.wrapperVo(entity));
        MicroServicePage<SystemResourceVO> microPage = new MicroServicePage<>(newPage, systemResourcePageReq.getPageable());
        SystemResourcePageResponse finalRes = new SystemResourcePageResponse(microPage);
        return BaseResponse.success(finalRes);
    }
