...
public class BaseConfigService {
	...
	public BaseConfig modify(BaseConfig entity) {
		BaseConfig baseConfig = this.getById(entity.getBaseConfigId());
		if(Objects.isNull(baseConfig)){
			baseConfigRepository.save(entity);
			return entity;
		}
		entity.setRegisterContent(baseConfig.getRegisterContent());
		entity.setCancellationContent(baseConfig.getCancellationContent());
		entity.setPayingMemberContent(baseConfig.getPayingMemberContent());
		baseConfigRepository.save(entity);
		return entity;
	}
	...
	public void modifyAgreement(BaseConfigUpdateRequest request) {
		baseConfigRepository.modifyAgreement(request);
	}
}
