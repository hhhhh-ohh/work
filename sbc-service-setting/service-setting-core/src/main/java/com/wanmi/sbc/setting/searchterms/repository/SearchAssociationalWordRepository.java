...
public interface SearchAssociationalWordRepository extends JpaRepository<SearchAssociationalWord,Long> ,
     ...
     */
    @Modifying
    @Query(value = "update SearchAssociationalWord set  delFlag = com.wanmi.sbc.common.enums.DeleteFlag.YES where id=?1")
    int  deleteSearchAssociationalWord(Long id);

    /**
     * 模糊搜索词
     */
    @Query("select s from SearchAssociationalWord s where s.searchTerms like concat('%', :associationalWordName, '%') and  s.delFlag = :deleteFlag order by s.createTime desc")
    List<SearchAssociationalWord> queryLikeSearchTermsAndDelFlag(@Param("associationalWordName") String associationalWordName,
