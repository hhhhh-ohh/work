...
public class ExpressCompanyService {
	...
	public ExpressCompany add(ExpressCompany entity) {
		ExpressCompany expressCompany = getByCode(entity.getExpressCode());
		if (expressCompany != null) {
			throw new SbcRuntimeException(SettingErrorCodeEnum.K070026);
		}
		int count = expressCompanyRepository.countExpressCompany();
		if(count>= Constants.S2B_EXPRESS_COMPANY_COUNT){
			throw new SbcRuntimeException(SettingErrorCodeEnum.K070084, new Object[]{String.valueOf(Constants.S2B_EXPRESS_COMPANY_COUNT)});
		}
	 ...
	 * @return   物流公司信息
	 */
	public ExpressCompany getByCode(String expressCode){
		return expressCompanyRepository.findExpressCompanyByExpressCodeAndDelFlag(expressCode, DeleteFlag.NO);
	}
