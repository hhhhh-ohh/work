...
public class YunService {
    ...
    public YunUploadResourceResponse uploadFile(YunUploadResourceRequest yunUploadResourceRequest) {
        ...
        SystemResourceVO systemResource;
        if (Objects.isNull(yunUploadResourceRequest.getStoreId())) {
            systemResource = saveFile(yunUploadResourceRequest);
        } else {
            systemResource = saveStoreFile(yunUploadResourceRequest);
        }
     ...
     */
    @Transactional
    public YunUploadResourceResponse saveUploadFile(YunUploadResourceRequest yunUploadResourceRequest) {
        String  artworkUrl = yunUploadResourceRequest.getArtworkUrl();
        String[] data = artworkUrl.split("/");

        String resourceKey = data[data.length-1];
        String fileName = yunUploadResourceRequest.getResourceName();
        if (Objects.isNull(resourceKey)) {
            resourceKey = getKey(fileName);
        }
        ...
        yunUploadResourceRequest.setArtworkUrl(resourceKey);
        yunUploadResourceRequest.setResourceKey(resourceKey);
        YunUploadResourceResponse response = YunUploadResourceResponse.builder().resourceUrl(artworkUrl).build();
        SystemResourceVO systemResource;
        if (Objects.isNull(yunUploadResourceRequest.getStoreId())) {
            systemResource = saveFile(yunUploadResourceRequest);
        } else {
    ...
    public void deleteResources(List<Long> resourceIds, Long storeId) {
        ...
        if (Objects.isNull(storeId)) {
            ...
            List<SystemResource> storeResources = systemResourceService.list(SystemResourceQueryRequest.builder()
                    .resourceIdList(resourceIds)
                    .storeId(storeId)
                    .delFlag(DeleteFlag.NO)
                    .build());
    ...
    private SystemResourceVO saveFile(YunUploadResourceRequest yunUploadResourceRequest) {
        ...
        if (cateId == null || cateId == 0) {
            ...
            if (Objects.isNull(resourceCates)) {
                if (redisTemplate.opsForValue().setIfAbsent(RESOURCE_KEY.concat("-0"), RESOURCE_KEY.concat("-0"))) {
                    ...
                    throw new SbcRuntimeException(SettingErrorCodeEnum.K070041);
                }
        ...
        if (Objects.nonNull(availableYun)) {
            if (isSaveResource) {
                ...
                resource.setSort(yunUploadResourceRequest.getSort());
                SystemResource add = systemResourceService.add(resource);
                //如果没有排序号，用id填充
                if(yunUploadResourceRequest.getSort() == null) {
                    add.setSort(add.getResourceId().intValue());
                    systemResourceService.add(add);
                }
     ...
     * @param yunUploadResourceRequest
     */
    private SystemResourceVO saveStoreFile(YunUploadResourceRequest yunUploadResourceRequest) {
        ...
        if (cateId == null || cateId == 0) {
            List<SystemResourceCate> storeResourceCates = systemResourceCateService.list(SystemResourceCateQueryRequest.builder()
                    .delFlag(DeleteFlag.NO)
                    .isDefault(DefaultFlag.YES)
                    .storeId(storeId)
                    .build());
            if (CollectionUtils.isEmpty(storeResourceCates)) {
                ...
                storeResourceCate = systemResourceCateService.add(storeResourceCate);
        ...
        if (Objects.nonNull(availableYun) && isSaveResource) {
            ...
            storeResource.setSort(yunUploadResourceRequest.getSort());
            SystemResource add = systemResourceService.add(storeResource);
            SystemResourceVO systemResourceVO = new SystemResourceVO();
            BeanUtils.copyProperties(add, systemResourceVO);
            return systemResourceVO;
        }
