...
public class CustomerLevelRightsQueryController implements CustomerLevelRightsQueryProvider {
    ...
    @Override
    public BaseResponse<CustomerLevelRightsListResponse> issueLevelRightsForPayingMember(@RequestBody @Valid CustomerLevelRightsQueryRequest customerLevelRightsListReq) {
        List<CustomerLevelRights> customerLevelRightsList = customerLevelRightsService
                .issueLevelRightsForPayingMember(customerLevelRightsListReq.getCustomerId(),
                        customerLevelRightsListReq.getRightsIdList(),customerLevelRightsListReq.getLevelState(),
                        customerLevelRightsListReq.getRecordId());
        List<CustomerLevelRightsVO> newList = customerLevelRightsList.stream().map(entity -> customerLevelRightsService.wrapperVo(entity)).collect(Collectors.toList());
        return BaseResponse.success(new CustomerLevelRightsListResponse(newList));
    }
