...
public class DepartmentController implements DepartmentProvider {
	...
	@Override
	public BaseResponse<DepartmentByIdResponse> deleteById(@RequestBody @Valid DepartmentDelByIdRequest departmentDelByIdRequest) {
		Department department = departmentService.deleteById(departmentDelByIdRequest.getDepartmentId());
		return BaseResponse.success(new DepartmentByIdResponse(departmentService.wrapperVo(department)));
	}

	@Override
	public BaseResponse modifyLeader(@Valid DepartmentModifyLeaderRequest request) {
		departmentService.modifyLeader(request.getOldEmployeeId(),request.getNewEmployeeId(),
				request.getDepartmentId(), request.getCompanyInfoId());
		return BaseResponse.SUCCESSFUL();
	}

	@Override
	public BaseResponse modifyDepartmentEmployeeNum(@RequestBody @Valid ModifyEmployeeNumRequest request) {
		departmentService.modifyDepartmentEmployeeNum(request);
		return BaseResponse.SUCCESSFUL();
	}
