...
public class CompanyInfoQueryController implements CompanyInfoQueryProvider {
    ...
    public BaseResponse<CompanyWaitCheckCountResponse> countCompanyByWaitCheck() {
        CompanyWaitCheckCountResponse response = CompanyWaitCheckCountResponse.builder().count(companyInfoService
                .countByTodo(StoreType.SUPPLIER))
                .providerCount(companyInfoService.countByTodo(StoreType.PROVIDER))
                .build();
        return BaseResponse.success(response);
    }
    ...
    public BaseResponse<CompanyInfoQueryByIdsResponse> queryByCompanyInfoIds(@RequestBody @Valid CompanyInfoQueryByIdsRequest request) {
        ...
        companyInfoList.forEach(info->{
            ...
            vo.setStoreVOList(KsBeanUtil.convert(info.getStoreList(),StoreVO.class, JSONWriter.Feature.ReferenceDetection));
            vo.setEmployeeVOList(KsBeanUtil.convert(info.getEmployeeList(),EmployeeVO.class, JSONWriter.Feature.ReferenceDetection));
    ...
    public BaseResponse<StoreListForDistributionResponse> queryByCompanyCode(@RequestBody @Valid
        ...
        StoreListForDistributionResponse response =
                new StoreListForDistributionResponse(Lists.newArrayList());
        Page<CompanyInfo> companyInfoPage = companyInfoService.findAll(KsBeanUtil.copyPropertiesThird(recordRequest, CompanyRequest.class));
        if(CollectionUtils.isNotEmpty(companyInfoPage.getContent())){
            companyInfoPage.getContent().forEach(companyInfo -> {
                ...
                response.getStoreSimpleInfos().add(storeSimpleInfo);
            });
        }
        ;
        return BaseResponse.success(response);
    ...
    @Override
    public BaseResponse<CompanyInfoVO> queryByPreciseCompanyCode(@RequestBody @Valid CompanyInfoForDistributionRecordRequest recordRequest){
        StoreListForDistributionResponse response =
                new StoreListForDistributionResponse(Lists.newArrayList());
        CompanyInfo company = companyInfoService.queryByPreciseCompanyCode(recordRequest.getCompanyCode());
        return BaseResponse.success(KsBeanUtil.copyPropertiesThird(company, CompanyInfoVO.class));
    }
