...
import com.wanmi.sbc.customer.util.mapper.CustomerMapper;
...
public class CustomerQueryController implements CustomerQueryProvider {
    ...
    @Override
    public BaseResponse<Long> count(@RequestBody @Valid CustomerDetailPageRequest request) {
        Long count = customerService.count(request);
    ...
    public BaseResponse<CustomerDetailListByConditionResponse> listCustomerDetailByCondition(@RequestBody @Valid CustomerDetailListByConditionRequest request) {
        List<CustomerDetailVO> detailResponseList = null;
        Optional<List<CustomerDetail>> customerListOptional = customerService.findDetailByCondition(request);
        if (customerListOptional.isPresent()) {
            detailResponseList = customerListOptional.get().stream().map
                    (customerDetail -> {
    ...
    public BaseResponse<CustomerDetailListByPageResponse> listCustomerDetailForDistributionRecord(@RequestBody @Valid
        ...
        if (Objects.isNull(request.getIsIgnoreStatus()) || BooleanUtils.isFalse(request.getIsIgnoreStatus())) {
    ...
    public BaseResponse<CustomerListByConditionResponse> listCustomerByCondition(@RequestBody @Valid
        ...
        Optional<List<Customer>> customerListOptional = customerService.findCustomerByCondition(request);
        if (customerListOptional.isPresent()) {
            customerVOList = customerListOptional.get().stream().map
                    (customer -> {
                        ...
                        vo.setCustomerDetail(customerDetailVO);
     ...
     */
    @Override
    @Cacheable(value = "CUSTOMER", key = "#request.getCustomerId()")
    public BaseResponse<CustomerGetByIdResponse> getCustomerNoThirdImgById(@RequestBody @Valid CustomerGetByIdRequest request) {
        CustomerGetByIdResponse response;
        ...
        response =  customerMapper.customerToCustomerGetByIdResponse(customer);
    ...
    public BaseResponse<CustomerGetByIdResponse> getCustomerById(@RequestBody @Valid CustomerGetByIdRequest request) {
        ...
        Customer customer = customerService.findById(request.getCustomerId());
        copyProperties(customer, response, request.getCustomerId());

        if (StringUtils.isBlank(customer.getHeadImg())){
    ...
    @Override
    public BaseResponse<CustomerGetByIdResponse> getCustomerInfoById(@RequestBody @Valid CustomerGetByIdRequest request) {
        CustomerGetByIdResponse response = new CustomerGetByIdResponse();

        Customer customer = customerService.findById(request.getCustomerId());

        this.copyField(customer, response);
        ThirdLoginRelation thirdLoginRelation =
                thirdLoginRelationService.findFirstByCustomerIdAndThirdType
                        (request.getCustomerId(),
                                ThirdLoginType.WECHAT);
    ...
    public BaseResponse<CustomerNotRelatedListResponse> listCustomerNotRelated(@RequestBody @Valid
                                                                                       ...
                                                                                       request) {
        return BaseResponse.success(new CustomerNotRelatedListResponse(customerService.getCustomerNotRelated(request.getCustomerAccount())));
    }
     ...
     * @param response
     */
    private void copyProperties(Customer customer, CustomerGetByIdResponse response, String customerId) {
        if (customer != null) {
            KsBeanUtil.copyPropertiesThird(customer, response);

            CustomerDetail customerDetail = customerDetailService.findByCustomerId(customerId);
            if (customerDetail != null) {
                CustomerDetailVO vo = new CustomerDetailVO();
            ...
            throw new SbcRuntimeException(customerService.getDeleteIndex(customerId), CustomerErrorCodeEnum.K010001);
        }
    }
     ...
     * @param response
     */
    private void copyField(Customer customer, CustomerGetByIdResponse response) {
        if (customer != null) {
            KsBeanUtil.copyPropertiesThird(customer, response);
    ...
    @Override
    public BaseResponse<CustomerSimplifyByIdResponse> simplifyById(@RequestBody @Valid CustomerSimplifyByIdRequest request) {
        Customer customer = customerService.findById(request.getCustomerId());
        ...
        simplify.setLogOutStatus(customer.getLogOutStatus());
     ...
     */
    @Override
    public BaseResponse<CustomerVO> getCustomerByCustomerId(CustomerDelFlagGetRequest request) {
        return BaseResponse.success(customerService.getCustomerByCustomerId(request.getCustomerId(), DeleteFlag.NO));
    }

    @Override
    public BaseResponse<CustomerIdsByEmployeeIdsResponse> listCustomerIdsByEmployeeIds(@RequestBody @Valid CustomerIdsByEmployeeIdsRequest request) {
        List<CustomerDetailBaseVO> customerIds = customerService.listCustomerIdsByEmployeeIds(request.getEmployeeIds(), request.getDeleteFlag());
        return BaseResponse.success(new CustomerIdsByEmployeeIdsResponse(customerIds));
    }

    @Override
    public BaseResponse<CustomerListCustomerIdByPageableResponse> listCustomerIdsByUpgradeTime(@RequestBody @Valid CustomerIdsByUpgradeTimeRequest request) {
        List<String> ids = customerService.pageByLevelIdsAndUpgradeTime(request);
        return BaseResponse.success(new CustomerListCustomerIdByPageableResponse(ids));
    }

    @Override
    public BaseResponse<Boolean> isIepCustomer(@RequestBody @Valid CustomerGetByIdRequest request) {
        return BaseResponse.success(customerService.isIepCustomer(request.getCustomerId()));
    }

    @Override
    public BaseResponse<Boolean> isDistributor(@RequestBody @Valid CustomerGetByIdRequest request) {
        return BaseResponse.success(customerService.isDistributor(request.getCustomerId()));
    }
     ...
     */
    @Override
    public BaseResponse<CustomerListByConditionResponse> listCustomerByConditionNew(@RequestBody @Valid
                                                                                            CustomerListByConditionRequest request) {
        List<CustomerVO> customerVOList = null;
        Optional<List<Customer>> customerListOptional = customerService.findCustomerByCondition(request);
        if (customerListOptional.isPresent()) {
            customerVOList = customerListOptional.get().stream().map
                    (customer -> KsBeanUtil.convert(customer, CustomerVO.class)).collect(Collectors.toList());
        }
        return BaseResponse.success(new CustomerListByConditionResponse(customerVOList));
    }
    ...
    @Override
    public BaseResponse<Integer> isNewCustomer(@RequestBody @Valid CustomerGetByIdRequest request) {
        Customer customer = customerService.findById(request.getCustomerId());
        return BaseResponse.success(customer.getIsNew());
    }

    @Override
    public BaseResponse<CustomerIdListResponse> CustomerIdsByMembershipExpiredTimeRequest(@RequestBody @Valid CustomerIdsByMembershipExpiredTimeRequest request) {
        List<String> customerIds = customerService.CustomerIdsByMembershipExpiredTime(request);
        return BaseResponse.success(new CustomerIdListResponse(customerIds));
    }
}
