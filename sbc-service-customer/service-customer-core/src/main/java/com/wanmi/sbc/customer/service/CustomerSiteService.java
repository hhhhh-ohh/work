...
public class CustomerSiteService {
    ...
    @MultiSubmit
    public Customer login(String customerAccount, char[] password) throws SbcRuntimeException {
        CustomerDetail customerDetail = getCustomerDetailByAccount(customerAccount);
        if (Objects.isNull(customerDetail) ||
                Objects.isNull(customerDetail.getCustomer()) ||
                customerDetail.getCustomer().getLogOutStatus().equals(LogOutStatus.LOGGED_OUT)
        ){
            ...
            if (error < PASS_WRONG_MAX_COUNTS) {
                //用户名或密码错误，还有几次机会
                throw new SbcRuntimeException(CustomerErrorCodeEnum.K010078, new Object[]{PASS_WRONG_MAX_COUNTS - error});
                ...
                //连续输错密码5次，请{0}分钟后重试
                throw new SbcRuntimeException(CustomerErrorCodeEnum.K010079, new Object[]{LocalDateTime.now().until
        ...
        //判断用户是否首次登录
        if (EnterpriseCheckState.INIT.equals(customer.getEnterpriseCheckState())) {
            if(customer.getCheckState().toValue() == CheckState.CHECKED.toValue()
                    && Objects.isNull(customer.getLoginTime())) {
                customer.setFirstLoginFlag(Boolean.TRUE);
            }
        } else {
        ...
        if (CustomerStatus.DISABLE.toValue() == customerDetail.getCustomerStatus().toValue()) {
            throw new SbcRuntimeException(CustomerErrorCodeEnum.K010087, new Object[]{"，原因为：" + customerDetail
                    .getForbidReason()});
        }
        ...
        if (customer.getLoginLockTime() != null) {
            if (now.isBefore(customer.getLoginLockTime().plus(LOCK_MINUTES, ChronoUnit.MINUTES))) {
                throw new SbcRuntimeException(CustomerErrorCodeEnum.K010079, new Object[]{now.until
                        (customer.getLoginLockTime().plus(LOCK_MINUTES, ChronoUnit.MINUTES), ChronoUnit.MINUTES)});
            } else {
                // 30分钟后解锁用户
        ...
        if (!customer.getCustomerPassword().equals(encryptPwd)) {
            ...
            if (currentErrCount + 1 >= PASS_WRONG_MAX_COUNTS) {
                ...
                throw new SbcRuntimeException(CustomerErrorCodeEnum.K010078, new Object[]{PASS_WRONG_MAX_COUNTS -
                        (currentErrCount + 1)});
            }
        }
        if(customer.getCheckState().toValue() == CheckState.CHECKED.toValue()
                && (EnterpriseCheckState.INIT.equals(customer.getEnterpriseCheckState()) || EnterpriseCheckState.CHECKED.equals(customer.getEnterpriseCheckState()) )) {
        ...
        KsBeanUtil.copyPropertiesThird(customerDetail, newDetail, new String[]{"customer"});
        customer.setCustomerDetail(newDetail);
    ...
    *
     ...
     **/
    @Transactional
    public void updateLoginTime(String customerId) {
        customerRepository.updateLoginTime(customerId, LocalDateTime.now(), HttpUtil.getIpAddr());
    }
     ...
     */
    @Transactional/*(noRollbackFor = SbcRuntimeException.class)*/
    public Customer register(Customer customer, String employeeId, boolean checkStatus) throws SbcRuntimeException {
        ...
        CustomerDetail customerDetail1 = customerDetailRepository.findByCustomerId(customer.getCustomerId());
        if (Objects.nonNull(customerDetail1)) {
        ...
        customer.setIsNew(Constants.ZERO);
        ...
        // 如果不是业务员
        if (employee.isPresent() && DefaultFlag.YES.toValue() == employee.get().getIsEmployee()) {
            //查询该店铺下商家主账号
    ...
    public Customer register(Customer customer, Employee emp, boolean checkStatus,Long customerLevelId,
                             Optional<Employee> employeeOptional,
                             Map<Long, Employee> longEmployeeMap,
                             boolean isPointsOpen,
                             List<ConfigVO> configs,Long points) throws SbcRuntimeException {

        Optional<Employee> employee = Optional.ofNullable(emp);
        //存储基本信息
        //employeeId 存在为商家客户 否则为平台客户
        customer.setCustomerType(Objects.isNull(emp) ? CustomerType.PLATFORM : CustomerType.SUPPLIER);
        customer.setCustomerLevelId(customerLevelId);
        customer.setUpgradeTime(LocalDateTime.now());
        customer.setGrowthValue(0L); // 成长值默认为0
        customer.setPointsAvailable(0L); // 可用积分默认为0
        customer.setPointsUsed(0L);// 已用积分默认为0
        ...
        String saltVal = SecurityUtil.getNewPsw();
        //生成加密后的登陆密码
        String encryptPwd = SecurityUtil.getStoreLogpwd(String.valueOf(customer.getCustomerId()), MD5Util.md5Hex(customer
                .getCustomerPassword()), saltVal);
        customer.setCustomerSaltVal(saltVal);
        customer.setCustomerPassword(encryptPwd);
        customer = customerRepository.save(customer);
        ...
        // 负责的业务员
        String mainEmployeeId = Objects.nonNull(emp) ? emp.getEmployeeId() : null;
        // 如果不是业务员
        if (employee.isPresent() && DefaultFlag.YES.toValue() == employee.get().getIsEmployee()) {
            //查询该店铺下商家主账号
            Employee mainEmployee = longEmployeeMap.get(employee.get().getCompanyInfoId());
            if (Objects.nonNull(mainEmployee)) {
                mainEmployeeId = mainEmployee.getEmployeeId();
            }
        }
        ...
        if(!points.equals(0L)){
            CustomerPointsDetailAddRequest request = CustomerPointsDetailAddRequest.builder()
                    ...
                    .points(points)
                    .build();
            customerPointsDetailService.saveCustomerPointsDetail(request,isPointsOpen,configs,customer);
        }
        return customer;
    }
    /**
     ...
     */
    @Transactional/*(noRollbackFor = SbcRuntimeException.class)*/
    public Customer registerCustomer(Customer customer, Employee emp, boolean checkStatus,Long customerLevelId,
                             Optional<Employee> employeeOptional,
                             Map<Long, Employee> longEmployeeMap) throws SbcRuntimeException {
        this.beforeRegister(customer);
        Optional<Employee> employee = Optional.ofNullable(emp);
        //存储基本信息
        //employeeId 存在为商家客户 否则为平台客户
        ...
        //初始化会员资金信息
        producerService.initCustomerFunds(customer.getCustomerId(), customer.getCustomerAccount(), customer.getCustomerAccount());
        return customer;
    }
    ...
    public Customer registerEnterprise(Customer customer, CustomerRegisterRequest customerRegisterRequest) throws SbcRuntimeException {
        ...
        producerService.initCustomerFunds(customer.getCustomerId(), customerDetail.getCustomerName(), customer.getCustomerAccount());
        customer.setCustomerDetail(customerDetail);
        customer.setEnterpriseInfoVO(enterpriseInfo);
        return customer;
    }
    ...
    public Customer registerEnterpriseAgain(CustomerRegisterRequest customerRegisterRequest) throws SbcRuntimeException {
        ...
        if (Objects.isNull(customer)) {
            throw new SbcRuntimeException(CustomerErrorCodeEnum.K010027);
        }
        if (customer.getEnterpriseCheckState() == EnterpriseCheckState.CHECKED){
        ...
        if (Objects.isNull(enterpriseInfo)) {
            throw new SbcRuntimeException(CustomerErrorCodeEnum.K010028);
        }
        enterpriseInfo.setEnterpriseName(customerRegisterRequest.getEnterpriseName());
        enterpriseInfo.setSocialCreditCode(customerRegisterRequest.getSocialCreditCode());
        enterpriseInfo.setBusinessNatureType(customerRegisterRequest.getBusinessNatureType());
        ...
        CustomerDetail detail = customerDetailRepository.findAnyByCustomerId(customer.getCustomerId());
    ...
    public void beforeRegister(String customerAccount) {
        ...
        if (oldCustomer != null && LogOutStatus.LOGGED_OUT!=oldCustomer.getLogOutStatus()) {
            ...
            if (CustomerStatus.DISABLE.toValue() == customerDetail.getCustomerStatus().toValue()) {
                throw new SbcRuntimeException(CustomerErrorCodeEnum.K010010);
            }
    ...
    public Customer perfectCustomer(CustomerDetail detail) {
        Customer customer =
                customerRepository.findById(detail.getCustomerId()).orElseThrow(() -> new SbcRuntimeException(CustomerErrorCodeEnum.K010023));

        CustomerDetail oldDetail = customerDetailRepository.findByCustomerId(detail.getCustomerId());
        if (Objects.isNull(oldDetail)) {
            throw new SbcRuntimeException(CustomerErrorCodeEnum.K010023);
        }
        ...
        CustomerDetail newDetail = new CustomerDetail();
        KsBeanUtil.copyPropertiesThird(oldDetail, newDetail, new String[]{"customer"});
        customer.setCustomerDetail(newDetail);
    ...
    public Customer getCustomerByAccount(String account) {
        ...
        return customerRepository.findByCustomerAccountAndDelFlagAndLogOutStatusIn(account, DeleteFlag.NO,
                logOutStatuses);
    }
     ...
     * 根据账号获取有效的Customer
     *
     * @param customerAccountList 账号
     * @return 客户
     */
    public List<Customer> getCustomerByAccountList(List<String> customerAccountList) {
        List<LogOutStatus> logOutStatuses = Lists.newArrayList();
        logOutStatuses.add(LogOutStatus.NORMAL);
        logOutStatuses.add(LogOutStatus.LOGGING_OFF);
        return customerRepository.findByCustomerAccountInAndDelFlagAndLogOutStatusIn(customerAccountList,
                DeleteFlag.NO, logOutStatuses);
    }
    ...
    public void editPassword(Customer customer) {
        //获取会员信息
        Customer retCustomer = customerRepository.findByCustomerIdAndDelFlag(customer.getCustomerId(), DeleteFlag.NO);
        customer.setSafeLevel(SafeLevelUtil.getSafeLevel(customer.getCustomerPassword()));
        ...
        customer.setUpdateTime(LocalDateTime.now());
        customer.setLogOutStatus(retCustomer.getLogOutStatus());
        customerRepository.save(customer);
    }
     ...
     */
    @Transactional(noRollbackFor = SbcRuntimeException.class)
    public void editPayPassword(String customerId, String payPassword) {
        //获取会员信息
        Customer retCustomer = customerRepository.findByCustomerIdAndDelFlag(customerId, DeleteFlag.NO);
        retCustomer.setPaySafeLevel(SafeLevelUtil.getSafeLevel(payPassword));
        //支付密码盐值
        String customerPaySaltVal = retCustomer.getCustomerPaySaltVal();
        if(StringUtils.isBlank(customerPaySaltVal)){
            customerPaySaltVal = SecurityUtil.getNewPsw(); //生成盐值
            retCustomer.setCustomerPaySaltVal(customerPaySaltVal);
        }
        retCustomer.setCustomerPayPassword(SecurityUtil.getStoreLogpwd(String.valueOf(customerId), payPassword, retCustomer.getCustomerPaySaltVal()));
        retCustomer.setUpdateTime(LocalDateTime.now());
        retCustomer.setLogOutStatus(retCustomer.getLogOutStatus());
        customerRepository.save(retCustomer);
    }
    ...
    public void bindThird(ThirdLoginRelation thirdLoginRelation) {
        ...
        thirdLoginRelation.setStoreId(thirdLoginRelation.getStoreId() == null ? Long.valueOf("-1") : thirdLoginRelation.getStoreId());
...
//            customer.setCustomerPassword(password);
            customer.setCustomerPassword(MD5Util.md5Hex(password));
            ...
            customer = this.register(customer, null,checkStatus);
            response = KsBeanUtil.convert(customer, ThirdLoginAndBindResponse.class);
            //发送密码到用户手机
    ...
    public void checkCustomerPayPwd(CustomerCheckPayPasswordRequest customerCheckPayPasswordRequest) {
        ...
        String payPassword = SecurityUtil.getStoreLogpwd(String.valueOf(customerCheckPayPasswordRequest.getCustomerId()),
                customerCheckPayPasswordRequest.getPayPassword(), customer.getCustomerPaySaltVal());
        //判断密码是否正确
        ...
        if (!payPassword.equals(customer.getCustomerPayPassword())) {
            if (customer.getPayErrorTime() != null && customer.getPayErrorTime() == Constants.THREE) {
                ...
                if (duration.toMinutes() < Constants.NUM_30) {
                    ...
                    throw new SbcRuntimeException(CustomerErrorCodeEnum.K010138);
                }
            }
            throw new SbcRuntimeException(CustomerErrorCodeEnum.K010139);
        } else {
            //密码输入正确--密码错误次数置为“0”
    ...
    public void checkCustomerPayPwdErrorEvent(CustomerCheckPayPasswordRequest customerCheckPayPasswordRequest) {
        ...
        if (customer.getPayErrorTime() == null) {
            customer.setPayErrorTime(Constants.ONE);
        } else if (customer.getPayErrorTime() < Constants.TWO) {
            customer.setPayErrorTime(customer.getPayErrorTime() + Constants.ONE);
        } else if (customer.getPayErrorTime() == Constants.TWO) {
            customer.setPayErrorTime(customer.getPayErrorTime() + Constants.ONE);
            //如果已经达到3次，修改支付锁定登录时间
        ...
        } else if (customer.getPayErrorTime() == Constants.THREE) {
            Duration duration = Duration.between(customer.getPayLockTime(), LocalDateTime.now());
            if (duration.toMinutes() > Constants.NUM_30) {
