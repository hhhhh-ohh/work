...
public interface CustomerSignRecordRepository extends JpaRepository<CustomerSignRecord, String>,
     ...
     */
    @Modifying
    @Query("update CustomerSignRecord set delFlag = com.wanmi.sbc.common.enums.DeleteFlag.YES where signRecordId = ?1")
     ...
     */
    @Modifying
    @Query("update CustomerSignRecord set delFlag = com.wanmi.sbc.common.enums.DeleteFlag.YES where signRecordId in ?1")
    int deleteByIdList(List<String> signRecordIdList);

    @Query("from CustomerSignRecord where customerId = ?1 " +
            " and date_format(signRecord, '%Y-%m') = date_format(now(),'%Y-%m')")
    List<CustomerSignRecord> listByMonth(String customerId);

    @Query("from CustomerSignRecord where customerId = ?1 and to_days(now()) - to_days(signRecord) = ?2")
