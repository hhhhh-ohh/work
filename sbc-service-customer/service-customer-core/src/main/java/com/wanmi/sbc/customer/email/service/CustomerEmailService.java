...
//    }
    ...
    public CustomerEmail addCustomerEmail(CustomerEmailAddRequest request) {
        if (StringUtils.isEmpty(request.getEmailAddress()) || request.getEmailAddress().length() > Constants.NUM_32) {
            throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
        }
        // 根据客户id查询财务邮箱
    ...
    public CustomerEmail modifyCustomerEmail(CustomerEmailModifyRequest request) {
        ...
        if (Objects.isNull(customerEmail)) {
            throw new SbcRuntimeException(CustomerErrorCodeEnum.K010144);
        }
        // 除自身外, 客户财务邮箱名称不能重复
