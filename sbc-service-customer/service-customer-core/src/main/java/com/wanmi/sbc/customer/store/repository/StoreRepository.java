...
public interface StoreRepository extends JpaRepository<Store, Long>, JpaSpecificationExecutor<Store> {
    ...
    @Query(value = "select s.company_info_id companyInfoId,s.store_id storeId,s.store_name storeName,s.supplier_name supplierName, " +
            ...
            "s.supplier_type,s.contract_audit_state,s.contract_audit_reason"+
            " from store s " +
            " where s.del_flag = 0  " +
    ...
    @Query(value = "select s.company_info_id companyInfoId,s.store_id storeId,s.store_name storeName,s.supplier_name supplierName, " +
            ...
            " from store s " +
            " where s.del_flag = 0  " +
            " and s.store_id in :idList"
            , nativeQuery = true)
    List<Object> queryStoreInfo(@Param("idList") List<Long> idList);
    ...
    @Modifying
    @Query(value = "update store set pickup_state = 0 where (company_type = 1 or company_type = null) and store_type = 1",nativeQuery = true)
    void closePickupSetting();

    @Modifying
    @Query(value = "update store set pickup_state = 0 where company_type = 0 and store_type = 1",nativeQuery = true)
    void closePickupSetting2();

    @Modifying
    @Query(value = "update store set pickup_state = 0 where store_type = 2",nativeQuery = true)
    void closePickupSettingForO2o();

    /**
     * 拉卡拉关店
     */
    @Modifying
    @Query(value = "update store set store_state = 1, lakala_reason = 1,store_closed_reason='商家未进件' where del_flag = 0 and lakala_state = 0 and (store_state = 0 or store_state is null )and lakala_reason = 0",nativeQuery = true)
    void closeStoreForLaKaLa();
     ...
     */
    @Modifying
    @Query(value = "update store set store_state = 0, lakala_reason = 0,store_closed_reason='' where del_flag = 0 and lakala_state = 0 and store_state = 1 and lakala_reason = 1",nativeQuery = true)
    void openStoreForLaKaLa();
     ...
     * @return
     */
    @Query("select storeId from Store where companyInfoId in (?1) and delFlag = com.wanmi.sbc.common.enums.DeleteFlag.NO")
    List<Long> findStoreByCompanyInfoIds(List<Long> companyInfoIds);
     ...
     */
    @Modifying
    @Query(value = "update store set store_state = 0, lakala_reason = 0, store_closed_reason='', lakala_state = 1 where company_info_id = ?1 and lakala_reason = 1",nativeQuery = true)
    void updateLaKaLaState(Long companyInfoId);
}
