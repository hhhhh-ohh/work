...
public class CustomerInvoiceQueryRequest extends BaseQueryRequest {
    ...
    public Specification<CustomerInvoice> getWhereCriteria() {
        return (root, cquery, cbuild) -> {
            ...
            if (CollectionUtils.isNotEmpty(customerInvoiceIdList)) {
                predicates.add(root.get("customerInvoiceId").in(customerInvoiceIdList));
            }
            ...
            if(invoiceStyle != null){
                predicates.add(cbuild.equal(root.get("invoiceStyple"),invoiceStyle));
            }
