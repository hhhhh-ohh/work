...
public interface VerifyQueryProvider {
    ...
    /**
     * 校验订单确认信息（一般初始化加载、切换收货地址调用）
     * 校验商品库存，上下架状态
     * @param request   {@link TradeVerifyPurchaseRequest} 包含以下参数：
     *                            addressId        会员地址id
     * @return  {@link BaseResponse}
     */
    @PostMapping("/order/${application.order.version}/trade/verify-trade")
    BaseResponse verifyPurchase(@RequestBody @Valid TradeVerifyPurchaseRequest request);
}
