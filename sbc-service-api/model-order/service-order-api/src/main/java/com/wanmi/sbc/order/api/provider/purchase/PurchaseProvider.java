...
public interface PurchaseProvider {
    ...
    /**
     * @description   批量加购支持部分容错
     * @author  wur
     * @date: 2022/10/22 13:10
     * @param request
     * @return
     **/
    @PostMapping("/order/${application.order.version}/purchase/batch-save-new")
    BaseResponse batchSaveNew(@RequestBody @Valid PurchaseBatchSaveRequest request);
     ...
     * @return
     */
    @PostMapping("/order/${application.order.version}/purchase/get-cart-list")
    BaseResponse<GoodsCartListResponse> getCartList(@RequestBody @Valid PurchaseInfoRequest request);

    /**
     * 更新购物车商品规格
     * @return
     */
    @PostMapping("/order/${application.order.version}/purchase/replace-purchase")
    BaseResponse replacePurchase(@RequestBody @Valid PurchaseReplaceRequest request);
     ...
     * @return {@link BaseResponse}
     */
    @PostMapping("/order/${application.order.version}/purchase/quick-order-add-follow")
    BaseResponse quickOrderAddFollow(@RequestBody @Valid PurchaseAddFollowRequest request);
}
