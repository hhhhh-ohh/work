...
public abstract class StandardGoodsBaseRequest extends BaseRequest {
    ...
    public void checkParam(){
        if (StringUtils.isBlank(goods.getGoodsUnit()) || !ValidateUtil.isChsEng(goods.getGoodsUnit())) {
            throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
        }

        if (goods.getGoodsType() == GoodsType.REAL_GOODS.toValue()) {
            for (StandardSkuDTO goodsInfo : goodsInfos) {
                if (Objects.isNull(goodsInfo.getGoodsCubage()) || goodsInfo.getGoodsCubage().compareTo(BigDecimal.ZERO) < 1) {
                    throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
                }

                if (Objects.isNull(goodsInfo.getGoodsWeight()) || goodsInfo.getGoodsWeight().compareTo(BigDecimal.ZERO) < 1) {
                    throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
                }
                if (Objects.isNull(goodsInfo.getMarketPrice())) {
                    throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
                }
            }
        } else if (goods.getGoodsType() == GoodsType.VIRTUAL_GOODS.toValue()) {
            for (StandardSkuDTO goodsInfo : goodsInfos) {
                if (Objects.isNull(goodsInfo.getMarketPrice())) {
                    throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
                }
