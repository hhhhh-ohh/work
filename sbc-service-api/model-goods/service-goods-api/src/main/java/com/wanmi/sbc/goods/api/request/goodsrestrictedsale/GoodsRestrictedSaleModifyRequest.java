...
public class GoodsRestrictedSaleModifyRequest extends GoodsBaseRequest {
	...
	public void checkParam() {
		//是否打开限售方式开关、是否打开起售数量开关 至少有一个是打开的
		if((Objects.isNull(restrictedWay) || restrictedWay.equals(DefaultFlag.NO))
				&& (Objects.isNull(restrictedStartNum) || restrictedStartNum.equals(DefaultFlag.NO))){
			throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
		}

		//打开限售方式开关
		if(Objects.nonNull(restrictedWay) && restrictedWay.equals(DefaultFlag.YES)){
			//限售方式 0: 按订单 1：按会员 必填
			...
			// 按订单
			if (RestrictedType.RESTRICTED_ORDER == restrictedType){
				if (Objects.isNull(restrictedNum) || restrictedNum < Constants.ONE
						|| restrictedNum >= Constants.NUM_1000){
					throw new SbcRuntimeException(CommonErrorCodeEnum.K000009);
				}
			}else if (RestrictedType.RESTRICTED_HUMAN == restrictedType){
				// 每人限售标识
				...
				if (DefaultFlag.YES.equals(restrictedAssignFlag)){
					...
					}else if (AssignPersonRestrictedType.RESTRICTED_ADDRESS == assignPersonRestrictedType){
		...
		//打开起售数量开关
		if(Objects.nonNull(restrictedStartNum) && restrictedStartNum.equals(DefaultFlag.YES)){
