...
public enum PointsServiceType {
    ...
    RIGHTS,
    @ApiEnumProperty("用户注销")
    LOG_OUT,
    @ApiEnumProperty("权益-赠送积分")
    RIGHTS_SEND_POINTS,
    @ApiEnumProperty("抽奖消耗")
    DRAW_CONSUME,
    @ApiEnumProperty("抽奖奖励")
    DRAW_AWARD,
    @ApiEnumProperty("手动增加")
    MANUAL_INCREASE,
    @ApiEnumProperty("手动减少")
    MANUAL_REDUCT,
    @ApiEnumProperty("手动覆盖")
    MANUAL_REPLACE,

    @ApiEnumProperty("老系统下单")
    OLD_SYSTEM_PLACE_ORDER,

    @ApiEnumProperty("老系统退单")
    OLD_SYSTEM_REFLOUND_ORDER;
