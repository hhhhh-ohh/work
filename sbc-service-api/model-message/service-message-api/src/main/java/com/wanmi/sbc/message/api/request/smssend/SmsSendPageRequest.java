...
public class SmsSendPageRequest extends BaseQueryRequest {
	 ...
	 */
	@Schema(description = "接收类型（0-全部，1-会员等级，2-会员人群，3-自定义）", hidden = true)
	 ...
	 * 发送类型（0-立即发送，1-定时发送）
	 */
	@Schema(description = "发送类型（0-立即发送，1-定时发送）", hidden = true)
	private SendType sendType;

	/**
	 * 搜索条件:发送时间开始
	 */
	@Schema(description = "搜索条件:发送时间开始", hidden = true)
	 ...
	 * 搜索条件:发送时间截止
	 */
	@Schema(description = "搜索条件:发送时间截止", hidden = true)
	@JsonSerialize(using = CustomLocalDateTimeSerializer.class)
	@JsonDeserialize(using = CustomLocalDateTimeDeserializer.class)
