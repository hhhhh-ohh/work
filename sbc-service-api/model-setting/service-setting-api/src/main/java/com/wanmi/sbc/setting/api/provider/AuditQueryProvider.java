...
import com.wanmi.sbc.common.cache.CacheProperties;
...
public interface AuditQueryProvider {
     ...
     */
    @PostMapping("/setting/${application.setting.version}/audit/is-goods-outofstock-show")
    BaseResponse<BossGoodsOutOfStockShowResponse> isGoodsOutOfStockShow();

    /**
     * 移动端商品展示字段设置
     * @return
     */
    @PostMapping("/setting/${application.setting.version}/audit/goods-column-show")
    BaseResponse<GoodsColumnShowResponse> goodsColumnShowForMobile(@RequestBody ConfigContextModifyByTypeAndKeyRequest request);
     ...
     * @return
     */
    @PostMapping("/setting/${application.setting.version}/audit/is-boss-goods-secondary-audit")
    BaseResponse<BossGoodsAuditResponse> isBossGoodsSecondaryAudit(@RequestBody @Valid GoodsSecondaryAuditRequest request);

    /**
     * 限时抢购海报列表
     * @return
     */
    @PostMapping("/setting/${application.setting.version}/audit/poster/list")
    BaseResponse<ConfigVO> posterList();

    /**
     * 限时抢购库存抢完后是否允许原价购买
     */
    @PostMapping("/setting/${application.setting.version}/audit/isAllowOriginalPrice")
    BaseResponse<ConfigVO> isAllowFlashPromotionOriginalPrice();

    /**
     * 限时抢购库存抢完后是否允许原价购买
     */
    @PostMapping("/setting/${application.setting.version}/audit/isAllowFlashSaleOriginalPrice")
    BaseResponse<ConfigVO> isAllowFlashSaleOriginalPrice();

    /**
     * 种草开关
     * @return
     */
    @PostMapping("/setting/${application.setting.version}/audit/recommend/recommendConfig")
    BaseResponse<ConfigVO> recommendConfig();

    /**
     * 发现页直播种草排序优先级
     * @return
     */
    @PostMapping("/setting/${application.setting.version}/audit/recommend/findShowType")
    BaseResponse<ConfigVO> findShowType();

    /**
     * 发现页直播种草排序优先级及开关配置
     * @return
     */
    @PostMapping("/setting/${application.setting.version}/audit/recommend/config")
    BaseResponse<RecommendStatusConfigResponse> config();

    /**
     * 根据type查询缓存configList
     *
     * @return 订单设置列表 {@link TradeConfigListResponse}
     */
    @PostMapping("/setting/${application.setting.version}/audit/cached-list-trade-config")
    BaseResponse<TradeConfigListResponse> cachedListTradeConfig();
