...
/**
 * @author lvzhenwei
 * @className ModifyCustomerAccountFundsService
 * @description 会员账户修改,触发账户模块-会员资金的会员账户修改
 * @date 2021/8/13 3:33 下午
 */
@Slf4j
@Service
public class ModifyCustomerAccountFundsService {

    @Autowired private AccountMqConsumerProvider accountMqConsumerProvider;

    @Bean
    public Consumer<Message<String>> mqModifyCustomerAccountFundsService() {
        return message->{
            ...
            accountMqConsumerProvider.modifyCustomerAccountFunds(accountMqConsumerRequest);
        };
    }
}
