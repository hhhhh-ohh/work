...
/**
 * @author edz
 * @className LakalaDistributionBindStateExportService
 * @description TODO
 * @date 2022/9/16 10:09
 **/
@Service
@Slf4j
public class LakalaDistributionBindStateExportService implements ExportBaseService {
    ...
    public BaseResponse exportReport(ExportData data) throws Exception {
        ...
        String fileName = String.format("批量导出商家分销员关系绑定_%s.xls"
        ...
        DistributionBindStateQueryRequest queryReq = JSON.parseObject(data.getParam(), DistributionBindStateQueryRequest.class);
        ...
        while (nextPage){
            BindStateQueryResponse bindStateQueryResponse =
                    ledgerReceiverRelQueryProvider.queryDistributionBindStatePage(queryReq).getContext();
            List<LakalaDistributionBindVO> dataRecords =
                    bindStateQueryResponse.getLakalaDistributionBindVOPage().getContent();
            excelHelper.addSXSSFSheetRow(sheet, columns, dataRecords, rowIndex + 1);
            rowIndex = rowIndex + dataRecords.size();
            nextPage = bindStateQueryResponse.getLakalaDistributionBindVOPage().hasNext();
        }
    ...
    public Column[] getColumns() {
        return new Column[]{
                new Column("供应商名称", new SpelColumnRender<LakalaDistributionBindVO>("distributionName")),
                new Column("供应商编码", new SpelColumnRender<LakalaDistributionBindVO>("distributionAccount")),
                new Column("进件状态", (cell, object) -> cell.setCellValue("已进件")),
                new Column("分账关系绑定状态", (cell, object) -> {
                    LakalaDistributionBindVO lakalaDistributionBindVO = (LakalaDistributionBindVO) object;
                    Integer bindState =  lakalaDistributionBindVO.getLedgerBindState();
                    String desc = "";
                    switch (bindState) {
                        case 0:
                            desc = "未绑定";
