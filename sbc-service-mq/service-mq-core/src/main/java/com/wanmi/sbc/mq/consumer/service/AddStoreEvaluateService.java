...
/**
 * @author lvzhenwei
 * @className StoreEvaluateAddService
 * @description 商家评价
 * @date 2021/8/16 1:45 下午
 */
@Slf4j
@Service
public class AddStoreEvaluateService {

    @Autowired private EsMqConsumerProvider esMqConsumerProvider;

    @Bean
    public Consumer<Message<String>> mqAddStoreEvaluateService() {
        return message->{
            String json = message.getPayload();
            EsMqConsumerRequest esMqConsumerRequest = new EsMqConsumerRequest();
            esMqConsumerRequest.setMqContentJson(json);
            esMqConsumerProvider.addStoreEvaluate(esMqConsumerRequest);
        };
    }
}
