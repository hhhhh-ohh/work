...
/**
 * @author lvzhenwei
 * @className OperateLogAddService
 * @description 记录操作日志
 * @date 2021/8/11 5:02 下午
 */
@Slf4j
@Service
public class OperateLogAddService {

    @Autowired private EsMqConsumerProvider esMqConsumerProvider;

    @Bean
    public Consumer<Message<String>> mqOperateLogAddService() {
        return message->{
            String json = message.getPayload();
            EsMqConsumerRequest esMqConsumerRequest = new EsMqConsumerRequest();
            esMqConsumerRequest.setMqContentJson(json);
            esMqConsumerProvider.operateLogAdd(esMqConsumerRequest);
        };
    }
}
