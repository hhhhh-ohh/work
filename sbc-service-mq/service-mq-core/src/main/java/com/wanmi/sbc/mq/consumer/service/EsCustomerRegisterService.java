...
/**
 * @author lvzhenwei
 * @className EsCustomerRegisteService
 * @description 会员注册成功，发送订单支付MQ消息，同步ES
 * @date 2021/8/11 4:11 下午
 */
@Slf4j
@Service
public class EsCustomerRegisterService {

    @Autowired private EsMqConsumerProvider esMqConsumerProvider;

    @Bean
    public Consumer<Message<String>> mqEsCustomerRegisterService() {
        return message->{
            String json = message.getPayload();
            EsMqConsumerRequest esMqConsumerRequest = new EsMqConsumerRequest();
            esMqConsumerRequest.setMqContentJson(json);
            esMqConsumerProvider.customerRegister(esMqConsumerRequest);
        };
    }
}
