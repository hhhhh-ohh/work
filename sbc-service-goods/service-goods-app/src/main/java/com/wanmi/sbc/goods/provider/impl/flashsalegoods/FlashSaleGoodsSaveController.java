...
import com.wanmi.sbc.goods.api.request.flashsalegoods.FlashSaleGoodsBatchAddRequest;
import com.wanmi.sbc.goods.api.request.flashsalegoods.FlashSaleGoodsBatchAddStockRequest;
import com.wanmi.sbc.goods.api.request.flashsalegoods.FlashSaleGoodsBatchMinusStockRequest;
import com.wanmi.sbc.goods.api.request.flashsalegoods.FlashSaleGoodsBatchModifyRequest;
import com.wanmi.sbc.goods.api.request.flashsalegoods.FlashSaleGoodsBatchPlusSalesVolumeRequest;
import com.wanmi.sbc.goods.api.request.flashsalegoods.FlashSaleGoodsBatchStockAndSalesVolumeRequest;
import com.wanmi.sbc.goods.api.request.flashsalegoods.FlashSaleGoodsCloseRequest;
import com.wanmi.sbc.goods.api.request.flashsalegoods.FlashSaleGoodsDelByActivityIdRequest;
import com.wanmi.sbc.goods.api.request.flashsalegoods.FlashSaleGoodsDelByIdListRequest;
import com.wanmi.sbc.goods.api.request.flashsalegoods.FlashSaleGoodsDelByIdRequest;
import com.wanmi.sbc.goods.api.request.flashsalegoods.FlashSaleGoodsDelByTimeListRequest;
import com.wanmi.sbc.goods.api.request.flashsalegoods.FlashSaleGoodsModifyRequest;
...
public class FlashSaleGoodsSaveController implements FlashSaleGoodsSaveProvider {
	...
	public BaseResponse<FlashSaleGoodsModifyResponse> modify(@RequestBody @Valid FlashSaleGoodsModifyRequest flashSaleGoodsModifyRequest) {
		...
		flashSaleGoods.setStock(flashSaleGoodsModifyRequest.getIncreaseStock());
	...
	@Override
	public BaseResponse close(@RequestBody @Valid FlashSaleGoodsCloseRequest request) {
		flashSaleGoodsService.close(request.getId());
		return BaseResponse.SUCCESSFUL();
	}

	@Override
	public BaseResponse deleteByActivityId(@RequestBody @Valid FlashSaleGoodsDelByActivityIdRequest flashSaleGoodsDelByActivityIdRequest) {
		flashSaleGoodsService.deleteByActivityId(flashSaleGoodsDelByActivityIdRequest.getActivityId());
		return BaseResponse.SUCCESSFUL();
	}

	@Override
	public BaseResponse<FlashSaleGoodsAddResponse> batchModify(@RequestBody @Valid FlashSaleGoodsBatchModifyRequest flashSaleGoodsBatchModifyRequest) {
		//秒杀活动实体
		FlashPromotionActivity flashPromotionActivity = KsBeanUtil.convert(flashSaleGoodsBatchModifyRequest.getFlashPromotionActivityVO(),
				FlashPromotionActivity.class);
		//活动商品实体
		List<FlashSaleGoods> flashSaleGoodsList =
				KsBeanUtil.convertList(flashSaleGoodsBatchModifyRequest.getFlashSaleGoodsVOList(),
						FlashSaleGoods.class);
		List<FlashSaleGoodsVO> flashSaleGoodsVOS =
				flashSaleGoodsService.batchModifyPromotion(flashPromotionActivity, flashSaleGoodsList).stream().map(flashSaleGoods-> flashSaleGoodsService.wrapperVo(flashSaleGoods)).collect(Collectors.toList());
		return BaseResponse.success(new FlashSaleGoodsAddResponse(flashSaleGoodsVOS));
	}

	@Override
	public BaseResponse<FlashSaleGoodsAddResponse> batchAddPromotion(@RequestBody @Valid FlashSaleGoodsBatchAddRequest flashSaleGoodsAddRequest) {
		//秒杀活动实体
		FlashPromotionActivity flashPromotionActivity = KsBeanUtil.convert(flashSaleGoodsAddRequest.getFlashPromotionActivityVO(),
				FlashPromotionActivity.class);
		//活动商品实体
		List<FlashSaleGoods> flashSaleGoodsList =
				KsBeanUtil.convertList(flashSaleGoodsAddRequest.getFlashSaleGoodsVOList(),
		...
		List<FlashSaleGoodsVO> flashSaleGoodsVOS =
				flashSaleGoodsService.batchAddPromotion(flashPromotionActivity, flashSaleGoodsList).stream().map(
						flashSaleGoods-> flashSaleGoodsService.wrapperVo(flashSaleGoods)).collect(Collectors.toList());
		return BaseResponse.success(new FlashSaleGoodsAddResponse(flashSaleGoodsVOS));
	}
