...
import com.wanmi.sbc.goods.api.response.storecate.StoreCateRelaCountResponse;
...
public class StoreCateGoodsRelaQueryController implements StoreCateGoodsRelaQueryProvider {
    ...
    @Override
    public BaseResponse<StoreCateRelaCountResponse> countByParams(@RequestBody @Valid StoreCateGoodsRelaCountRequest request) {
        Long count = storeCateGoodsRelaService.countByStoreCateIdsAndGoodsIds(request.getStoreCateIds(), request.getGoodsIds());
        return BaseResponse.success(new StoreCateRelaCountResponse(count));
    }
}
