...
public class GoodsCateQueryController implements GoodsCateQueryProvider {
    ...
    public BaseResponse<GoodsCateListByConditionResponse> listByCondition(
        ...
        if (Boolean.TRUE.equals(request.getFillChildren())) {
            List<GoodsCateVO> cateVOList = goodsCateVOList.stream().filter(v -> !v.getCateGrade().equals(3)).collect(Collectors.toList());
            if (CollectionUtils.isNotEmpty(cateVOList)) {
                List<GoodsCateVO> goodsCates = JSON.parseArray(JSON.toJSONString(goodsCateService.queryGoodsCateAll()),GoodsCateVO.class);
                for (GoodsCateVO goodsCateVO : cateVOList) {
                    goodsCateService.findChildren(goodsCateVO,goodsCates);
                }
            }
        }
     ...
     */
    @Override
    public BaseResponse<GoodsCateListByConditionResponse> listByParentId(GoodsCateByParentIdRequest request) {
        GoodsCateQueryRequest goodsCateQueryRequest = new GoodsCateQueryRequest();
        goodsCateQueryRequest.setSortMap(request.getSortMap());
        goodsCateQueryRequest.setDelFlag(Constants.no);
        if(Objects.nonNull(request.getGoodsType())){
            goodsCateQueryRequest.setGoodsType(request.getGoodsType());
        }
        if (Objects.isNull(request.getCateParentId())){
            goodsCateQueryRequest.setCateGrade(Constants.ONE);
        }else {
            goodsCateQueryRequest.setCateParentId(request.getCateParentId());
        }
        List<GoodsCate> goodsCateList = goodsCateService.query(goodsCateQueryRequest);

        GoodsCateListByConditionResponse response = new GoodsCateListByConditionResponse();
        List<GoodsCateVO> goodsCateVOList = KsBeanUtil.convert(goodsCateList, GoodsCateVO.class);

        //处理下级标识
        if(Objects.isNull(request.getCateParentId()) || (request.getHasSonFlag() && CollectionUtils.isNotEmpty(goodsCateVOList))) {
            List<Long> cateIdList = goodsCateVOList.stream().map(GoodsCateVO :: getCateId).collect(Collectors.toList());
            goodsCateQueryRequest = new GoodsCateQueryRequest();
            goodsCateQueryRequest.setDelFlag(Constants.no);
            goodsCateQueryRequest.setCateParentIds(cateIdList);
            if(Objects.nonNull(request.getGoodsType())){
                goodsCateQueryRequest.setGoodsType(request.getGoodsType());
            }
            List<GoodsCate> sonCateList = goodsCateService.query(goodsCateQueryRequest);
            if (CollectionUtils.isNotEmpty(sonCateList)){
                List<Long> parentIdList  = sonCateList.stream().filter(goodsCate -> Objects.nonNull(goodsCate.getCateParentId())).map(GoodsCate :: getCateParentId).collect(Collectors.toList());
                if (CollectionUtils.isNotEmpty(parentIdList)) {
                    goodsCateVOList.forEach(goodsCateVO -> {
                        goodsCateVO.setHasSonFlag(parentIdList.contains(goodsCateVO.getCateId()));
                    });
                }
            }
        }
    ...
    public BaseResponse<GoodsCateListResponse> list() {
        List<GoodsCateVO> goodsCateList = goodsCateService.queryGoodsCate();

        GoodsCateListResponse response = new GoodsCateListResponse();
        response.setGoodsCateVOList(goodsCateList);
        return BaseResponse.success(response);
    }

    @Override
    public BaseResponse<List<GoodsCateVO>> queryForMapWechat() {
        return BaseResponse.success(goodsCateService.queryForMapWechat());
    }
     ...
     */
    @Override
    public BaseResponse<GoodsCateListResponse> listAll() {
        List<GoodsCateVO> goodsCateList = goodsCateService.queryGoodsCateAll(true);
     ...
     */
    @Override
    public BaseResponse<GoodsCateMapResponse> allCateMap() {
        List<GoodsCate> goodsCateList = goodsCateService.queryGoodsCateAll();
        return BaseResponse.success(new GoodsCateMapResponse(WmCollectionUtils.notEmpty2Map(KsBeanUtil
                .convert(goodsCateList, GoodsCateVO.class), GoodsCateVO::getCateId)));
    }
     ...
     */
    @Override
    public BaseResponse<GoodsCateByIdsResponse> findParentIdByIds(@RequestBody @Valid GoodsCateByIdsRequest request) {
        List<Long> cateIdList = request.getCateIds();
        List<GoodsCate> goodsCateList = goodsCateService.findParentIdByIds(cateIdList);
