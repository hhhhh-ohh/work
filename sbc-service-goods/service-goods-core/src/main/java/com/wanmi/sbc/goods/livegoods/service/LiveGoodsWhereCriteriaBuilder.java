    ...
    public static Specification<LiveGoods> build(LiveGoodsQueryRequest queryRequest) {
        return (root, cquery, cbuild) -> {
            ...
            if (queryRequest.getMinStock() != null) {
                predicates.add(cbuild.greaterThanOrEqualTo(root.get("stock"),
                        queryRequest.getMinStock()));
            }
            // 小于或等于 搜索条件:价格结束截止
            if (queryRequest.getMaxStock() != null) {
                predicates.add(cbuild.lessThanOrEqualTo(root.get("stock"),
                        queryRequest.getMaxStock()));
            }

            // 大于或等于 搜索条件:库存开始
            if (queryRequest.getMinMarketPrice() != null) {
                predicates.add(cbuild.greaterThanOrEqualTo(root.get("price"),
                        queryRequest.getMinMarketPrice()));
            }
            // 小于或等于 搜索条件:库存截止
            if (queryRequest.getMaxMarketPrice() != null) {
                predicates.add(cbuild.lessThanOrEqualTo(root.get("price"),
                        queryRequest.getMaxMarketPrice()));
            }
            //商品类型
