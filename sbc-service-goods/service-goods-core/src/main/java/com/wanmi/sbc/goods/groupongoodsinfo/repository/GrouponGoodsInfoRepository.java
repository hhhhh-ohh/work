...
public interface GrouponGoodsInfoRepository extends JpaRepository<GrouponGoodsInfo, String>,
	 ...
	 * @description 关闭活动
	 * @author  xuyunpeng
	 * @date 2021/6/24 10:09 上午
     * @param grouponActivityId 拼团活动id
	 * @return
	 */
	@Modifying
	@Query("update GrouponGoodsInfo set endTime = now() where grouponActivityId = ?1")
	void closeActivity(String grouponActivityId);

	/**
	 * 根据活动ids查询商品id
	 *
	 * @param grouponActivityIds  活动ids
	 * @return 商品skuIds
	 */
	@Query("select count(g.grouponGoodsId) from GrouponGoodsInfo g where g.grouponActivityId in ?1 and g.goodsInfoId in ?2")
	Long countBySkuIdsByActivityIds(List<String> grouponActivityIds, List<String> skuIds);
}
