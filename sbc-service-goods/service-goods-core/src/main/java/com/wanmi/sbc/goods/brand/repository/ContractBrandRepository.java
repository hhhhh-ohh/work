...
public interface ContractBrandRepository extends JpaRepository<ContractBrand, Long>, JpaSpecificationExecutor<ContractBrand> {
     ...
     * @param storeId
     */
    @Query("delete from ContractBrand c where c.storeId = :storeId")
    @Modifying
     ...
     * @return
     */
    @Query("select new com.wanmi.sbc.goods.brand.entity.ContractBrandBase(c.contractBrandId,c.brandId, c.storeId) from ContractBrand c where c.storeId in :storeIds  and c.brandId = :brandId")
    List<ContractBrandBase> findStoreIdAndBrandId(@Param("storeIds") List<Long> storeIds, @Param("brandId") Long brandId);
}
