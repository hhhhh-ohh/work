...
public interface ContractCateRepository extends JpaRepository<ContractCate, Long>, JpaSpecificationExecutor<ContractCate> {
    ...
    @Query("from ContractCate c where c.goodsCate.cateId in (:cateIds) and c.storeId = :storeId")
    List<ContractCate> queryByCateIdAndStoreId(@Param("cateIds") List<Long> cateIds, @Param("storeId") Long storeId);
     ...
     */
    @Modifying
    @Query("delete from ContractCate c where c.storeId = :storeId ")
