...
public class GoodsCateQueryRequest extends BaseQueryRequest {
    ...
    public Specification<GoodsCate> getWhereCriteria(){
        return (root, cquery, cbuild) -> {
            ...
            // 精准分类名称
            if(StringUtils.isNotBlank(equalCateName)){
                predicates.add(cbuild.equal(root.get("cateName"), equalCateName.trim()));
            }
            //默认标记
            if(isDefault != null){
                predicates.add(cbuild.equal(root.get("isDefault"), DefaultFlag.fromValue(isDefault)));
            }
            //删除标记
            if(delFlag != null){
            ...
            //查询虚拟商品商品分类
            if(Objects.nonNull(goodsType) && goodsType != GoodsType.REAL_GOODS){
                predicates.add(cbuild.equal(root.get("containsVirtual"), BoolFlag.YES));
            }
