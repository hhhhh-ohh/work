    ...
    public static Specification<PriceAdjustmentRecordDetail> build(PriceAdjustmentRecordDetailQueryRequest queryRequest) {
        return (root, cquery, cbuild) -> {
            ...
            if (queryRequest.getAloneFlag() != null) {
                predicates.add(cbuild.equal(root.get("aloneFlag"), queryRequest.getAloneFlag().equals(Constants.yes)));
            }

            // 销售类别(0:批发,1:零售)
            if (queryRequest.getSaleType() != null) {
                predicates.add(cbuild.equal(root.get("saleType"), SaleType.fromValue(queryRequest.getSaleType())));
            }

            // 设价类型,0:按客户(等级)1:按订货量(阶梯价)2:按市场价
            if (queryRequest.getPriceType() != null) {
                predicates.add(cbuild.equal(root.get("priceType"), GoodsPriceType.fromValue(queryRequest.getPriceType())));
            }
            ...
            if (queryRequest.getAdjustResult() != null) {
                predicates.add(cbuild.equal(root.get("adjustResult"), PriceAdjustmentResult.fromValue(queryRequest.getAdjustResult())));
            }
            ...
            if (queryRequest.getConfirmFlag() != null) {
                predicates.add(cbuild.equal(root.get("confirmFlag"), DefaultFlag.fromValue(queryRequest.getConfirmFlag())));
            }
