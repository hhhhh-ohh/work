...
import com.wanmi.sbc.goods.bean.dto.GoodsCateSaveDTO;
...
public class GoodsCateExcelService {
    ...
    @Transactional
    public Boolean importGoodsCate(List<GoodsCateSaveDTO> goodsCateList) {
        //处理层级结构
        List<GoodsCateSaveDTO> list = goodsCateList.stream().filter(cate -> cate.getCateParentId().intValue() == 0)
