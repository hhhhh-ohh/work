...
//    @ListCache(prefix = CacheKeyConstant.PURCHASE_GOODS, clazz = GoodsVO.class, seconds = 60 * 5)
    public List<GoodsVO> listGoodsByIds(List<String> goodsIds) {
        List<GoodsVO> goodsList = KsBeanUtil.convertList(goodsRepository.findAll(
                GoodsQueryRequest.builder().goodsIds(goodsIds).build().getWhereCriteria()), GoodsVO.class);
            ...
            if (stocks != null) {
                for (GoodsVO goodsVO : goodsList) {
                    if (ThirdPlatformType.LINKED_MALL.equals(goodsVO.getThirdPlatformType())) {
                        Optional<LinkedMallStockVO> optional = stocks.stream().filter(v -> String.valueOf(v.getItemId()).equals(goodsVO.getThirdPlatformSpuId())).findFirst();
    ...
    public List<GoodsInfoVO> listGoodsInfosByIds(List<String> goodsInfoIds) {
        // 单品查询
        ...
        goodsInfos = goodsInfoService.fillGoodsStatus(goodsInfos);
        ...
        List<GoodsInfoSpecDetailRel> relList = goodsInfoSpecDetailRelRepository.findByGoodsInfoIds(goodsInfoIds);
        List<GoodsLevelPrice> goodsLevelPrices = goodsLevelPriceRepository.findSkuByGoodsInfoIds(goodsInfoIds);
