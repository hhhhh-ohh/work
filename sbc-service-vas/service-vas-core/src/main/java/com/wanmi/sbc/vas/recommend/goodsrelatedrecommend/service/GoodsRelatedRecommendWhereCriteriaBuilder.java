...
/**
 * <p>商品相关性推荐动态查询条件构建器</p>
 * @author lvzhenwei
 * @date 2020-11-24 16:13:10
 */
public class GoodsRelatedRecommendWhereCriteriaBuilder {
    public static Specification<GoodsRelatedRecommend> build(GoodsRelatedRecommendQueryRequest queryRequest) {
        return (root, cquery, cbuild) -> {
            ...
            // 模糊查询 - 关联商品id
            if (StringUtils.isNotEmpty(queryRequest.getRelatedGoodsId())) {
                predicates.add(cbuild.like(root.get("relatedGoodsId"), StringUtil.SQL_LIKE_CHAR
                        .concat(XssUtils.replaceLikeWildcard(queryRequest.getRelatedGoodsId()))
                        .concat(StringUtil.SQL_LIKE_CHAR)));
            }
            // 批量查询-主键idList
            if (CollectionUtils.isNotEmpty(queryRequest.getRelatedGoodsIds())) {
                predicates.add(root.get("relatedGoodsId").in(queryRequest.getRelatedGoodsIds()));
            }
